...
def test_complete_mbti_flow():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"   🔑 Token received: {token[:20]}..." if token else "   ⚠️  No token received")
        else:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"   ✅ User info: {data.get('email', 'Unknown')} ({data.get('role', 'Unknown')})")

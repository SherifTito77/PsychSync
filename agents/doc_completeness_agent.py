...
class DocstringChecker:
    ...
    def check_file(self, file_path: str) -> Dict:
        ...
        try:
            ...
            documented_items = (
                sum(1 for f in result['functions'] if f['has_docstring']) +
                sum(1 for c in result['classes'] if c['has_docstring'])
            )

            result['coverage'] = (documented_items / total_items * 100) if total_items > 0 else 100
    ...
    def _check_function(self, func_node: ast.FunctionDef) -> Dict:
        ...
        if docstring:
            ...
            if params:
                ...
                documented_params = []
                for param in params:
                    if f":param {param}:" in docstring or f"Args:\n    {param}" in docstring:
                        documented_params.append(param)

                missing_params = set(params) - set(documented_params)
                if missing_params:
                    doc_issues.append(f"Missing parameter docs: {', '.join(missing_params)}")

            # Check for return documentation
            if func_node.returns and ':return:' not in docstring and 'Returns:' not in docstring:
    ...
    def get_summary(self) -> Dict:
        ...
        function_coverage = (
            ...
            if self.total_functions > 0 else 100
        )
        class_coverage = (
            (self.documented_classes / self.total_classes * 100)
            if self.total_classes > 0 else 100
        )
        module_coverage = (
            (self.documented_modules / self.total_modules * 100)
            if self.total_modules > 0 else 100
        )
        ...
        return {
            ...
            'overall_coverage': (function_coverage + class_coverage + module_coverage) / 3
        }
    ...
    def scan(self):
        ...
        for root, dirs, files in os.walk(self.code_path):
            ...
            if any(f.startswith('README') for f in files):
                readme_path = os.path.join(root, next(f for f in files if f.startswith('README')))
            ...
            for path, methods in paths.items():
                for method, details in methods.items():
                    ...
                    if 'responses' not in endpoint_details:
                        issues.append(f"No responses documented for {method.upper()} {path}")
                        score -= 10
                    else:
                        responses = endpoint_details['responses']
                        if '200' not in responses and '2XX' not in responses:
                            issues.append(f"No success response documented for {method.upper()} {path}")
                            score -= 5
...
class DocumentationReporter:
    ...
    def __init__(self, code_path: str, output_path: str = 'reports/doc_coverage.json', spec_path: str = None):
        self.code_path = code_path
        ...
        if spec_path is None:
            ...
            for possible_spec in ['openapi.json', 'swagger.json', 'openapi.yaml', 'swagger.yaml']:
    ...
    def scan_all(self) -> Dict:
                    ...
                    for func in result['functions']:
                        for issue in func['issues']:
                            self.checker.issues.append({
                                ...
                                'line': func['line'],
                                'issue': issue,
                                'severity': 'minor' if 'No docstring' in issue else 'info'
                            })
        ...
        overall_score = (
            docstring_summary['overall_coverage'] * 0.5 +  # 50% weight
            (100 - readme_summary['directories_missing_readme'] * 2) * 0.3 +  # 30% weight
            spec_result['completeness'] * 0.2  # 20% weight
        )
        ...
        report = {
            ...
            'issues_by_severity': {
                'minor': sum(1 for i in self.checker.issues if i['severity'] == 'minor'),
                'info': sum(1 for i in self.checker.issues if i['severity'] == 'info')
            },
            'recommendations': self._generate_recommendations(docstring_summary, readme_summary, spec_result)
        }
    ...
    def _generate_recommendations(self, docstring_summary, readme_summary, spec_result) -> List[str]:
        ...
        if readme_summary['directories_missing_readme'] > 5:
            recommendations.append(f"Add README files to {readme_summary['directories_missing_readme']} directories")

        if not spec_result['exists']:
            recommendations.append("Create OpenAPI/Swagger specification")
        elif spec_result['completeness'] < 70:
            recommendations.append("Improve OpenAPI spec completeness (add descriptions, examples, etc.)")
...
Examples:
  ...
  python agents/doc_completeness_agent.py --code-path app/ --min-coverage 80
    ...
    parser.add_argument('--output', default='reports/doc_coverage.json', help='Output report path')
    parser.add_argument('--spec-path', default=None, help='Path to OpenAPI/Swagger spec (auto-detected if not provided)')
    parser.add_argument('--min-coverage', type=float, default=0, help='Minimum acceptable coverage score (0-100)')

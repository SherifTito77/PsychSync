...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("agents/doc_syncer.log"), logging.StreamHandler()],
)
...
class DocumentationSyncer:
    ...
    def scan_and_sync(self, commit_hash: str = None) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"✅ Documentation sync complete: {len(doc_updates)} files updated"
            )
            return changes
    ...
    def _detect_code_changes(self, commit_hash: str = None) -> List[Dict]:
        ...
        for file_path in changed_files:
            ...
            if any(
                file_path.startswith(src_dir)
                for src_dir in ["app/", "ai/", "frontend/src/"]
            ):
                    ...
                    for decorator in node.decorator_list:
                        if isinstance(decorator, ast.Call):
                            ...
                            if (
                                ...
                                and "router" in decorator.func.attr
                            ):
    ...
    def _is_user_facing_feature(self, file_path: str) -> bool:
        ...
        user_facing_patterns = ["assessments", "analytics", "dashboard", "reports"]
    ...
    def _apply_documentation_updates(self, update_plan: Dict) -> List[Dict]:
        ...
        if update_plan["database_schema"]:
            schema_updates = self._update_database_schema(
                update_plan["database_schema"]
            )
    ...
    def _update_database_schema(self, models: List[Dict]) -> List[Dict]:
        ...
        for model in models:
            updates.append(
                {"type": "database_schema", "action": "add_model", "details": model}
            )
...
"""
    ...
    def run_continuous(self, interval_minutes: int = 60):
        ...
        logger.info(
            f"🔄 Running continuous sync (checking every {interval_minutes} minutes)..."
        )

...
def watch_logs(
    log_path: str, pattern: str = "*", threshold: float = 3.0, watch: bool = False
):
                                ...
                                if line:
                                    try:
                                        ...
                                        if anomaly or entry.is_anomaly():
                                            ...
                                            print(
                                                f"[{timestamp}] {icon} {anomaly['message'] if anomaly else entry.message}"
                                            )
...
Examples:
  ...
  python agents/log_anomaly_agent.py --log-path logs/ --pattern "app*.log" --threshold 5.0
    ...
    parser.add_argument(
        "--log-path", required=True, help="Path to log file or directory"
    )
    parser.add_argument(
        "--pattern", default="*.log", help="Log file pattern to match (default: *.log)"
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=3.0,
        help="Anomaly threshold multiplier (default: 3.0)",
    )
    parser.add_argument(

...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "notify_on_crisis_alert", sa.Boolean(), nullable=True, server_default="true"
        ),
        sa.Column(
            "notify_on_high_risk", sa.Boolean(), nullable=True, server_default="true"
        ),
        sa.Column(
            "notify_on_moderate_risk",
            sa.Boolean(),
            nullable=True,
            server_default="false",
        ),
        sa.Column(
            "notify_on_pending_review",
            sa.Boolean(),
            nullable=True,
            server_default="true",
        ),
        ...
        sa.Column(
            "quiet_hours_enabled", sa.Boolean(), nullable=True, server_default="true"
        ),
        sa.Column(
            "quiet_hours_start", sa.TIME(), nullable=True, server_default="22:00"
        ),
        sa.Column("quiet_hours_end", sa.TIME(), nullable=True, server_default="08:00"),
        sa.Column(
            "timezone", sa.String(), nullable=True, server_default="America/New_York"
        ),
        sa.Column(
            "bypass_quiet_hours_for_critical",
            sa.Boolean(),
            nullable=True,
            server_default="true",
        ),
    ...
    op.create_index(
        "idx_notification_prefs_user", "notification_preferences", ["user_id"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "delivery_status", sa.String(), nullable=True, server_default="pending"
        ),
        ...
        sa.ForeignKeyConstraint(
            ["entity_id"], ["clinical_screenings.id"], ondelete="CASCADE"
        ),
    )
    op.create_index(
        "idx_notifications_recipient", "notifications", ["recipient_id", "created_at"]
    )
    op.create_index(
        "idx_notifications_unread", "notifications", ["recipient_id", "read"]
    )
    op.create_index(
        "idx_notifications_status", "notifications", ["delivery_status", "created_at"]
    )
    ...
    op.create_index(
        "idx_notification_queue_recipient",
        "notification_queue",
        ["recipient_id", "status"],
    )

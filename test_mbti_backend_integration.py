...
class MBTIBackendIntegrationTest:
    ...
    def test_backend_mbti_questions_api(self):
        ...
        try:
            response = requests.get(f"{self.backend_url}/assessment-questions/mbti", timeout=10)
            ...
            if response.status_code == 200:
                ...
                self.log_test("Backend MBTI Questions API", False, f"HTTP {response.status_code}")
                return None
    ...
    def test_frontend_page_loads(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                has_react_content = "react" in content.lower() or "vite" in content.lower()
                ...
                self.log_test("Frontend Assessment Page", False, f"HTTP {response.status_code}")
                return False
        ...
        try:
            ...
            quality_checks = {
                ...
                "has_proper_structure": all(
                    q.get("question_text") and q.get("options") and len(q.get("options", [])) == 2
                    for q in questions
                ),
                ...
                "has_meaningful_questions": all(
                    len(q.get("question_text", "")) > 10 and "?" in q.get("question_text", "")
                    for q in questions
                ),
                "has_valid_options": all(
                    all(opt.get("text") and opt.get("value") for opt in q.get("options", []))
                    for q in questions
                )
            }
    ...
    def test_complete_integration_flow(self):
        ...
        questions_quality = self.test_question_quality(mbti_data) if mbti_data else False

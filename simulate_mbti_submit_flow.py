...
class MBTISubmitFlowSimulator:
    ...
    def step_1_user_visits_assessment_page(self):
        ...
        try:
            ...
            response = self.session.get(f"{self.frontend_url}/assessments/mbti/start", timeout=10)

            if response.status_code == 200:
                self.log_step("✅ Frontend page loads successfully", f"Status: {response.status_code}")
                ...
                self.log_step("❌ Failed to load assessment page", f"HTTP {response.status_code}")
                return False
    ...
    def step_3_user_clicks_submit(self, user_answers: Dict[int, str]):
        ...
        self.log_step("🧠 Client-side MBTI calculation", f"Calculated type: {calculated_type}")

        # Submit to backend using the new assessment results service
        self.log_step("📤 Frontend calls assessmentResultsService.submitMBTIAssessment()")
        ...
        try:
            ...
            self.log_step("🌐 API Call to Backend", f"POST {self.backend_url}/assessment-results-test")
            ...
            self.log_step("📡 Backend response received", f"Status: {response.status_code}, Time: {response_time:.0f}ms")
    ...
    def _get_question_text(self, question_id: int, answer: str) -> str:
        ...
        questions = {
            1: {"E": "Talk to many people, including strangers", "I": "Talk to a few people you know well"},
            2: {"N": "Imagine possibilities and think about abstract concepts", "S": "Focus on the real world and practical matters"},
            3: {"F": "Consider how it will affect people involved", "T": "Rely on logic and objective analysis"},
            4: {"J": "Plan things in advance and stick to the plan", "P": "Be spontaneous and adapt to new situations"},
            5: {"E": "Enjoy working in teams and brainstorming", "I": "Prefer working independently and concentrating"},
            6: {"N": "Like to understand the overall concept first", "S": "Prefer step-by-step instructions"},
            7: {"F": "Consider feelings and delivery", "T": "Focus on facts and logical improvements"},
            8: {"P": "Leave options open and decide spontaneously", "J": "Plan activities and have a schedule"}
        }
    ...
    def simulate_complete_flow(self):
        ...
        print("and clicks 'Submit Assessment' on http://localhost:5173/assessments/mbti/start")
        print()

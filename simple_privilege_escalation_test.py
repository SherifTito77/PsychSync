...
class SimplePrivilegeEscalationTester:
    ...
    def execute_permission_test(self, test_case):
        ...
        try:
            ...
            for sql_op, description in dangerous_operations:
                try:
                    ...
                    if rows:
                        ...
                        result["findings"].append(f"DANGEROUS: {description} granted to {test_case['user']}")

                except psycopg2.Error as e:
                    if "permission denied" in str(e).lower():
                        result["findings"].append(f"GOOD: Permission denied for {description}")
                    else:
            ...
            if "password authentication failed" in str(e).lower():
                result["findings"].append(f"GOOD: Authentication failed for {test_case['user']}")
            elif "database" in str(e).lower() and "does not exist" in str(e).lower():
                result["findings"].append(f"INFO: Database {test_case['database']} does not exist")
            else:
    ...
    def test_sql_injection_privilege_escalation(self):
        ...
        for payload in injection_payloads:
            ...
            try:
                ...
                escalation_keywords = ["UPDATE users SET is_admin", "INSERT INTO users", "DROP TABLE"]
                if any(keyword in payload for keyword in escalation_keywords):
                    result["privilege_escalation"] = True
                    result["injection_successful"] = True
                    result["finding"] = f"CRITICAL: Payload contains privilege escalation command: {payload}"
    ...
    def generate_security_report(self, all_results):
        ...
        # Count issues
        db_issues = len([r for r in all_results.get("database_permissions", [])
                        if r.get("privilege_escalation", False)])
        injection_issues = len([r for r in all_results.get("sql_injection", [])
                               if r.get("privilege_escalation", False)])
        config_issues = sum(len(r.get("issues", [])) for r in all_results.get("config_files", []))
    ...
    def run_comprehensive_test(self):
        ...
        db_issues = [r for r in results["database_permissions"]
                    if r.get("privilege_escalation", False)]
        if db_issues:
        ...
        injection_issues = [r for r in results["sql_injection"]
                           if r.get("privilege_escalation", False)]
        if injection_issues:
        ...
        # Save report
        with open("/Users/sheriftito/Downloads/psychsync/privilege_escalation_report.json", "w") as f:

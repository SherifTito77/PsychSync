    ...
    async def test_privilege_escalation(self) -> AuthorizationTestResult:
        ...
        for directory in [endpoints_dir, services_dir]:
            ...
            for file_path in directory.glob("*.py"):
                ...
                if re.search(r'(change_role|update_role|set_role|make_admin|promote)', content, re.IGNORECASE):
                    # Check if properly protected
                    func_context = content

                    if 'get_current_user' in func_context:
                        # Check for admin verification
                        if 'is_admin' in func_context or 'admin' in func_context.lower():
                            findings.append(f"Role change found in {file_path.name} with admin check")
                        else:
                            findings.append(f"Role change in {file_path.name} may lack admin verification")
                            recommendations.append("Verify admin role before allowing role changes")
                            severity = "high"
                            status = "fail"
                    else:
                        findings.append(f"Role change in {file_path.name} lacks authentication")
                        recommendations.append("Add authentication to role change operations")
                        severity = "critical"
                        status = "fail"

                # Check for permission modification
                if re.search(r'(add_permission|remove_permission|grant|revoke)', content, re.IGNORECASE):
                    if 'is_admin' not in content:
                        findings.append(f"Permission modification in {file_path.name} may lack admin check")
                        recommendations.append("Require admin verification for permission changes")
                        severity = "high"
                        status = "warning"
            ...
            if 'role' in content.lower() or 'is_admin' in content.lower():
                ...
                if 'role = Column' in content or 'is_admin = Column' in content:
                    ...
                    if 'validator' not in content.lower():
                        findings.append("Role field may be directly mutable without validation")
                        recommendations.append("Add validators to prevent unauthorized role changes")
                        status = "warning"
    ...
    async def test_idor(self) -> AuthorizationTestResult:
        ...
        if endpoints_dir.exists():
            ...
            for endpoint_file in endpoints_dir.glob("*.py"):
                ...
                for route_match in routes:
                    ...
                    if route_method in ['put', 'delete', 'patch']:
                        ...
                        has_ownership_check = (
                            'owner' in route_context.lower() or
                            'created_by' in route_context.lower() or
                            'user_id' in route_context.lower() or
                            'organization_id' in route_context.lower()
                        )

                        has_auth_check = 'get_current_user' in route_context

                        if not has_auth_check:
                            findings.append(f"{route_method} {route_path} - no authentication")
                            idor_risk_count += 1
                            severity = "critical"
                            status = "fail"
                        elif not has_ownership_check and '{id}' in route_path:
                            findings.append(f"{route_method} {route_path} - may lack ownership check")
                            recommendations.append("Verify user owns resource before allowing access")
                            idor_risk_count += 1
            ...
            if idor_risk_count > 0:
                findings.append(f"Found {idor_risk_count} potential IDOR vulnerabilities")
            else:
    ...
    async def test_api_key_leakage(self) -> AuthorizationTestResult:
            ...
            for endpoint_file in endpoints_dir.glob("*.py"):
                ...
                if 'raise HTTPException' in content or 'raise Exception' in content:
                    ...
                    if re.search(r'HTTPException.*?\{.*?\}', content):
                        findings.append(f"Error responses may include variable data in {endpoint_file.name}")
                        recommendations.append("Sanitize error messages to prevent data leakage")
                        status = "warning"
        ...
        for log_file in project_files[:50]:
            ...
            try:
                ...
                if re.search(r'logger\.(info|debug|error).*?(key|token|secret|password)', content, re.IGNORECASE):
                    findings.append(f"Potential sensitive data logging in {log_file.name}")
        ...
        if endpoints_dir.exists():
            ...
            for endpoint_file in endpoints_dir.glob("*.py"):
                ...
                for route_match in admin_routes:
                    ...
                    has_admin_check = 'is_admin' in route_context or 'admin' in route_context.lower()

                    if not has_auth:
                        findings.append(f"Admin operation without auth in {endpoint_file.name}")
                        missing_auth_count += 1
                        severity = "critical"
                        status = "fail"
                    elif not has_admin_check:
                        findings.append(f"Admin operation without admin check in {endpoint_file.name}")
                        missing_auth_count += 1
                        severity = "high"
                        status = "warning"
                ...
                for route_match in org_routes:
                    ...
                    has_org_check = (
                        'organization_id' in route_context or
                        'member' in route_context.lower() or
                        'belongs_to' in route_context.lower()
                    )

                    if not has_org_check and 'get_current_user' in route_context:
                        findings.append(f"Organization operation may lack org check in {endpoint_file.name}")
                        recommendations.append("Verify user belongs to organization before access")
                        missing_auth_count += 1
            ...
            if missing_auth_count > 0:
                findings.append(f"Found {missing_auth_count} operations with missing authorization")
            else:
        ...
        if admin_files:
            for admin_file in admin_files:
                ...
                if unprotected_admin_routes > 0:
                    findings.append(f"Found {unprotected_admin_routes} unprotected admin routes in {admin_file.name}")
                    recommendations.append("Add admin verification to all admin endpoints")
                else:
                    findings.append(f"Admin routes in {admin_file.name} appear protected")
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for test_name, test_method in test_methods:
            ...
            try:
                ...
                self.results.append(AuthorizationTestResult(
                    category=test_name,
                    test_name=test_name,
                    severity="error",
                    status="error",
...
async def main():
    ...
    # Save report to JSON
    output_file = project_root / f"authorization_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

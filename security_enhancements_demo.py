...
from app.core.security_monitoring import security_monitor, SecurityAlert
from app.core.failed_login_tracker import failed_login_tracker, LockoutReason
from app.core.siem_integration import siem_integration, send_security_event
...
async def demo_failed_login_tracking():
    ...
    print(f"⏱️  Lockout duration: {int(failed_login_tracker.lockout_duration.total_seconds() / 60)} minutes")
    ...
    for i in range(1, failed_login_tracker.max_attempts + 2):
        ...
        if lockout_info and lockout_info.is_locked:
            ...
            if lockout_info:
                ...
                print(f"   Attempt {i}: Failed ({attempts}/{failed_login_tracker.max_attempts}) {warning}")
            else:
                print(f"   Attempt {i}: Failed ({attempts}/{failed_login_tracker.max_attempts})")
...
async def demo_siem_integration():
    ...
    print(f"   Status: {'✅ Connected' if test_result['success'] else '⚠️  Not configured'}")

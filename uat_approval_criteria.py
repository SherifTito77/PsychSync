...
class UATApprovalCriteriaGenerator:
    ...
    def evaluate_criteria(self, criteria: List[ApprovalCriterion],
                         test_results: Dict[str, Any]) -> Tuple[float, ApprovalStatus, List[str]]:
        ...
        for criterion in criteria:
            ...
            if criterion_score < 70 and criterion.approval_level == ApprovalLevel.CRITICAL:
            ...
            if 70 <= criterion_score < 85:
                conditions.append(f"Improve {criterion.title} to achieve better outcomes")
    ...
    def _calculate_criterion_score(self, criterion: ApprovalCriterion,
    ...
    def generate_approval_package(self, criteria: List[ApprovalCriterion],
                                test_results: Dict[str, Any],
                                stakeholder_inputs: Dict[str, Any]) -> UATApprovalPackage:
        """Generate complete approval package"""

        overall_score, approval_status, conditions = self.evaluate_criteria(criteria, test_results)
        ...
        for stakeholder in StakeholderGroup:
            stakeholder_signoffs[stakeholder.value] = {
                ...
                "concerns": [] if overall_score >= 85 else ["Minor improvements needed"]
            }
    ...
    def generate_approval_report(self, package: UATApprovalPackage) -> Dict[str, Any]:
        ...
        report = {
            ...
            "executive_summary": {
                ...
                "critical_criteria": len([c for c in package.criteria if c.approval_level == ApprovalLevel.CRITICAL]),
                "stakeholder_coverage": len(package.stakeholder_signoffs),
                "approval_likelihood": "HIGH" if package.overall_score >= 85 else "MEDIUM" if package.overall_score >= 75 else "LOW"
            },
            "criteria_analysis": {},
            "stakeholder_analysis": {},
            "risk_assessment": {
                "high_risks": package.risks,
                "risk_mitigation": []
            },
            "implementation_readiness": {
                "ready_for_production": package.recommendation == ApprovalStatus.APPROVED,
...
def main():
    ...
    for level, level_criteria in criteria_by_level.items():
        ...
        for criterion in level_criteria:
            ...
            print(f"     Stakeholder: {criterion.stakeholder_group.value} | Weight: {criterion.weight:.2f}")
    ...
    for stakeholder, stakeholder_criteria in criteria_by_stakeholder.items():
        ...
        top_criteria = sorted(stakeholder_criteria, key=lambda x: x.weight, reverse=True)[:2]
        for criterion in top_criteria:
            print(f"   • {criterion.criterion_id}: {criterion.title} (Weight: {criterion.weight:.2f})")
    ...
    approval_package = generator.generate_approval_package(criteria, test_results, stakeholder_inputs)
    ...
    output_file = f"uat_approval_package_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

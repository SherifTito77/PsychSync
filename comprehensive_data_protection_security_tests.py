...
class DataProtectionSecurityTester:
    ...
    async def test_encryption_at_rest(self) -> DataProtectionTestResult:
        ...
        if models_dir.exists():
            for model_file in models_dir.glob("*.py"):
                ...
                sensitive_fields = ['password', 'ssn', 'credit_card', 'token', 'secret', 'key']
                for field in sensitive_fields:
                    if field in content.lower():
                        # Check if encrypted
                        if 'encrypted' in content.lower() or 'hash' in content.lower():
                            findings.append(f"Sensitive field '{field}' appears protected in {model_file.name}")
                        else:
                            findings.append(f"Sensitive field '{field}' in {model_file.name} - verify encryption")
                            status = "warning"

        # Check for backup encryption
        backup_files = list(self.project_root.rglob("*.sql")) + list(self.project_root.rglob("*.backup"))
        if backup_files:
            encrypted_backups = [f for f in backup_files if '.enc' in str(f) or 'password' in str(f)]

            findings.append(f"Found {len(backup_files)} backup files")
            if encrypted_backups:
                findings.append(f"Found {len(encrypted_backups)} encrypted backups (good)")
            else:
            ...
            for schema_file in schemas_dir.glob("*.py"):
                ...
                if 'password' in content.lower():
                    ...
                    if 'exclude' in content or 'write_only' in content:
                        findings.append(f"Password field properly protected in {schema_file.name}")
                    else:
                        findings.append(f"Password field may be exposed in {schema_file.name}")
                        recommendations.append("Use exclude=True or write_only=True for password fields")
                        severity = "high"
                        status = "warning"
        ...
        if endpoints_dir.exists():
            for endpoint_file in endpoints_dir.glob("*.py"):
                ...
                if 'return.*User' in content or 'response.*user' in content:
                    findings.append(f"{endpoint_file.name} may return full user objects")
                    recommendations.append("Use response schemas to filter sensitive fields")
                    status = "warning"
        ...
        if auth_service.exists():
            ...
            for algo in weak_algos:
                if algo in content.lower():
                    ...
                    recommendations.append(f"Replace {algo.upper()} with bcrypt or argon2")
                    severity = "critical"
                    status = "fail"
    ...
    async def test_pii_handling(self) -> DataProtectionTestResult:
        ...
        if not gdpr_found:
            ...
            recommendations.append("Implement GDPR compliance endpoints (delete, export, anonymize)")
            status = "warning"
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for test_name, test_method in test_methods:
            ...
            try:
                ...
                self.results.append(DataProtectionTestResult(
                    category=test_name,
                    test_name=test_name,
                    severity="error",
                    status="error",
...
async def main():
    ...
    # Save report to JSON
    output_file = project_root / f"data_protection_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

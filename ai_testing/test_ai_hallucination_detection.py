...
class AIHallucinationDetector:
        ...
        scenarios.append(
            {
                ...
                "ground_truth": {"team_performance": 85, "improvement_potential": 15},
                "ai_output": """
        ...
        scenarios.append(
            {
                ...
                "ground_truth": {"team_members": ["INTJ", "ENFP"]},
                "ai_output": """
        ...
        scenarios.append(
            {
                ...
                "ground_truth": {"team_cohesion": "high"},
                "ai_output": """
        ...
        scenarios.append(
            {
                ...
                "ground_truth": {"variables": ["personality", "performance"]},
                "ai_output": """
    ...
    def detect_factual_hallucinations(
        self, ai_output: str, ground_truth: Dict[str, Any]
        ...
        valid_enneagram = set(
            self.knowledge_base["valid_personality_types"]["enneagram"]
        )
        ...
        enneagram_matches = re.finditer(
            r"\bType\s*([1-9][0-9]*|[0])\b", ai_output, re.IGNORECASE
        )
    ...
    def detect_invented_data(
        ...
        framework_pattern = (
            r"\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+(?:framework|assessment|test|index)"
        )
    ...
    def detect_logical_inconsistencies(
        self, ai_output: str
    ) -> List[HallucinationDetection]:
    ...
    def calculate_hallucination_score(
        ...
        weighted_score = sum(
            severity_weights[h.severity] * h.confidence_score for h in hallucinations
        ) / len(hallucinations)
    ...
    async def test_hallucination_detection(
        ...
        for scenario in scenarios:
            ...
            all_hallucinations = (
                factual_hallucinations + contradictions + invented_data + logical_issues
            )
            ...
            factual_accuracy_score = self.calculate_factual_accuracy_score(
                unique_hallucinations
            )
            ...
            if hallucination_score > 0.7:
                recommendations.append(
                    "Critical hallucination issues detected - review model training data"
                )
            elif hallucination_score > 0.4:
                recommendations.append(
                    "Moderate hallucination risk - implement output validation"
                )
            elif hallucination_score > 0.2:
                recommendations.append(
                    "Minor hallucination risk - enhance fact-checking mechanisms"
                )
            else:
                recommendations.append(
                    "Low hallucination risk - current safeguards are effective"
                )

            # Specific recommendations based on detected issues
            if any(
                h.hallucination_type == HallucinationType.IMPOSSIBLE_METRICS
                for h in unique_hallucinations
            ):
                recommendations.append("Implement metric validation constraints")

            if any(
                h.hallucination_type == HallucinationType.FACTUAL_INCORRECTNESS
                for h in unique_hallucinations
            ):
                recommendations.append("Enhance knowledge base with verified facts")

            if any(
                h.hallucination_type == HallucinationType.CONTRADICTORY_STATEMENTS
                for h in unique_hallucinations
            ):
            ...
            print(
                f"   ⚠️  Expected vs Actual: {scenario['expected_hallucinations']} vs {len(unique_hallucinations)}"
            )
    ...
    async def run_comprehensive_hallucination_tests(self) -> Dict[str, Any]:
        ...
        avg_hallucination_score = statistics.mean(
            [r.overall_hallucination_score for r in results]
        )
        avg_factual_accuracy = statistics.mean(
            [r.factual_accuracy_score for r in results]
        )
        ...
        high_risk_results = [
            r for r in results if 0.4 < r.overall_hallucination_score <= 0.7
        ]
        ...
        if avg_factual_accuracy >= 0.8:
            recommendations.append(
                "✅ Excellent factual accuracy - AI outputs are reliable"
            )
        elif avg_factual_accuracy >= 0.6:
            recommendations.append("⚠️ Good factual accuracy with room for improvement")
        else:
            recommendations.append(
                "❌ Significant factual accuracy issues - immediate action required"
            )
        ...
        report = {
            ...
            "risk_assessment": {
                ...
                "low_risk_scenarios": len(results)
            ...
            "detailed_results": [
                {
                    ...
                    "hallucination_types": list(
                        set(
                            h.hallucination_type.value
                            for h in result.hallucinations_detected
                        )
                    ),
                }
            ...
            "quality_metrics": {
                ...
                "high_severity_rate": total_severity_distribution.get("critical", 0)
                / max(1, total_hallucinations),
                "consistency_issues": total_severity_distribution.get("low", 0)
                + total_severity_distribution.get("medium", 0),
            },
        }

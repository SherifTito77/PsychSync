...
class PIIEncryptionMigrator:
    ...
    PII_FIELDS_TO_MIGRATE = [
        ...
        FieldMigration("users", "email", "CITEXT", is_pii=True, priority="CRITICAL",
                      notes="Need hashed version for indexing"),
        FieldMigration("users", "full_name", "String", is_pii=True, priority="HIGH",
                      notes="Encrypted version exists in user_secure"),
        FieldMigration("users", "phone_number", "String", is_pii=True, priority="HIGH",
                      notes="Encrypted version exists in user_secure"),
        FieldMigration("users", "address", "Text", is_pii=True, priority="HIGH"),

        # Organization model (app/db/models/organization.py)
        FieldMigration("organizations", "display_name", "String", is_pii=False, priority="MEDIUM",
                      notes="May contain sensitive info"),
        FieldMigration("organizations", "address", "Text", is_pii=True, priority="HIGH"),
        FieldMigration("organizations", "phone", "String", is_pii=True, priority="HIGH"),
        FieldMigration("organizations", "email", "String", is_pii=True, priority="HIGH"),

        # Slack workspace (app/db/models/slack_workspace.py)
        FieldMigration("slack_workspaces", "token", "Text", is_pii=True, priority="CRITICAL",
                      notes="OAuth tokens should be encrypted"),

        # Email connection (app/db/models/email_connection.py)
        FieldMigration("email_connections", "token", "Text", is_pii=True, priority="CRITICAL",
                      notes="OAuth tokens should be encrypted"),
        FieldMigration("email_connections", "email", "String", is_pii=True, priority="HIGH"),
        FieldMigration("email_connections", "address", "String", is_pii=True, priority="HIGH"),
    ]
    ...
    def scan_models_for_unencrypted_pii(self) -> List[Dict]:
        ...
        pii_keywords = [
            'email', 'phone', 'address', 'full_name', 'name',
            'token', 'secret', 'password', 'ssn', 'social_security',
            'credit_card', 'birth', 'dob', 'location'
        ]
...
'''
    ...
    def generate_summary_report(self) -> Dict:
        ...
        return {
            ...
            "fields_by_priority": {
                "CRITICAL": [f for f in self.PII_FIELDS_TO_MIGRATE if f.priority == "CRITICAL"],
                "HIGH": [f for f in self.PII_FIELDS_TO_MIGRATE if f.priority == "HIGH"],
                "MEDIUM": [f for f in self.PII_FIELDS_TO_MIGRATE if f.priority == "MEDIUM"],
            },
            "summary": {
                "critical": sum(1 for f in self.PII_FIELDS_TO_MIGRATE if f.priority == "CRITICAL"),
                "high": sum(1 for f in self.PII_FIELDS_TO_MIGRATE if f.priority == "HIGH"),
                "medium": sum(1 for f in self.PII_FIELDS_TO_MIGRATE if f.priority == "MEDIUM"),
            }
        }
...
def main():
    ...
    parser.add_argument("--plan", action="store_true", help="Generate SQL migration plan")
    parser.add_argument("--alembic", action="store_true", help="Generate Alembic migration file")
    parser.add_argument("--scan", action="store_true", help="Scan models for unencrypted PII")
    ...
    if args.scan:
        ...
        migration_file = migrator.migrations_dir / f"{migration_id}_encrypt_pii_fields.py"

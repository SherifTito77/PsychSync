...
class InternalAPISecurityTester:
    ...
    def discover_api_endpoints(self) -> Dict[str, Any]:
            ...
            if file_path.exists():
                ...
                try:
                    ...
                    if "@app.route(" in content and "methods=" not in content:
                        result["security_issues"].append(f"Missing HTTP method restrictions in {api_file}")

                    if "cors_origins=" in content and "*" in content:
                        result["security_issues"].append(f"CORS configured to allow all origins in {api_file}")
        ...
        for endpoint in all_endpoints:
            ...
            if any(keyword in endpoint_lower for keyword in ["admin", "config", "system", "debug", "health"]):
                admin_endpoints.append(endpoint)
            elif any(keyword in endpoint_lower for keyword in ["users", "data", "internal", "manage"]):
    ...
    def test_api_access_controls(self) -> Dict[str, Any]:
        ...
        for endpoint in test_endpoints:
            ...
            try:
                ...
                if response.status_code == 401:
                    ...
                    result["vulnerabilities"].append(f"Unauthenticated access allowed: {endpoint}")
            ...
            for endpoint in test_endpoints:
                try:
                    ...
                    test_result = {
                        ...
                        "authorized": response.status_code not in scenario["expected_status"],
                        "vulnerable": response.status_code not in scenario["expected_status"]
                    }

                    scenario_result["test_results"].append(test_result)

                    if test_result["vulnerable"]:
                        result["vulnerabilities"].append(f"Access control bypass: {endpoint} with {scenario['name']}")
        ...
        result["risk_level"] = "HIGH" if len(result["vulnerabilities"]) > 5 else "MEDIUM"
        ...
        for endpoint in internal_endpoints:
            ...
            try:
                ...
                if response.status_code == 200:
                    result["vulnerabilities"].append(f"Internal endpoint exposed: {endpoint}")
            ...
            try:
                ...
                security_headers = {
                    ...
                    "cors_restricted": "Access-Control-Allow-Origin" not in response.headers,
                    "secure_headers": response.headers.get("X-Frame-Options") is not None
                }
                exposure_result["security_measures"] = security_headers

                if not security_headers["auth_required"]:
                    result["vulnerabilities"].append(f"Missing authentication for {endpoint}")
    ...
    def test_api_rate_limiting(self) -> Dict[str, Any]:
        ...
        for endpoint in rate_limit_endpoints:
            ...
            try:
                ...
                rate_limit_result["requests_per_second"] = requests_made / (time.time() - start_time) if time.time() > start_time else 0

                if not rate_limit_result["rate_limit_detected"] and requests_made >= 10:
                    result["vulnerabilities"].append(f"No rate limiting detected: {endpoint} ({requests_made} requests made)")

            except requests.exceptions.RequestException:
                result["vulnerabilities"].append(f"Rate limiting test failed for {endpoint}")
                ...
                try:
                    ...
                    for pattern, description in insecure_patterns:
                        if re.search(pattern, content, re.IGNORECASE):
                            result["vulnerabilities"].append(f"Insecure CORS setting in {cors_file}: {description}")

                except Exception as e:
                    result["vulnerabilities"].append(f"Error analyzing CORS in {cors_file}: {e}")
    ...
    def generate_api_security_recommendations(self, test_results: List[Dict]) -> List[Dict]:
...
def main():
    ...
    try:
        ...
        print(f"🎯 Overall API Security Score: {summary['overall_api_security_score']}/100")
        ...
        if endpoint_discovery:
            ...
            print(f"   Admin Endpoints: {len(endpoint_discovery.get('admin_endpoints', []))}")
            print(f"   Internal Endpoints: {len(endpoint_discovery.get('internal_endpoints', []))}")
            print(f"   External Endpoints: {len(endpoint_discovery.get('external_endpoints', []))}")
        ...
        with open("/Users/sheriftito/Downloads/psychsync/internal_api_security_report.json", "w") as f:

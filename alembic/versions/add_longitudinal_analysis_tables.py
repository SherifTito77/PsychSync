...
def upgrade():
    ...
    op.create_table(
        ...
        ),  # count, rate, duration, score
        sa.Column(
            "time_bucket", sa.DateTime(timezone=True), nullable=False
        ),  # hour/day/week bucket
        sa.Column(
            "bucket_size", sa.String(20), nullable=False
        ),  # hour, day, week, month
        ...
        sa.Index(
            "idx_behavioral_time_series_metric_time", "metric_name", "time_bucket"
        ),
        sa.Index(
            "idx_behavioral_time_series_org_time", "organization_id", "time_bucket"
        ),
        sa.UniqueConstraint(
            "user_id",
            "metric_name",
            "time_bucket",
            "bucket_size",
            name="uq_behavioral_time_series",
        ),
    )
    ...
    op.create_table(
        ...
        ),  # trend, level, variance, pattern
        sa.Column(
            "detection_method", sa.String(50), nullable=False
        ),  # cusum, edivisive, bayesian, ml
        ...
        sa.Column(
            "impact_level", sa.String(20), nullable=False
    ...
    op.create_table(
        ...
        sa.Index(
            "idx_trend_analysis_period", "analysis_period_start", "analysis_period_end"
        ),
        sa.UniqueConstraint(
            "user_id",
            "metric_name",
    ...
    op.create_table(
        ...
        sa.Column("metric_name", sa.String(100), nullable=False),
        sa.Column(
            "baseline_type", sa.String(50), nullable=False
        ),  # personal, peer_group, organizational
    ...
    op.create_table(
        ...
        sa.Column(
            "cohort_definition", postgresql.JSONB(), nullable=False
        ),  # criteria for cohort membership
        sa.Column(
            "cohort_type", sa.String(50), nullable=False
        ),  # static, dynamic, time_based
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            server_default=sa.text("now()"),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "cohort_id", "user_id", "joined_at", name="uq_cohort_memberships"
        ),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "analysis_type", sa.String(50), nullable=False
        ),  # cohort_comparison, time_comparison, ab_test
        sa.Column("control_group", postgresql.JSONB(), nullable=True),
        sa.Column("treatment_group", postgresql.JSONB(), nullable=True),
        sa.Column(
            "comparison_period_start", sa.DateTime(timezone=True), nullable=False
        ),
        ...
        sa.Index(
            "idx_comparative_analysis_period",
    ...
    op.create_table(
        ...
        sa.Column(
            "retention_periods", postgresql.JSONB(), nullable=False
        ),  # Period-by-period retention data
    ...
    op.execute(
        "COMMENT ON TABLE behavioral_time_series IS 'Time series data for behavioral metrics analysis'"
    )
    op.execute(
        "COMMENT ON TABLE change_detection_events IS 'Detected changes in behavioral patterns over time'"
    )
    op.execute(
        "COMMENT ON TABLE trend_analysis IS 'Statistical trend analysis results for user metrics'"
    )
    op.execute(
        "COMMENT ON TABLE behavioral_baselines IS 'Baseline metrics for behavioral comparison'"
    )
    op.execute(
        "COMMENT ON TABLE longitudinal_cohorts IS 'Definition of user cohorts for longitudinal analysis'"
    )
    op.execute(
        "COMMENT ON TABLE cohort_memberships IS 'User membership in longitudinal cohorts'"
    )
    op.execute(
        "COMMENT ON TABLE comparative_analysis IS 'Results of comparative behavioral analysis'"
    )
    op.execute(
        "COMMENT ON TABLE retention_analysis IS 'User retention and churn analysis over time'"
    )

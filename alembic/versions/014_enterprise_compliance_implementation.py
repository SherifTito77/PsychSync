...
def upgrade() -> None:
    ...
    op.add_column(
        "users", sa.Column("mfa_enabled", sa.Boolean(), nullable=False, default=False)
    )
    ...
    op.add_column(
        "users",
        sa.Column("gdpr_consent_given", sa.Boolean(), nullable=False, default=False),
    )
    op.add_column(
        "users",
        sa.Column(
            "data_processing_consent", sa.Boolean(), nullable=False, default=False
        ),
    )
    op.add_column(
        "users",
    ...
    op.add_column(
        "users",
        sa.Column("security_question_hash", sa.String(length=255), nullable=True),
    )
    ...
    op.add_column(
        "assessments",
        sa.Column("retention_schedule", sa.DateTime(timezone=True), nullable=True),
    )
    op.add_column(
        "assessments",
    ...
    op.add_column(
        ...
        sa.Column("response_encrypted", sa.Boolean(), nullable=False, default=False),
    )
    op.add_column(
        "assessment_responses",
    ...
    op.create_index(
        "idx_assessments_classification", "assessments", ["data_classification"]
    )
    ...
    op.create_index(
        "idx_assessment_responses_encrypted",
        "assessment_responses",
        ["response_encrypted"],
    )
    ...
    if op.get_bind().dialect.name == "postgresql":
        ...
        for month_offset in range(-2, 4):  # Past 2 months to next 3 months
            month_name = datetime.now().replace(day=1) + timedelta(

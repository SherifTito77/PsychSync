...
def upgrade():
    ...
    op.create_table(
        'analytics_insights',
        sa.Column('id', sa.UUID(), nullable=False),
        sa.Column('insight_type', sa.String(50), nullable=False),  # user, assessment, team, system
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('metric_name', 'dimension', 'date_key', 'organization_id', 'team_id', name='uq_time_series_record')
    )
    ...
    op.create_table(
        ...
        sa.Column('metric_unit', sa.String(20), nullable=True),
        sa.Column('period_type', sa.String(20), nullable=False),  # daily, weekly, monthly, yearly
    ...
    op.execute("COMMENT ON TABLE analytics_events IS 'Tracks all user interaction events for analytics'")
    op.execute("COMMENT ON TABLE dashboard_metrics_cache IS 'Cache table for dashboard metric calculations'")
    op.execute("COMMENT ON TABLE user_engagement IS 'Daily engagement metrics per user'")
    op.execute("COMMENT ON TABLE system_metrics IS 'System performance and health metrics'")
    op.execute("COMMENT ON TABLE analytics_insights IS 'AI-generated insights from analytics data'")
    op.execute("COMMENT ON TABLE time_series_data IS 'Aggregated time series data for fast dashboard queries'")
    op.execute("COMMENT ON TABLE business_metrics IS 'Business KPI and financial metrics'")

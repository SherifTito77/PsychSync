...
def upgrade():
    ...
    op.create_table(
        ...
        # Screening metadata
        sa.Column('screening_type', sa.String(50), nullable=False, comment='PHQ9, GAD7, CSSRS, etc.'),
        ...
        sa.Column('severity_level', sa.String(50), comment='minimal, mild, moderate, moderately_severe, severe'),
        sa.Column('subscale_scores', postgresql.JSONB, comment='Multi-dimensional scores'),
    ...
    op.create_index('idx_clinical_user_type', 'clinical_screenings', ['user_id', 'screening_type'])
    op.create_index('idx_clinical_crisis', 'clinical_screenings', ['crisis_alert', 'created_at'])
    op.create_index('idx_clinical_org_type', 'clinical_screenings', ['org_id', 'screening_type'])
    op.create_index('idx_clinical_risk_level', 'clinical_screenings', ['risk_level', 'created_at'])

    # Create clinical_alerts table
    op.create_table(
        'clinical_alerts',
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), primary_key=True),
        sa.Column('screening_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('clinical_screenings.id', ondelete='CASCADE')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('org_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('organizations.id', ondelete='CASCADE'), nullable=False),

        # Alert details
        sa.Column('alert_type', sa.String(50), nullable=False, comment='suicide_risk, self_harm, severe_symptoms'),
        sa.Column('severity', sa.String(20), nullable=False, comment='moderate, high, critical'),
        ...
        # Resolution
        sa.Column('resolution_status', sa.String(50), server_default='pending', comment='pending, in_progress, resolved, escalated'),
        ...
        sa.Column('escalation_level', sa.String(50), comment='supervisor, clinical_team, emergency_services'),
    ...
    op.create_index('idx_alerts_unresolved', 'clinical_alerts', ['resolution_status', 'created_at'])
    op.create_index('idx_alerts_critical', 'clinical_alerts', ['severity', 'acknowledged'])
    op.create_index('idx_alerts_org', 'clinical_alerts', ['org_id', 'resolution_status'])
    ...
    op.create_table(
        ...
        sa.Column('id', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), primary_key=True),
        sa.Column('screening_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('clinical_screenings.id', ondelete='CASCADE')),
        sa.Column('alert_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('clinical_alerts.id')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id', ondelete='CASCADE'), nullable=False),
        sa.Column('org_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('organizations.id', ondelete='CASCADE'), nullable=False),

        # Referral details
        sa.Column('referral_type', sa.String(100), nullable=False, comment='therapist, psychiatrist, EAP, crisis_line'),
        sa.Column('urgency', sa.String(20), nullable=False, comment='routine, urgent, emergency'),
        ...
        # Status
        sa.Column('status', sa.String(50), server_default='pending', comment='pending, contacted, scheduled, completed, declined'),
    ...
    op.create_index('idx_referrals_pending', 'clinical_referrals', ['status', 'urgency', 'created_at'])
    ...
    op.create_table(
        ...
        # What was accessed
        sa.Column('entity_type', sa.String(50), nullable=False, comment='screening, alert, referral'),
        sa.Column('entity_id', postgresql.UUID(as_uuid=True), nullable=False),

        # Who accessed it
        sa.Column('accessed_by', postgresql.UUID(as_uuid=True), sa.ForeignKey('users.id'), nullable=False),
        sa.Column('user_role', sa.String(50), nullable=False),

        # Action details
        sa.Column('action', sa.String(50), nullable=False, comment='view, create, update, delete, export'),
        ...
        # Authorization
        sa.Column('authorization_method', sa.String(50), comment='role_based, explicit_consent, emergency_access'),
    ...
    op.create_index('idx_audit_entity', 'clinical_audit_logs', ['entity_type', 'entity_id'])
    op.create_index('idx_audit_user', 'clinical_audit_logs', ['accessed_by', 'created_at'])
    ...
    op.create_table(
        ...
        # Consent details
        sa.Column('consent_type', sa.String(50), nullable=False, comment='screening, data_sharing, referral, crisis_contact'),
        sa.Column('consent_version', sa.String(20), nullable=False, comment='Track consent form versions'),

        # Consent status
        sa.Column('consented', sa.Boolean, server_default='false'),
        sa.Column('consent_text', sa.Text, nullable=False, comment='Full consent language shown to user'),

        # Consent scope
        sa.Column('screening_types', postgresql.JSONB, comment='Which assessments are covered'),
        sa.Column('data_sharing_scope', postgresql.JSONB, comment='What data can be shared, with whom'),
    ...
    op.create_index('idx_consent_user_type', 'clinical_consents', ['user_id', 'consent_type', 'consented'])
    op.create_index('idx_consent_expiry', 'clinical_consents', ['expires_at'])

    # Add comments for documentation
    op.execute("COMMENT ON TABLE clinical_screenings IS 'HIPAA: Stores PHI for mental health screening assessments'")
    op.execute("COMMENT ON TABLE clinical_alerts IS 'HIPAA: Crisis alerts requiring immediate clinical response'")
    op.execute("COMMENT ON TABLE clinical_referrals IS 'HIPAA: Referrals to mental health professionals'")
    op.execute("COMMENT ON TABLE clinical_audit_logs IS 'HIPAA: Immutable audit trail - 6 year minimum retention'")
    op.execute("COMMENT ON TABLE clinical_consents IS 'HIPAA: Explicit consent tracking for clinical assessments'")

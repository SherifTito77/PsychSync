...
def upgrade() -> None:
    ...
    op.create_table('ab_conversions',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=True),
        sa.Column('experiment_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('variant_id', postgresql.UUID(as_uuid=True), nullable=False),
    ...
    op.create_index('idx_ab_conversions_experiment', 'ab_conversions', ['experiment_id'])
    ...
    op.create_index('idx_feature_requests_search', 'feature_requests', ['search_vector'], postgresql_using='gin')
    op.create_index('idx_feature_requests_customer', 'feature_requests', ['customer_id'])

    # Feature request relations table
    op.create_table('feature_request_relations',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        ...
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.ForeignKeyConstraint(['parent_request_id'], ['feature_requests.id'], ondelete='CASCADE'),
        sa.ForeignKeyConstraint(['child_request_id'], ['feature_requests.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index('idx_feature_votes_request', 'feature_request_votes', ['feature_request_id'])
    ...
    op.create_table('churn_risk_scores',
        ...
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        sa.CheckConstraint('overall_score BETWEEN 0 AND 100', name='check_overall_score_range')
    )
    ...
    op.create_table('churn_trigger_executions',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False),
    ...
    op.create_index('idx_churn_trigger_date', 'churn_trigger_executions', ['executed_at'])

    # Churn trigger cooldowns table
    op.create_table('churn_trigger_cooldowns',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('trigger_name', sa.String(length=100), nullable=False),
    ...
    op.create_table('user_activation',
        sa.Column('id', postgresql.UUID(as_uuid=True), nullable=False, server_default=sa.text('gen_random_uuid()')),
        sa.Column('user_id', postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column('segment', sa.String(length=50), nullable=False, server_default='individual_free'),
        # Timestamps
        sa.Column('signup_timestamp', sa.DateTime(timezone=True), nullable=False),
        sa.Column('first_assessment_timestamp', sa.DateTime(timezone=True), nullable=True),
        sa.Column('first_results_viewed_timestamp', sa.DateTime(timezone=True), nullable=True),
        ...
        sa.Column('first_invite_sent_timestamp', sa.DateTime(timezone=True), nullable=True),
        sa.Column('first_invite_accepted_timestamp', sa.DateTime(timezone=True), nullable=True),
        # Premium-specific
        sa.Column('upgraded_to_premium', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('upgrade_timestamp', sa.DateTime(timezone=True), nullable=True),
        # Enterprise-specific
        sa.Column('completed_onboarding', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('onboarding_completed_timestamp', sa.DateTime(timezone=True), nullable=True),
        sa.Column('configured_sso', sa.Boolean(), nullable=False, server_default='false'),
        # Metadata
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.Column('updated_at', sa.DateTime(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
    ...
    op.create_index('idx_user_activation_is_activated', 'user_activation', ['is_activated'])
    op.create_index('idx_user_activation_segment', 'user_activation', ['segment'])
    op.create_index('idx_user_activation_timestamp', 'user_activation', ['signup_timestamp'])

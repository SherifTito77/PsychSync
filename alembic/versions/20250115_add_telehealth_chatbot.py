...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            comment="Requires explicit consent",
        ),
        ...
        sa.Column(
            "diagnosis_codes", postgresql.ARRAY(sa.String(10)), comment="ICD-10 codes"
        ),
        sa.Column("treatment_plan", sa.Text),
        sa.Column("follow_up_recommendations", sa.Text),
        # Cancellation/rescheduling
        sa.Column(
            "cancelled_by", postgresql.UUID(as_uuid=True), sa.ForeignKey("users.id")
        ),
    ...
    op.create_index(
        "idx_telehealth_user_status", "telehealth_sessions", ["user_id", "status"]
    )
    op.create_index(
        "idx_telehealth_clinician_schedule",
        "telehealth_sessions",
        ["clinician_id", "scheduled_time"],
    )
    op.create_index(
        "idx_telehealth_org", "telehealth_sessions", ["org_id", "created_at"]
    )
    op.create_index(
        "idx_telehealth_upcoming", "telehealth_sessions", ["scheduled_time", "status"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            sa.String(100),
            nullable=False,
            comment="Unique conversation session",
        ),
        sa.Column(
            "conversation_number",
            sa.Integer,
            comment="Sequential conversation for user",
        ),
        # Message details
        sa.Column(
            "message_text", sa.Text, nullable=False, comment="User or AI message"
        ),
        sa.Column(
            "is_user_message", sa.Boolean, nullable=False, comment="True=user, False=AI"
        ),
        sa.Column("message_sequence", sa.Integer, comment="Order in conversation"),
        # AI response metadata
        sa.Column(
            "ai_model_used", sa.String(100), comment="gpt-4, gpt-3.5-turbo, etc."
        ),
        sa.Column("ai_response", sa.Text, comment="AI generated response"),
        sa.Column(
            "confidence_score", sa.Numeric(5, 4), comment="AI confidence in response"
        ),
        sa.Column("tokens_used", sa.Integer, comment="OpenAI tokens consumed"),
        # Crisis detection (CRITICAL)
        sa.Column(
            "crisis_detected", sa.Boolean, server_default="false", nullable=False
        ),
        sa.Column(
            "crisis_level", sa.String(20), comment="low, moderate, high, emergency"
        ),
        sa.Column(
            "crisis_keywords", postgresql.ARRAY(sa.Text), comment="Triggering keywords"
        ),
        sa.Column("escalated_to_human", sa.Boolean, server_default="false"),
        sa.Column(
            "escalated_to_clinician_id",
            postgresql.UUID(as_uuid=True),
        ...
        sa.Column(
            "context_screening_type",
            sa.String(50),
            comment="Related clinical assessment",
        ),
        sa.Column("context_score", sa.Integer, comment="Related assessment score"),
        sa.Column(
            "suggested_resources", postgresql.JSONB, comment="Resources provided by AI"
        ),
        sa.Column(
            "user_sentiment",
            sa.String(20),
            comment="positive, neutral, negative, crisis",
        ),
        # RAG (Retrieval Augmented Generation)
        sa.Column(
            "rag_sources_used",
            postgresql.ARRAY(sa.String),
            comment="Knowledge base sources",
        ),
    ...
    op.create_index(
        "idx_chatbot_user_session", "chatbot_conversations", ["user_id", "session_id"]
    )
    op.create_index(
        "idx_chatbot_crisis", "chatbot_conversations", ["crisis_detected", "created_at"]
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "device_token", sa.String(500), nullable=False, comment="FCM or APNs token"
        ),
        ...
        sa.Column(
            ...
            postgresql.JSONB,
            comment="Custom notification settings",
        ),
    ...
    op.create_index(
        "idx_mobile_user_token", "mobile_devices", ["user_id", "device_token"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "assessments_by_type",
            postgresql.JSONB,
            comment="PHQ9: 150, GAD7: 120, etc.",
        ),
        # Risk distribution
        sa.Column(
            "risk_level_distribution",
            postgresql.JSONB,
            comment="low: 70%, moderate: 20%, etc.",
        ),
        # Crisis metrics

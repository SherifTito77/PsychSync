...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('resting_heart_rate', sa.Numeric(precision=5, scale=2), nullable=True),
        sa.Column('heart_rate_variability', sa.Numeric(precision=6, scale=2), nullable=True),
        ...
        sa.Column('blood_pressure_timestamp', sa.TIMESTAMP(timezone=True), nullable=True),
        ...
        sa.Column('sleep_quality_score', sa.Numeric(precision=3, scale=2), nullable=True),
        ...
        sa.Column('stress_body_indicator', sa.Numeric(precision=3, scale=2), nullable=True),
        ...
        sa.Column('body_temperature_celsius', sa.Numeric(precision=4, scale=2), nullable=True),
        ...
        sa.ForeignKeyConstraint(['organization_id'], ['organizations.id'], name=op.f('fk_biometric_health_data_organization_id')),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_biometric_health_data_user_id')),
        comment='Biometric health data from wearables and health devices'
    )

    # Create indexes for biometric_health_data
    op.create_index('idx_biometric_user_date', 'biometric_health_data', ['user_id', 'measurement_date'], unique=False)
    op.create_index('idx_biometric_org_date', 'biometric_health_data', ['organization_id', 'measurement_date'], unique=False)
    op.create_index('idx_biometric_source', 'biometric_health_data', ['data_source', 'measurement_date'], unique=False)
    op.create_index('idx_biometric_rhr', 'biometric_health_data', ['resting_heart_rate'], unique=False)
    op.create_index('idx_biometric_hrv', 'biometric_health_data', ['heart_rate_variability'], unique=False)
    op.create_index('idx_biometric_sleep', 'biometric_health_data', ['sleep_hours'], unique=False)
    ...
    op.create_table(
        ...
        sa.Column('biometric_collection', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('biometric_processing', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('biometric_sharing', sa.Boolean(), nullable=False, server_default='false'),
        sa.Column('research_participation', sa.Boolean(), nullable=False, server_default='false'),
        ...
        sa.Column('anonymization_allowed', sa.Boolean(), nullable=False, server_default='false'),
        ...
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.Column('updated_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
        sa.ForeignKeyConstraint(['organization_id'], ['organizations.id'], name=op.f('fk_health_data_consent_organization_id')),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], name=op.f('fk_health_data_consent_user_id')),
    ...
    op.create_index('idx_health_consent_org', 'health_data_consent', ['organization_id', 'consent_given'], unique=False)

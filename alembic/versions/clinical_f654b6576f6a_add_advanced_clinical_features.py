...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Constraints
        sa.CheckConstraint("assessment_type IN ('LSAS', 'EAT26', 'YBOCS', 'PHQ9', 'GAD7', 'ASRS', 'ISI')",
                          name='valid_assessment_type_extended'),
        sa.CheckConstraint("severity_level IN ('minimal', 'mild', 'moderate', 'moderately_severe', 'severe')",
    ...
    op.create_index('idx_clinical_ext_user_type', 'clinical_assessments_extended',
                    ['user_id', 'assessment_type', 'completed_at'])
    op.create_index('idx_clinical_ext_crisis', 'clinical_assessments_extended',
                    ['crisis_alert', 'completed_at'],
                    postgresql_where=sa.text('crisis_alert = true'))
    op.create_index('idx_clinical_ext_review', 'clinical_assessments_extended',
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.UniqueConstraint('user_id', 'assessment_type', name='uq_user_assessment_trend')
    )

    op.create_index('idx_trends_user_type', 'assessment_trends',
                    ['user_id', 'assessment_type'])
    op.create_index('idx_trends_worsening', 'assessment_trends',
                    ['trend_direction'],
    ...
    op.create_table(
        ...
        sa.CheckConstraint("session_type IN ('initial', 'follow_up', 'crisis', 'routine')",
                          name='valid_session_type'),
        sa.CheckConstraint("status IN ('scheduled', 'in_progress', 'completed', 'cancelled', 'no_show')",
                          name='valid_session_status')
    )

    op.create_index('idx_telehealth_user', 'telehealth_sessions',
                    ['user_id', 'scheduled_time'])
    op.create_index('idx_telehealth_clinician', 'telehealth_sessions',
                    ['clinician_id', 'scheduled_time'])
    op.create_index('idx_telehealth_upcoming', 'telehealth_sessions',
                    ['scheduled_time'],
    ...
    op.create_index('idx_chatbot_user_session', 'chatbot_conversations',
                    ['user_id', 'session_id', 'created_at'])
    op.create_index('idx_chatbot_crisis', 'chatbot_conversations',
                    ['crisis_detected', 'created_at'],
                    postgresql_where=sa.text('crisis_detected = true'))
    op.create_index('idx_chatbot_escalation', 'chatbot_conversations',
    ...
    op.create_index('idx_mobile_active', 'mobile_devices',
                    ['user_id', 'last_active'],
    ...
    op.add_column('users',
        sa.Column('clinician_license_number', sa.String(100))
    )
    op.add_column('users',
        sa.Column('clinician_specialization', postgresql.ARRAY(sa.String(100)))
    )
    op.add_column('users',
        sa.Column('clinician_bio', sa.Text)
    )
    op.add_column('users',
        sa.Column('available_for_crisis', sa.Boolean, server_default='false')
    )
    ...
    op.create_index('idx_pop_health_type_month', 'population_health_stats',
                    ['assessment_type', 'month'], unique=True)

...
class AIEngineTester:
    ...
    def test_all_mbti_types(self):
        ...
        for mbti_type in mbti_types:
            ...
            try:
                ...
                response = requests.post(f"{self.base_url}/personality-assessments/process-public",
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success'):
                        ...
                        if (personality == mbti_type and
                            len(description) > 10 and
                            "PsychSync AI" in processed_by and
                            len(insights) > 0):
                            self.log_result(f"MBTI {mbti_type}", True,
                                          f"{description[:30]}...", response_time)
                        else:
                            self.log_result(f"MBTI {mbti_type}", False,
                                          "Invalid response structure", response_time)
                    else:
                        self.log_result(f"MBTI {mbti_type}", False,
                                      "API returned success=false", response_time)
                else:
                    self.log_result(f"MBTI {mbti_type}", False,
    ...
    def test_enneagram_types(self):
        ...
        for ennea_type in enneagram_types:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success'):
                        ...
                        if personality == ennea_type and "PsychSync AI" in processed_by:
                            self.log_result(f"Enneagram {ennea_type}", True,
                                          "Processed successfully", response_time)
                        else:
                            self.log_result(f"Enneagram {ennea_type}", False,
                                          "Invalid response", response_time)
                    else:
                        self.log_result(f"Enneagram {ennea_type}", False,
                                      "API returned success=false", response_time)
                else:
                ...
                self.log_result(f"Enneagram {ennea_type}", False, f"Exception: {str(e)}", 0)
    ...
    def test_frameworks_endpoint(self):
        ...
        start_time = time.time()
        try:
            response = requests.get(f"{self.base_url}/personality-assessments/frameworks", timeout=10)
            response_time = (time.time() - start_time) * 1000
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    if total_frameworks >= 5 and access_level == 'public':
                        ...
                        self.log_result("Frameworks Endpoint", True,
                                      f"{total_frameworks} frameworks: {', '.join(framework_names[:3])}...",
                                      response_time)
                    else:
                        self.log_result("Frameworks Endpoint", False,
                                      "Invalid framework data", response_time)
                else:
                    self.log_result("Frameworks Endpoint", False,
                                  "API returned success=false", response_time)
            else:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success'):
                        ...
                        if len(insights) >= 3:
                            insight_quality = "Good" if all(len(insight) > 20 for insight in insights) else "Poor"
                            personality = result.get('type')
                            self.log_result(f"AI Insights Test {i+1} ({personality})", True,
                                          f"{len(insights)} insights, Quality: {insight_quality}", response_time)
                        else:
                            self.log_result(f"AI Insights Test {i+1}", False,
                                          "Insufficient insights", response_time)
                    else:
                        self.log_result(f"AI Insights Test {i+1}", False,
                                      "API returned success=false", response_time)
                else:
                ...
                self.log_result(f"AI Insights Test {i+1}", False, f"Exception: {str(e)}", 0)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('success') and data.get('results'):
                        self.log_result(f"Error Handling Test {i+1}", True,
                                      "Graceful error handling", response_time)
                    else:
                        self.log_result(f"Error Handling Test {i+1}", False,
                                      "Poor error handling", response_time)
                else:
                    self.log_result(f"Error Handling Test {i+1}", False,
                                  f"HTTP {response.status_code}", response_time)

            except Exception as e:
                self.log_result(f"Error Handling Test {i+1}", False, f"Exception: {str(e)}", 0)
        ...
        for i in range(10):
            ...
            try:
                ...
                if response.status_code != 200 or not response.json().get('success'):
                    self.log_result(f"Performance Test {i+1}", False, "Request failed", response_time)
                    return

            except Exception as e:
                self.log_result(f"Performance Test {i+1}", False, f"Exception: {str(e)}", 0)
                return
    ...
    def generate_report(self):
        ...
        report = {
            "summary": {
                ...
                "average_response_time_ms": round(avg_response_time, 2) if response_times else 0
            },

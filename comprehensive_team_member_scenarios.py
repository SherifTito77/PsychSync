...
class ComprehensiveTeamMemberScenarios:
    ...
    def _generate_all_scenarios(self) -> List[TeamMemberScenario]:
        ...
        scenarios.append(TeamMemberScenario(
            scenario_id="TM001",
        ...
        scenarios.append(TeamMemberScenario(
            ...
            user_role=UserRole.OWNER,
            member_email="very.long.email.address.that.exceeds.maximum.allowed.length.for.validation.testing.purposes@" + "x"*50 + ".com",
            member_role=UserRole.MEMBER,
            expected_result="VALIDATION_ERROR",
            test_steps=[
    ...
    async def _simulate_api_call(self, scenario: TeamMemberScenario, headers: Dict[str, str], payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if scenario.user_role == UserRole.ADMIN and scenario.member_role == UserRole.ADMIN:
            return {
                "status": "FORBIDDEN",
                "validation_results": {
        ...
        if scenario.scenario_id in ["TM012", "TM013"]:
            return {
                "status": "DUPLICATE_ERROR" if scenario.scenario_id == "TM012" else "INVITATION_EXISTS",
                "validation_results": {
    ...
    def _display_scenario_result(self, scenario: TeamMemberScenario, result: Dict[str, Any]):

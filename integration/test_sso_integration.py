...
class MockOAuthProvider:
            ...
            base_profile.update(
                {
                    ...
                    "family_name": (
                        user_id.split("_")[3]
                        if "_" in user_id and len(user_id.split("_")) > 3
                        else "Doe"
                    ),
                    "locale": "en",
                }
    ...
    def check_rate_limit(self, client_id: str) -> bool:
        ...
        self.rate_limit_tracker[client_id] = [
            req_time
            for req_time in self.rate_limit_tracker[client_id]
...
class SSOIntegrationTester:
    ...
    async def test_oauth_authorization_flow(self, provider: str) -> SSOTestResult:
        ...
        token_response = mock_provider.exchange_code_for_token(
            authorization_code, getattr(self.config, f"{provider.lower()}_client_id")
        )
        ...
        profile_response = mock_provider.get_user_profile(
        ...
        if "error" in profile_response:
            return SSOTestResult(
                ...
                details={
                    "token_valid": True,
                    "profile_error": profile_response["error"],
                },
    ...
    async def test_token_refresh(self, provider: str) -> SSOTestResult:
        ...
        initial_token_response = mock_provider.exchange_code_for_token(
            auth_code, getattr(self.config, f"{provider.lower()}_client_id")
        )
        ...
        if old_access_token in mock_provider.access_tokens:
            mock_provider.access_tokens[old_access_token][
                "expires_at"
        ...
        return SSOTestResult(
            ...
            response_time=end_time - start_time,
            details={
                "old_token_invalidated": old_access_token
                not in mock_provider.access_tokens,
    ...
    async def test_concurrent_sessions(self, provider: str) -> SSOTestResult:
        ...
        for i in range(session_count):
            ...
            async def create_session(index: int):
                ...
                state = secrets.token_urlsafe(32)
                auth_code = mock_provider.generate_authorization_code(
                    state, f"{provider.lower()}_user_{index}"
                )
        ...
        return SSOTestResult(
            ...
            >= session_count * 0.95,  # 95% success rate
            response_time=end_time - start_time,
            details={
                ...
                "unique_users": len(
                    set(r["user_id"] for r in successful_sessions if r["user_id"])
                ),
            },
        )
        ...
        error_scenarios = [
            {
                ...
                "action": lambda: mock_provider.exchange_code_for_token(
                    "invalid_code", "client_id"
                ),
            },
            {
    ...
    async def test_state_parameter_security(self, provider: str) -> SSOTestResult:
        ...
        for scenario in scenarios:
            ...
            state_valid = (
                scenario["state"] is not None
                and scenario["auth_code"] in mock_provider.authorization_codes
                and mock_provider.authorization_codes[scenario["auth_code"]]["state"]
                == scenario["state"]
            )
            ...
            results.append(
                {
                    ...
                    "security_check_passed": state_valid
    ...
    def _test_expired_code(self, mock_provider: MockOAuthProvider) -> Dict[str, Any]:
        ...
        if auth_code in mock_provider.authorization_codes:
            mock_provider.authorization_codes[auth_code][
                "expires_at"
            ] = datetime.now() - timedelta(minutes=1)
    ...
    def _test_expired_token(self, mock_provider: MockOAuthProvider) -> Dict[str, Any]:
        ...
        if "access_token" in token_response:
            ...
            if access_token in mock_provider.access_tokens:
                mock_provider.access_tokens[access_token][
                    "expires_at"
                ] = datetime.now() - timedelta(minutes=1)
        ...
        for provider in providers:
            ...
            for test_func in test_functions:
                try:
                    ...
                    print(
                        f"{status} {provider}: {result.test_name} ({result.response_time:.3f}s)"
                    )
...
async def main():
    ...
    for result in results["test_results"]:
        ...
        print(
            f"  {status} {result['provider']} - {result['name']}: {result['response_time']:.3f}s"
        )
        if result["error_message"]:
            print(f"       Error: {result['error_message']}")

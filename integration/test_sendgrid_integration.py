...
class SendGridMockService:
        ...
        for email in to_emails:
            if email in self.error_conditions:
                ...
                return {
                    ...
                    "body": json.dumps(
                        {"errors": [self.error_conditions[email]["message"]]}
                    ),
    ...
    def set_error_condition(self, email: str, status_code: int, message: str):
        ...
        self.error_conditions[email] = {"status_code": status_code, "message": message}
...
class SendGridIntegrationTester:
    ...
    async def test_bulk_email_sending(
        self, recipient_count: int = 100
    ) -> EmailTestResult:
        ...
        email_data = {
            "personalizations": [
                {"to": recipients, "subject": "Team Assessment Notification"}
            ],
            "from": {"email": self.config.from_email},
            "content": [
                {
        ...
        try:
            ...
            return EmailTestResult(
                ...
                details={
                    ...
                    "dynamic_data_keys": list(
                        email_data["personalizations"][0][
                            "dynamic_template_data"
                        ].keys()
                    ),
                    "api_response": json.loads(result["body"]),
    ...
    async def test_error_handling(self) -> EmailTestResult:
        ...
        self.mock_service.set_error_condition(
            "invalid@example", 400, "Invalid email address"
        )
        self.mock_service.set_error_condition(
            "bounced@example.com", 422, "Email address has bounced"
        )
        self.mock_service.set_error_condition(
            "blocked@example.com", 403, "Email address is blocked"
        )
        ...
        error_scenarios = [
            {"email": "invalid@example", "expected_error": "Invalid email address"},
            {
                ...
                "expected_error": "Email address has bounced",
            },
            {
                "email": "blocked@example.com",
                "expected_error": "Email address is blocked",
            },
        ...
        for scenario in error_scenarios:
            email_data = {
                "personalizations": [
                    {"to": [{"email": scenario["email"]}], "subject": "Test Email"}
                ],
                "from": {"email": self.config.from_email},
    ...
    async def test_rate_limiting(self) -> EmailTestResult:
        ...
        successful_sends = sum(
            1 for r in results if isinstance(r, dict) and r.get("status_code") == 202
        )
        ...
        try:
            ...
            return EmailTestResult(
                ...
                details={
                    ...
                    "attachment_types": [
                        att.get("type") for att in email_data.get("attachments", [])
                    ],
                    "api_response": json.loads(result["body"]),
                },
            )
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "average_response_time": sum(r.response_time for r in self.test_results)
                / total_tests,
            },
            "test_results": [
                {
            ...
            "mock_service_stats": {
                ...
                "average_response_time": (
                    sum(self.mock_service.response_times)
                    / len(self.mock_service.response_times)
                    if self.mock_service.response_times
                    else 0
                ),
            },
        }

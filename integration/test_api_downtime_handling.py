...
class MockAPIServer:
    ...
    def __init__(self, base_url: str = "http://localhost:8000"):
        ...
        self.endpoints = {
            ...
            '/api/v1/assessments': [{'id': 1, 'type': 'MBTI'}, {'id': 2, 'type': 'Big Five'}],
            '/api/v1/analytics': {'users': 150, 'assessments': 500, 'completion_rate': 87.3},
            '/api/v1/teams': [{'id': 1, 'name': 'Team Alpha'}, {'id': 2, 'name': 'Team Beta'}]
        }
    ...
    async def handle_request(self, endpoint: str, method: str = 'GET') -> APIResponse:
        ...
        if self.status == APIStatus.OFFLINE:
            ...
            if random.random() < 0.7:  # 70% failure rate in degraded mode
                self._increment_circuit_breaker()
                error_mode = random.choice(self.error_modes) if self.error_modes else 'timeout'
...
class FrontendClient:
    ...
    async def make_request(self, endpoint: str, method: str = 'GET', use_cache: bool = True) -> Dict[str, Any]:
    ...
    async def _handle_fallback(self, endpoint: str, last_response: Optional[APIResponse]) -> Dict[str, Any]:
...
class APIDowntimeTester:
    ...
    async def test_healthy_api_response(self) -> DowntimeTestResult:
        ...
        return DowntimeTestResult(
            ...
            success=successful_requests == len(endpoints) and api_requests == len(endpoints),
    ...
    async def test_intermittent_failures(self) -> DowntimeTestResult:
        ...
        endpoints = ['/api/v1/users/profile', '/api/v1/analytics', '/api/v1/assessments']
        results = []
        ...
        successful_requests = sum(1 for r in results if r['success'])
        graceful_degradations = sum(1 for r in results if r['source'] in ['cache_fallback', 'fallback'])
    ...
    async def test_complete_api_outage(self) -> DowntimeTestResult:
        ...
        endpoints = ['/api/v1/users/profile', '/api/v1/analytics', '/api/v1/teams', '/api/v1/health']
        results = []
    ...
    async def test_circuit_breaker_behavior(self) -> DowntimeTestResult:
        ...
        return DowntimeTestResult(
            ...
            success=circuit_breaker_triggered and successful_requests >= len(results) - 5,
            response_time=end_time - start_time,
            details={
    ...
    async def test_cache_effectiveness(self) -> DowntimeTestResult:
        ...
        cache_hits = sum(1 for r in results if r['source'] in ['cache', 'cache_fallback'])
    ...
    async def test_recovery_after_outage(self) -> DowntimeTestResult:
        ...
        successful_recoveries = sum(1 for r in recovery_responses if r.get('source') == 'api')

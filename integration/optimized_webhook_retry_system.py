...
class OptimizedWebhookEndpoint:
    ...
    async def receive_webhook(self, event: WebhookEvent) -> DeliveryResult:
        ...
        recent_requests = [
            req_time
            for req_time in self.current_requests
...
class OptimizedWebhookRetryService:
    ...
    def create_webhook_event(
        ...
        headers = {
            ...
            "X-PsychSync-Retry-Strategy": (
                retry_strategy or RetryStrategy.EXPONENTIAL_BACKOFF
            ).value,
        }
    ...
    def _calculate_retry_delay(self, event: WebhookEvent, attempt: int) -> int:
        ...
        if event.retry_strategy == RetryStrategy.LINEAR_BACKOFF:
            ...
            if event.response_time_history:
                avg_response_time = sum(event.response_time_history) / len(
                    event.response_time_history
                )
                if avg_response_time > 5.0:  # Slow responses, back off more
                    return int(
                        self.retry_intervals[
                            min(attempt, len(self.retry_intervals) - 1)
                        ]
                        * 1.5
                    )
    ...
    async def deliver_webhook_with_optimization(
        self, event: WebhookEvent, endpoint: OptimizedWebhookEndpoint
    ) -> bool:
    ...
    async def process_pending_retries_optimized(
        self, endpoints: Dict[str, OptimizedWebhookEndpoint]
    ) -> Dict[str, int]:
        ...
        ready_events = [
            ...
            and (event.next_retry_at is None or current_time >= event.next_retry_at)
        ]
        ...
        async def process_single_event(event: WebhookEvent):
            async with semaphore:
                ...
                if endpoint:
                    success = await self.deliver_webhook_with_optimization(
                        event, endpoint
                    )
...
class OptimizedWebhookTester:
    ...
    async def test_optimized_retry_mechanism(self) -> Dict[str, Any]:
        ...
        for strategy in strategies:
            ...
            for attempt in range(event.max_attempts):
                success = await self.retry_service.deliver_webhook_with_optimization(
                    event, endpoint
                )
            ...
            results.append(
                {
                    ...
                    "avg_response_time": (
                        sum(event.response_time_history)
                        / len(event.response_time_history)
                        if event.response_time_history
                        else 0
                    ),
                }
        ...
        return {
            ...
            "success": len(successful_strategies)
            >= 1,  # At least one strategy succeeds
            "details": {
                "total_strategies_tested": len(strategies),
                "successful_strategies": len(successful_strategies),
                "strategy_results": results,
                "best_strategy": (
                    min(results, key=lambda x: x["attempts"]) if results else None
                ),
                "adaptive_better": any(
                    r["strategy"] == "adaptive_backoff" and r["success"]
                    for r in results
                ),
            },
        }
    ...
    async def test_priority_based_delivery(self) -> Dict[str, Any]:
        ...
        delivery_results = await self.retry_service.process_pending_retries_optimized(
            endpoints
        )
        ...
        delivered_events = [
            ...
            for e in events
            if e.status == WebhookStatus.DELIVERED
        ]
        ...
        return {
            ...
            "details": {
                ...
                "priority_respected": len(delivered_events) > 0
                and delivered_events[0][0] in ["critical", "high"],
                "processing_time": end_time - start_time,
            },
        }
    ...
    async def run_optimized_tests(self) -> Dict[str, Any]:
        ...
        return {
            "summary": {
                ...
                "meets_target": (
                    (successful_tests / total_tests) * 100 >= 90
                    if total_tests > 0
                    else False
                ),
            },

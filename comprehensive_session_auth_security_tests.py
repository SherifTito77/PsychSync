...
class SessionAuthSecurityTester:
    ...
    async def test_jwt_token_security(self) -> SessionAuthTestResult:
        ...
        for config_file in [config, security, auth_deps]:
            ...
            if 'SECRET_KEY' in content or 'JWT_SECRET' in content or 'secret' in content.lower():
                ...
                for pattern, description in weak_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        ...
                        recommendations.append("Use cryptographically secure random secret (32+ bytes)")
                ...
                if 'os.getenv' in content or 'os.environ' in content:
                    findings.append("JWT secret loaded from environment (good practice)")

                # Check secret length
                secret_match = re.search(r'secret\s*=\s*["\']([^"\']+)["\']', content, re.IGNORECASE)
                if secret_match and not jwt_weak_secret:
                    secret_value = secret_match.group(1)
                    if len(secret_value) < 32:
                        findings.append(f"JWT secret too short: {len(secret_value)} characters (minimum 32 recommended)")
                        recommendations.append("Use longer JWT secret (32+ bytes/characters)")
                        severity = "high"
                        status = "fail"

            # Check JWT algorithm
            if 'JWT_ALGORITHM' in content or 'algorithm' in content.lower():
                algo_match = re.search(r'JWT_ALGORITHM\s*=\s*["\']([^"\']+)["\']', content)
                if algo_match:
                    algorithm = algo_match.group(1)
                    if algorithm == 'HS256':
                        findings.append(f"JWT algorithm: {algorithm} (HMAC with SHA-256)")
                    elif algorithm == 'RS256':
                        findings.append(f"JWT algorithm: {algorithm} (RSA with SHA-256 - more secure)")
                        recommendations.append("RS256 provides better security but requires key management")
                    else:
                        findings.append(f"JWT algorithm: {algorithm}")
                        if algorithm in ['none', 'None', 'NONE']:
                            findings.append("CRITICAL: 'none' algorithm allows token bypass!")
                            severity = "critical"
                            status = "fail"
        ...
        if config.exists():
            ...
            if 'ACCESS_TOKEN_EXPIRE' in content or 'TOKEN_EXPIRATION' in content:
                expire_match = re.search(r'(ACCESS_TOKEN_EXPIRE|TOKEN_EXPIRATION).*?(\d+)', content)
                if expire_match:
                    ...
                    if 'minute' in content.lower() or 'MINUTE' in content:
                        ...
                        # Assume seconds
                        expire_minutes = expire_value // 60 if expire_value > 3600 else expire_value

                    if expire_minutes > 1440:  # More than 24 hours
                        findings.append(f"Access token expiration too long: {expire_minutes} minutes")
                        recommendations.append("Reduce access token expiration to 15-30 minutes")
                        severity = "high"
                        status = "warning"
                ...
                recommendations.append("Set access token expiration (15-30 minutes recommended)")
                status = "warning"
        ...
        if auth_service.exists():
            ...
            if 'refresh_token' in content.lower() or 'refresh_token' in content.lower():
                ...
                recommendations.append("Implement refresh token mechanism for better security")
                status = "warning"
        ...
        if auth_service.exists():
            ...
            if 'blacklist' in content.lower() or 'revoke' in content.lower() or 'invalidate' in content.lower():
                findings.append("Token revocation mechanism detected")
            else:
                findings.append("No token blacklisting/revocation found")
                recommendations.append("Implement token blacklisting for logout scenarios")
                status = "info"
    ...
    async def test_session_fixation(self) -> SessionAuthTestResult:
        ...
        for session_file in session_files:
            ...
            if 'expire' in content.lower() or 'timeout' in content.lower() or 'ttl' in content.lower():
                findings.append("Session expiration/timeout configured")
            else:
                findings.append("No session timeout configuration found")
                recommendations.append("Set session timeout (15-30 minutes for inactivity)")
                status = "warning"
        ...
        if auth_service.exists():
            ...
            if 'jwt' in content.lower() or 'token' in content.lower():
                ...
                if 'secrets.token_urlsafe' in content or 'uuid' in content.lower():
                    findings.append("Token generation uses cryptographically secure random")
                else:
                    findings.append("Token generation may use weak randomness")
                    recommendations.append("Use secrets.token_urlsafe() for token generation")
                    status = "warning"
    ...
    async def test_cookie_security(self) -> SessionAuthTestResult:
        ...
        for config_file in [main_app, auth_endpoints]:
            ...
            if 'cookie' in content.lower() or 'Cookie' in content:
                ...
                if 'httponly' in content.lower() or 'HttpOnly' in content:
                    ...
                    recommendations.append("Set HttpOnly flag on all authentication cookies")
                    severity = "high"
                    status = "fail"
                ...
                if 'secure' in content.lower() and 'cookie' in content.lower():
                    ...
                    recommendations.append("Set Secure flag on all authentication cookies (HTTPS only)")
                    status = "warning"

                # Check for SameSite
                if 'samesite' in content.lower() or 'SameSite' in content:
                    samesite_match = re.search(r'SameSite\s*=\s*["\']?(\w+)["\']?', content)
                    if samesite_match:
                    ...
                    recommendations.append("Set SameSite=Lax or SameSite=Strict on cookies")
                    status = "warning"

                # Check for cookie domain/path
                if 'cookie_domain' in content.lower() or 'cookie_path' in content.lower():
        ...
        if auth_endpoints.exists():
            ...
            if 'Authorization' in content or 'authorization' in content:
                findings.append("Using Authorization header for tokens (recommended over cookies)")
            ...
            if 'password' in content.lower():
                ...
                if 'Field(' in content and 'min_length' in content:
                    ...
                    if min_length_match:
                        ...
                        if min_length < 8:
                            findings.append(f"Password minimum length too short: {min_length} characters")
                            recommendations.append("Set minimum password length to 8+ characters")
                            severity = "high"
                            status = "fail"
                        else:
                            findings.append(f"Password minimum length: {min_length} characters (good)")
                ...
                if re.search(r'regex.*?password', content, re.IGNORECASE):
                    ...
                    recommendations.append("Add password complexity validation (uppercase, lowercase, numbers, symbols)")
                    status = "warning"

                # Check for strength validator
                if 'strength' in content.lower() or 'validate_password' in content.lower():
            ...
            if 'hash' in content.lower() or 'bcrypt' in content.lower():
                ...
                if bcrypt_rounds:
                    ...
                    if rounds >= 12:
                        ...
                        findings.append(f"Bcrypt rounds: {rounds} (too low, recommend 12+)")
                        recommendations.append("Increase bcrypt work factor to 12+")
                        status = "warning"

            # Check for common password detection
            if 'common_password' in content.lower() or 'haveibeenpwned' in content.lower():
    ...
    async def test_brute_force_protection(self) -> SessionAuthTestResult:
        ...
        for auth_file in [auth_endpoints, auth_service]:
            ...
            if 'rate_limit' in content.lower() or 'limiter' in content.lower():
                findings.append("Rate limiting on authentication endpoints (helps prevent brute force)")
                brute_force_protection = True
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for test_name, test_method in test_methods:
            ...
            try:
                ...
                self.results.append(SessionAuthTestResult(
                    category=test_name,
                    test_name=test_name,
                    severity="error",
                    status="error",
...
async def main():
    ...
    # Save report to JSON
    output_file = project_root / f"session_auth_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

...
class DependencySecurityTester:
    ...
    async def test_known_vulnerabilities(self) -> DependencyTestResult:
        ...
        if requirements.exists():
            ...
            for vuln_package, cve in vulnerable_packages.items():
                package_name = vuln_package.split('==')[0].split('>')[0].split('<')[0].split('=')[0]
                if package_name.lower() in content.lower():
                    # Check if it's the vulnerable version
                    if vuln_package.split('==')[1] in content if '==' in vuln_package else True:
                        findings.append(f"Potentially vulnerable package: {package_name} ({cve})")
                        recommendations.append(f"Update {package_name} to latest secure version")
                        severity = "high"
                        status = "warning"
    ...
    async def test_outdated_packages(self) -> DependencyTestResult:
        ...
        if requirements.exists():
            ...
            if outdated_count > 0:
                findings.append(f"Found {outdated_count} packages with old version pins")
                recommendations.append("Review and update outdated packages")
                status = "warning"

            # Check for unpinned versions
            unpinned = [l for l in content.strip().split('\n') if l.strip() and not l.startswith('#') and '==' not in l and '>=' not in l and '<=' not in l]
            if unpinned:
    ...
    async def test_supply_chain_attacks(self) -> DependencyTestResult:
        ...
        if requirements.exists():
            ...
            for suspicious, correct in suspicious_patterns:
                if suspicious in content:
                    findings.append(f"Suspicious package name: {suspicious} (typosquatting?)")
                    recommendations.append(f"Verify {suspicious} is legitimate, should it be {correct}?")
                    severity = "high"
                    status = "warning"
        ...
        if lock_files_found:
            findings.append(f"Found {len(lock_files_found)} dependency lock files (good)")
        else:
            findings.append("No dependency lock files found")
            recommendations.append("Use dependency lock files (Pipfile.lock, package-lock.json)")
            status = "warning"
    ...
    async def test_license_compliance(self) -> DependencyTestResult:
        ...
        if frontend_package.exists():
            try:
                ...
                recommendations.append("Add license checker: npm install -g license-checker")
                status = "info"
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for test_name, test_method in test_methods:
            ...
            try:
                ...
                self.results.append(DependencyTestResult(
                    category=test_name,
                    test_name=test_name,
                    severity="error",
                    status="error",
...
async def main():
    ...
    # Save report to JSON
    output_file = project_root / f"dependency_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

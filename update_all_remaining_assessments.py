...
def update_disc_assessment():
    ...
    updated_content = content[:questions_start] + new_questions + content[questions_end + 2:]
...
def main():
    ...
    if update_disc_assessment():
        ...
        try:
            import requests
            response = requests.get("http://localhost:8000/api/v1/assessment-questions/disc", timeout=5)
    ...
    if update_social_styles_assessment():
        ...
        try:
            import requests
            response = requests.get("http://localhost:8000/api/v1/assessment-questions/social-styles", timeout=5)
            if response.status_code == 200:
                data = response.json()
                q_count = len(data.get("assessment", {}).get("questions", []))

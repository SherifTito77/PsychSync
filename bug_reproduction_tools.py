...
class BugReproductionTool:
    ...
    def generate_minimal_reproduction_steps(self, bug_report: str) -> List[ReproductionStep]:
        ...
        elif any(word in bug_report.lower() for word in ["report", "dashboard", "analytics"]):
    ...
    def suggest_root_causes(self, bug_description: str, context: Dict[str, Any] = None) -> List[RootCause]:
        ...
        # UI/Interface issues
        if any(pattern in bug_lower for pattern in ["crash", "freeze", "unresponsive", "ui", "interface"]):
            causes.append(RootCause(
                ...
                components_involved=["React Components", "State Management", "Event Handlers"]
            ))

        # Data issues
        if any(pattern in bug_lower for pattern in ["data", "null", "undefined", "missing"]):
            causes.append(RootCause(
                cause_id="DATA-001",
                category="Data Handling",
                likelihood="HIGH",
                description="Null/undefined data causing application failure",
                evidence=["Data-related error mentioned", "Null reference patterns"],
                suggested_fix="Add proper data validation and null checks",
                components_involved=["Data Layer", "API Integration", "State Management"]
            ))

        # Performance issues
        if any(pattern in bug_lower for pattern in ["slow", "performance", "timeout", "lag"]):
            causes.append(RootCause(
                cause_id="PERF-001",
        ...
        # Integration issues
        if any(pattern in bug_lower for pattern in ["api", "integration", "connection", "server"]):
            causes.append(RootCause(
                cause_id="INT-001",
                category="Integration",
                likelihood="HIGH",
                description="API integration failure or network connectivity issues",
                evidence=["API or integration mentioned", "Server-related problems"],
                suggested_fix="Verify API endpoints and implement retry logic",
                components_involved=["API Layer", "Network", "Error Handling"]
            ))

        # Authentication issues
        if any(pattern in bug_lower for pattern in ["login", "auth", "permission", "access"]):
            causes.append(RootCause(
                cause_id="AUTH-001",
                ...
                components_involved=["Auth Service", "JWT Validation", "Role Management"]
            ))
        ...
        if not causes:
            causes.append(RootCause(
                ...
                components_involved=["Application Core", "Logging", "Error Handling"]
            ))
    ...
    def analyze_stack_trace(self, stack_trace: str) -> StackTraceAnalysis:
        ...
        for i, (method, file, line) in enumerate(stack_frames[:10]):  # Analyze top 10 frames
        ...
        if "TimeoutError" in exception_type or "TimeoutException" in exception_type:
            potential_causes.append("Operation timeout - possible performance or network issue")
    ...
    def rewrite_bug_report_professionally(self, raw_bug_report: str,
                                        additional_context: Dict[str, Any] = None) -> BugReport:
    ...
    def generate_severity_priority_rating(self, bug_description: str,
                                       affected_users: int = 1,
                                       frequency: str = "once") -> Tuple[SeverityLevel, PriorityLevel]:
        """Generate severity and priority rating for a defect"""

        # Severity assessment (impact on system)
        severity_keywords = {
            SeverityLevel.CRITICAL: [
                "crash", "security", "data loss", "corruption", "system down",
                "unrecoverable", "production down", "security breach"
            ],
            SeverityLevel.HIGH: [
                "major functionality", "blocked", "cannot use", "significant impact",
                "workaround impossible", "core feature broken"
            ],
            SeverityLevel.MEDIUM: [
                "minor functionality", "workaround available", "inconvenience",
                "incorrect behavior", "performance issue"
            ],
            SeverityLevel.LOW: [
                "cosmetic", "minor issue", "spelling", "ui improvement",
                "rare occurrence", "edge case"
            ]
        }
        ...
        if severity == SeverityLevel.CRITICAL:
            ...
            priority = PriorityLevel.HIGH if frequency.lower() in ["frequently", "often"] else PriorityLevel.MEDIUM
        else:
...
A thorough investigation is required to identify the root cause and implement an appropriate fix.
    ...
    def _assess_priority(self, bug_report: str, severity: SeverityLevel) -> PriorityLevel:
        """Assess bug priority"""
...
def main():
    ...
    print("-" * 60)
    severity, priority = tool.generate_severity_priority_rating(example_bug, affected_users=50, frequency="always")
    ...
    for point in stack_analysis.failure_points[:3]:
        print(f"  Frame {point['frame_number']}: {point['method']} ({point['file']}:{point['line_number']})")

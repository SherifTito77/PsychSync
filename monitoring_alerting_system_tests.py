...
class MonitoringAlertingTestSuite:
    ...
    async def test_health_endpoints(self) -> None:
        ...
        for endpoint in health_endpoints:
            ...
            if result.status_code == 200 and result.response_time < 1000:
                ...
                print(f"⚠️  {endpoint} - Issues detected (Status: {result.status_code}, Time: {result.response_time:.2f}ms)")
    ...
    async def test_metrics_collection(self) -> None:
        ...
        for endpoint in metrics_endpoints:
            ...
            if result.success and len(result.metrics_collected) > 0:
                print(f"✅ {endpoint} - {len(result.metrics_collected)} metrics collected")
            else:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if isinstance(metrics_data, dict):
                        ...
                        for key in ["requests_total", "request_duration_seconds", "error_rate", "active_users"]:
    ...
    async def test_database_query_performance(self) -> MonitoringTestResult:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(f"{self.base_url}/api/v1/teams", headers={"Authorization": "Bearer test-token"})

            response_time = (time.time() - start_time) * 1000

            success = response.status_code in [200, 401, 403]  # Accept auth errors as successful DB operation
            details = {"status_code": response.status_code}
                ...
                try:
                    async with httpx.AsyncClient(timeout=2.0) as client:
                        ...
                        if i < total_requests * 0.8:
                            ...
                            response = await client.get(f"{self.base_url}/api/v1/invalid-endpoint")
                            error_requests += 1
                except Exception:
                    error_requests += 1
    ...
    async def test_dependency_health_check(self, dependency: str) -> MonitoringTestResult:
    ...
    def generate_monitoring_report(self, total_time: float) -> Dict[str, Any]:
        ...
        if healthy_endpoints == health_checks_total and health_checks_total > 0:
            ...
            print(f"├─ Health Endpoints: ⚠️  {healthy_endpoints}/{health_checks_total} healthy")
        ...
        report_data = {
            ...
            "health_checks": {
                ...
                "health_rate_percent": (healthy_endpoints/health_checks_total*100) if health_checks_total > 0 else 0
            },
            "performance_metrics": {

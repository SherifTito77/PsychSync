...
def add_config_to_schema(file_path: str, schema_configs: dict):
    ...
    for class_name, config_data in schema_configs.items():
        ...
        def add_config(match):
            ...
            modifications += 1
            example_json = str(config_data).replace("'", '"').replace('True', 'true').replace('False', 'false')
...
def main():
    ...
    schema_files = {
        "app/schemas/user.py": {"UserCreate": SCHEMA_CONFIGS["UserCreate"], "UserUpdate": SCHEMA_CONFIGS["UserUpdate"]},
        "app/schemas/team.py": {"TeamCreate": SCHEMA_CONFIGS["TeamCreate"]},
        "app/schemas/assessment.py": {"AssessmentCreate": SCHEMA_CONFIGS["AssessmentCreate"]},

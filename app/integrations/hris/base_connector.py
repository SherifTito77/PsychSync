...
class HRISConnector(ABC):
    ...
    def sync_employees_to_database(self, employees: list[Employee], db_connection: Any):
        """
        ...
        # Write to database
        df.to_sql("hris_employees", con=db_connection, if_exists="replace", index=False)

        logger.info(f"Synced {len(employees)} employees to database")

    def get_sync_statistics(self, start_date: date, end_date: date) -> dict:
        """
...
class CSVConnector(HRISConnector):
    ...
    def test_connection(self) -> bool:
        ...
        return all(
            os.path.exists(path)
            for path in [self.employees_csv, self.attendance_csv, self.leave_csv]
            if path
        )

...
class HRISIntegrationManager:
    ...
    def create_connector(self, hris_type: str, config: dict) -> HRISConnector | None:
        """
    ...
    def get_connector(self, hris_type: str) -> HRISConnector | None:
        """Get existing active connector."""
        return self.active_connectors.get(hris_type.lower())

    def sync_all_data(self, connector: HRISConnector, days_back: int = 30) -> dict:
        """
        ...
        results = {"success": False, "timestamp": date.today().isoformat(), "data": {}}
        ...
        try:
            ...
            if employees:
                path = connector.export_to_csv(
                    employees, f"employees_{date.today()}.csv", output_dir
                )
            ...
            if attendance:
                path = connector.export_to_csv(
                    attendance, f"attendance_{start_date}_to_{end_date}.csv", output_dir
                )
            ...
            if leaves:
                path = connector.export_to_csv(
                    leaves, f"leaves_{start_date}_to_{end_date}.csv", output_dir
                )
                exports["leaves"] = path
    ...
    def get_integration_status(self) -> dict:
        ...
        status = {"total_connectors": len(self.active_connectors), "connectors": {}}

...
import requests
import logging
from .base_connector import HRISConnector, Employee, AttendanceRecord, LeaveRecord, PerformanceReview
...
class QuickBooksWorkforceConnector(HRISConnector):
    ...
    def _make_api_request(self, endpoint: str, method: str = 'GET', **kwargs) -> Optional[Dict]:
        ...
        try:
            ...
            for user_data in response['results']:
                ...
                employee = Employee(
                    ...
                    department=self._get_user_department(user_data['id'])[0] if self._get_user_department(user_data['id']) else None,
                    ...
                    manager_id=str(user_data.get('manager_id')) if user_data.get('manager_id') else None,
    ...
    def get_attendance(
        ...
        try:
            ...
            for timesheet in response['results']:
                ...
                details_endpoint = f'timesheets/{timesheet["id"]}/details?company_id={self.company_id}'
                ...
                if details_response and 'results' in details_response:
                    for detail in details_response['results']:
                        record = AttendanceRecord(
                            ...
                            hours_worked=float(detail.get('duration', 0)) / 3600 if detail.get('duration') else None,

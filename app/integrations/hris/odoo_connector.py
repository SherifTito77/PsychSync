...
class OdooHRConnector(HRISConnector):
    ...
    def get_employee_by_id(self, employee_id: str) -> Employee | None:
        ...
        try:
            ...
            employees_data = self._execute(
                "hr.employee", "read", [[int(employee_id)]], {"fields": fields}
            )
    ...
    def get_attendance(
        ...
        for item in attendances:
            check_in = (
                datetime.strptime(item["check_in"], "%Y-%m-%d %H:%M:%S")
                if item.get("check_in")
                else None
            )
            check_out = (
                datetime.strptime(item["check_out"], "%Y-%m-%d %H:%M:%S")
    ...
    def get_leave_records(
        ...
        leaves = self._execute("hr.leave", "read", [leave_ids], {"fields": fields})

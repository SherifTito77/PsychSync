...
class EmailMetadataExtractor:
    ...
    def extract_from_gmail_message(self, message_data: Dict[str, Any], user_id: int,
                                   connection_id: int, organization_id: int) -> EmailMetadata:
        """
        ...
        try:
            ...
            # Behavioral flags
            is_after_hours = sent_at.hour < self.WORK_HOURS_START or sent_at.hour >= self.WORK_HOURS_END
    ...
    def extract_from_outlook_message(self, message_data: Dict[str, Any], user_id: int,
                                     connection_id: int, organization_id: int) -> EmailMetadata:
        """
        ...
        try:
            ...
            received_at = datetime.fromisoformat(message_data['receivedDateTime'].replace('Z', '+00:00'))

            # Extract sender
            sender_email = message_data['from']['emailAddress']['address']

            # Extract recipients
            to_emails = [r['emailAddress']['address'] for r in message_data.get('toRecipients', [])]
            cc_emails = [r['emailAddress']['address'] for r in message_data.get('ccRecipients', [])]
            bcc_emails = [r['emailAddress']['address'] for r in message_data.get('bccRecipients', [])]
    ...
    def calculate_behavioral_signals(
        ...
        signals['thread_length_avg'] = sum(thread_lengths) / len(thread_lengths) if thread_lengths else 0
    ...
    def detect_burnout_indicators(self, signals: Dict[str, Any]) -> List[str]:
        ...
        if signals.get('hourly_distribution'):
            late_night = sum(signals['hourly_distribution'][22:] + signals['hourly_distribution'][:6])
    ...
    def _analyze_urgency(self, subject: str) -> tuple:
        ...
        if any(keyword in subject_lower for keyword in self.URGENCY_KEYWORDS['critical']):
        ...
        async with httpx.AsyncClient() as client:
            ...
            for msg in messages[:max_results]:
                response = await client.get(
                    ...
                    headers={'Authorization': f'Bearer {self.access_token}'},
                    params={'format': 'metadata', 'metadataHeaders': ['From', 'To', 'Cc', 'Bcc', 'Subject', 'Date', 'In-Reply-To', 'References']}
                )
                response.raise_for_status()

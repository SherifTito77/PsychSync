...
class MetricsCollector:
    ...
    async def get_metrics_summary(self, minutes: int = 5) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            avg_response_time = (
                sum(t.duration_ms for t in recent_traces) / total_requests
                if total_requests > 0
                else 0
            )
            ...
            avg_db_duration = (
                sum(d.duration_ms for d in recent_db_metrics) / total_db_queries
                if total_db_queries > 0
                else 0
            )
            db_error_rate = (
                len([d for d in recent_db_metrics if not d.success]) / total_db_queries
                if total_db_queries > 0
                else 0
            )
    ...
    async def get_endpoint_stats(self, minutes: int = 5) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            for endpoint, durations in self.endpoint_stats.items():
                ...
                recent_durations = [
                    d
                    for _, trace in zip(durations, self.request_traces)
...
class CustomMetrics:
    ...
    async def record_user_activity(
        self, user_id: str, activity: str, metadata: dict[str, Any] = None
    ):
        ...
        await self.metrics_collector.add_metric(
            PerformanceMetric(
                ...
                unit="count",
                timestamp=datetime.utcnow(),
                tags={"user_id": user_id, "activity": activity, **(metadata or {})},
            )
        )

    async def record_business_metric(
        self, name: str, value: float, business_context: dict[str, Any] = None
    ):
...
_custom_metrics: CustomMetrics | None = None
_db_apm: DatabaseAPM | None = None
...
async def trace_operation(operation_name: str, tags: dict[str, Any] = None):
    ...
    try:
        ...
        await get_metrics_collector().add_metric(
            PerformanceMetric(
            ...
            try:
                ...
                await get_metrics_collector().add_metric(
                    PerformanceMetric(
                        ...
                        unit="ms",
                        timestamp=datetime.utcnow(),
                        tags={"function": name, "success": "true"},
                    )
                )
                ...
                await get_metrics_collector().add_metric(
                    PerformanceMetric(
                        ...
                        unit="count",
                        timestamp=datetime.utcnow(),
                        tags={"function": name, "error_type": type(e).__name__},
                    )
                )
                raise

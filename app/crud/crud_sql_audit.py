...
from app.db.models.sql_audit import SQLQuery, SQLVulnerability, SQLScanReport
...
class CRUDSQLQuery:
    ...
    async def get_by_hash(self, db: AsyncSession, *, query_hash: str) -> Optional[SQLQuery]:
        result = await db.execute(select(SQLQuery).where(SQLQuery.query_hash == query_hash))
        return result.scalar_one_or_none()
    async def get_unfixed(self, db: AsyncSession, *, skip: int = 0, limit: int = 100) -> list[SQLQuery]:
        result = await db.execute(
            select(SQLQuery).where(SQLQuery.is_fixed == 0.0)
            .order_by(SQLQuery.risk_score.desc()).offset(skip).limit(limit)
        )
        return list(result.scalars().all())
    async def get_multi(self, db: AsyncSession, *, skip: int = 0, limit: int = 100) -> list[SQLQuery]:
        result = await db.execute(select(SQLQuery).offset(skip).limit(limit))
        return list(result.scalars().all())
    async def get_recent(self, db: AsyncSession, *, skip: int = 0, limit: int = 100) -> list[SQLQuery]:
    ...
    async def create(self, db: AsyncSession, *, obj_in: dict) -> SQLQuery:
        ...
        return db_obj
    async def update(self, db: AsyncSession, *, db_obj: SQLQuery, obj_in: dict) -> SQLQuery:
...
class CRUDSQLVulnerability:
    ...
    async def get(self, db: AsyncSession, id: UUID) -> Optional[SQLVulnerability]:
        result = await db.execute(select(SQLVulnerability).where(SQLVulnerability.id == id))
        return result.scalar_one_or_none()
    async def get_multi(self, db: AsyncSession, *, skip: int = 0, limit: int = 100) -> list[SQLVulnerability]:

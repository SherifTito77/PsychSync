...
from app.db.models.caching_config import (
...
class CRUDCacheEntry:
    ...
    async def get_low_hit_rate(
        ...
        *,
        ...
        result = await db.execute(
            select(CacheEntry)
            .where(CacheEntry.hit_rate < threshold)
            .order_by(CacheEntry.hit_rate.asc())
            .offset(skip)
            .limit(limit)
        )
...
class CRUDCachePerformance:
    ...
    async def get(self, db: AsyncSession, id: UUID) -> Optional[CachePerformance]:
        result = await db.execute(
            select(CachePerformance).where(CachePerformance.id == id)
        )
        return result.scalar_one_or_none()

    async def get_multi(
        self, db: AsyncSession, *, skip: int = 0, limit: int = 100
    ) -> list[CachePerformance]:
...
class CRUDCacheOptimization:
    ...
    async def get(self, db: AsyncSession, id: UUID) -> Optional[CacheOptimization]:
        result = await db.execute(
            select(CacheOptimization).where(CacheOptimization.id == id)
        )
        return result.scalar_one_or_none()
...
class CRUDCacheConfigurationReport:
    ...
    def calculate_configuration_grade(
        self, overall_hit_rate, avg_response_time_ms, optimization_opportunities
    ) -> str:
        score = 100
    ...
    async def get_latest(self, db: AsyncSession) -> Optional[CacheConfigurationReport]:
        result = await db.execute(
            select(CacheConfigurationReport)
            .order_by(CacheConfigurationReport.created_at.desc())
            .limit(1)
        )
        return result.scalar_one_or_none()

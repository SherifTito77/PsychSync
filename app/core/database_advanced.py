...
def handle_error(exception_context):
    ...
    if context:
        db_monitor.record_query(
            str(context.statement) if context.statement else "UNKNOWN", 0.0, error=True
        )
...
class QueryOptimizer:
    ...
    async def analyze_table(db: AsyncSession, table_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error analyzing table {table_name}: {e!s}")
            return {}
...
class DatabaseMaintenance:
    ...
    async def vacuum_analyze_table(db: AsyncSession, table_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"VACUUM ANALYZE failed for {table_name}: {e!s}")
            return False
    ...
    async def update_table_statistics(db: AsyncSession, table_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Statistics update failed for {table_name}: {e!s}")
            return False
    ...
    async def reindex_table(db: AsyncSession, table_name: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Reindex failed for {table_name}: {e!s}")
            return False
...
async def init_advanced_db():
    ...
    try:
        ...
        logger.error(f"Advanced database initialization failed: {e!s}")
        raise

...
class ResourceAttributes:
    ...
    organization_id: str | None
    created_at: datetime
    requires_clearance: ClearanceLevel | None = None
...
class AccessPolicy:
    ...
    def evaluate(
        ...
        try:
            ...
            logger.error(f"Policy evaluation error ({self.name}): {e!s}")
...
class ABACService:
    ...
    def _init_default_policies(self):
        ...
        self.policies.append(
            AccessPolicy(
                ...
                condition=lambda u, r, e: (u.user_id == r.owner_id or u.is_superuser),
        ...
        self.policies.append(
            AccessPolicy(
                ...
                condition=lambda u, r, e: (
                    r.team_id is None or r.team_id in u.team_ids or u.is_superuser
                ),
    ...
    def _compare_clearance(
        self, user_clearance: ClearanceLevel, required_clearance: ClearanceLevel
    ) -> bool:
        """
    ...
    def _check_classification(
        ...
        if data_classification == DataClassification.RESTRICTED:
            return user_clearance in [ClearanceLevel.SECRET, ClearanceLevel.TOP_SECRET]
    ...
    def extract_environmental_context(self, request: Request) -> EnvironmentalContext:
        """
    ...
    def evaluate_access(
        self, user: User, resource_attrs: ResourceAttributes, request: Request
    ) -> tuple[bool, list[str]]:
        """
        ...
        for policy in self.policies:
            try:
                ...
                logger.error(f"Policy evaluation error: {e!s}")
...
# Decorator for ABAC
def require_abac(
    resource_type: str, classification: DataClassification = DataClassification.INTERNAL
):
    """

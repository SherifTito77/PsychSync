    ...
    async def execute_migration_plan(
        ...
        try:
            ...
            return {
                ...
                "duration_minutes": (datetime.utcnow() - datetime.fromisoformat(
                    execution_results[0]["start_time"] if execution_results else
                    migration_id.split("_")[1] + "_" + migration_id.split("_")[2]
                )).total_seconds() / 60 if execution_results else 0,
        ...
        try:
            async with self.engine.begin() as conn:
                ...
                if step.batch_size and step.batch_size > 0:
                    ...
                    await self.execute_batch_migration(
                        conn, step.sql, step.batch_size
                    )
                else:
                ...
                return {
                    ...
                    "rows_affected": "verification_completed" if step.verification_query else "not_verified"
                }
    ...
    async def verify_database_health(self) -> bool:
        ...
        try:
            async with self.engine.begin() as conn:
                ...
                long_queries = await conn.execute(text("""
                ...
                # Check table locks
                table_locks = await conn.execute(text("""
    ...
    async def verify_migration_success(self, plan: MigrationPlan) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_failures = [
                ...
                if r["critical"] and not r["exists"]
            ]
            ...
            return {
                ...
                "issues": [
                    f"Missing critical table: {r['table']}"

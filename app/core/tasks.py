...
class TaskRegistry:
    ...
    def register_task(
        self, task_id: str, task_name: str, max_retries: int = 3
    ) -> TaskMetadata:
        """Register a new task"""
    ...
    def get_task_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failure_rate": (
                (len(failed_tasks) / total_tasks * 100) if total_tasks > 0 else 0
            ),
            "avg_runtime": (
...
class CustomTask(Task):
    ...
    def on_retry(self, exc, task_id, args, kwargs, einfo):
        ...
        logger.warning(
            f"Task {task_id} retrying (attempt {task.retry_count if task else 1})"
        )
...
def process_assessment_results(
    assessment_id: str, user_id: str, framework: str, raw_data: Dict[str, Any]
) -> AsyncResult:

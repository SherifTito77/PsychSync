...
from app.dependency_injection.integration import setup_di_integration, di_health_check
...
def create_application(
    ...
    **kwargs
    ...
    try:
        ...
        conflicting_params = ['docs_url', 'redoc_url', 'openapi_url', 'debug', 'title', 'description', 'version', 'lifespan']
        filtered_kwargs = {k: v for k, v in kwargs.items()
                          if k not in conflicting_params}

        # Debug logging to see what parameters are causing conflicts
        if any(param in kwargs for param in conflicting_params):
            factory_logger.warning(f"Found conflicting parameters in kwargs: {[k for k in kwargs if k in conflicting_params]}")
        ...
        app = FastAPI(
            ...
            description=description or "PsychSync AI - Enterprise Psychological Assessment Platform",
            ...
            openapi_url="/openapi.json" if (debug if debug is not None else settings.DEBUG) else None,
            **filtered_kwargs
        )
...
def create_application_for_environment(environment: Optional[str] = None, **kwargs) -> FastAPI:
    """

...
class SecurityConfig:
    ...
    DEVICE_FINGERPRINTING_ENABLED: bool = Field(
        default=True,
        env="DEVICE_FINGERPRINTING_ENABLED"
    )
    ...
    REQUIRE_PASSWORD_COMPLEXITY: bool = Field(
        default=True,
        env="REQUIRE_PASSWORD_COMPLEXITY"
    )
    ...
    TOKEN_BLACKLIST_ENABLED: bool = Field(
        default=True,
        env="TOKEN_BLACKLIST_ENABLED"
    )
    BLACKLIST_CLEANUP_HOURS: int = Field(
        default=24,
        env="BLACKLIST_CLEANUP_HOURS"
    )
    ...
    AUDIT_SENSITIVE_OPERATIONS_ONLY: bool = Field(
        default=False,
        env="AUDIT_SENSITIVE_OPERATIONS_ONLY"
    )
    ...
    def validate_secret_key(cls, v, values):
        ...
        if environment == "production":
            ...
            if len(v) < 64:
                raise ValueError(
                    "SECRET_KEY must be at least 64 characters in development"
                )

        # Global forbidden patterns
        global_blacklist = [
            'secret', 'password', 'admin', 'root', 'test', 'demo', 'debug',
            'development', 'staging', 'production', 'example', 'sample',
            'changeme', 'default', '123456', 'qwerty', 'abc123'
        ]
        ...
        security_config_logger.info(
            f"SECRET_KEY validation passed",
            extra={
                "length": len(v),
                "entropy": entropy,
    ...
    def _has_sequential_pattern(key: str) -> bool:
        ...
        for i in range(len(key) - 2):
            ...
            if all(c.isalpha() for c in [char1, char2, char3]):
                if (ord(char2) == ord(char1) + 1 and
                    ord(char3) == ord(char2) + 1):
                    return True
                if (ord(char2) == ord(char1) - 1 and
                    ord(char3) == ord(char2) - 1):
                    return True

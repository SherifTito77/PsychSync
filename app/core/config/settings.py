...
class Settings(BaseSettings, ApplicationConfig, SecurityConfig, DatabaseConfig):
    ...
    SIEM_ENDPOINT_URL: Optional[str] = Field(default=None, env="SIEM_ENDPOINT_URL")
    SIEM_TOKEN: Optional[str] = Field(default=None, env="SIEM_TOKEN")
    SIEM_INDEX: Optional[str] = Field(default="psychsync-security", env="SIEM_INDEX")
    ...
    def _validate_cross_module_settings(self) -> None:
        ...
        if self.CACHE_ENABLED and not self.REDIS_URL:
            settings_logger.warning(
                "Cache enabled but Redis URL not configured"
            )

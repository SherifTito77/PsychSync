    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            try:
                ...
                logger.info(f"Database operation '{operation_name}' completed successfully", extra={
                ...
                logger.warning(f"Integrity error in {operation_name}: {error_message}", extra={
                    "operation": operation_name,
                ...
                logger.error(f"Operational error in {operation_name}: {error_message}", extra={
                    "operation": operation_name,
                ...
                if db:
                    try:
                        ...
                        logger.error(f"Failed to rollback transaction in {operation_name}: {rollback_error}")

                error_message = str(e)

                logger.error(f"Database error in {operation_name}: {error_message}", extra={
                    "operation": operation_name,
                ...
                logger.warning(f"Value error in {operation_name}: {str(e)}", extra={
                    "operation": operation_name,
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs) -> Any:
            try:
                ...
                logger.warning(f"Validation error in {service_name}.{func.__name__}: {e.message}", extra={
                ...
                logger.warning(f"Value error in {service_name}.{func.__name__}: {str(e)}", extra={
                    "service": service_name,
                    "function": func.__name__,
                    "error_type": "value_error",
                    "error_detail": str(e)
                ...
                logger.error(f"Unexpected error in {service_name}.{func.__name__}: {str(e)}", extra={
                    "service": service_name,
                    "function": func.__name__,
                    "error_type": "unexpected_error",
                    "error_detail": str(e),
...
class CircuitBreaker:
    ...
    def _should_attempt_reset(self) -> bool:
        ...
        import time
        return (time.time() - self.last_failure_time) >= self.timeout if self.last_failure_time else True

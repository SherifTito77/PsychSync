...
from app.core.redis_client import get_redis_client, redis_get, redis_set, redis_delete
from app.core.security import verify_jwt_token, generate_secure_token
...
class EnhancedSessionManager:
    ...
    async def invalidate_session(
        ...
        try:
            ...
            await redis_set(blacklist_key, json.dumps({
                "reason": reason,
    ...
    async def _store_session(self, session_data: SessionData):
        ...
        # Store session data
        await redis_set(session_key, json.dumps(session_data.to_dict()), expire_seconds=self.session_ttl)
        ...
        try:
            ...
            if current_level == DeviceTrustLevel.NEW:
                ...
                if usage_pattern["trust_score"] > 0.8:
                    ...
                    logger.info(f"Device {device_fingerprint.fingerprint_hash[:8]} promoted to trusted")
    ...
    async def _is_location_change_suspicious(
        self,
        old_ip: str,
        new_ip: str
    ) -> bool:
    ...
    async def _analyze_device_usage(self, user_id: str, device_fingerprint: DeviceFingerprint) -> Dict[str, Any]:

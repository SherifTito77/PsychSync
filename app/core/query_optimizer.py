...
class SQLQueryOptimizer:
    ...
    def _initialize_optimization_rules(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "suggestions": [
                    OptimizationType.ADD_INDEX,
                    OptimizationType.ADD_QUERY_HINT
                ],
    ...
    def _calculate_query_complexity(self, query: str) -> QueryComplexity:
        ...
        # Count subqueries
        subquery_count = len(re.findall(r'\bSELECT\b.*\bFROM\b.*\bWHERE\b.*\bIN\s*\(', query, re.IGNORECASE))
        complexity_score += subquery_count * 3

        # Count aggregate functions
        aggregate_count = len(re.findall(r'\b(COUNT|SUM|AVG|MAX|MIN|GROUP_CONCAT)\s*\(', query, re.IGNORECASE))
        complexity_score += aggregate_count

        # Count window functions
        window_count = len(re.findall(r'\b(OVER|PARTITION BY|ROW_NUMBER|RANK|DENSE_RANK)\b', query, re.IGNORECASE))
    ...
    def generate_optimization_report(self, query_metrics: QueryMetrics) -> List[OptimizationSuggestion]:
    ...
    def _parse_execution_plan(self, execution_plan: List[str]) -> Dict[str, Any]:
        ...
        for line in execution_plan:
                    ...
                    if "cost=" in line:
                        ...
                        if cost_match:
                            ...
                            if join_cost > 1000:  # High cost join
                                metrics["bottlenecks"].append(f"High-cost {join_type} (cost: {join_cost})")
            ...
            if "Workers" in line or "Parallel" in line:
                ...
                if workers_match:
                    metrics["parallel_workers"] = max(metrics["parallel_workers"], int(workers_match.group(1)))
        ...
        if metrics["seq_scans"] > 0:
            metrics["optimization_opportunities"].append("Consider adding indexes to eliminate sequential scans")

        if metrics["sort_operations"] > 2:
            metrics["optimization_opportunities"].append("Consider reducing sort operations or adding appropriate indexes")

        if metrics["temp_files"] > 0:
            metrics["optimization_opportunities"].append("Increase work_mem to avoid disk-based operations")

        if len(metrics["join_methods"]) > 3:
            metrics["optimization_opportunities"].append("Complex multi-join queries may benefit from denormalization or query restructuring")
    ...
    def get_slow_queries(
        self,
        hours: int = 24,
        min_execution_time: float = 500
    ) -> List[QueryMetrics]:
        """Get slow queries from history"""
        cutoff_time = datetime.utcnow() - timedelta(hours=hours)

        slow_queries = [
            metrics for metrics in self.query_history.values()
            if (metrics.timestamp >= cutoff_time and
                metrics.execution_time_ms >= min_execution_time)
        ]
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        slow_queries_count = len([m for m in self.query_history.values() if m.execution_time_ms > self.slow_query_threshold])

        total_suggestions = sum(len(m.optimization_suggestions) for m in self.query_history.values())
...
class AutomaticQueryOptimizer:
    ...
    def _apply_eager_loading_optimization(
        self,
        query: Select,
        metrics: QueryMetrics
    ) -> Select:
                ...
                try:
                    for relationship in relationships:
                        if 'selectinload' in relationship:
                            ...
                            rel_name = relationship.split('"')[1] if '"' in relationship else relationship
                ...
                for i in range(len(tables_columns)):
                    for j in range(i + 1, len(tables_columns)):
                        ...
                        if table1 == table2:  # Same table - good candidate for composite index
                            ...
                            index_suggestions[index_key]['avg_execution_time'] += metrics.execution_time_ms
                            index_suggestions[index_key]['estimated_improvement'] += metrics.execution_time_ms * 0.6
        ...
        for index_data in index_suggestions.values():
            if index_data['query_count'] >= 3:  # Only suggest if used by multiple queries
                ...
                recommendations.append({
                    ...
                    'priority': 'HIGH' if index_data['estimated_improvement'] > 1000 else 'MEDIUM'
                })

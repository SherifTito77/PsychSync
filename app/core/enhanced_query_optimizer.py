...
from sqlalchemy.schema import Index, CreateIndex
...
class EnhancedQueryOptimizer:
    ...
    async def analyze_query_performance(
        ...
        async with self._lock:
            if query_hash not in self.query_stats:
                self.query_stats[query_hash] = QueryStatistics(
        ...
        self.logger.info(
            ...
            cache_hit=f"{stats.cache_hits/(stats.cache_hits+stats.cache_misses)*100:.1f}%" if stats.cache_hits + stats.cache_misses > 0 else "0%"
        )
    ...
    async def get_query_optimization_recommendations(
        ...
        return sorted(
            [r for r in all_recommendations if priority_order[r.priority] <= priority_order[min_priority]],
    ...
    async def get_performance_report(self, hours: int = 24) -> Dict[str, Any]:
        ...
        slow_queries = [
            (hash_key, stats) for hash_key, stats in recent_stats.items()
        ...
        return {
            ...
            "slow_queries": [
                ...
                for hash_key, stats in sorted(slow_queries, key=lambda x: x[1].avg_time_ms, reverse=True)
            ],
            "cache_hit_rate": (
                ...
                if sum(stats.cache_hits + stats.cache_misses for stats in recent_stats.values()) > 0 else 0
            ) * 100,
    ...
    async def _identify_query_pattern(self, query: str) -> Optional[QueryPattern]:

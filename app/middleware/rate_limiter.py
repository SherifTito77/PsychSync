...
class RateLimitExceeded(HTTPException):
    ...
    def __init__(self, retry_after: int = None, detail: str = None):
        super().__init__(
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self,
                 app,
                 redis_url: str = None,
                 default_limits: Optional[Dict[str, RateLimitConfig]] = None):
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        remaining_minute, remaining_hour = await self._get_remaining_tokens(
    ...
    async def _get_applicable_limits(
        ...
        # Add default limits
        rate_limits['default'] = self.default_limits.get('default', RateLimitConfig(60, 1000, 10000, 10))
        ...
        if path.startswith('/api/v1/auth/'):
            ...
            rate_limits['admin'] = self.default_limits.get('admin', RateLimitConfig(30, 500, 2000, 5))
        elif path.startswith('/api/v1/'):
            rate_limits['api'] = self.default_limits.get('api', RateLimitConfig(120, 5000, 50000, 20))
        elif path.startswith('/health'):
            rate_limits['health'] = self.default_limits.get('health', RateLimitConfig(1000, 10000, 100000, 50))
    ...
    async def _check_burst_limit(
        ...
        try:
            ...
            logger.error(f"Error checking burst limit: {str(e)}")
            return True
        ...
        try:
            ...
            if identifier.startswith('user:'):
                ...
                if user_config:
                    ...
                    remaining_minute = max(0, min(remaining_minute, user_config.calls_per_minute - user_minute_count))
                    remaining_hour = max(0, min(remaining_hour, user_config.calls_per_hour - user_hour_count))
            ...
            if identifier:
                ...
                for limit_name, config in rate_limits.items():
                    try:
                        ...
                        logger.error(f"Error getting limit status for {limit_name}: {str(e)}")
                        continue
    ...
    async def get_rate_limit_statistics(
        ...
        try:
            ...
            daily_pattern = f"rate_limit:*:*:day"
...
# Public wrapper function for backward compatibility
# Allows endpoints to import check_rate_limit from this module
async def check_rate_limit(
    ...
    """
    Public rate limit checking function for use in endpoints.

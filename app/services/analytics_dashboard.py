...
class AnalyticsDashboard:
    ...
    def __init__(self, db_session: Session, config: DashboardConfig | None = None):
        self.db = db_session
        ...
        try:
            ...
            if self.redis_client:
                await self.redis_client.setex(
                    cache_key, self.config.cache_ttl, json.dumps(overview, default=str)
                )
    ...
    async def _get_user_metrics(
        self, time_period: TimePeriod, organization_id: str | None, team_id: str | None
    ) -> dict[str, Any]:
    ...
    async def _get_business_metrics(
        self, time_period: TimePeriod, organization_id: str | None
    ) -> dict[str, Any]:

...
class BigFiveScorer:
    ...
    def _determine_profile(factor_scores: dict[str, float]) -> str:
        ...
        if "neuroticism" in low_factors and len(high_factors) >= 2:
            return "Emotionally stable with positive traits"
        if "openness" in high_factors and "conscientiousness" in high_factors:
            return "Creative and organized achiever"
        if "extraversion" in high_factors and "agreeableness" in high_factors:

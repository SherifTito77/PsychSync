...
class DISCScorer:
    ...
    def calculate_scores(
        response: AssessmentResponse, config: AssessmentScoringConfig
    ) -> dict[str, Any]:
        ...
        interpretation = DISCScorer._get_interpretation(
            dimension_scores, primary_style, secondary_style
        )
    ...
    def _calculate_dimension_score(
        dimension: str, question_ids: list[int], responses: dict[str, Any]
    ) -> float:
    ...
    def _get_interpretation(scores: dict[str, float], primary: str, secondary: str = None) -> str:
        ...
        for dimension, score in sorted(scores.items(), key=lambda x: x[1], reverse=True):
            ...
            interpretation.append(
                f"- {dimension} ({dim_info.get('name')}): {score:.1f}/100 ({level})"
            )
    ...
    def _get_communication_style(style: str) -> list[str]:
        ...
        styles = {
            ...
            "S": [
                ...
                "Good listener",
                "Prefers calm, non-confrontational dialogue",
            ],
            "C": [
                "Precise and detailed",
                "Prefers written communication",
                "Wants data and evidence",
            ],
        }
    ...
    def create_default_config(
        assessment_id: int, question_mapping: dict[str, list[int]]
    ) -> dict[str, Any]:
        """

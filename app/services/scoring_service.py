...
class ScoringService:
    ...
    async def _calculate_enneagram_scores(
        ...
        return {
            ...
            "response_count": len(responses),
            "interpretation": ScoringService._get_enneagram_interpretation(primary_type, final_scores),
            "assessment_date": assessment.completed_at or datetime.utcnow()
        }
    ...
    def _extract_personality_traits(scores: Dict[str, Any]) -> Dict[str, float]:
        ...
        if framework == "big_five":
            return scores.get("trait_scores", {
                "openness": 60.0,
            ...
            return {
                "extraversion": 80.0 if dichotomies.get("E_I", {}).get("preference") == "E" else 30.0,
                "openness": 80.0 if dichotomies.get("S_N", {}).get("preference") == "N" else 40.0,
                "agreeableness": 80.0 if dichotomies.get("T_F", {}).get("preference") == "F" else 40.0,
                "conscientiousness": 80.0 if dichotomies.get("J_P", {}).get("preference") == "J" else 50.0,

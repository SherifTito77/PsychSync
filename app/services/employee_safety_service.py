...
class WellnessAssessmentData:
    ...
    physical_wellness: Optional[float] = None
...
class EmployeeSafetyService:
    ...
    def __init__(self, db: Session):
        ...
        self.wellness_thresholds = {
            ...
            'mental_health_score': {'elevated': 5.0, 'high': 4.0, 'critical': 3.0},  # Lower is worse
    ...
    async def report_incident(self, incident_data: IncidentReportData, reporter_id: UUID,
                             organization_id: UUID, team_id: Optional[UUID] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if incident_data.incident_type in [
            ...
            return {
                "success": False,
                "error": f"Failed to report incident: {str(e)}"
            }

    async def get_incident(self, incident_id: UUID, user_id: UUID) -> Optional[Dict[str, Any]]:
        """Get incident details with appropriate access control"""
        incident = self.db.query(SafetyIncident).filter(
    ...
    async def update_incident_status(self, incident_id: UUID, new_status: IncidentStatus,
                                   investigator_id: UUID, notes: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def get_incidents_dashboard(self, organization_id: UUID, team_id: Optional[UUID] = None,
        ...
        try:
            ...
            critical_incidents = [inc for inc in incidents
            ...
            return {
                ...
                "incidents": [self._serialize_incident(inc) for inc in incidents[:50]]  # Last 50 incidents
            }

        except Exception as e:
            logger.error(f"Error getting incidents dashboard: {str(e)}")
            return {"error": f"Failed to load dashboard: {str(e)}"}

    # Wellness Monitoring

    async def create_wellness_assessment(self, assessment_data: WellnessAssessmentData) -> Dict[str, Any]:
        ...
        try:
            ...
            insights, recommendations = await self._generate_wellness_insights(assessment_data, risk_factors)
            ...
            logger.info(f"Wellness assessment created for user {assessment_data.user_id}: score {overall_score}")
            ...
            logger.error(f"Error creating wellness assessment: {str(e)}")
            self.db.rollback()
            return {"success": False, "error": f"Failed to create assessment: {str(e)}"}

    async def get_wellness_dashboard(self, organization_id: UUID, team_id: Optional[UUID] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            assessments = query.filter(
                WellnessAssessment.assessment_date >= thirty_days_ago
            ).order_by(desc(WellnessAssessment.assessment_date)).all()
            ...
            # High-risk employees
            high_risk_assessments = [a for a in assessments
            ...
            return {
                ...
                "recent_assessments": [self._serialize_wellness_assessment(a) for a in assessments[:20]]
            }

        except Exception as e:
            logger.error(f"Error getting wellness dashboard: {str(e)}")
            return {"error": f"Failed to load wellness dashboard: {str(e)}"}

    async def get_employee_wellness_history(self, user_id: UUID,
        ...
        try:
            ...
            logger.error(f"Error getting employee wellness history: {str(e)}")
            return []

    # Safety Resources and Training

    async def create_safety_resource(self, organization_id: UUID, title: str, description: str,
                                   resource_type: str, content: Optional[str] = None,
                                   file_url: Optional[str] = None, category: Optional[str] = None,
        ...
        try:
            ...
            logger.error(f"Error creating safety resource: {str(e)}")
            self.db.rollback()
            return {"success": False, "error": f"Failed to create resource: {str(e)}"}

    async def get_safety_resources(self, organization_id: UUID, resource_type: Optional[str] = None,
                                 category: Optional[str] = None, user_role: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    def _can_access_incident(self, incident: SafetyIncident, user_id: UUID) -> bool:
        ...
        if incident.team_id:
            team_member = self.db.query(Team).filter(
    ...
    def _serialize_incident(self, incident: SafetyIncident) -> Dict[str, Any]:
        ...
        return {
            ...
            "affected_user_id": str(incident.affected_user_id) if incident.affected_user_id else None,
    ...
    def _serialize_safety_resource(self, resource: SafetyResource) -> Dict[str, Any]:
        ...
        return {
            ...
            "next_review_date": resource.next_review_date.isoformat() if resource.next_review_date else None,
    ...
    def _assess_wellness_risks(self, assessment_data: WellnessAssessmentData) -> Tuple[List[str], AlertLevel]:
        ...
        for metric_name, value in [
            ...
            if metric_name in self.wellness_thresholds:
                ...
                lower_is_worse = metric_name in ["work_life_balance", "mental_health_score", "sleep_quality", "engagement_level"]
                ...
                if lower_is_worse:
                    if value <= thresholds["critical"]:
                        ...
                        max_alert_level = max(max_alert_level, AlertLevel.HIGH, key=lambda x: ["normal", "elevated", "high", "critical"].index(x.value))
                    elif value <= thresholds["elevated"]:
                        risk_factors.append(f"Elevated {metric_name.replace('_', ' ')}")
                        max_alert_level = max(max_alert_level, AlertLevel.ELEVATED, key=lambda x: ["normal", "elevated", "high", "critical"].index(x.value))
                else:
    ...
    async def _generate_wellness_insights(self, assessment_data: WellnessAssessmentData,
                                        risk_factors: List[str]) -> Tuple[Dict[str, Any], List[str]]:
    ...
    async def _create_wellness_alerts(self, assessment: WellnessAssessment,
                                    alert_level: AlertLevel, risk_factors: List[str]) -> None:
        ...
        try:
            ...
            for risk_factor in risk_factors[:3]:  # Limit to top 3
                alert = WellnessAlert(
                    ...
                    trigger_metrics={risk_factor.lower(): getattr(assessment, risk_factor.lower(), None)},
                    recommended_actions=[
            ...
            logger.error(f"Error creating wellness alerts: {str(e)}")
    ...
    async def _send_status_update_notifications(self, incident: SafetyIncident,
    ...
    async def _send_critical_wellness_alerts(self, assessment: WellnessAssessment,
                                           risk_factors: List[str]) -> None:
        """Send notifications for critical wellness alerts"""
        # Implementation for critical wellness notifications
        pass

    def _calculate_average_wellness_scores(self, assessments: List[WellnessAssessment]) -> Dict[str, float]:
        """Calculate average wellness scores from list of assessments"""
        if not assessments:
            return {}

        metrics = [
            'stress_level', 'burnout_risk', 'work_life_balance', 'mental_health_score',
            'physical_wellness', 'sleep_quality', 'social_connection', 'job_satisfaction', 'engagement_level'
        ]
        ...
        # Add overall score
        overall_scores = [a.overall_wellness_score for a in assessments if a.overall_wellness_score is not None]
        if overall_scores:
            averages['overall_score'] = round(sum(overall_scores) / len(overall_scores), 2)

        return averages

    def _calculate_wellness_trends(self, assessments: List[WellnessAssessment]) -> Dict[str, List[float]]:
        """Calculate wellness trends over time"""
        # Group assessments by week
        weekly_data = {}
        for assessment in assessments:
            week_start = assessment.assessment_date - timedelta(days=assessment.assessment_date.weekday())

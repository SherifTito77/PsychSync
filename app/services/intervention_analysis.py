...
class InterventionAnalyzer:
    ...
    ) -> Optional[InterventionAnalysisResult]:
    ...
    async def _get_intervention_metrics(self, intervention_id: str) -> List[str]:
        ...
        pre_metrics = self.db.query(PreInterventionMeasurement.metric_name).filter(
            PreInterventionMeasurement.intervention_id == intervention_id
        ).distinct().all()

        # Get unique metrics from post measurements
        post_metrics = self.db.query(PostInterventionMeasurement.metric_name).filter(
    ...
    async def _get_pre_post_measurements(
        ...
        ).group_by(PreInterventionMeasurement.user_id).subquery()
        ...
        metadata = {
            ...
            "data_collection_dates": {
                ...
                "latest_post": max([row.post_date for row in paired_data]).isoformat() if paired_data else None
            }
        }
    ...
    async def _calculate_effect_sizes(self, data: PrePostData) -> List[EffectSizeResult]:
        ...
        try:
            ...
            if pre_std > 0:
                ...
                se_d = math.sqrt((len(data.pre_values) / len(data.post_values)) +
    ...
    def _determine_clinical_significance(self, effect_sizes: List[EffectSizeResult]) -> Optional[str]:
    ...
    def _generate_recommendations(
        ...
        if significant_tests:
            if len(significant_tests) >= 2:  # Multiple significant tests
                recommendations.append("Strong evidence of intervention effectiveness - consider scaling")
            else:
                recommendations.append("Moderate evidence of effectiveness - consider replication with larger sample")
        else:
            recommendations.append("No statistically significant effects detected - reconsider intervention approach")

        # Check effect sizes
        large_effects = [
            es for es in effect_sizes
            if es.magnitude in ["large", "very_large"]
        ]

        if large_effects:
            recommendations.append("Large practical effects observed - high potential for organizational impact")
        elif effect_sizes:
            medium_effects = [
                es for es in effect_sizes
                if es.magnitude == "medium"
            ]
            if medium_effects:
                recommendations.append("Medium effects observed - valuable but may need optimization")

        # Sample size considerations
        if len(data.pre_values) < 20:
            recommendations.append("Small sample size - interpret results with caution, consider larger scale implementation")
        elif len(data.pre_values) < 50:
            recommendations.append("Moderate sample size - results promising but would benefit from larger scale validation")
        ...
        for test in statistical_tests:
            if test.assumptions_met:
                ...
                if violated_assumptions:
                    limitations.append(f"{test.test_name.value} assumptions violated: {', '.join(violated_assumptions)}")

        # Study design limitations
        limitations.append("Pre-post design without control group - results may be influenced by confounding factors")

        # Measurement limitations
        if len(set(data.measurement_dates)) == 1:
            limitations.append("All measurements taken on same date - may limit temporal generalizability")
    ...
    def _perform_power_analysis(
        ...
        diff = np.array(data.post_values) - np.array(data.pre_values)
        mean_diff = np.mean(diff)
        pooled_sd = np.sqrt((np.var(data.pre_values, ddof=1) + np.var(data.post_values, ddof=1)) / 2)
        ...
        try:
            ...
            power = 1 - stats.nct.cdf(critical_t, n - 1, noncentral_t) + \

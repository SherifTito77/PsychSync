...
class TrendAnalysisService:
    ...
    async def _process_trend_data(
        ...
        for response in responses:
            try:
                ...
                trend_point = {
                    "date": response.completed_at.isoformat() if response.completed_at else datetime.utcnow().isoformat(),
    ...
    def _get_assessment_type(self, response: Response) -> str:
        ...
        if response.assessment_template:
            ...
            elif "phq" in template_name or "gad" in template_name:
                return "Mental Health Screening"
            elif "mbti" in template_name:
                return "Personality Assessment"
            elif "big five" in template_name:
    ...
    async def _generate_trend_insights(
        ...
        try:
            ...
            if overall_trend == "improving":
                insights.append("Your overall wellness is showing positive improvement over time. Keep up the great work!")
            elif overall_trend == "declining":
                insights.append("Your wellness scores show a declining trend. Consider focusing on self-care and professional support.")
            else:
                insights.append("Your wellness levels are relatively stable. Small, consistent changes can lead to significant improvements.")

            # Domain-specific insights
            if not domains or 'all' in domains:
                domain_trends = {}
                for domain in ['physical', 'emotional', 'social', 'intellectual', 'spiritual', 'occupational', 'environmental']:
                    scores = [d["domain_scores"].get(domain, 0.5) for d in trend_data if domain in d["domain_scores"]]
                    if scores:
                ...
                if improving_domains:
                    insights.append(f"Your {improving_domains[0]} wellness is showing the most improvement. This is a great strength to build upon.")

                # Areas needing attention
                declining_domains = [d for d, t in domain_trends.items() if t == "declining"]
                if declining_domains:
                    insights.append(f"Consider focusing more attention on your {declining_domains[0]} wellness, which shows room for improvement.")
            ...
            if assessment_frequency < 0.5:  # Less than 2 assessments per month
                insights.append("More frequent assessments can provide better insights and help track progress more effectively.")
            ...
            if len(recent_data) >= 3:
                ...
                if abs(recent_change) > 0.1:
                    if recent_change > 0:
                        insights.append("Recent assessments show significant improvement. Your current approach is working well!")
                    else:
                        insights.append("Recent scores indicate some challenges. This is normal - focus on self-compassion and consider reaching out for support.")

        except Exception as e:
            logger.error(f"Error generating trend insights: {e}")
            insights.append("Trend analysis is available. Continue regular assessments for more detailed insights.")
    ...
    async def _identify_patterns(self, trend_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            weekly_averages = {day: np.mean(scores) for day, scores in weekly_scores.items() if scores}
            if weekly_averages:
            ...
            patterns["variability"] = {
                ...
                "stability": "high" if score_variance < 0.01 else "medium" if score_variance < 0.05 else "low"
            }
    ...
    async def get_domain_comparison(
        ...
        try:
            ...
            domain_analysis = {}
            domains = ['physical', 'emotional', 'social', 'intellectual', 'spiritual', 'occupational', 'environmental']
            ...
            return {
                ...
                "data": {
                    ...
                    "strongest_domain": max(domain_analysis.keys(), key=lambda d: domain_analysis[d]["average"]) if domain_analysis else None,
                    "most_improved_domain": max(domain_analysis.keys(), key=lambda d: domain_analysis[d]["improvement"]) if domain_analysis else None,
                    "attention_needed": [d for d, analysis in domain_analysis.items() if analysis["trend"] == "declining"]
                }
            }

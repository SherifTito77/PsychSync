"""
SECURE Email Service for sending notifications and communications
...
class EmailService:
    ...
    def _validate_email_address(self, email: str) -> bool:
        ...
        """
        try:
            # Basic format validation using our security validator
    ...
    def _check_rate_limit(self, sender_email: str) -> bool:
        ...
        # In production, this would check a cache/Redis for rate limiting
        return True
    ...
    async def send_email(
        ...
        try:
            ...
            if not self._validate_email_address(email_to):
                AuditLogger.log_security_event(
            ...
            # Check subject length and sanitize
            if len(subject) > 200:
                subject = subject[:200] + "..."
            subject = bleach.clean(subject, tags=[], strip=True)

            # Validate sender email if provided
            if sender_email and not self._validate_email_address(sender_email):
                AuditLogger.log_security_event(
                    event_type=SecurityEventType.INVALID_INPUT,
            ...
            # Check rate limiting
            if not self._check_rate_limit(sender_email or "default"):
                AuditLogger.log_security_event(
            ...
            # PREPARE MESSAGE WITH SECURITY HEADERS
            tracking_id = tracking_id or secrets.token_urlsafe(16)
            ...
            try:
                ...
                AuditLogger.log_security_event(
                    event_type=SecurityEventType.DATA_ACCESS,
                    details=f"Email sent successfully to {email_to}",
                    additional_data={
                ...
                # LOG SENDING ERROR (without exposing sensitive data)
                AuditLogger.log_security_event(
                    event_type=SecurityEventType.SYSTEM_ERROR,
                    details=f"Email sending failed for {email_to}",
                    additional_data={
                        "recipient_email": email_to,
                        "subject": subject,
                        "tracking_id": tracking_id,
    ...
    async def send_verification_email(self, email: str, token: str, name: str):
        ...
        return await self.send_email(email, subject, verify_link, template_content)

    async def send_welcome_email_legacy(self, email: str, name: str):
        """Send welcome email after verification (legacy method)"""
        if not settings.SMTP_HOST or not settings.SMTP_USER:
        ...
        try:
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                    ...
                    <div class="warning-box" style="background: #fef3c7; border-left: 4px solid #f59e0b; padding: 15px; margin: 20px 0; border-radius: 0 6px 6px 0;">
                        <h4 style="margin-top: 0;">⚠️ Security Notice:</h4>
                        <p style="margin-bottom: 0;">This link will expire in 1 hour for your security. If you didn't request a password reset, please ignore this email.</p>
                    </div>
                    <p style="font-size: 14px; color: #64748b;">If you have any trouble, contact our support team.</p>
                </div>
            </body>
            </html>
            """
        ...
        try:
            ...
            <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
                <div style="background: linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%); padding: 30px 20px; text-align: center; border-radius: 8px 8px 0 0;">
                    <h1 style="color: white; font-size: 28px; margin: 0;">You're Invited to Join {team_name}!</h1>
                </div>
                <div style="background: white; padding: 30px 20px; border: 1px solid #e5e7eb; border-radius: 0 0 8px 8px;">
                    ...
                    <p><strong>{inviter_name}</strong> has invited you to join their team <strong>{team_name}</strong> on PsychSync, the AI-powered team intelligence platform.</p>

                    <div style="background: #f0f9ff; border-left: 4px solid #0284c7; padding: 20px; margin: 20px 0; border-radius: 0 6px 6px 0;">
                        <h4 style="margin-top: 0;">What is PsychSync?</h4>
                        <ul style="margin-bottom: 0;">
                            <li>AI-powered personality assessments</li>
                            <li>Team optimization and insights</li>
                            <li>Collaboration improvement tools</li>
                            <li>Data-driven team building</li>
                        </ul>
                    </div>
                    ...
                    <p style="font-size: 14px; color: #64748b; text-align: center; margin: 20px 0;">Or copy this link: <code style="background: #f1f5f9; padding: 2px 6px; border-radius: 4px;">{invite_link}</code></p>
                    <p style="font-size: 12px; color: #9ca3af; text-align: center; margin-top: 30px;">This invitation will expire in 7 days.</p>
                </div>
            </body>
            </html>
            """

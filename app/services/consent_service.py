...
from sqlalchemy import Column, String, Text, DateTime, Boolean, Integer, ForeignKey, and_, or_
...
class ConsentPurpose(Base):
    ...
    legal_basis = Column(String(100), nullable=False)  # consent, contract, legal_obligation, vital_interests, public_task, legitimate_interests
...
class ConsentHistory(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    consent_record_id = Column(UUID(as_uuid=True), ForeignKey("user_consent_records.id"), nullable=False)
    ...
    admin_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True)  # If changed by admin
...
class ConsentManagementService:
    ...
    async def initialize_consent_purposes(self, db: Session):
        ...
        for purpose_data in default_purposes:
            existing = db.query(ConsentPurpose).filter(
                ConsentPurpose.consent_type == purpose_data["consent_type"]
            ).first()
    ...
    async def grant_consent(
        ...
        try:
            ...
            purpose = db.query(ConsentPurpose).filter(
                ConsentPurpose.consent_type == consent_type
            ).first()
    ...
    async def withdraw_consent(
        ...
        try:
            ...
            consent_record = db.query(UserConsentRecord).filter(
                and_(
                    UserConsentRecord.user_id == user_id,
                    UserConsentRecord.consent_type == consent_type,
                    UserConsentRecord.status == ConsentStatus.ACTIVE
    ...
    async def get_user_consents(
        ...
        try:
            ...
            for purpose in purposes:
                user_consent = next((c for c in consent_records if c.consent_type == purpose.consent_type), None)
                ...
                consent_status[purpose.consent_type] = {
                    ...
                    "expires_at": user_consent.expires_at.isoformat() if user_consent and user_consent.expires_at else None,
                    "granular_preferences": user_consent.granular_preferences if user_consent else None,
            ...
            result = {
                ...
                "last_updated": max((c.updated_at for c in consent_records), default=datetime.utcnow()).isoformat()
            }
        ...
        try:
            ...
            for consent_item in consent_data:
                try:
                    ...
                    errors.append(f"Failed to process consent item {consent_item}: {str(e)}")
            ...
            logger.error(f"Failed to process consent request: {str(e)}")
            raise
    ...
    async def get_consent_analytics(
        ...
        try:
            ...
            consent_records = db.query(UserConsentRecord).filter(
                and_(
            ...
            for consent in consent_records:
                ...
                analytics["consent_by_type"][consent_type] = analytics["consent_by_type"].get(consent_type, 0) + 1

                # By language
                language = consent.consent_language or "en"
                analytics["consent_by_language"][language] = analytics["consent_by_language"].get(language, 0) + 1
                ...
                analytics["consent_form_versions"][version] = analytics["consent_form_versions"].get(version, 0) + 1

            # Get current active consents
            active_consents = db.query(UserConsentRecord).filter(
                and_(
                    UserConsentRecord.status == ConsentStatus.ACTIVE,
                    UserConsentRecord.expires_at > datetime.utcnow() if UserConsentRecord.expires_at.isnot(None) else True
                )
            ).all()
            ...
            for consent in active_consents:
                consent_type = consent.consent_type
                analytics["active_consent_breakdown"][consent_type] = analytics["active_consent_breakdown"].get(consent_type, 0) + 1

            return analytics

        except Exception as e:
            logger.error(f"Failed to get consent analytics: {str(e)}")
            raise
    ...
    async def _log_consent_change(
        ...
        try:
            ...
            logger.error(f"Failed to log consent change: {str(e)}")
    ...
    async def _get_consent_history(
        ...
        try:
            ...
            ).order_by(ConsentHistory.timestamp.desc()).limit(limit).all()
    ...
    async def cleanup_expired_consents(self, db: Session):
        ...
        try:
            ...
            expired_consents = db.query(UserConsentRecord).filter(
                and_(
                    UserConsentRecord.status == ConsentStatus.ACTIVE,

...
class AIMetric:
    ...
    threshold_breach: Optional[float] = None
...
class AIMonitoringService:
    ...
    async def get_ai_health_status(self) -> AIHealthStatus:
        ...
        try:
            ...
            overall_health_score = sum(performance_scores) / len(performance_scores) if performance_scores else 1.0
            ...
            recommendations = await self._generate_recommendations(recent_metrics, overall_health_score)
    ...
    async def _health_check_loop(self) -> None:
        ...
        while self.monitoring_active:
            try:
                ...
                # Log health status
                logger.info(f"AI Health Status: {health_status.overall_status} (Score: {health_status.health_score:.2f})")
    ...
    async def _check_threshold_breach(self, metric: AIMetric) -> Optional[float]:
        ...
        try:
            ...
            elif metric.value <= thresholds.get("critical_low", float('-inf')):
                return thresholds["critical_low"]
            elif metric.value <= thresholds.get("warning_low", float('-inf')):
    ...
    def _calculate_health_score(self, metric_type: MetricType, value: float) -> float:
        ...
        try:
            ...
            if metric_type in [MetricType.ACCURACY_SCORE, MetricType.CONFIDENCE_SCORE,
                             MetricType.ENGAGEMENT_PREDICTION_ACCURACY,
                             MetricType.PERSONALIZATION_EFFECTIVENESS, MetricType.CACHE_HIT_RATE]:
                if value >= 0.8:
                    return 1.0
                elif value >= 0.6:
                    return 0.8
                elif value >= thresholds.get("critical", 0.5):
                    return 0.6
                else:
                    return 0.3
            ...
            # For metrics where lower is better (processing time, error rate, etc.)
                ...
                if metric_type == MetricType.PROCESSING_TIME:
                    if value <= 1000:  # 1 second
                        ...
                        return 0.8
                    elif value <= thresholds.get("warning", 2000):
                        return 0.6
                    else:
                        return 0.3
                    ...
                    if value <= 0.01:  # 1%
                        ...
                        return 0.8
                    elif value <= thresholds.get("warning", 0.05):
                        return 0.6
                    else:
                        return 0.3
            ...
            for metric_type in MetricType:
                ...
                if type_metrics:
                    ...
                    if metric_type == MetricType.PROCESSING_TIME and avg_value > 2000:
                        recommendations.append("Consider optimizing AI algorithms for faster processing")
                        ...
                        recommendations.append("Optimize AI memory usage or allocate more resources")
    ...
    async def _handle_critical_health(self, health_status: AIHealthStatus) -> None:
        ...
        try:
            ...
            logger.error(f"CRITICAL: AI Engine health is critical - Score: {health_status.health_score:.2f}")

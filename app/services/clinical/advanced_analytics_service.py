...
class AdvancedAnalyticsService:
    ...
    async def calculate_user_trends(
        self, user_id: str, assessment_type: str, min_data_points: int = 3
        ...
        if len(assessments) < min_data_points:
            logger.info(
                f"Insufficient data points for trend analysis: {len(assessments)} < {min_data_points}"
            )
            return None

        # Extract scores and dates
        scores_data = [(a.completed_at, float(a.total_score)) for a in assessments]
    ...
    async def _calculate_period_change(
        self, user_id: str, assessment_type: str, days: int
    ) -> Optional[float]:
    ...
    def _generate_trend_interpretation(
        self, direction: str, slope: float, r_squared: float, assessment_type: str
    ) -> str:
    ...
    async def get_population_health_metrics(
        ...
        if group_by == "week":
            date_trunc = func.date_trunc(
                "week", ClinicalAssessmentExtended.completed_at
            )
        elif group_by == "month":
            date_trunc = func.date_trunc(
                "month", ClinicalAssessmentExtended.completed_at
            )
        else:
        ...
        query = (
            select(
                ...
                func.count(func.distinct(ClinicalAssessmentExtended.user_id)).label(
                    "unique_users"
                ),
                func.avg(ClinicalAssessmentExtended.total_score).label("mean_score"),
                func.percentile_cont(0.5)
                .within_group(ClinicalAssessmentExtended.total_score)
                .label("median_score"),
                func.stddev(ClinicalAssessmentExtended.total_score).label("std_dev"),
                func.count(func.distinct(ClinicalAssessmentExtended.user_id))
                .filter(ClinicalAssessmentExtended.risk_level == "high")
                .label("high_risk_count"),
                func.count(func.distinct(ClinicalAssessmentExtended.user_id))
                .filter(ClinicalAssessmentExtended.crisis_alert == True)
                .label("crisis_count"),
            )
            .where(
                and_(
        ...
        for row in rows:
            ...
            metrics.append(
                PopulationHealthMetrics(
                    ...
                    crisis_rate=(
                        float((row.crisis_count or 0) / row.unique_users * 100)
                        if row.unique_users > 0
                        else 0.0
                    ),
                    high_risk_rate=(
                        float((row.high_risk_count or 0) / row.unique_users * 100)
                        if row.unique_users > 0
                        else 0.0
                    ),
    ...
    async def _get_severity_distribution(
        self, assessment_type: str, start_date: datetime, end_date: datetime
    ) -> Dict[str, int]:
    ...
    async def identify_high_risk_users(
        self, assessment_type: str, limit: int = 50
    ) -> List[Dict[str, any]]:
        """
            ...
            .where(
                and_(
                    ...
                    or_(
                        ...
                        AssessmentTrend.mean_score
                        > 15,  # Threshold depends on assessment type
        ...
        return [
            {
                ...
                "change_30d": (
                    float(trend.score_change_30d) if trend.score_change_30d else None
                ),
                "high_risk_episodes": trend.high_risk_episodes,
                "risk_level": (
                    "high"
                    if trend.mean_score > 15 or trend.high_risk_episodes > 0
                    else "moderate"
                ),
            }
            for trend in trends
    ...
    async def refresh_materialized_view(self):
        ...
        await self.db.execute(
            text("REFRESH MATERIALIZED VIEW CONCURRENTLY population_health_stats")
        )
        await self.db.commit()
...
async def calculate_user_trends_task(user_id: str, assessment_types: List[str]):
    """
    ...
    async for db in get_async_db():
        ...
        for assessment_type in assessment_types:
            try:
                ...
                logger.error(
                    f"Failed to calculate trends for user {user_id}, assessment {assessment_type}: {e}"
                )

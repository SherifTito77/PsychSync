...
class ClinicalScreeningAnalytics:
    ...
    async def get_screening_completion_stats(
        ...
        if screening_type:
            screening_base = screening_base.where(ClinicalScreening.screening_type == screening_type)
        ...
        unique_users_query = select(func.count(func.distinct(ClinicalScreening.user_id))).select_from(screening_base)
        unique_users_result = await self.db.execute(unique_users_query)
        unique_users_completed = unique_users_result.scalar() or 0

        # Calculate completion rate
        completion_rate = round((unique_users_completed / total_eligible * 100) if total_eligible > 0 else 0, 2)
        ...
        by_screening_type = {
            row.screening_type: {
                ...
                'percentage': round((row.unique_users / total_eligible * 100) if total_eligible > 0 else 0, 2)
            }
        ...
        if screening_type:
            weekly_trend_query = weekly_trend_query.where(ClinicalScreening.screening_type == screening_type)
        ...
        trend_over_time = [
            {
                ...
                'total_screenings': row.count,
                'unique_users': row.unique_users,
                'completion_rate': round((row.unique_users / total_eligible * 100) if total_eligible > 0 else 0, 2)
            }
        ...
        by_team_query = select(
            User.team_id,
            func.count(func.distinct(User.id)).label('team_size'),
            func.count(func.distinct(ClinicalScreening.user_id)).label('completed_users')
        ).select_from(
        ...
        if screening_type:
            by_team_query = by_team_query.where(ClinicalScreening.screening_type == screening_type)
        ...
        by_team = [
            {
                ...
                'completion_rate': round((row.completed_users / row.team_size * 100) if row.team_size > 0 else 0, 2)
            }
    ...
    async def get_severity_distribution(
        ...
        if screening_type:
            severity_query = severity_query.where(ClinicalScreening.screening_type == screening_type)
        ...
        severity_percentages = {
            level: round((count / total_screenings * 100) if total_screenings > 0 else 0, 2)
        ...
        ).group_by(ClinicalScreening.screening_type, ClinicalScreening.severity_level)
        ...
        if screening_type:
            high_risk_query = high_risk_query.where(ClinicalScreening.screening_type == screening_type)
    ...
    async def get_crisis_alert_metrics(
        ...
        by_type_result = await self.db.execute(by_type_query)
        alerts_by_type = {row.alert_type: row.count for row in by_type_result.fetchall()}
        ...
        average_response_time = round(avg_seconds / 60, 2) if avg_seconds else None  # Convert to minutes
        ...
        resolution_rate = round((resolved_count / total_alerts * 100) if total_alerts > 0 else 0, 2)
    ...
    async def get_population_health_summary(
        ...
        # Sort and get top 10
        top_concerns = sorted(flag_counts.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    async def get_clinician_workload_metrics(
        ...
        reviewed_query = select(
            ...
            func.count(ClinicalScreening.id).label('count'),
            func.count(func.distinct(ClinicalScreening.user_id)).label('unique_patients')
        ).where(
            and_(
                ClinicalScreening.org_id == org_id,
                ClinicalScreening.completed_at >= start_date,
        ...
        average_review_time_hours = round((avg_seconds / 3600), 2) if avg_seconds else None  # Convert to hours

        # Alert response statistics
        alert_responses_query = select(
            ClinicalAlert.acknowledged_by,
            func.count(ClinicalAlert.id).label('count'),
            func.count(ClinicalAlert.id).filter(
                ClinicalAlert.resolved_at.isnot(None)
            ).label('resolved')
        ).where(
            and_(
        ...
        for row in alert_responses_result.fetchall():
            responder_id = str(row.acknowledged_by) if row.acknowledged_by else 'unknown'
        ...
        # Get clinician names
        clinician_ids = list(set(
            [item['clinician_id'] for item in screenings_reviewed if item['clinician_id'] != 'unknown'] +
            [item['responder_id'] for item in alert_responses if item['responder_id'] != 'unknown']
        ))
        ...
        if clinician_ids:
            ...
            clinician_names = {str(row.id): row.full_name for row in clinician_result.fetchall()}

        # Add names to results
        for item in screenings_reviewed:
            item['clinician_name'] = clinician_names.get(item['clinician_id'], 'Unknown Clinician')

        for item in alert_responses:
            item['responder_name'] = clinician_names.get(item['responder_id'], 'Unknown Clinician')

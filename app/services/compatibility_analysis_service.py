...
class TeamCompatibilityAnalysisService:
    ...
    async def analyze_team_compatibility(self, team_id: UUID) -> TeamCompatibilityReport:
        ...
        try:
            ...
            overall_compatibility = await self._calculate_overall_compatibility(compatibility_matrix)
    ...
    async def analyze_member_compatibility(
        ...
        try:
            ...
            communication_match = await self._calculate_communication_compatibility(profile1, profile2)
            ...
            logger.error(f"Error analyzing member compatibility: {str(e)}")
            raise
    ...
    async def suggest_team_composition(
        ...
        try:
            ...
            logger.error(f"Error suggesting team composition: {str(e)}")
            raise
    ...
    async def _get_member_profiles(self, members: List[Any]) -> List[Dict[str, Any]]:
        ...
        for member in members:
            ...
            responses = self.db.query(UserAssessmentResponse).filter(
                UserAssessmentResponse.user_id == user_id
            ).all()
    ...
    async def _extract_work_style(self, responses: List[Any]) -> Dict[str, Any]:
        ...
        for response in responses:
            if hasattr(response, 'answers') and response.answers:
                ...
                collaborative_responses = sum(1 for ans in response.answers if 'team' in str(ans).lower() or 'collabor' in str(ans).lower())
                analytical_responses = sum(1 for ans in response.answers if 'analy' in str(ans).lower() or 'data' in str(ans).lower())
    ...
    async def _extract_communication_style(self, responses: List[Any]) -> Dict[str, Any]:
        ...
        for response in responses:
            if hasattr(response, 'text_responses'):
                ...
                formal_count = sum(1 for indicator in formal_indicators if indicator in text_responses.lower())
                informal_count = sum(1 for indicator in informal_indicators if indicator in text_responses.lower())
    ...
    async def _calculate_compatibility_matrix(self, profiles: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
    ...
    async def _calculate_personality_compatibility(self, profile1: Dict, profile2: Dict) -> float:
        ...
        if p1['extraversion'] < 0.5 and p2['extraversion'] > 0.5:
            compatibility += 0.1
        elif p1['extraversion'] > 0.5 and p2['extraversion'] < 0.5:
            compatibility += 0.1
    ...
    async def _calculate_work_style_compatibility(self, profile1: Dict, profile2: Dict) -> float:
        ...
        elif ws1.get('decision_making_style') in ['analytical', 'data-driven'] and ws2.get('decision_making_style') in ['intuitive', 'creative']:
        ...
        elif ws1.get('preferred_environment') == 'collaborative' and ws2.get('preferred_environment') == 'independent':
    ...
    async def _calculate_communication_compatibility(self, profile1: Dict, profile2: Dict) -> float:
        ...
        if formality1 == formality2:
            compatibility_score += 0.4
        elif abs(['informal', 'moderate', 'formal'].index(formality1) - ['informal', 'moderate', 'formal'].index(formality2)) == 1:
    ...
    async def _calculate_overall_compatibility(self, compatibility_matrix: Dict[str, Dict[str, float]]) -> float:
    ...
    async def _calculate_diversity_metrics(self, profiles: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        diversity_metrics['work_environment_diversity'] = unique_environments / max(1, len(work_styles))
        ...
        diversity_metrics['communication_diversity'] = unique_formalities / max(1, len(communication_styles))

        return diversity_metrics

    async def _analyze_role_fits(self, team: Team, profiles: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    async def _generate_optimization_suggestions(
        ...
        if low_compatibility_pairs:
            suggestions.append(f"Consider team building activities for {len(low_compatibility_pairs)} low-compatibility pairs")
            suggestions.append("Facilitate communication workshops to improve understanding")

        if high_compatibility_pairs:
            suggestions.append(f"Leverage {len(high_compatibility_pairs)} high-compatibility pairs for mentoring and collaboration")
        ...
        if diversity_metrics.get('communication_diversity', 0) < 0.3:
            suggestions.append("Establish clear communication protocols to accommodate different styles")
    ...
    async def _identify_risk_factors(
        ...
        if min_compatibility < 0.3:
            risk_factors.append("Very low compatibility between some team members may cause conflicts")

        # Check for communication style mismatches
        communication_styles = [p.get('communication', {}) for p in profiles]
        formal_members = sum(1 for cs in communication_styles if cs.get('formality_level') == 'formal')
        informal_members = sum(1 for cs in communication_styles if cs.get('formality_level') == 'informal')

        if formal_members > 0 and informal_members > 0:
            if min(formal_members, informal_members) / len(profiles) > 0.3:
                risk_factors.append("Mixed communication formality levels may cause misunderstandings")

        # Check for extreme work style differences
        collaborative_count = sum(1 for p in profiles if p.get('work_style', {}).get('preferred_environment') == 'collaborative')
    ...
    async def _identify_potential_conflicts(self, profile1: Dict, profile2: Dict) -> List[str]:
        ...
        if p1.get('neuroticism', 0.5) > 0.7 and p2.get('neuroticism', 0.5) > 0.7:
            conflicts.append("Both may be sensitive to stress - establish clear conflict resolution processes")
    ...
    async def _generate_compatibility_recommendations(self, profile1: Dict, profile2: Dict) -> List[str]:
    ...
    async def _score_candidate_for_team(
        self, candidate: Dict[str, Any], current_profiles: List[Dict[str, Any]],
        required_skills: List[str], skill_gaps: List[str]
    ) -> Dict[str, float]:
        ...
        score['skill_fit'] = (matching_required + filling_gaps * 1.5) / max(1, len(required_skills_set))

        # Compatibility with current team
        if current_profiles:
            compatibility_scores = []
            for current_profile in current_profiles:
                compatibility = await self._calculate_personality_compatibility(candidate, current_profile)

...
class CrisisSupportService:
    ...
    async def assess_crisis_severity(
        self, user_id: str, responses: dict[str, str], timestamp: str
    ) -> dict[str, Any]:
        """
    ...
    async def _identify_safety_concerns(
        self, responses: dict[str, str], risk_level: str
    ) -> list[str]:
        ...
        try:
            ...
            if responses.get("suicidal_thoughts") == "yes":
                if responses.get("harm_plan") == "yes":
                    concerns.append(
                        "Immediate suicidal thoughts with specific plan - requires emergency intervention"
                    )
                else:
                    concerns.append(
                        "Suicidal thoughts present - requires immediate professional support"
                    )
    ...
    async def _generate_immediate_actions(
        self, risk_level: str, responses: dict[str, str]
    ) -> list[str]:
    ...
    async def create_personalized_safety_plan(
        ...
        try:
            ...
            return {"success": True, "data": safety_plan}

        except Exception as e:
            logger.error(f"Error creating safety plan: {e}")
            # Return basic safety plan
            return {"success": True, "data": await self._get_basic_safety_plan(user_id)}
    ...
    async def _log_crisis_assessment(
        self, user_id: str, risk_level: str, severity_score: float, timestamp: str
    ) -> None:
    ...
    async def get_existing_safety_plan(self, user_id: str) -> dict[str, Any] | None:

...
class TeamOptimizationService:
    ...
    async def optimize_team_composition(
        ...
        existing_members = []
        if existing_team_id:
            existing_members = await self._get_existing_members(db, existing_team_id)
    ...
    async def analyze_team(
        self, db: AsyncSession, team_id: int, organization_id: int
    ) -> dict[str, Any]:
        """
    ...
    async def check_member_compatibility(
        self, db: AsyncSession, user_id_1: int, user_id_2: int, organization_id: int
    ) -> dict[str, Any]:
        """
        ...
        return {
            "user_1": {"id": user1.id, "name": user1.full_name, "email": user1.email},
            "user_2": {"id": user2.id, "name": user2.full_name, "email": user2.email},
    ...
    async def get_candidate_pool(
        ...
        """
        candidates = await self._build_candidate_pool(db, organization_id, filters)
    ...
    async def _user_to_profile(self, db: AsyncSession, user: User) -> TeamMemberProfile:
    ...
    async def _count_user_teams(self, db: AsyncSession, user_id: int) -> int:

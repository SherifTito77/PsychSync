...
class BehavioralPipelineOrchestrator:
    ...
    async def _process_email_data(
        ...
        if platform == "gmail":
            ...
            raw_emails = await gmail_integration.fetch_recent_emails(
            ...
            raw_emails = await outlook_integration.fetch_recent_emails(
                days=time_window_days
            )
        ...
        # Calculate signals
        signals = self.email_extractor.calculate_behavioral_signals(
            emails, time_window_days
        )
    ...
    async def _process_calendar_data(
        ...
        # Calculate signals
        signals = self.calendar_extractor.calculate_behavioral_signals(
    ...
    async def _process_slack_data(
        ...
        # Calculate signals
        signals = self.slack_extractor.calculate_behavioral_signals(
            user_messages, time_window_days
        )
    ...
    def _calculate_work_life_balance(self, all_signals: Dict[str, Dict]) -> float:
        ...
        calendar_signals = all_signals.get("calendar", {})
        after_hours = (
            calendar_signals.get("after_hours_meetings_count", 0) / 10.0
        )  # Normalize
    ...
    def _determine_severity(
        self, indicators: List[str], critical_threshold: int
    ) -> InsightSeverity:

    ...
    Operation result
        ...
        async with AsyncSessionLocal() as db:
            ...
            for i in range(0, total_users, self.batch_size):
                ...
                print(f"   Progress: {progress:.1f}% ({users_processed}/{total_users} users)")
        ...
        print(f"   Average time per user: {duration / users_processed:.3f} seconds" if users_processed > 0 else "")
            ...
            try:
                ...
                if executed:
                    ...
                    if risk_data["overall_risk"] in ["critical", "high"]:
                        print(f"   ⚠️  User {user_id}: {risk_data['overall_risk'].upper()} risk (score: {risk_data['overall_score']})")
                        print(f"      Factors: {', '.join(risk_data['primary_risk_factors'])}")
    ...
    Operation result
        ...
        async with AsyncSessionLocal() as db:
            ...
            if total_users == 0:
                ...
                return {"users_processed": 0, "triggers_executed": 0, "duration_seconds": 0}
...
Raises:
    NotFoundError: If resource doesn't exist
        ...
        async with AsyncSessionLocal() as db:
            ...
            result = await db.execute(
                select(
                    ChurnRiskScore.overall_risk,
                    func.count(ChurnRiskScore.id)
                )
            ...
            result = await db.execute(
                ...
                .where(
                    ...
                    ChurnRiskScore.calculated_at >= datetime.utcnow() - timedelta(days=7)
                )
...
Returns:
    ...
    if args.mode == "all":
        ...
        for user in summary['high_risk_users']:
            print(f"      {user['user_id']}: {user['risk_level'].upper()} (score: {user['score']})")

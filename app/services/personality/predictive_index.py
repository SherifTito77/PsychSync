...
class PredictiveIndexProcessor(PersonalityFrameworkProcessor):
    ...
    def process(self, raw_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            dimensions = {
                "dominance": self._clamp_value(
                    self._safe_get(raw_data, "dominance", 0.5)
                ),
                "extraversion": self._clamp_value(
                    self._safe_get(raw_data, "extraversion", 0.5)
                ),
                "patience": self._clamp_value(
                    self._safe_get(raw_data, "patience", 0.5)
                ),
                "formality": self._clamp_value(
                    self._safe_get(raw_data, "formality", 0.5)
                ),
            }

...
class NLPAnalysisService:
    ...
    def _extract_subject_features(self, subject: str) -> dict[str, Any]:
        ...
        politeness_words = len(
            re.findall(
                r"\b(please|thank|thanks|appreciate|kindly|could you|would you)\b",
                subject,
                re.IGNORECASE,
            )
        )
    ...
    def analyze_communication_patterns(
        ...
        response_times = [
            email.response_time_minutes
            for email in email_metadata_list
        ...
        if subjects:
            ...
            politeness_indicators = sum(
                len(re.findall(r"\b(please|thank|appreciate)\b", s, re.IGNORECASE))
                for s in subjects
            )
            ...
            communication_style = "neutral"
            urgency_indicators = politeness_indicators = question_indicators = (
        ...
        leadership_score = min(
            ...
            (
                sum(
                    email.is_thread_initiator
                    for email in email_metadata_list
                    if email.is_thread_initiator
                )
                / max(1, total_emails)
        ...
        conflict_tendency = min(
            1.0,
            urgency_indicators / max(1, total_emails)
            + exclamation_indicators / max(1, total_emails),
        )
        ...
        burnout_risk = min(
            1.0,
            (
                (avg_response_time / (24 * 60))
                if avg_response_time > 0
                else 0  # convert to days
    ...
    def calculate_psychological_safety(
        self, user_patterns: dict[str, Any], team_patterns: dict[str, Any]
    ) -> PsychologicalSafetyIndicators:
    ...
    async def analyze_email_batch(
        self, db: Session, email_metadata_list: list[EmailMetadata]
    ) -> list[CommunicationAnalysis]:
        ...
        for email_meta in email_metadata_list:
            try:
                ...
                sentiment_result = await self.analyze_sentiment_from_subject(
                ...
                conflict_probability = self._calculate_conflict_probability(
                    email_meta, sentiment_result, subject_features
                )
    ...
    def _calculate_conflict_probability(
        self, email: EmailMetadata, sentiment: SentimentResult, features: dict[str, Any]
    ) -> float:
        """Calculate probability of conflict based on email characteristics"""

        # Base probability from sentiment
        base_conflict = (
            max(0, -sentiment.score) * 0.5
        )  # Negative sentiment increases conflict probability
    ...
    async def generate_insights_summary(
        self, db: Session, user_id: str, days_back: int = 30
    ) -> dict[str, Any]:

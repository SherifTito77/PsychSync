...
class TeamRequirements:
    ...
    max_same_department: int | None = None

    # Seniority balance
    min_senior_members: int | None = None
    max_junior_members: int | None = None
...
class TeamOptimizationEngine:
    ...
    def optimize_team(
        ...
        existing_members: list[TeamMemberProfile] | None = None,
    ) -> OptimizedTeam:
        """
        ...
        diversity_scores = self._calculate_diversity_scores(
            eligible, existing_members, requirements
        )

        # Calculate overall scores
        overall_scores = self._calculate_overall_scores(
            compatibility_matrix, skill_scores, diversity_scores, eligible
        )
        ...
        optimized_team = self._create_team_result(
            all_members, requirements, overall_scores[selected_indices].mean()
        )

        logger.info(
            f"Team optimized: {len(all_members)} members, score: {optimized_team.overall_score:.2f}"
        )
        ...
        if existing_members:
            for i, candidate in enumerate(candidates):
                avg_existing_compat = np.mean(
                    [
                        self._calculate_personality_compatibility(candidate, existing)
    ...
    def _calculate_personality_compatibility(
        self, member1: TeamMemberProfile, member2: TeamMemberProfile
    ) -> float:
        """
    ...
    def _calculate_skill_scores(
        self, candidates: list[TeamMemberProfile], requirements: TeamRequirements
    ) -> np.ndarray:
    ...
    def _select_team_members(
        self, scores: np.ndarray, num_to_select: int, requirements: TeamRequirements
    ) -> list[int]:
    ...
    def _create_team_result(
        ...
        strengths, gaps, recommendations = self._generate_team_insights(
            members, requirements, role_dist, skill_coverage
        )
    ...
    def _calculate_team_compatibility(self, members: list[TeamMemberProfile]) -> float:
    ...
    def _calculate_skill_coverage_score(
        self, members: list[TeamMemberProfile], requirements: TeamRequirements
    ) -> float:
    ...
    def _calculate_team_diversity(self, members: list[TeamMemberProfile]) -> float:
    ...
    def _calculate_team_balance(self, members: list[TeamMemberProfile]) -> float:
    ...
    def _generate_team_insights(
        ...
        for skill, min_level in requirements.required_skills.items():
            ...
            if coverage >= min_level:
                strengths.append(f"Strong {skill} capability: {coverage:.1f}%")
            else:
                gaps.append(f"Low {skill} proficiency: {coverage:.1f}%")
                recommendations.append(f"Provide {skill} training or hire specialist")

        # Analyze diversity
        departments = list(set(m.department for m in members))
        if len(departments) >= 3:
            strengths.append(f"Cross-functional team: {len(departments)} departments")
        elif len(departments) == 1:
            gaps.append("Limited departmental diversity")
            recommendations.append("Consider including members from other departments")

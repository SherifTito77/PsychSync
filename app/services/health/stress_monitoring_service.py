...
from app.db.models.wellness_burnout import BurnoutRiskLevel, WellnessMetrics
...
class StressMonitoringService:
    ...
    async def analyze_health_risks(
        ...
        communication_stress = await self._analyze_communication_stress(
            user_id, cutoff_date
        )
        ...
        stress_level = self._calculate_stress_level(
            work_patterns, communication_stress, wellness_metrics
        )
        ...
        cardiovascular_risk = self._calculate_cardiovascular_risk(
            work_patterns, stress_level, biometric_data, wellness_metrics
        )

        mental_health_risk = self._calculate_mental_health_risk(
            communication_stress, wellness_metrics, work_patterns
        )

        # Identify risk factors and protective factors
        risk_factors = self._identify_risk_factors(
            work_patterns, communication_stress, wellness_metrics, biometric_data
        )
        ...
        urgent_intervention = self._requires_urgent_intervention(
            stress_level, cardiovascular_risk, risk_factors
        )

        recommend_medical = cardiovascular_risk > 0.7 or any(
            "cardiovascular" in r.lower() or "blood pressure" in r.lower()
        ...
        recommend_workload_reduction = (
            work_patterns.get("weekly_hours", 0) > 55
            or work_patterns.get("continuous_days", 0) > 10
        )
        ...
        return HealthRiskIndicators(
            ...
            sleep_disruption_score=(
                wellness_metrics.sleep_disruption if wellness_metrics else 0.0
            ),
            social_isolation_score=(
                wellness_metrics.social_withdrawal if wellness_metrics else 0.0
            ),
    ...
    async def _analyze_work_patterns(
        ...
        try:
            ...
            emails_query = (
                ...
                .join(
                    EmailConnection, EmailMetadata.connection_id == EmailConnection.id
                )
                .where(
                    and_(
            ...
            if emails:
                ...
                work_hours_emails = [
                    e for e in emails if e.sent_at.hour >= 9 and e.sent_at.hour < 18
                ]
                after_hours_emails = [
                    e for e in emails if e.sent_at.hour < 9 or e.sent_at.hour >= 18
                ]
                ...
                # Weekend work
                weekend_emails = [
                    e for e in emails if e.sent_at.weekday() >= 5  # Sat=5, Sun=6
                ]
                patterns["weekend_work_percentage"] = (
                    len(weekend_emails) / len(emails) if emails else 0
                )
                ...
                patterns["early_morning_work_days"] = len(
                    {e.sent_at.date() for e in after_hours_emails if e.sent_at.hour < 7}
                )
                ...
                patterns["weekly_hours"] = min(
                    estimated_work_hours, 80
                )  # Cap at 80 hours
    ...
    async def _analyze_communication_stress(
        ...
        try:
            ...
            if analyses:
                ...
                # Sentiment analysis
                sentiments = [
                    a.sentiment_score for a in analyses if a.sentiment_score is not None
                ]
                if sentiments:
                    stress_indicators["negative_sentiment_avg"] = (
                        sum(s for s in sentiments if s < 0) / len(sentiments)
    ...
    def _calculate_stress_level(
        ...
        if wellness_metrics:
            ...
            if (
                wellness_metrics.exhaustion_level
                and wellness_metrics.exhaustion_level >= 8
            ):
                stress_score += 0.15
    ...
    def _determine_burnout_stage(
        ...
        if wellness_metrics:
            if (
                wellness_metrics.engagement_level
                and wellness_metrics.engagement_level <= 3
            ):
                burnout_indicators += 1
            ...
            if (
                wellness_metrics.professional_efficacy
                and wellness_metrics.professional_efficacy <= 3
            ):
                burnout_indicators += 1
    ...
    def _calculate_cardiovascular_risk(
        ...
        if wellness_metrics:
            if (
                wellness_metrics.physical_wellness
                and wellness_metrics.physical_wellness <= 4
            ):
                risk_score += 0.10
        ...
        if biometric_data:
            if (
                biometric_data.resting_heart_rate
                and biometric_data.resting_heart_rate > 80
            ):
                risk_score += 0.15

            if (
                biometric_data.heart_rate_variability
                and biometric_data.heart_rate_variability < 50
            ):
                risk_score += 0.10  # Low HRV = chronic stress

            if (
                biometric_data.blood_pressure_systolic
                and biometric_data.blood_pressure_systolic > 140
            ):
                risk_score += 0.25
            ...
            if (
                biometric_data.oxygen_saturation
                and biometric_data.oxygen_saturation < 95
            ):
                risk_score += 0.15
    ...
    def _calculate_mental_health_risk(
        ...
        if wellness_metrics:
            if (
                wellness_metrics.mental_wellness
                and wellness_metrics.mental_wellness <= 4
            ):
                risk_score += 0.20

            if (
                wellness_metrics.emotional_wellness
                and wellness_metrics.emotional_wellness <= 4
            ):
                risk_score += 0.15

            if (
                wellness_metrics.social_wellness
                and wellness_metrics.social_wellness <= 4
            ):
                risk_score += 0.10
    ...
    def _identify_risk_factors(
        ...
        if wellness_metrics:
            if (
                wellness_metrics.engagement_level
                and wellness_metrics.engagement_level <= 4
            ):
        ...
        if biometric_data:
            if (
                biometric_data.resting_heart_rate
                and biometric_data.resting_heart_rate > 85
            ):
                risk_factors.append(
            ...
            if (
                ...
                risk_factors.append(
                    "High blood pressure - immediate medical evaluation needed"
                )
    ...
    def _identify_protective_factors(
        self, wellness_metrics: Optional[WellnessMetrics]
    ) -> List[str]:
        ...
        if wellness_metrics:
            if (
                wellness_metrics.resilience_score
                and wellness_metrics.resilience_score >= 7
            ):
                protective.append("High psychological resilience")

            if (
                wellness_metrics.social_wellness
                and wellness_metrics.social_wellness >= 7
            ):
                protective.append("Strong social connections")

            if (
                wellness_metrics.professional_wellness
                and wellness_metrics.professional_wellness >= 7
            ):
                protective.append("High professional satisfaction")

            if (
                wellness_metrics.support_systems_quality
                and wellness_metrics.support_systems_quality >= 7
            ):
    ...
    def _requires_urgent_intervention(
        ...
        if any(
            keyword in " ".join(risk_factors).lower() for keyword in critical_keywords
        ):
            return True

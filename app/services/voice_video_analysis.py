...
class VoiceVideoAnalysisEngine:
    ...
    async def process_video_recording(
        self, video_data: bytes, user_id: str, config: VideoRecordingConfig
    ) -> dict[str, Any]:
    ...
    async def get_transcription_history(
        self, user_id: str, limit: int = 50, date_from: datetime | None = None
    ) -> list[dict[str, Any]]:
    ...
    async def _save_video_file(
        self, video_data: bytes, analysis_id: str, format: VideoFormat
    ) -> str:
    ...
    def _select_transcription_model(self, quality: TranscriptionQuality) -> str:
        ...
        if quality == TranscriptionQuality.STANDARD:
            return self.transcription_models.get("whisper_base", "whisper_base")
        return "fallback"

    async def _transcribe_with_whisper(
        self, audio_path: str, model_name: str, config: TranscriptionConfig
    ) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "quality_metrics": {
                    ...
                    "words_per_minute": len(result["text"].split())
                    / (result.get("duration", 1) / 60),
                },
            }
    ...
    async def _transcribe_with_sphinx(
        ...
        try:
            ...
            return {
                ...
                "quality_metrics": {"model_used": "sphinx", "fallback_used": True},
            }
    ...
    async def _analyze_single_face(
        self, frame: np.ndarray, face: tuple[int, int, int, int], timestamp: float
    ...
    async def _analyze_audio_chunk_sentiment(
        self, chunk: np.ndarray, sr_rate: int, timestamp: float
        ...
        try:
            ...
            if avg_eye_contact < 0.7:
                recommendations.append(
                    "Try to maintain more consistent eye contact to improve engagement"
                )
    ...
    def _assess_risks(
        ...
        try:
            ...
            stress_indicators_count = sum(
                len(result.stress_indicators) for result in voice_sentiment
            )

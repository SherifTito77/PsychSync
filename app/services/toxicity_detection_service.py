...
class ToxicityDetectionService:
    ...
    async def _analyze_toxicity_patterns(
        self, communications: List[CommunicationAnalysis]
    ) -> Dict[str, Any]:
                ...
                significant_patterns.append(
                    {
                        ...
                        "pattern_definition": self.toxicity_patterns.get(
                            pattern_type, {}
                        ),
                        "detection_confidence": min(
                            frequency / len(communications), 1.0
                        ),
                    }
                )
        ...
        return {
            ...
            "high_frequency_patterns": [
                p for p in significant_patterns if p["frequency"] >= 3
            ],
        }
    ...
    async def _detect_toxicity_in_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        for toxicity_type, pattern_config in self.toxicity_patterns.items():
            ...
            if keyword_matches or pattern_matches:
                confidence = min(
                    (len(keyword_matches) * 0.3 + len(pattern_matches) * 0.7), 1.0
                )
    ...
    def _analyze_behavioral_indicators(
        ...
        avg_sentiments = {
            k: v / len(communications) for k, v in total_sentiments.items()
        }
        ...
        return {
            ...
            "negative_communication_ratio": negative_communications
            / len(communications),
    ...
    def _calculate_toxicity_risk(
        self, toxicity_analysis: Dict[str, Any], behavioral_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if toxicity_analysis.get("patterns"):
            pattern_severity = sum(
                p["severity_score"] for p in toxicity_analysis["patterns"]
            )
            pattern_risk = min(
                pattern_severity / len(toxicity_analysis["patterns"]), 1.0
            )
    ...
    def _generate_toxicity_recommendations(
        ...
        if overall_risk["toxicity_level"] in [
            ToxicityLevel.HIGH,
            ...
            for pattern in toxicity_analysis.get("patterns", []):
                ...
                existing_pattern = (
                    ...
                    .filter(
                        and_(
                            ...
                            ToxicityPattern.detection_date
                ...
                if existing_pattern:
                    ...
                    toxicity_pattern = ToxicityPattern(
                        ...
                        severity_level=self._severity_from_score(
                            pattern["severity_score"]
                        ),
                        confidence_score=pattern["detection_confidence"],
                        behavioral_indicators=pattern.get("pattern_definition", {}).get(
                            "behavioral_signs", []
                        ),
    ...
    def get_toxicity_trends(
        ...
        try:
                ...
                trend_data.append(
                    {
                        ...
                        "critical_patterns": severity_counts.get(
                            ToxicityLevel.CRITICAL, 0
                        ),
            ...
            if len(trend_data) >= 2:
                recent_avg = (
                    trend_data[-7:]["pattern_count"]
                    if len(trend_data) >= 7
                    else [p["pattern_count"] for p in trend_data[-3:]]
                )
                earlier_avg = (
                    trend_data[-14:-7]["pattern_count"]
                    if len(trend_data) >= 14
                    else [p["pattern_count"] for p in trend_data[-8:-4]]
                )
            ...
            return {
                ...
                "summary": {
                    ...
                    "most_common_type": (
                        Counter([p.pattern_type for p in patterns]).most_common(1)[0]
                        if patterns
                        else None
                    ),
                    "patterns_requiring_intervention": len(
                        [p for p in patterns if p.intervention_required]
                    ),
                },
            }

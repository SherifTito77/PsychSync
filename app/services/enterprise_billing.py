...
class PricingConfig:
    ...
    TIERS = {
        PlanType.FREE: PricingTier(
            ...
            limits={"users": 5, "assessments": 10, "responses": 100, "storage_mb": 100},
            included_users=5,
        ),
...
class EnterpriseBillingService:
        ...
        try:
            ...
            if quantity > tier.included_users:
                ...
                if tier.price_per_user:
                    unit_price += (additional_users * tier.price_per_user) / Decimal(
                        quantity
                    )
            ...
            subscription = Subscription(
                ...
                trial_start=(
                    datetime.fromtimestamp(stripe_subscription.trial_start, tz=UTC)
            ...
            logger.error(
                f"❌ Failed to create subscription for org {organization_id}: {e}"
            )
            raise
    ...
    async def update_subscription(
        ...
        try:
                    ...
                    **update_data,
                    metadata={
                        ...
                        "plan_type": (
                            plan_type.value
                            if plan_type
                            else current_sub.plan_type.value
                        ),
                    },
                )

            # Update local record
            updated_subscription = await self._update_subscription_from_stripe(
                stripe_subscription
            )
            ...
            await self._log_event(
                ...
                {"subscription_id": subscription_id, "updates": update_data},
            )
    ...
    async def create_invoice(
        ...
        try:
            ...
            stripe_invoice = stripe.Invoice.create(
                ...
                custom_fields=[{"name": "Organization ID", "value": organization_id}],
            )
            ...
            invoice = Invoice(
                ...
                amount_remaining=Decimal(str(stripe_invoice.amount_remaining))
                / Decimal(100),
                ...
                metadata={"stripe_invoice_id": stripe_invoice.id},
            )
            ...
            logger.info(
                f"✅ Created invoice {invoice.invoice_id} for org {organization_id}"
            )
            return invoice
    ...
    async def process_usage_billing(self, organization_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"❌ Failed to process usage billing for org {organization_id}: {e}"
            )
            raise
    ...
    async def _log_event(
        self, event_type: EventType, organization_id: str, data: dict[str, Any]
    ):
        """Log billing event"""
    ...
    async def _get_usage_data(self, organization_id: str) -> dict[str, int]:
        ...
        return {"additional_users": 5, "additional_storage": 2, "api_calls": 15000}

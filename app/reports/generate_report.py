...
class PDFReportGenerator:
    ...
    def generate_outcome_summary_report(
        self, summary_stats: dict, clients_data: pd.DataFrame, output_file: str
    ):
        """
...
class ExcelExporter:
    ...
    @staticmethod
    def export_aggregate_data(dataframes: dict[str, pd.DataFrame], output_file: str):
        """
...
class CSVExporter:
    ...
    @staticmethod
    def export_with_metadata(df: pd.DataFrame, metadata: dict, output_file: str):
        """Export DataFrame with metadata header."""
        with open(output_file, "w") as f:
            # Write metadata as comments
            f.write("# PsychSync Data Export\n")
...
if __name__ == "__main__":
    ...
    try:
        ExcelExporter.export_client_data(
            client_info, assessment_scores, session_data, "client_data_export.xlsx"
        )
    ...
    builder.add_section(
        "Client Overview",
        f"Client ID: {client_info['client_id']}, Age: {client_info['age']}",
        "text",
    )
    builder.add_section("Assessment Scores", assessment_scores, "table")
    builder.add_section(
        "Summary",
        f"Total sessions: {session_summary['total_sessions']}, Attendance: {session_summary['attendance_rate']}%",
        "text",
    )

...
from app.dependency_injection.container import (
    container, register_singleton, register_scoped, register_transient, register_configuration,
    register_singleton_by_name, register_scoped_by_name, register_transient_by_name
)
from app.dependency_injection.container import Lifetime
...
def register_configuration_values():
    ...
    try:
        ...
        register_configuration("redis_url", getattr(settings, 'REDIS_URL', 'redis://localhost:6379'))
        ...
        register_configuration("access_token_expire_minutes", getattr(settings, 'ACCESS_TOKEN_EXPIRE_MINUTES', 30))
        ...
        register_configuration("email_verification_enabled", getattr(settings, 'ENABLE_EMAIL_VERIFICATION', False))
        register_configuration("mfa_enabled", False)  # MFA not implemented yet
        register_configuration("audit_logging_enabled", getattr(settings, 'AUDIT_LOGGING_ENABLED', True))
...
def register_development_services():
    ...
    register_scoped_by_name("dev_tools", lambda: {
        "sql_echo": True,
        "debug_requests": True,
...
def validate_service_registrations():
    ...
    try:
        ...
        service_logger.info(f"Service registration validation passed. Registered {len(service_info)} services")

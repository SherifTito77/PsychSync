...
class DataAnonymizer:
    ...
    def anonymize_dataframe(
        self,
        df: pd.DataFrame,
        config: dict[str, AnonymizationConfig] | None = None,
    ...
    def _suggest_method(self, column_name: str, series: pd.Series) -> str:
        ...
        if any(x in col_lower for x in ["name", "patient", "client"]):
            return "hash"
        if any(x in col_lower for x in ["email", "phone"]):
            return "mask"
        if any(x in col_lower for x in ["age"]):
            return "generalize_age"
        if any(x in col_lower for x in ["zip", "postal"]):
            return "generalize_zip"
        if any(x in col_lower for x in ["note", "comment", "text", "description"]):
            return "redact_text"
        if any(x in col_lower for x in ["ssn", "mrn", "id"]):
            return "hash"
...
class AuditLogger:
    ...
    def get_logs(self, user_id: str | None = None, limit: int = 100) -> list[dict]:
...
if __name__ == "__main__":
    ...
    if recent_logs:
        ...
        print(
            f"Last event: {last_log['action']} by {last_log['user_id']} at {last_log['timestamp']}"
        )

...
class SentimentService:
    ...
    def analyze_text(self, text: str) -> Dict:
        ...
        combined_sentiment = self._combine_sentiments(
            textblob_sentiment, vader_sentiment, clinical_sentiment, progress_score
        )
    ...
    def _clinical_sentiment(self, doc) -> Dict:
        ...
        positive_count = sum(
            1 for term in self.progress_indicators if term in text_lower
        )
        negative_count = sum(
            1 for term in self.concern_indicators if term in text_lower
        )
    ...
    def _calculate_progress_score(self, text: str) -> Dict:
        ...
        return {
            ...
            "interpretation": (
                "improving"
                if score > 0.2
                else "declining" if score < -0.2 else "stable"
            ),
    ...
    def _combine_sentiments(self, textblob, vader, clinical, progress) -> Dict:
        ...
        weights = {"textblob": 0.2, "vader": 0.2, "clinical": 0.4, "progress": 0.2}
        ...
        variance = sum((x - combined_score) ** 2 for x in score_values) / len(
            score_values
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Progress Interpretation: {result['detailed_scores']['progress']['interpretation']}"
    )
    print(f"\nTop Keywords:")

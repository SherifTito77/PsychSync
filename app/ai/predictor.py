...
class OutcomePredictor:
    ...
    def predict_batch(
        self, features_list: List[ClientFeatures]
...
class ResponsePredictor:
    ...
    def predict_response_trajectory(
        self, features: ClientFeatures, weeks_ahead: int = 4
    ) -> Dict:
        """
...
if __name__ == "__main__":
    ...
    print(
        f"   CV R² (mean ± std): {metrics['cv_r2_mean']:.3f} ± {metrics['cv_r2_std']:.3f}"
    )
    ...
    for point in trajectory["trajectory"][:4]:  # Show first 4 weeks
        print(
            f"      Week {point['week']}: {point['predicted_severity']:.1f} "

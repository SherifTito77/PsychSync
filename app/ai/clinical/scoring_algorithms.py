...
class CSSRSScorer:
    ...
    def score(responses: Dict[str, any]) -> ScoringResult:
        ...
        """
        normalized = CSSRSScorer._normalize_responses(responses)

        recent_attempt = bool(normalized.get("q11", False))
        active_ideation = bool(
            normalized.get("q3", False)
            or normalized.get("q4", False)
            or normalized.get("q5", False)
        )
        passive_ideation = bool(
            normalized.get("q1", False) or normalized.get("q2", False)
        )
    ...
    def _normalize_responses(responses: Dict[str, any]) -> Dict[str, any]:
        """
        Support both the legacy direct-test payload and the API's q1/q11 payload.

        The public API passes q1-q5/q11-q13 keys, while older tests use a more
        semantic set of field names. Keeping both forms avoids changing the API
        contract while preserving compatibility.
        """

        if any(key.startswith("q") for key in responses):
            return responses

        suicidal_intent = responses.get("suicidal_intent", 0) or 0
        suicidal_thoughts = bool(responses.get("suicidal_thoughts", False))
        suicidal_plan = bool(responses.get("suicidal_plan", False))
        suicidal_attempts = responses.get("suicidal_attempts", 0) or 0

        return {
            "q1": bool(responses.get("wish_dead", False)),
            "q2": suicidal_thoughts,
            "q3": suicidal_thoughts or suicidal_intent >= 1,
            "q4": suicidal_intent >= 1,
            "q5": suicidal_plan,
            "q11": suicidal_attempts >= 1,
            "q12": False,
            "q13": False,
        }
...
def score_phq9(responses: Dict[str, int]) -> Dict[str, any]:
    ...
    """
    # Convert string keys to integer item numbers and default missing items to 0.
    item_responses = {i: 0 for i in range(1, 10)}
    for key, value in responses.items():
        ...
        if item_num not in item_responses:
            continue
        ...
        # Clamp invalid values to the legal 0-3 PHQ-9 range.
        item_responses[item_num] = max(0, min(3, item_value))
...
def score_mdq(responses: Dict[str, any]) -> Dict[str, any]:
    ...
    from app.services.clinical.additional_scorers import MDQScorer

    result = MDQScorer.score(responses)
    symptom_count = int(result.subscale_scores.get("symptom_count", result.total_score))
...
def score_aq10(responses: Dict[str, int]) -> Dict[str, any]:
    ...
    # Legacy scoring used by the tests:
    # - Items 1,2,4,5,7,9,10 score 1 for "agree" (4)
    # - Items 3,6,8 score 1 for lower responses (1-2)

    score = 0
    for item, value in responses.items():
        item_num = int(item)
        if item_num in [1, 2, 4, 5, 7, 9, 10]:
            if value == 4:
                score += 1
        elif item_num in [3, 6, 8]:
            if value <= 2:
                score += 1

...
class EmotionDetector:
    ...
    def _assess_psychological_states(self, emotion_freq: Dict[str, float]) -> Dict:
        ...
        for state_name, criteria in self.psychological_states.items():
            ...
            primary_score = sum(
                emotion_freq.get(e, 0) for e in criteria["primary_emotions"]
            )
            secondary_score = (
                sum(emotion_freq.get(e, 0) for e in criteria["secondary_emotions"])
                * 0.5
            )
            ...
            risk_level = (
                ...
                else "moderate" if total_score >= criteria["threshold"] * 0.7 else "low"
            )
    ...
    def _calculate_emotional_stability(self, history: List[Dict]) -> Dict:
        ...
        level = (
            ...
            else "moderate" if stability_score >= 0.4 else "unstable"
        )
    ...
    def _detect_emotional_patterns(self, history: List[Dict]) -> List[Dict]:
        ...
        for i in range(len(history) - 2):
            ...
            if sequence[0] == sequence[2] and sequence[0] != sequence[1]:
                patterns.append(
                    {"type": "cycling", "emotions": sequence, "position": i}
                )

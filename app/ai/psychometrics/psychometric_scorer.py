...
class PsychometricScorer:
    ...
    def score_assessment(
        self, assessment_type: str, responses: List[Dict], framework_config: Dict
    ) -> Dict:
        """
    ...
    def _score_mbti(self, responses: List[Dict], config: Dict) -> Dict:
        ...
        for dimension, scores in dimensions.items():
            ...
            if scores[options[0]] > scores[options[1]]:
                ...
                strength = scores[options[0]] / (
                    scores[options[0]] + scores[options[1]]
                )
            else:
                mbti_type += options[1]
                preference = options[1]
                strength = scores[options[1]] / (
                    scores[options[0]] + scores[options[1]]
                )
            ...
            preferences[dimension] = {
                ...
                "clarity": (
                    "clear"
                    if strength > 0.65
                    else "moderate" if strength > 0.55 else "slight"
                ),
            }
        ...
        for response in responses:
            if "answer_value" in response:
                ...
                if not (config["min_value"] <= value <= config["max_value"]):
                    errors.append(
                        f"Invalid value {value} for question {response['question_id']}"
                    )

...
class IRTModel(Enum):
    ...
    THREE_PL = (
        "3PL"  # Three Parameter Logistic (difficulty + discrimination + guessing)
    )
...
class IRTEngine:
    ...
    def __init__(
        self, item_bank: List[ItemParameters], model: IRTModel = IRTModel.TWO_PL
    ):
        """
    ...
    def estimate_ability_mle(
        ...
        # Optimize
        result = minimize(
            neg_log_likelihood, x0=initial_theta, method="BFGS", bounds=[(-4, 4)]
        )
...
class AdaptiveTestEngine:
    ...
    def select_next_item(
        ...
        available = [
            item
            for item in self.irt.item_bank
        ...
        if method == "max_info":
            ...
            info_scores = [
                self.irt.information(current_theta, item) for item in available
            ]
    ...
    def administer_test(
        ...
        for item_num in range(self.max_items):
            ...
            next_item = self.select_next_item(
                theta_est, administered, method=selection_method
            )
    ...
    def simulate_test(
        ...
        result = self.administer_test(
            simulated_response, selection_method=selection_method, verbose=verbose
        )
...
if __name__ == "__main__":
    ...
    cat = AdaptiveTestEngine(irt, min_items=5, max_items=10, se_threshold=0.4)

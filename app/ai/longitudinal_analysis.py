...
class LongitudinalAnalyzer:
    ...
    def compare_phases(
        self, data: pd.DataFrame, phase_column: str, value_column: str = "score"
    ) -> Dict:
        """
        ...
        return {"phase_statistics": phase_stats, "pairwise_comparisons": comparisons}
    ...
    def analyze_intervention_effectiveness(
        ...
        # Extract data
        pre_data = data[
            (data[date_column] >= pre_start) & (data[date_column] < intervention_date)
        ][value_column]

        post_data = data[
            (data[date_column] >= intervention_date) & (data[date_column] <= post_end)
        ][value_column]
    ...
    def calculate_growth_metrics(
        self, data: pd.DataFrame, value_column: str = "score"
    ) -> Dict:
        """
        ...
        if len(values) >= 4:
            early_slope = stats.linregress(
                x[: len(x) // 2], values[: len(values) // 2]
            )[0]
            recent_slope = stats.linregress(
                x[len(x) // 2 :], values[len(values) // 2 :]
            )[0]
...
class TimeSeriesForecaster:
    ...
    def forecast_simple_moving_average(
        self, data: pd.Series, window: int = 7, periods_ahead: int = 7
    ) -> Dict:
        """
    ...
    def forecast_exponential_smoothing(
        self, data: pd.Series, alpha: float = 0.3, periods_ahead: int = 7
    ) -> Dict:
        """
...
if __name__ == "__main__":
    ...
    for pred in trajectory.predicted_values:
        print(
            f"      {pred['date']}: {pred['predicted_value']:.2f} "
    ...
    rci_result = analyzer.calculate_reliable_change(
        baseline=scores[0], current=scores[-1], test_reliability=0.90, population_sd=5.0
    )
    ...
    print(
        f"   Change: {intervention_effect.change:.2f} ({intervention_effect.percent_change:.1f}%)"
    )
    ...
    forecast = forecaster.forecast_exponential_smoothing(
        data["score"], alpha=0.3, periods_ahead=4
    )

    print(f"   Forecast type: {forecast['forecast_type']}")
    for f in forecast["forecasts"]:
        print(
            f"      Period +{f['period_ahead']}: {f['forecast']:.2f} "

...
class PIIRedactor:
        ...
        try:
                ...
                for pattern in self.compiled_patterns[category]:
                    ...
                    for match in matches:
                        ...
                        if self._validate_match(matched_text, category):
                            ...
                            redaction_token = (
                                f"{redaction_strategy}-{category.value.upper()}"
                            )
                            # Only redact if not already redacted
                            if not self._is_already_redacted(
                                matched_text, redaction_strategy
                            ):
            ...
            if findings:
                redacted_text = self._apply_redactions(
                    text, findings, redaction_strategy
                )
    ...
    def _apply_redactions(
        self, text: str, findings: List[Dict], redaction_strategy: str
    ) -> str:
        """
    ...
    def get_risk_assessment(self, text: str) -> Dict[str, any]:
        ...
        return {
            ...
            "high_risk_findings": [
                f for f in result.findings if f["risk_score"] >= 0.8
            ],

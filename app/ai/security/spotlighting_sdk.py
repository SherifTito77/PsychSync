...
class DelimitingSpotlighting:
    ...
    def generate_delimiter_pair(self) -> Tuple[str, str]:
        ...
        start_delimiter = (
            f"{bracket_open}{symbols}USER_INPUT_START{symbols}{bracket_open}"
        )

        # Combine for end delimiter
        end_delimiter = (
            f"{bracket_close}{symbols}USER_INPUT_END{symbols}{bracket_close}"
        )
    ...
    def verify(self, content: str, result: SpotlightingResult) -> bool:
        ...
        """
        return (
            result.delimiter_start in content
            and result.delimiter_end in content
            and content.index(result.delimiter_start)
...
class EncodingSpotlighting:
    ...
    def __init__(self, method: str = "base64", add_prefix: bool = True):
        ...
        if method not in self.ENCODING_METHODS:
            raise ValueError(
                f"Invalid encoding method. Choose from: {self.ENCODING_METHODS}"
            )
...
class SpotlightingSDK:
    ...
    def verify(self, content: str, original_result: SpotlightingResult) -> bool:
        """
...
def spotlight_datamarking(
    content: str, marker: Optional[str] = None
) -> SpotlightingResult:

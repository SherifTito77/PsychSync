...
class BuildFailureBase(BaseModel):
    ...
    failure_type: str = Field(
        ...,
        description="Type of failure: test_failure, compilation_error, lint_issue, deployment_failure",
    )
    failure_stage: str = Field(
        ...,
        description="Stage where failure occurred: build, test, deploy, integration",
    )
    ...
    failed_tests: Optional[list[str]] = Field(
        None, description="List of failed test names"
    )
    changed_files: Optional[list[str]] = Field(
        None, description="Files changed in this build"
    )
...
class BuildFailureCreate(BuildFailureBase):
    ...
    root_cause_category: str = Field(
        ...,
        description="Category: code_bug, dependency_issue, environment_problem, test_flake, infrastructure_issue",
    )
    suspected_culprit_file: Optional[str] = Field(
        None, description="File likely causing the failure"
    )
    ai_suggested_fix: Optional[str] = Field(
        None, description="AI-generated fix suggestion"
    )
    priority: str = Field(
...
class RootCauseAnalysisCreate(RootCauseAnalysisBase):
    ...
    similar_failures: list[UUID] = Field(
        default_factory=list, description="Links to similar past failures"
    )
...
class BuildPatternBase(BaseModel):
    ...
    pattern_type: str = Field(
        ...,
        description="Pattern type: flaky_test, slow_build, frequent_failure, dependency_conflict",
    )
...
class BuildPatternCreate(BuildPatternBase):
    ...
    affected_branches: list[str] = Field(
        ..., description="Branches where pattern appears"
    )
    affected_developers: list[str] = Field(
        default_factory=list, description="Developers typically affected"
    )
    remediation_priority: str = Field(
        default="medium", description="Priority for addressing pattern"
    )
    ai_remediation_suggestion: Optional[str] = Field(
        None, description="AI-suggested fix"
    )

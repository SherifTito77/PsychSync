...
class TeamCompositionResponse(BaseModel):
    ...
    openness: Optional[DimensionStats] = Field(
        None, description="Openness dimension statistics"
    )
    conscientiousness: Optional[DimensionStats] = Field(
        None, description="Conscientiousness dimension statistics"
    )
    extraversion: Optional[DimensionStats] = Field(
        None, description="Extraversion dimension statistics"
    )
    agreeableness: Optional[DimensionStats] = Field(
        None, description="Agreeableness dimension statistics"
    )
    neuroticism: Optional[DimensionStats] = Field(
        None, description="Neuroticism dimension statistics"
    )
    strengths: List[str] = Field(
        default_factory=list, description="Team strengths based on personality"
    )
    gaps: List[str] = Field(
        default_factory=list, description="Potential areas for development"
    )
    internal_compatibility: Optional[float] = Field(
        None, description="How well personalities complement each other (0-1)"
    )
    diversity_score: Optional[float] = Field(
        None, description="Personality diversity score (0-1)"
    )
    updated_at: str = Field(
        ..., description="When this composition was last calculated"
    )
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "gaps": ["May struggle with organization and follow-through"],
...
class TeamComparisonResponse(BaseModel):
    ...
    teams: List[TeamCompositionResponse] = Field(
        ..., description="Team composition data"
    )

...
class QuickAssessmentRequest(BaseModel):
    ...
    team_size: Optional[str] = Field(
        None, description="Team size (e.g., '5-10', '10-20')"
    )
...
class Insight(BaseModel):
    ...
    confidence_score: float = Field(
        ..., ge=0, le=1, description="Confidence in insight"
    )
...
class QuickInsights(BaseModel):
    ...
    conversion_probability: float = Field(
        ..., ge=0, le=1, description="Likelihood to convert"
    )
...
class TeamInsightRequest(BaseModel):
    ...
    assessment_data: Optional[Dict[str, Any]] = Field(
        None, description="Assessment responses"
    )
    team_composition: Optional[List[Dict[str, Any]]] = Field(
...
class TeamProfile(BaseModel):
    ...
    personality_distribution: Dict[str, float] = Field(
        ..., description="Personality type distribution"
    )
    ...
    current_performance: float = Field(
        ..., ge=0, le=1, description="Current performance score"
    )
    potential_performance: float = Field(
        ..., ge=0, le=1, description="Potential performance score"
    )
...
class TeamInsightResponse(BaseModel):
    ...
    detailed_insights: List[DetailedInsight] = Field(
    ...
    implementation_roadmap: List[ImplementationRoadmap] = Field(
...
class ValueMetrics(BaseModel):
    ...
    team_satisfaction_score: float = Field(
        ..., ge=0, le=1, description="Team satisfaction"
    )

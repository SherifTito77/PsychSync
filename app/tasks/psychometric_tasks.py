...
from app.db.models.psychometric_session import ProgressReport, PsychometricSession
...
def analyze_session_async(
    ...
    try:
        ...
        result = service.analyze_client_session(
            session_text, client_id, session_id, datetime.fromisoformat(session_date)
        )
...
def generate_progress_report_async(client_id: str, start_date: str, end_date: str):
    ...
    try:
        ...
        for client_id in client_ids:
            ...
            if anomalies.get("anomalies_detected", 0) > 0:
                ...
                logger.warning(
                    f"Anomalies detected for client {client_id}: {anomalies['anomalies_detected']}"
                )

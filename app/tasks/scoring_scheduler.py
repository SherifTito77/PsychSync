...
from app.db.models.assessment import Assessment, AssessmentResponse, AssessmentStatus
...
def calculate_assessment_scores(
    self, assessment_id: int, response_ids: list[int] = None
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for response in responses:
            try:
                ...
                score_data = AssessmentService.calculate_scores(db, response.id)
                ...
                logger.error(f"Error scoring response {response.id}: {e!s}")
                errors.append({"response_id": response.id, "error": str(e)})
        ...
        logger.error(f"Error in calculate_assessment_scores: {e!s}")
...
@celery_app.task(
    base=DatabaseTask, bind=True, name="tasks.generate_assessment_report", max_retries=3
)
def generate_assessment_report(
    self, response_id: int, report_format: str = "pdf"
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            return {"status": "error", "message": str(e), "response_id": response_id}
...
@celery_app.task(base=DatabaseTask, bind=True, name="tasks.cleanup_expired_assessments")
def cleanup_expired_assessments(self) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error in cleanup_expired_assessments: {e!s}")
        return {"status": "error", "message": str(e)}


@celery_app.task(base=DatabaseTask, bind=True, name="tasks.generate_daily_reports")
def generate_daily_reports(self) -> dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error in generate_daily_reports: {e!s}")
        return {"status": "error", "message": str(e)}

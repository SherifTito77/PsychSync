"""
Slack Metadata Analysis API Endpoints

Privacy-first Slack intelligence: analyzes ONLY metadata (message counts,
timestamps, channel breadth, presence). Never reads message content.
"""
...
router = APIRouter(prefix="/slack-metadata", tags=["Slack Metadata Analysis"])


def _get_analyzer():
    from app.services.slack_metadata_service import SlackMetadataAnalyzer

    return SlackMetadataAnalyzer()


def _get_registry():
    from app.services.slack_metadata_service import slack_metadata_registry

    return slack_metadata_registry


@router.get("/signals/{org_id}")
async def get_slack_metadata_signals(
    org_id: str,
    days: int = Query(14, ge=1, le=90),
    current_user: User = Depends(get_current_user),
):
    """Full Slack metadata behavioral signals for an organization."""
    ...
    for info in registry.list_connectors():
        ...
        if connector:
            try:
                ...
                logger.warning("Slack connector %s failed: %s", info["name"], e)
...
async def get_slack_daily_breakdown(
    org_id: str,
    days: int = Query(14, ge=1, le=90),
    current_user: User = Depends(get_current_user),
):
    """Per-day Slack volume breakdown for dashboard charts."""
    registry = _get_registry()
    analyzer = _get_analyzer()
...
async def get_slack_burnout_signals(
    org_id: str,
    days: int = Query(14, ge=1, le=90),
    current_user: User = Depends(get_current_user),
):
    """Burnout risk signals derived from Slack metadata only."""
    registry = _get_registry()
    analyzer = _get_analyzer()
    ...
    return {
        ...
        "burnout": {
            ...
            "isolation_risk": signals.isolation_risk_score,
...
@router.get("/status")
async def get_slack_metadata_status(
    current_user: User = Depends(get_current_user),
):
    """Check which Slack metadata connectors are registered."""
    registry = _get_registry()
    return {
        "success": True,
        "connectors": registry.list_connectors(),
        "available": len(registry.list_connectors()) > 0,
        "privacy_note": "Only Slack analytics API used. Message content is never accessed.",
    }

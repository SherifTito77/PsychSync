...
async def submit_anonymous_feedback(
    ...
    try:
        logger.info(
            f"Anonymous feedback submission received: {feedback_data.feedback_type}/{feedback_data.category}"
        )
        ...
        if result["success"]:
            logger.info(
                f"Anonymous feedback submitted successfully - Tracking ID: {result['tracking_id'][:8]}..."
            )
            return result
        logger.error(
            f"Anonymous feedback submission failed: {result.get('error', 'Unknown error')}"
        )
        raise HTTPException(
            status_code=500,
            detail={
...
async def get_feedback_for_review(
    ...
    try:
        ...
        logger.info(
            f"Feedback review requested by user {current_user.id} for org {organization_id}"
        )
        ...
        raise HTTPException(
            status_code=403, detail="Insufficient permissions to review feedback"
        ) from e
    except Exception as e:
...
async def get_anonymous_feedback_statistics(
    ...
    try:
        ...
        logger.info(
            f"Feedback statistics requested by user {current_user.id} for org {organization_id}"
        )

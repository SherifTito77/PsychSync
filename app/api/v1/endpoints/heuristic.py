#!/usr/bin/env python3
"""
Secure Heuristic Endpoints with Spotlighting Protection

Demonstrates best practices for implementing Heuristic-powered endpoints with
...
async def secure_chat(
    ...
    """
    Secure Heuristic chat endpoint with full security controls.
    ...
    try:
        ...
        # Step 2: Generate Heuristic response (with spotlighted input)
        # In production, this would call your actual Heuristic service
        heuristic_output = await _mock_heuristic_generate(safe_input)

        # Step 3: Validate LLM output for malicious content
        is_valid, issues = spotlighting_engine.validate_heuristic_output(
            heuristic_output
        )
        ...
        if not is_valid:
            logger.warning(
                ...
                extra={
                    "user_id": current_user.id,
                    "issues": issues,
                    "output_preview": heuristic_output[:200],
        ...
        logger.info(
            ...
            extra={
                "user_id": current_user.id,
                "validated": True,
                "response_length": len(heuristic_output),
                "event_type": "chat_response",
            },
        )

        return ChatResponse(
            response=heuristic_output,
            validated=True,
            validation_issues=[],
            timestamp=datetime.utcnow(),
        )
...
async def analyze_assessment_secure(
    ...
    """
    Secure assessment analysis with Heuristic.

    Security Measures:
    1. Spotlight assessment data
    2. Validate Heuristic insights
    3. User ownership verification
    4. Analysis type validation

    ✅ GOOD: Shows real-world Heuristic security pattern
    """
    try:
        # Step 1: Get assessment (verify ownership)
        from app.api.v1.endpoints.assessments.crud import get_assessment
        ...
        analysis = await _mock_heuristic_generate(safe_prompt)

        # Step 5: Validate analysis output
        is_valid, issues = spotlighting_engine.validate_heuristic_output(analysis)
...
async def secure_chat_stream(
    ...
    try:
        ...
        async def generate():
            ...
            # Mock streaming - in production, use actual Heuristic service
...
async def _mock_heuristic_generate(prompt: str) -> str:
    """Mock Heuristic generation for demo purposes.

    In production, replace with actual Heuristic service call:
    - OpenHeuristic API
    - Anthropic Claude
    - Local LLM
    - Custom Heuristic service
    """

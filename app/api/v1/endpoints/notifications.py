...
async def get_notification_preferences(
    ...
    if current_user.role not in ['clinician', 'admin']:
        raise HTTPException(403, "Notification preferences only available to clinicians and administrators")
...
async def update_notification_preferences(
    ...
    try:
        ...
        raise HTTPException(400, "Invalid time format. Use HH:MM format (e.g., '22:00')")
    ...
    if prefs:
        ...
        prefs.bypass_quiet_hours_for_critical = preferences.bypass_quiet_hours_for_critical
...
async def list_notifications(
    ...
    # Check role
    if current_user.role not in ['clinician', 'admin']:
        raise HTTPException(403, "Notifications only available to clinicians and administrators")
...
async def trigger_pending_review_notifications(
    ...
    if current_user.role != 'admin':
        raise HTTPException(403, "Only administrators can manually trigger notifications")
    ...
    try:
        ...
        logger.info(f"Pending review notifications triggered by admin {current_user.id}: {result}")

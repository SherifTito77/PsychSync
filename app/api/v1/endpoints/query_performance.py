...
async def get_query_performance_stats(
    ...
    """
    try:
        stats = query_monitor.get_real_time_stats()
...
async def get_slow_queries(
    ...
    """
    try:
        slow_queries = query_monitor.get_recent_slow_queries(limit)
...
async def get_query_errors(
    ...
    """
    try:
        errors = query_monitor.get_recent_errors(limit)
...
async def get_performance_alerts(
    ...
    """
    try:
        alerts = query_monitor.get_recent_alerts(limit)
...
async def get_optimization_opportunities(
    ...
    """
    try:
        opportunities = performance_dashboard._get_optimization_opportunities()
...
async def get_performance_report(
    ...
    """
    try:
        report = performance_dashboard.export_performance_report(hours)
...
async def analyze_query(
    ...
    try:
        ...
        execute_query = query_data.get("execute", False)
        ...
        logger.info(
            EventType.DATABASE_OPERATION,
            f"Query analyzed: {metrics.complexity.value} complexity",
...
async def reset_performance_stats(
    ...
    """
    try:
        query_monitor.reset_statistics()
...
async def apply_automatic_optimizations(
    ...
    try:
        ...
        current_report = auto_optimizer.get_optimization_report()
...
async def get_query_performance_health(
    ...
    try:
        stats = query_monitor.get_real_time_stats()
        alerts = query_monitor.get_recent_alerts(limit=10)
        ...
        if stats.total_queries > 0:
            slow_rate = (stats.slow_query_count / stats.total_queries) * 100
            if slow_rate > 10:
                health_score -= 20
                issues.append(f"High slow query rate: {slow_rate:.1f}%")

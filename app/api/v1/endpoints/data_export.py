...
async def create_export_request(
    ...
    - **format**: Export format (json, csv, xml, pdf, excel)
    - **scope**: Data scope to export (profile, assessments, team_data, activity_log, settings, all)
    - **date_range_start**: Optional start date for filtering
    - **date_range_end**: Optional end date for filtering
    - **include_anonymized_data**: Include anonymized data
    - **include_deleted_data**: Include deleted/archived data
    - **filters**: Additional filters for data selection

    Creates a background job to process the export request.
    """
    try:
...
async def get_export_statistics(
    ...
    Returns statistics about user's export history and usage.
    """
    try:
        export_service = get_data_export_service()
...
async def cleanup_expired_exports(
    ...
    Removes expired export files from the system.
    """
    try:
        export_service = get_data_export_service()
...
async def get_available_scopes(
    ...
    Returns information about data export scopes.
    """
    try:
        scopes = {
        ...
        logger.error(f"Failed to get available scopes: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
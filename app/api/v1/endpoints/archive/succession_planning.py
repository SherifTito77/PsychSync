...
class SuccessionScenarioRequest(BaseModel):
    ...
    include_external_hiring: bool = Field(
        False, description="Whether to consider external hiring"
    )
...
class GapAnalysisRequest(BaseModel):
    ...
    competency_categories: Optional[List[str]] = Field(
        None, description="Specific competency categories to analyze"
    )
    min_gap_threshold: float = Field(
        10, description="Minimum gap percentage to consider"
    )
    include_development_plan: bool = Field(
        True, description="Include development plan recommendations"
    )


# Helper functions to convert dataclasses to response models
def _convert_leadership_pipeline(
...
def _convert_succession_candidate(
...
def _convert_succession_scenario(
    scenario: SuccessionScenario,
) -> SuccessionScenarioResponse:
...
async def get_leadership_pipeline(
    organization_id: Optional[str] = Query(
        None, description="Organization ID to analyze"
    ),
    ...
    try:
        ...
        if not (
            ...
            or current_user.role in ["admin", "hr", "executive"]
        ):
            raise HTTPException(
                status_code=403,
        ...
        return [
            _convert_leadership_pipeline(pipeline)
            for pipeline in pipeline_analysis.values()
        ]
...
@router.get(
    "/succession-candidates/{role_id}", response_model=List[SuccessionCandidateResponse]
)
async def get_succession_candidates(
    ...
    organization_id: Optional[str] = Query(
        None, description="Organization ID to search within"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    ...
    try:
        ...
        if not (
            ...
            raise HTTPException(
                status_code=403, detail="Not authorized to view succession candidates"
            )
        ...
        return [
            _convert_succession_candidate(candidate)
            @ rate_limit(
    ...
    try:
        ...
        for competency, gaps in gap_analysis["competency_gaps"].items():
            gap_analysis["competency_gaps"][competency] = {
                ...
                "max_gap": max(gaps),
                "min_gap": min(gaps),
                "candidates_with_gap": len(
                    [g for g in gaps if g >= request.min_gap_threshold]
                ),
            }

        # Generate development recommendations if requested
        if request.include_development_plan:
            gap_analysis["development_recommendations"] = (
                await _generate_gap_recommendations(
...
async def simulate_succession_scenarios(
    ...
    try:
        ...
        scenario_results = await planner.simulate_succession_scenarios(
            org_id, scenario_configs
        )
...
async def create_development_programs(
    target_roles: Optional[List[str]] = Query(
        None, description="Target roles for development"
    ),
    organization_id: Optional[str] = Query(None, description="Organization ID"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
    ...
    try:
        ...
        if not (current_user.is_superuser or current_user.role in ["admin", "hr"]):
            raise HTTPException(
                status_code=403, detail="Not authorized to create development programs"
            )
        ...
        planner = SuccessionPlanner(db)
        development_programs = await planner.create_development_programs(
            org_id, target_roles
        )
...
async def get_succession_dashboard(
    time_horizon_months: int = Query(
        24, description="Time horizon for analysis in months"
    ),
    organization_id: Optional[str] = Query(None, description="Organization ID"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
    ...
    try:
        ...
        if not (
            ...
            raise HTTPException(
                status_code=403, detail="Not authorized to view succession dashboard"
            )
        ...
        planner = SuccessionPlanner(db)
        dashboard_data = await planner.generate_succession_dashboard(
            org_id, time_horizon_months
        )
...
async def get_succession_risk_assessment(
    organization_id: Optional[str] = Query(None, description="Organization ID"),
    include_mitigation_strategies: bool = Query(
        True, description="Include risk mitigation strategies"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    ...
    try:
        ...
        for level, pipeline in pipeline_analysis.items():
            ...
            risk_assessment["risk_by_level"][level] = {
                ...
                "position_risk": (pipeline.gap_percentage / 100)
                * pipeline.total_positions,
            }
        ...
        risk_assessment["overall_risk_score"] = (
            total_risk / level_count if level_count > 0 else 0
        )
...
async def get_readiness_timeline(
    ...
    try:
        ...
        if not (current_user.is_superuser or current_user.role in ["admin", "hr"]):
            raise HTTPException(
                status_code=403, detail="Not authorized to view readiness timeline"
            )
        ...
        if role_id:
            ...
            for level, pipeline in pipeline_analysis.items():
                role_profile = planner.role_profiles.get(
                ...
                if role_profile:
                    ...
                    ready_candidates = [
                        candidate
                        for candidate in candidates
                        if candidate.candidate.readiness_level
                    ...
                    timeline_data["overall_timeline"][level] = {
                        ...
                        "average_timeline": (
                            sum(
                                candidate.candidate.promotion_timeline
                    ...
                    for candidate in candidates:
                        ...
                        timeline_data["readiness_distribution"][
                            readiness_category
                        ].append(
                            {
                                "user_id": candidate.candidate.user_id,
...
async def _generate_gap_recommendations(
    critical_gaps: Dict[str, List[Dict]], organization_id: str
) -> List[Dict[str, Any]]:
    ...
    for competency, gap_data in critical_gaps.items():
        ...
        recommendation = {
            ...
            "estimated_cost": avg_gap
            * affected_candidates
            * 1000,  # Simplified calculation

"""
AI-Enhanced Analytics API Endpoints
...
async def get_ai_enhanced_dashboard(
    ...
    - **time_period_days**: Analysis time window (1-365 days)

    Returns comprehensive analytics including:
    - AI-generated insights and recommendations
    - Predictive metrics with confidence scores
    - Risk assessment and mitigation strategies
    - Opportunity identification
    - Team health analysis using AI
    """
    try:
...
async def get_ai_insights(
    ...
    - **insight_type**: Filter by prediction, anomaly_detection, trend_analysis, recommendation, risk_assessment, opportunity_identification
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve AI insights: {e!s}"
        ) from e
...
async def get_predictive_metrics(
    ...
    - **metric_type**: Filter by specific metric name
    ...
    try:
        ...
        predictions = dashboard_data.get("predictive_metrics", {}).get(
            "predictions", []
        )
        ...
        if metric_type:
            predictions = [
                p for p in predictions if p.get("metric_name") == metric_type
            ]
...
async def get_risk_assessment(
    ...
    include_user_details: bool = Query(
        False, description="Include detailed user risk profiles"
    ),
    current_user: User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    - **include_user_details**: Include detailed risk profiles for individual users
    """
    try:
        ai_analytics = AIEnhancedAnalyticsService(db)
        ...
        # Remove sensitive user details if not requested
        if not include_user_details:
            risk_assessment.pop("high_risk_users", None)
...
async def get_opportunities(
    ...
    - **opportunity_type**: Filter by high_potential_users, team_optimization, engagement, development
    """
    try:
        ai_analytics = AIEnhancedAnalyticsService(db)
        ...
        # Apply type filter
        if opportunity_type:
            opportunities = {opportunity_type: opportunities.get(opportunity_type, [])}
...
async def get_team_health_ai_analysis(
    ...
    - **include_recommendations**: Include AI-generated improvement recommendations
    """
    try:
        ai_analytics = AIEnhancedAnalyticsService(db)
        ...
        if not include_recommendations:
            team_health.pop("recommendations", None)
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze team health: {e!s}"
        ) from e
...
async def refresh_ai_analytics(
    ...
    force_refresh: bool = Query(
        False, description="Force complete refresh of AI models"
    ),
    current_user: User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    - **force_refresh**: Force complete refresh of AI models and insights (resource intensive)
    """
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to refresh AI analytics: {e!s}"
        ) from e

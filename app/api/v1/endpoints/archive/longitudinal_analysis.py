...
class ChangeDetectionRequest(BaseModel):
    ...
    detection_mode: str = Field(
        "streaming", description="Detection mode (streaming, batch, hybrid)"
    )
...
class BaselineCalculationRequest(BaseModel):
    ...
    baseline_period_days: int = Field(
        30, description="Period for baseline calculation in days"
    )
...
class UserProgressionRequest(BaseModel):
    ...
    include_change_points: bool = Field(
        True, description="Include change point detection"
    )
...
class ComparisonRequest(BaseModel):
    ...
    include_change_points: bool = Field(
        True, description="Include change points in comparison"
    )
    ...
    try:
        ...
        response_data = {
            ...
            "analysis_period": {
                ...
                "days": (
                    trend_analysis.analysis_period_end
                    - trend_analysis.analysis_period_start
                ).days,
            },
            ...
            "statistical_tests": {
                ...
                "seasonal_test": (
                    ...
                    if trend_analysis.seasonal_component
                    else None
                ),
    ...
    try:
        ...
        response_data = {
            ...
            "baseline_period": {
                ...
                "days": (
                    baseline.baseline_period_end - baseline.baseline_period_start
                ).days,
            },
            "statistics": {
                ...
                try:
                    ...
                    if metric in analysis["metrics_analysis"]:
                        user_data = {
                            ...
                            "change_points": analysis["metrics_analysis"][metric][
                                "change_points"
                            ],
                            "baseline": analysis["metrics_analysis"][metric].get(
                                "baseline"
                            ),
                            "current_value": analysis["metrics_analysis"][metric].get(
                                "current_value"
                            ),
                        }
                    ...
                    logger.error(
                        f"Error analyzing user {user_id}, metric {metric}: {e}"
                    )
                    continue
        ...
        for i in range(n_users):
            for j in range(i + 1, n_users):
                ...
                user1_data = [
                    ...
                    if d["user_id"] == request.user_ids[i]
                    and d["metric"] in request.metrics
                ]
                user2_data = [
                    d
                    for d in comparison_data
                    if d["user_id"] == request.user_ids[j]
                    and d["metric"] in request.metrics
                ]
                ...
                if user1_data and user2_data:
                    ...
                    trend1 = (
                        user1_data[0]["trend"]["trend_direction"]
                        if user1_data and user1_data[0]["trend"]
                        else "stable"
                    )
                    trend2 = (
                        user2_data[0]["trend"]["trend_direction"]
                        if user2_data and user2_data[0]["trend"]
                        else "stable"
                    )
                    ...
                    if user1_data[0].get("current_value") and user2_data[0].get(
                        "current_value"
                    ):
                        value1 = user1_data[0]["current_value"]
                        value2 = user2_data[0]["current_value"]
                        value_similarity = 1.0 - min(
                            1.0, abs(value1 - value2) / max(value1, value2, 1)
                        )
                        similarity = (
                            similarity + 0.5
                        ) / 2  # Average with trend similarity
...
async def get_user_insights(
    ...
    metrics: Optional[str] = Query(
        None, description="Comma-separated metrics to analyze"
    ),
    current_user: User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_db),
):
    ...
    try:
        ...
        time_range_days = {"7d": 7, "30d": 30, "90d": 90, "180d": 180}.get(
            time_range, 90
        )

        # Get user insights
        progression_analysis = await analyzer.analyze_user_progression(
            user_id=user_id, metrics=metrics_list, time_range_days=time_range_days
        )
        ...
        user_insights = {
            ...
            "recommendations": progression_analysis["recommendations"],
            "risk_indicators": progression_analysis["risk_indicators"],
            "metrics_summary": {metric: progression_analysis["metrics_analysis"]},

...
class LearningPathRequest(BaseModel):
    target_skills: List[str] = Field(
        ..., description="List of target skills to develop"
    )
    learning_style_preference: Optional[str] = Field(
        None, description="Preferred learning style"
    )
    max_duration_weeks: Optional[int] = Field(
        None, description="Maximum duration in weeks"
    )
...
class SkillAnalysisRequest(BaseModel):
    ...
    min_gap_percentage: Optional[float] = Field(
        0, description="Minimum gap percentage threshold"
    )
...
def _convert_learning_recommendation(
...
def _convert_development_program(
    program: DevelopmentProgram,
) -> DevelopmentProgramResponse:
...
def _convert_career_trajectory(
...
async def get_individual_skill_gaps(
    user_id: Optional[str] = Query(
        None, description="User ID to analyze (defaults to current user)"
    ),
...
async def get_individual_skill_summary(
    ...
    try:
        ...
        medium_priority_gaps = len(
            [gap for gap in skill_gaps if gap.priority == "medium"]
        )
        low_priority_gaps = len([gap for gap in skill_gaps if gap.priority == "low"])

        average_gap_size = (
            sum(gap.gap_percentage for gap in skill_gaps) / len(skill_gaps)
            if skill_gaps
            else 0
        )
...
async def get_organizational_skill_gaps(
    ...
    try:
        ...
        if not (
            current_user.is_superuser or current_user.role in ["admin", "team_lead"]
        ):
            raise HTTPException(
                status_code=403,
        ...
        for dept, gaps in org_gaps["department_gaps"].items():
            converted_department_gaps[dept] = [
                _convert_skill_assessment(gap) for gap in gaps
            ]

        # Convert critical gaps
        converted_critical_gaps = [
            _convert_skill_assessment(gap) for gap in org_gaps["critical_gaps"]
        ]
...
async def get_future_skill_demands(
    timeframe_months: int = Query(
        24, ge=1, le=60, description="Timeframe in months for prediction"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    ...
    try:
        ...
        skill_demands = await analyzer.predict_future_skill_demands(
            org_id, timeframe_months
        )
...
@router.post(
    "/individual/learning-path", response_model=List[LearningRecommendationResponse]
)
async def generate_learning_path(
    request: LearningPathRequest,
    user_id: Optional[str] = Query(
        None, description="User ID to generate path for (defaults to current user)"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    ...
    try:
        ...
        recommendations = await analyzer.recommend_learning_path(
            target_user_id, request.target_skills
        )
...
@router.get(
    "/individual/development-programs", response_model=List[DevelopmentProgramResponse]
)
async def get_development_programs(
    user_id: Optional[str] = Query(
        None, description="User ID to get programs for (defaults to current user)"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
...
@router.get(
    "/individual/career-trajectories", response_model=List[CareerTrajectoryResponse]
)
...
async def batch_skill_analysis(
    ...
    try:
        ...
        if not (current_user.is_superuser or current_user.role in ["admin", "hr"]):
            raise HTTPException(
                status_code=403, detail="Not authorized to perform batch skill analysis"
            )
        ...
        if request.user_ids:
            ...
            for user_id in request.user_ids:
                try:
                    ...
                    results[user_id] = [
                        _convert_skill_assessment(gap) for gap in user_gaps
                    ]
            ...
            results["organization"] = [
                _convert_skill_assessment(gap) for gap in filtered_gaps
            ]
            results["departments"] = {
...
async def get_skill_distribution_analytics(
    ...
    try:
        ...
        if not (
            ...
            or current_user.role in ["admin", "hr", "team_lead"]
        ):
            raise HTTPException(
                status_code=403,
        ...
        for dept, gaps in org_gaps["department_gaps"].items():
            ...
            analytics["department_comparison"][dept] = {
                ...
                "critical_gaps": len(
                    [gap for gap in gaps if gap.priority == "critical"]
                ),
            }

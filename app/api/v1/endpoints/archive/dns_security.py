"""
DNS Security Monitoring Endpoints
...
async def get_dns_security_status(
    ...
    """
    try:
        status_data = dns_security_manager.get_dns_health_status()
        ...
        logger.log_error(
            e, operation="get_dns_security_status", user_id=str(current_user.id)
        )
...
async def get_dns_security_summary(
    ...
    """
    try:
        summary_data = dns_security_manager.get_security_summary()
        ...
        logger.log_error(
            e, operation="get_dns_security_summary", user_id=str(current_user.id)
        )
        return APIResponse.server_error(
...
async def clear_dns_cache(
    ...
    """
    try:
        old_cache_size = len(dns_security_manager.query_cache)
        dns_security_manager.clear_cache()
...
async def test_dns_resolution(
    ...
    try:
        ...
        result = await dns_security_manager.resolve_hostname_secure(
        ...
        logger.info(
            ...
            operation_name="test_dns_resolution",
            user_id=str(current_user.id),
            hostname=hostname,
        ...
        logger.log_error(
            e, operation="test_dns_resolution", user_id=str(current_user.id)
        )
        return APIResponse.server_error(
...
async def get_dns_security_events(
    ...
    try:
        ...
        logger.log_error(
            e, operation="get_dns_security_events", user_id=str(current_user.id)
        )
        return APIResponse.server_error(

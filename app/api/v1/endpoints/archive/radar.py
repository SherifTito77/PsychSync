...
async def get_radar_view(
    ...
    - Early warning signals (burnout prediction)
    - Behavioral pattern analysis
    - Psychological safety metrics
    - Active intervention tracking

    Returns unified zone classification (green/yellow/red) with actionable insights
    """
    try:
        # Verify user has access to this organization
        # Allow superusers to access any organization
        # For demo purposes, allow access if user has no organization_id
        if (
            ...
            logger.warning(
                f"User {current_user.email} attempted to access organization "
                f"{request.organization_id} but belongs to {current_user.organization_id}"
            )
            # For demo purposes, allow access anyway with a warning
            logger.info(
                f"DEMO MODE: Allowing access to organization {request.organization_id}"
            )
...
async def get_quick_stats(
    ...
    Returns summary metrics including zone classification,
    pattern counts, and trend direction
    """
    try:
        # Verify user has access (demo mode - allow access for testing)
        if (
            ...
            logger.warning(
                f"User {current_user.email} accessing organization {organization_id} "
                f"(belongs to {current_user.organization_id}) - DEMO MODE"
            )
        ...
        return QuickStatsResponse(
            ...
            trend_direction=trends.get("summary", {}).get("trend_direction", "unknown"),
        )
...
async def get_zone_history(
    ...
    Shows how the organization has moved between green/yellow/red zones over time
    """
    try:
        # Verify user has access (demo mode - allow access for testing)
        if (
            ...
            logger.info(
                f"User {current_user.email} accessing zone history for {organization_id} - DEMO MODE"
            )
...
async def get_hotspots(
    ...
    Returns prioritized list of behavioral and toxicity hotspots
    """
    try:
        # Verify user has access (demo mode - allow access for testing)
        if (
            ...
            logger.info(
                f"User {current_user.email} accessing hotspots for {organization_id} - DEMO MODE"
            )
...
async def get_concentric_zones(
    ...
    try:
        ...
        if (
            ...
            logger.info(
                f"User {current_user.email} accessing concentric zones for {organization_id} - DEMO MODE"
            )

...
from app.core.logging_config import get_logger  # Get logger function
...
async def rate_limit_dependency(request: Request):
    """Dependency that applies rate limiting"""
    await check_rate_limit(request, limit=30, window=60)


@router.get("/health/public", summary="Public Health Check")
@rate_limit(limit=60, window=60)  # Rate limit public health checks
async def public_health_check(
    request: Request, _rate_limit: None = Depends(rate_limit_dependency)
):
    """
...
@rate_limit(limit=60, window=60)  # Rate limit basic health checks
async def health_check(request: Request):
    """
    Basic health check endpoint (no authentication required)
    Returns system status and basic metrics for monitoring systems
    """
    logger.info("health_check endpoint called - about to check rate limit")
    # Apply rate limiting
    await check_rate_limit(request, limit=30, window=60)
    logger.info("health_check - rate limit passed")
...
@rate_limit(limit=30, window=60)  # Stricter limit for detailed checks
...
# NOTE: /metrics endpoint removed to avoid conflict with prometheus_metrics router
# The comprehensive prometheus_metrics endpoint provides standard Prometheus format metrics
# at /metrics without authentication requirements. See: app/api/v1/endpoints/prometheus_metrics.py
# If you need application-specific metrics, use a different path like /api/v1/health/metrics

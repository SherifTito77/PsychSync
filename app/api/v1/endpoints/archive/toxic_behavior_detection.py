...
async def submit_anonymous_report(
    request: AnonymousReportRequest, db: AsyncSession = Depends(get_db)
):
    """
...
async def check_anonymous_report_status(
    tracking_id: str, db: AsyncSession = Depends(get_db)
):
    """
...
async def get_toxicity_dashboard(
    ...
    try:
        ...
        # Build query for recent patterns using SQLAlchemy 2.0 async syntax
        pattern_query = select(ToxicityPattern).filter(
            and_(
                ToxicityPattern.organization_id == organization_id,
        ...
        intervention_query = select(func.count(BehavioralIntervention.id)).filter(
            and_(
                BehavioralIntervention.toxicity_pattern_id.in_(pattern_ids),
        ...
        active_interventions = intervention_result.scalar() or 0
        ...
        latest_ps = ps_result.scalar_one_or_none()

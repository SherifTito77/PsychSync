...
async def get_growth_dashboard(
    ...
    try:
        ...
        # Generate dashboard
        dashboard = (
            await growth_analytics_service.generate_comprehensive_growth_dashboard(
...
async def list_conversion_funnels(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        for (
            funnel_name,
            funnel_def,
        ) in growth_analytics_service.funnel_definitions.items():
            funnels[funnel_name] = {
...
async def get_acquisition_metrics(
    ...
    try:
        ...
        metrics = await growth_analytics_service._calculate_acquisition_metrics(
            date_range_days
        )
        ...
        if channel:
            metrics["channel_breakdown"] = {
                channel: metrics.get("channels", {}).get(channel, {})
            }
...
async def get_engagement_metrics(
    ...
    try:
        ...
        metrics = await growth_analytics_service._calculate_engagement_metrics(
            date_range_days
        )

...
# Helper function to safely get organization_id
def _get_organization_id(user: User) -> str:
    """Safely get organization_id from user, fallback to user ID if not present"""
    org_id = getattr(user, "organization_id", None)
    return str(org_id) if org_id else str(user.id)
...
class HealthAnalysisRequest(BaseModel):
    ...
    include_biometric: bool = Field(
        False, description="Include biometric data if available"
    )
...
class ConsentRequest(BaseModel):
    ...
    biometric_collection: bool = Field(
        False, description="Consent to collect biometric data"
    )
    biometric_processing: bool = Field(
        False, description="Consent to process for wellness"
    )
    biometric_sharing: bool = Field(
        False, description="Consent to share with healthcare"
    )
    data_sources: Optional[List[str]] = Field(
        None, description="Consented data sources"
    )
    data_retention_days: int = Field(
        365, ge=30, le=2555, description="Days to retain data"
    )
    ...
    try:
        ...
        health_risks = HealthRiskIndicators(
            stress_level=StressLevel(
                request.health_risks.get("stress_level", "normal")
            ),
            burnout_stage=BurnoutStage(
                request.health_risks.get("burnout_stage", "none")
            ),
            cardiovascular_risk_score=request.health_risks.get(
            ...
            sleep_disruption_score=request.health_risks.get(
                "sleep_disruption_score", 0
            ),
            social_isolation_score=request.health_risks.get(
                "social_isolation_score", 0
            ),
            urgent_intervention_needed=request.health_risks.get(
                "urgent_intervention_needed", False
            ),
            recommend_medical_evaluation=request.health_risks.get(
                "recommend_medical_evaluation", False
            ),
            recommend_immediate_break=request.health_risks.get(
                "recommend_immediate_break", False
            ),
            recommend_workload_reduction=request.health_risks.get(
...
async def check_manager_access(
    ...
    """
    try:
        # Check if user has superuser privileges or is active
        has_access = current_user.is_superuser or current_user.is_active
...
async def get_manager_dashboard(
    ...
    """
    try:
        # Verify user has manager/HR role (is_superuser or is_active for development)
        # For production, you may want to implement proper role-based access control
        if not (current_user.is_superuser or current_user.is_active):
            raise HTTPException(
                status_code=403, detail="Manager dashboard requires admin privileges"
            )
        ...
        if team_id:
            ...
            team_result = await db.execute(
                select(Team).filter(Team.id == uuid.UUID(team_id))
            )
        ...
        team_stats = {
            ...
            "cardiovascular_risk_distribution": {"low": 18, "medium": 3, "high": 1},
        }
        ...
        if team_stats["high_risk_members_count"] > 0:
            org_risks.append(
                f"{team_stats['high_risk_members_count']} team members at elevated health risk"
            )

        if team_stats["critical_interventions_active"] > 0:
            org_risks.append(
                f"{team_stats['critical_interventions_active']} critical intervention(s) active"
            )
        ...
        return ManagerDashboardResponse(
            ...
            cardiovascular_risk_distribution=team_stats[
...
async def update_consent_preferences(
    ...
    try:
        ...
        consent_record = await loop.run_in_executor(
            None,
            lambda: db.query(HealthDataConsent)
            .filter(HealthDataConsent.user_id == current_user.id)
            .first(),
        )
        ...
        if consent_record:
            ...
            consent_record = HealthDataConsent(
                ...
                consent_given=request.biometric_collection
                or request.biometric_processing,
                consent_date=datetime.utcnow().date(),
            )
    ...
    try:
        ...
        return {
            ...
            "consent_date": (
                ...
                if consent_record.consent_date
                else None
            ),
...
def _generate_recommended_actions(health_risks: HealthRiskIndicators) -> List[str]:
    ...
    if health_risks.recommend_medical_evaluation:
        ...
        actions.append(
            "Call emergency services if experiencing chest pain, shortness of breath, or severe headache"
        )

...
async def send_chatbot_message(
    ...
    # Handle session ID conversion to uuid.UUID
    try:
        if request.session_id:
            # Try parsing as uuid.UUID
            session_id = str(uuid.UUID(request.session_id))
        else:
            session_id = str(uuid4())
    except (ValueError, AttributeError):
        # If not a valid uuid.UUID string (like frontend's 'chat_123'),
        # use a deterministic uuid.UUID based on the string
        import hashlib

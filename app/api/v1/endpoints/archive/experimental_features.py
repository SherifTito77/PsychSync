...
async def get_experiments(
    ...
    try:
        ...
        lab = ExperimentalFeaturesLab(db)
        await lab.get_experiment_dashboard()
...
async def opt_in_experimental_features(
    opt_in: bool = Body(..., description="Whether to opt in to experimental features"),
    feature_types: Optional[List[str]] = Body(
...
async def submit_experimental_feature_feedback(
    feature_type: str = Body(..., description="Type of experimental feature"),
    feedback_data: Dict[str, Any] = Body(..., description="Feedback data"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """

...
class MFADisableRequest(BaseModel):
    ...
    password: str = Field(
        ..., min_length=8, description="Current password for confirmation"
    )
...
class BackupCodesRegenerateResponse(BaseModel):
    ...
    message: str = Field(
        "Save these new backup codes securely. Old codes are no longer valid."
    )


class BackupCodeVerifyResponse(BaseModel):
    """Response model for backup code verification"""
...
async def setup_mfa(
    ...
    try:
        ...
        # MFASetupError indicates a business logic error, not a server error
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
...
async def verify_backup_code(
    ...
    try:
        ...
        await mfa_service.verify_backup_code(
            current_user, request.code, db, consume=True
        )

        return BackupCodeVerifyResponse(
            message="Backup code verified successfully",
            remaining_codes=mfa_service.get_mfa_status(current_user)[
                "backup_codes_count"
            ],
        )
...
async def regenerate_backup_codes(
    ...
    try:
        ...
        # BackupCodeError indicates a business logic error (e.g., MFA not enabled), not a server error
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def disable_mfa(
    ...
    **WARNING**: This reduces account security significantly.
    ...
    if not verify_password(request.password, current_user.password_hash):
        # Use 400 BAD_REQUEST because user IS authenticated - this is a validation error
        # 401 is for missing/invalid authentication, not for incorrect data
        raise HTTPException(
    ...
    try:
        ...
        # MFASetupError indicates a business logic error (e.g., MFA not enabled), not a server error
        # Use 400 BAD_REQUEST for client-side state issues
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,

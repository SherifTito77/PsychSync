...
def pivot_response_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    The input DataFrame should have columns: user_id, question_id, answer_value.
    Returns a DataFrame where index is user_id and columns are question_ids.
    """
    if df.empty:
        return pd.DataFrame()
    ...
    # Handle multiple responses from same user for same question (take latest)
    ...
    # Ensure IDs are strings to avoid uuid.UUID issues during pivoting/serialization
    df = df.copy()
    df["user_id"] = df["user_id"].astype(str)
    df["question_id"] = df["question_id"].astype(str)

    # Pivot: Index = Respondents, Columns = Items, Values = Responses
    try:
        pivot_df = df.pivot(
            index="user_id", columns="question_id", values="answer_value"
        )
        # Fill missing values with 0 or NaN as appropriate for psychometrics
        # For reliability, we typically want numeric types
...
async def analyze_reliability(
    ...
    try:
        ...
        # Pivot to wide format (Respondents x Items)
...
async def analyze_validity(
    ...
    try:
        ...
        if request.validity_type == ValidityType.CONVERGENT:
            ...
            # Calculate convergent validity (joining on user_id index)
...
async def analyze_items(
    ...
    try:
        ...
        # Calculate total scores per respondent
    ...
    try:
        ...
        if item_responses.empty:
            return JSONResponse(
                ...
                content={
                    "success": False,
                    "error_message": "No valid response data found for this assessment",
                },
            )

        # Calculate basic statistics on pivoted data

...
async def login_for_access_token_fixed(
    ...
    try:
        ...
        refresh_token = create_secure_token_for_user(str(user.id), user.email, expires_delta=timedelta(days=7))
        ...
        response = Response(
            content='{"message": "Login successful", "user": {"id": "' + str(user.id) + '", "email": "' + user.email + '", "full_name": "' + user.full_name + '"}}',

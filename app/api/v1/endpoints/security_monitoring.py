...
from app.api.v1.deps import get_db, get_current_active_user, get_current_admin_user
...
from app.core.security_monitoring import security_monitor, AlertSeverity, AnomalyType, RiskLevel
...
async def get_security_dashboard(
    ...
    try:
        ...
        dashboard_data = {
            ...
            "security_score": max(0, 100 - (len(critical_alerts) * 20) - (len(high_alerts) * 10) - (len(medium_alerts) * 5)),
            "recent_events": recent_events,
            "active_threats": len([a for a in alerts if not a.resolved and a.severity in [AlertSeverity.HIGH, AlertSeverity.CRITICAL]]),
...
async def resolve_security_alert(
    alert_id: str,
    request: Request,
    resolution_note: Optional[str] = Query(None, description="Note about resolution"),
    current_user: User = Depends(get_current_admin_user)
):
    """
    ...
    try:
        ...
        if not success:
            raise HTTPException(
                status_code=404,
                detail=f"Security alert not found: {alert_id}"
            )
...
async def get_user_risk_assessment(
    ...
    try:
        ...
        from app.core.security_monitoring import UserBehaviorProfile
        ...
        assessment_data = {
            ...
            "security_recommendations": self._generate_security_recommendations(risk_level, user_alerts)
        }
...
async def get_threat_intelligence_summary(
    ...
    try:
        ...
        for alert in alerts:
            anomaly_type = alert.anomaly_type.value
            threat_patterns["top_anomaly_types"][anomaly_type] = threat_patterns["top_anomaly_types"].get(anomaly_type, 0) + 1

        # Analyze severity distribution
        for alert in alerts:
            severity = alert.severity.value
            threat_patterns["severity_distribution"][severity] = threat_patterns["severity_distribution"].get(severity, 0) + 1
        ...
        metrics = {
            ...
            "security_status": "healthy" if len([a for a in alerts if a.severity == AlertSeverity.CRITICAL]) == 0 else "critical"
        }
        ...
        logger.log_error(e, operation="get_threat_intelligence_summary", user_id=str(current_user.id))
        return APIResponse.server_error(
...
async def record_security_event(
    ...
    try:
        ...
        ip_address = event_data.get("ip_address", request.client.host if request.client else "unknown")
...
def _generate_system_recommendations(alerts: List, metrics: Dict[str, Any]) -> List[str]:
    ...
    if critical_alerts > 0:
        recommendations.append(f"IMMEDIATE ACTION REQUIRED: {critical_alerts} critical security alerts detected")

    if metrics["average_risk_score"] > 60:
        recommendations.append("Review overall security posture and implement additional protections")

    if metrics["unique_users_affected"] > 10:
        recommendations.append("Investigate potential widespread security issue affecting multiple users")

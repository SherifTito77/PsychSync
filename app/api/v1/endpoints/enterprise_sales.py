...
async def get_account_health(
    ...
    try:
        ...
        return {
            ...
            "last_login": (
                health_metrics.last_login.isoformat()
                if health_metrics.last_login
                else None
            ),
        }
        ...
        logger.error(
            f"Failed to get account health for org {organization_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        return {
            ...
            "total_potential_value": sum(
                opp.get("potential_value", 0) for opp in opportunities
            ),
        }
        ...
        logger.error(
            f"Failed to get expansion opportunities for org {organization_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def monitor_sla_compliance(
    ...
    try:
        ...
        sla_compliance = await enterprise_sales_service.monitor_sla_compliance(
            sla_tier=sla_tier, date_range_start=start_date, date_range_end=end_date
        )
...
async def schedule_qbr(
    ...
    try:
        ...
        qbr_details = await enterprise_sales_service.schedule_qbr(
            organization_id=organization_id, qbr_type=qbr_type, attendees=attendees
        )
        ...
        return {"success": True, "qbr_scheduled": qbr_details}
...
async def get_enterprise_dashboard(
    ...
    try:
        ...
        enterprise_orgs = await loop.run_in_executor(
            None,
            lambda: db.query(Organization)
            .filter(Organization.subscription_tier.in_(["enterprise", "clinical"]))
            .all(),
        )
        ...
        for org in enterprise_orgs:
            try:
                health = await enterprise_sales_service.calculate_customer_health(
                    org.id, db
                )
                opportunities = (
                    await enterprise_sales_service.generate_expansion_opportunities(
                        org.id, db
                    )
                )
                ...
                dashboard_data["summary"]["health_distribution"][health_status] = (
                    dashboard_data["summary"]["health_distribution"].get(
                        health_status, 0
                    )
                    + 1
                )
                ...
                for opp in opportunities[:3]:  # Top 3 per account
                    dashboard_data["top_opportunities"].append(
                        {"organization_name": org.name, "opportunity": opp}
                    )

            except Exception as e:
                logger.warning(
                    f"Failed to process enterprise account {org.id}: {str(e)}"
                )
                continue
...
async def get_enterprise_features():
    ...
    try:
        ...
        return {
            "tiers": {
                tier.value: {"tier": tier.value, "features": features_list}
...
async def trigger_intervention_playbook(
    ...
    try:
        ...
        await enterprise_sales_service._schedule_success_plays(
            account, intervention_type
        )
        ...
        return {
            ...
            "scheduled_plays": len(
                enterprise_sales_service.success_plays.get(intervention_type, [])
            ),
        ...
        logger.error(
            f"Failed to trigger intervention for org {organization_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def _notify_customer_success_team(
    organization_id: int, intervention_type: str, triggered_by: str
):
    """Notify customer success team (background task)"""
    try:
        logger.info(
            f"Customer success team notified about {intervention_type} for organization {organization_id}"
        )

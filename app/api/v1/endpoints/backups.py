...
class BackupResponse(BaseModel):
    ...
    restoration_test_date: Optional[datetime]
...
@router.delete("/backups/{backup_id}", response_model=SuccessResponse[Dict[str, str]], dependencies=[Depends(get_current_user)])
...
async def cleanup_old_backups(
    ...
    try:
        ...
        logger.info(f"Backup cleanup completed by user {current_user.email}. Deleted {deleted_count} backups.")
...
async def test_storage_connection(
    ...
    try:
        ...
        logger.error(f"Failed to test storage connection: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
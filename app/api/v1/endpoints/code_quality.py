...
async def get_quality_summary(
    ...
    if previous_metrics:
        ...
        if previous.quality_score > 0:
            change = (
                (latest.quality_score - previous.quality_score) / previous.quality_score
            ) * 100
    ...
    return CodeQualitySummary(
        ...
        total_issues=latest.code_violations_count
        + latest.bugs_count
        + latest.security_hotspots_count,
...
async def get_quality_hotspots(
...
async def get_pull_request_quality(
    skip: int = Query(0, ge=0),
    limit: int = Query(100, ge=1, le=200),

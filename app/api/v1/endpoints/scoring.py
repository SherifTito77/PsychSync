...
async def submit_assessment(assessment_data: dict):
    ...
    try:
        ...
        if assessment_type == "mbti":
            ...
            avg_consistency = sum([
                max(dimensions['E-I'].values()) / sum(dimensions['E-I'].values()) if sum(dimensions['E-I'].values()) > 0 else 0,
                max(dimensions['S-N'].values()) / sum(dimensions['S-N'].values()) if sum(dimensions['S-N'].values()) > 0 else 0,
                max(dimensions['T-F'].values()) / sum(dimensions['T-F'].values()) if sum(dimensions['T-F'].values()) > 0 else 0,
                max(dimensions['J-P'].values()) / sum(dimensions['J-P'].values()) if sum(dimensions['J-P'].values()) > 0 else 0
            ]) / 4

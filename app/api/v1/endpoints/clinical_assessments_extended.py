...
async def get_lsas_history(
    ...
    query = (
        ...
        .order_by(ClinicalAssessmentExtended.completed_at.desc())
        .limit(limit)
    )
...
async def submit_eat26_assessment(
    ...
    try:
        # Score assessment
        result = EAT26Scorer.score(request.responses, behavioral=request.behavioral)
        ...
        assessment = ClinicalAssessmentExtended(
            ...
            risk_flags=result.risk_flags,
            response_metadata=(
                {"behavioral": request.behavioral} if request.behavioral else None
            ),
            completed_at=datetime.utcnow(),
        )
...
async def get_user_trends(
    assessment_type: str = Query(
        ..., description="Assessment type (PHQ9, GAD7, LSAS, EAT26, YBOCS)"
    ),
    current_user: User = Depends(get_current_active_user),
    db: AsyncSession = Depends(get_async_db),
):
...
async def get_population_metrics(
    ...
    if current_user.role not in ["clinician", "admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only clinicians and admins can access population metrics",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to calculate population metrics"
        )
...
async def get_high_risk_users(
    ...
    if current_user.role not in ["clinician", "admin"]:
        raise HTTPException(
            status_code=403,
            detail="Only clinicians and admins can access high-risk user lists",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to identify high-risk users"
        )
...
async def refresh_analytics_views(
    ...
    if current_user.role not in ["clinician", "admin"]:
        raise HTTPException(
            status_code=403, detail="Only clinicians and admins can refresh analytics"
        )
...
async def _trigger_crisis_alert(
    ...
    try:
        ...
        logger.warning(
            f"Crisis alert triggered for user {user_id}, assessment {assessment_type}, severity {severity}"
        )
...
async def get_gad7_extended_analytics(
    ...
    try:
        ...
        # Get trend data
        trend = await analytics.calculate_user_trends(
            user_id=user_id, assessment_type="GAD7", min_data_points=2
        )
        ...
        query = (
            ...
            .order_by(ClinicalAssessmentExtended.completed_at.asc())
        )
        ...
        return {
            ...
            "recommendations": _generate_gad7_recommendations(
                trend, avg_score, volatility
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail="Failed to generate extended analytics"
        )


def _generate_gad7_recommendations(
    trend: Dict, avg_score: Optional[float], volatility: Optional[float]
) -> List[str]:
    ...
    if not trend:
        return [
            "Complete regular GAD-7 assessments to enable personalized recommendations."
        ]
    ...
    if avg_score is not None:
        if avg_score >= 15:
            recommendations.append(
                "Consider discussing additional treatment options with provider"
            )
        elif avg_score <= 5:
            recommendations.append(
                "Excellent symptom control - maintain self-care practices"
            )

    if volatility is not None and volatility > 10:
        recommendations.append(
            "High symptom variability detected - consider tracking triggers and patterns"
        )

...
class SimpleEmailConnection(BaseModel):
    ...
    app_password: str = Field(
        ..., min_length=1, description="App password (not regular password)"
    )
    provider: str = Field(..., description="Email provider")
    display_name: Optional[str] = Field(
        None, description="Display name for this email account"
    )
    ...
    def validate_provider(cls, v):
        if v not in IMAP_PROVIDERS:
            raise ValueError(
                f'Provider must be one of: {", ".join(IMAP_PROVIDERS.keys())}'
            )
        return v
...
class SyncOptions(BaseModel):
    ...
    days_back: int = Field(
        30, ge=1, le=365, description="Number of days of emails to analyze"
    )
    max_emails: int = Field(
        1000, ge=100, le=2000, description="Maximum emails to fetch"
    )
...
async def quick_test_connection(test_data: QuickTest):
    ...
    try:
        ...
        is_valid, message = free_email_connector_service.test_imap_connection(
            test_data.email_address, test_data.app_password
        )
...
async def connect_email_simple(
    ...
    try:
        logger.info(
            f"User {current_user.id} connecting email {connection_data.email_address}"
        )
        ...
        is_valid, test_message = free_email_connector_service.test_imap_connection(
            connection_data.email_address, connection_data.app_password
        )
        ...
        email_connection = await free_email_connector_service.create_free_connection(
            ...
            account_name=connection_data.display_name
            or connection_data.email_address.split("@")[0],
            custom_imap_config={
                ...
                "use_starttls": IMAP_PROVIDERS[connection_data.provider].get(
                    "use_starttls", False
                ),
            },
        )
        ...
        try:
            ...
            logger.warning(
                f"Initial sync failed for {connection_data.email_address}: {sync_error}"
            )
        ...
        logger.info(
            f"Successfully connected email {connection_data.email_address} for user {current_user.id}"
        )
...
async def get_my_connections(
    ...
    try:
        ...
        connections_with_stats = (
            await email_connection_service.get_connections_with_stats(
                db, current_user.id
            )
        )
...
async def sync_emails_simple(
    ...
    try:
        ...
        if (
            ...
            and (datetime.utcnow() - connection.last_sync_at).total_seconds() < 1800
        ):  # 30 minutes
            raise HTTPException(
                status_code=429,
        ...
        raise HTTPException(status_code=500, detail="Failed to sync emails") from e
...
async def delete_connection_simple(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.id} deactivated email connection {connection_id}"
        )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete email connection"
        ) from e
    ...
    try:
        ...
        return {
            ...
            "needs_attention": (
                ...
                or (days_since_sync and days_since_sync > 7)
                or connection.sync_status == "error"
            ),
        }

            ...
            campaigns.append(
                {
                    ...
                    "trigger_events": [
                        event.value for event in campaign.trigger_events
                    ],
...
async def list_user_segments():
    ...
    try:
        ...
        return {"segments": segments, "total_count": len(segments)}
...
async def generate_referral_code(current_user: User = Depends(get_current_user)):
    ...
    """
    try:
        referral_code = await growth_service.generate_referral_code(
            str(current_user.id)
        )
        ...
        logger.error(
            f"Failed to generate referral code for user {current_user.id}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def track_referral_conversion(
    referral_code: str, new_user_id: str, current_user: User = Depends(get_current_user)
):
    """
...
async def list_available_triggers():
    ...
    try:
        ...
        return {"triggers": triggers, "total_count": len(triggers)}
...
async def handle_user_signup_webhook(
    user_data: Dict[str, Any], background_tasks: BackgroundTasks
):
    """

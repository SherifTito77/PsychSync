...
class PerformanceMetrics:
    ...
    def add_measurement(self, name: str, duration_ms: float, metadata: Dict[str, Any] = None):
...
class TestDatabasePerformance:
    ...
    async def test_user_query_performance(self, perf_metrics):
        ...
        # Check performance
        threshold_check = perf_metrics.check_threshold('user_lookup', PERFORMANCE_THRESHOLDS['user_lookup'])
    ...
    async def test_assessment_query_performance(self, perf_metrics):
        ...
        # Check performance
        threshold_check = perf_metrics.check_threshold('assessment_list', PERFORMANCE_THRESHOLDS['assessment_list'])
        assert threshold_check['passed'], (
    ...
    async def test_complex_join_query_performance(self, perf_metrics):
        ...
        threshold_check = perf_metrics.check_threshold('complex_join', 300)  # 300ms for complex query
        assert threshold_check['passed'], (
...
class TestAPIPerformance:
    ...
    def test_health_endpoint_performance(self, test_client, perf_metrics):
        ...
        # Check performance
        threshold_check = perf_metrics.check_threshold('health_check', 50)  # Health check should be fast
        assert threshold_check['passed'], (
    ...
    def test_api_docs_performance(self, test_client, perf_metrics):
        ...
        threshold_check = perf_metrics.check_threshold('api_docs', 1000)  # 1 second for docs
        assert threshold_check['passed'], (
...
class TestConnectionPoolPerformance:
    ...
    async def test_concurrent_connection_performance(self, perf_metrics):
        ...
        async def run_query():
            async with measure_database_operation(perf_metrics, 'concurrent_query') as conn:
...
class TestPerformanceRegression:
    ...
    async def test_no_performance_regression(self, perf_metrics):
        ...
        async with measure_database_operation(perf_metrics, 'assessment_count_query') as conn:
        ...
        assert all_checks_passed, f"Performance regressions detected: {failed_operations}"
...
class TestPerformanceTrends:
    ...
    async def test_performance_trend_analysis(self, perf_metrics):
        ...
        for i in range(5):
            async with measure_database_operation(perf_metrics, 'trend_test_query') as conn:
                await conn.execute(text("SELECT COUNT(*) FROM users"))
        ...
        measurements = [m['duration_ms'] for m in perf_metrics.measurements if m['name'] == 'trend_test_query']

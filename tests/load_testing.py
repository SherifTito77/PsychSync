...
class LoadTestRunner:
    ...
    async def __aenter__(self):
        ...
        timeout = aiohttp.ClientTimeout(
            total=self.config.timeout, connect=5, sock_read=self.config.timeout
        )
        ...
        for i in range(50):  # Create 50 test users
            ...
            try:
                async with self.session.post(
                    f"{self.config.base_url}/api/v1/auth/register", json=user_data
                ) as response:
                    if response.status == 201:
                        ...
                        async with self.session.post(
                            f"{self.config.base_url}/api/v1/auth/token", data=login_data
                        ) as token_response:
        ...
        self.test_data["assessment_templates"] = [
            {
                ...
                "content": {
                    ...
                    "questions": [
                        ...
                        {
                            "id": 3,
                            "text": "I see myself as dependable",
                            "type": "scale",
                        },
                        {"id": 4, "text": "I see myself as anxious", "type": "scale"},
                        {
                            "id": 5,
                            "text": "I see myself as open to new experiences",
                            "type": "scale",
                        },
                    ],
                },
    ...
    async def run_load_test(self, test_name: str) -> LoadTestResults:
        ...
        for i in range(self.config.concurrent_users):
            ...
            task = asyncio.create_task(self.user_simulation(i, delay, total_weight))
            tasks.append(task)
    ...
    async def user_simulation(
        self, user_id: int, start_delay: float, total_weight: int
    ):
    ...
    def generate_results(
        ...
        throughput_mbps = (
            (total_bytes * 8) / (test_duration * 1024 * 1024)
            if test_duration > 0
            else 0
        )
...
class TestLoadTesting:
    ...
    async def test_baseline_performance(self):
        ...
        async with LoadTestRunner(config) as runner:
            ...
            ), f"Error rate too high: {results.error_rate}%"
            assert (
                results.avg_response_time < 1.0
            ), f"Average response time too high: {results.avg_response_time}s"
            assert (
                results.p95_response_time < 2.0
            ), f"P95 response time too high: {results.p95_response_time}s"
            assert (
                results.requests_per_second > 40
            ), f"RPS too low: {results.requests_per_second}"
    ...
    async def test_stress_performance(self):
        ...
        async with LoadTestRunner(config) as runner:
            ...
            assert (
                results.error_rate < 20.0
            ), f"Error rate too high under stress: {results.error_rate}%"
            assert (
                results.avg_response_time < 5.0
            ), f"Average response time too high under stress: {results.avg_response_time}s"
            assert (
                results.requests_per_second > 200
            ), f"RPS too low under stress: {results.requests_per_second}"
    ...
    async def test_api_endpoints_performance(self):
        ...
        async with LoadTestRunner(config) as runner:
            ...
            for endpoint, stats in results.endpoints_stats.items():
                ...
                ), f"High error rate for {endpoint}: {stats['error_rate']}%"
                assert (
                    stats["avg_response_time"] < 2.0
                ), f"Slow response time for {endpoint}: {stats['avg_response_time']}s"
...
async def run_load_test_scenario(scenario_name: str):
    ...
    print(f"{'='*60}")
    print(
        f"Test Duration: {(results.end_time - results.start_time).total_seconds():.1f}s"
    )

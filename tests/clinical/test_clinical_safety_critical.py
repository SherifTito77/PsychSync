...
class TestPHQ9Safety:
    ...
    def test_phq9_score_calculation_accuracy(self, screening_service, valid_phq9_responses):
        ...
        assert result['risk_level'] == RiskLevel.MILD.value, \
    ...
    def test_phq9_severe_risk_detection(self, screening_service):
        ...
        severe_responses = {
            "assessment_type": "phq9",
            "responses": {f"phq9_{i}": 3 for i in range(1, 10)},  # All "Nearly every day"
        ...
        assert result['risk_level'] == RiskLevel.SEVERE.value, \
    ...
    def test_suicide_risk_detection(self, screening_service):
        ...
        for response_value, description, expected_risk, requires_attention in suicide_risk_cases:
            responses = {
                "assessment_type": "phq9",
                "responses": {
                    f"phq9_{i}": 0 if i != 9 else response_value
                    for i in range(1, 10)
                },
                "consent_given": True
            }
            ...
            if response_value >= 2:
                assert result['suicide_risk_detected'] is True, \
                    f"Suicide risk not detected for response '{description}'"
                assert result['requires_crisis_intervention'] is True, \
    ...
    def test_incomplete_assessment_handling(self, screening_service):
        ...
        for responses in incomplete_cases:
            ...
            assert result.get('valid', True) is False or 'error' in result, \
                f"Invalid responses {responses} should be rejected or flagged"

    # 🔴 CRITICAL: Time-based Safety Checks
    def test_assessment_timing_validation(self, screening_service, valid_phq9_responses):
        ...
        for completion_time, should_be_valid in timing_cases:
            ...
            if should_be_valid:
                assert result.get('timing_valid', True), \
                    f"Time {completion_time}s should be valid"
            else:
                assert result.get('timing_valid', True) is False, \
                    f"Time {completion_time}s should be flagged as invalid"

    def test_concurrent_assessment_prevention(self, screening_service, valid_phq9_responses):
        ...
        with patch.object(screening_service, '_get_recent_assessment',
                         return_value=recent_assessment):

            result = screening_service.process_assessment(valid_phq9_responses)

            assert result['duplicate_assessment'] is True, \
                "Should prevent concurrent assessments within 24-hour window"
            assert 'wait_until' in result, \
                "Should provide next assessment time"
...
class TestGAD7Safety:
    ...
    def test_gad7_score_calculation_accuracy(self, screening_service, valid_gad7_responses):
    ...
    def test_gad7_anxiety_level_classification(self, screening_service):
        ...
        for score, expected_risk in boundary_cases:
            responses = {
                "assessment_type": "gad7",
                "responses": {
                    f"gad7_{i}": min(3, max(0, score // 7 + (1 if i <= score % 7 else 0)))
                    for i in range(1, 8)
                },
                "consent_given": True
            }
            ...
            assert RiskLevel(result['risk_level']) in [expected_risk, RiskLevel.MILD], \
...
class TestCrisisDetectionAndIntervention:
    ...
    def test_crisis_intervention_resources(self, crisis_service):
        ...
        assert 'emergency_contacts' in crisis_result, \
            "Should provide emergency contacts"
        assert 'hotlines' in crisis_result, \
            "Should provide crisis hotlines"
        assert 'local_resources' in crisis_result, \
            "Should provide local resources"

        # Verify critical resources are included
        emergency_contacts = crisis_result['emergency_contacts']
        assert any('911' in str(contact) for contact in emergency_contacts), \
...
class TestDataPrivacyAndCompliance:
    ...
    def test_phi_data_encryption(self):
        ...
        assert 'assessment_results' in phi_data, \
            "PHI should contain assessment results"
    ...
    def test_data_retention_policy(self):
        ...
        assert old_date < datetime.utcnow(), \
...
class TestClinicalSystemIntegration:
    ...
    def test_assessment_result_consistency(self):
        ...
        assert len(set(results)) == 1, \
            "Assessment calculations should be deterministic"
...
if __name__ == "__main__":
    pytest.main([__file__, "-v", "--tb=short", "-x"])  # Stop on first failure for safety
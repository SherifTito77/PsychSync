...
class MockBillingService:
    ...
    def create_payment_intent(payment_data):
        ...
        raise stripe.error.CardError(
            "Card declined for testing", param="amount", code="card_declined"
        )
    ...
    def analyze_failure_patterns(failure_attempts):
        ...
        return {
            ...
            "recommendation": (
                "temporary_block" if len(failure_attempts) >= 5 else "monitor"
            ),
        }
    ...
    def detect_payment_amount_anomaly(amount, user_history):
        ...
        return {
            ...
            "recommendation": (
                "additional_verification" if deviation_factor > 5.0 else "proceed"
            ),
        }
    ...
    def handle_payment_failure_notifications(payment_failure):
        ...
        return {"email_sent": True, "push_sent": True, "in_app_created": True}
    ...
    def generate_payment_failure_analytics(failure_events):
        ...
        return {
            ...
            "average_failure_amount": sum(e["amount"] for e in failure_events)
            / len(failure_events),
    ...
    def calculate_payment_failure_dashboard_metrics(dashboard_data, time_window):
        ...
        return {
            ...
            "failure_rate": (
                dashboard_data["failed_payments"] / total_payments
                if total_payments > 0
                else 0
            ),
            "recovery_rate": (
                dashboard_data["revenue_recovered"] / dashboard_data["revenue_lost"]
                if dashboard_data["revenue_lost"] > 0
                else 0
            ),
            "net_revenue_impact": dashboard_data["revenue_lost"]
            - dashboard_data["revenue_recovered"],
    ...
    async def get_user_payment_state(user_id):
        ...
        return {"pending_attempts": 0, "failure_count": 1}
...
class TestFailedPaymentBehavior:
    ...
    def test_connection_timeout_during_payment(self):
        ...
        mock_timeout_error = stripe.error.APIConnectionError(
            message="Request timed out.", should_retry=True
        )

        with patch("stripe.PaymentIntent.create", side_effect=mock_timeout_error):
            result = BillingService.process_payment_with_retry(
                payment_data, max_retries=3, retry_delay=0.1  # Fast retry for testing
            )
    ...
    def test_smart_retry_logic_for_transient_failures(self):
        ...
        test_scenarios = [
            {
                ...
                "error": stripe.error.APIConnectionError(
                    "Network timeout", should_retry=True
                ),
                "should_retry": True,
                "expected_retries": 3,
            },
            ...
            },
            {
                ...
                "error": stripe.error.InvalidRequestError(
                    "Invalid card number", param="number"
                ),
                "should_retry": False,
                "expected_retries": 0,
            },
        ...
        for scenario in test_scenarios:
            with patch("stripe.PaymentIntent.create", side_effect=scenario["error"]):
                ...
                result = BillingService.process_payment_with_retry(
                    payment_data, max_retries=3, retry_delay=0.01
                )
        ...
        def mock_payment_create(*args, **kwargs):
            ...
            if len(retry_intervals) == 1:
                ...
                raise stripe.error.APIConnectionError(
                    "Final timeout", should_retry=True
                )
    ...
    def test_payment_method_blacklist_handling(self):
        ...
        with patch("stripe.PaymentIntent.create") as mock_create:
            mock_create.side_effect = stripe.error.InvalidRequestError(
                "Payment method is blacklisted", param="payment_method"
            )
    ...
    def test_unusual_payment_amount_detection(self):
        ...
        anomaly_score = BillingService.detect_payment_amount_anomaly(
            suspicious_payment["amount"], user_history
        )
    ...
    def test_user_friendly_error_messages(self):
        ...
        for scenario in error_scenarios:
            error_result = BillingService.generate_user_friendly_error(
                scenario["error_type"]
            )
            ...
            for suggestion in scenario["expected_suggestions"]:
                assert any(
                    suggestion.lower() in s.lower() for s in error_result["suggestions"]
                )
    ...
    def test_payment_failure_notification_system(self):
        ...
        with patch(
            "app.services.notification_service.send_payment_failure_email"
        ) as mock_email:
            with patch(
                "app.services.notification_service.send_push_notification"
            ) as mock_push:
                with patch(
                    "app.services.notification_service.create_in_app_notification"
                ) as mock_app:

                    notifications_sent = (
                        BillingService.handle_payment_failure_notifications(
                            payment_failure
                        )
                    )
    ...
    def test_subscription_management_after_failure(self):
        ...
        grace_period_result = BillingService.handle_subscription_payment_failure(
            subscription_data, failure_type="card_declined"
        )
    ...
    def test_payment_failure_analytics_tracking(self):
        ...
        analytics_report = BillingService.generate_payment_failure_analytics(
            failure_events
        )
    ...
    def test_payment_failure_dashboard_metrics(self):
        ...
        assert (
            metrics["net_revenue_impact"]
            == dashboard_data["revenue_lost"] - dashboard_data["revenue_recovered"]
        )
    ...
    def test_payment_failure_during_plan_upgrade(self):
        ...
        with patch(
            ...
            assert (
                result["current_plan"] == "professional"
    ...
    def test_webhook_failure_during_payment_events(self):
        ...
        with patch(
            "app.services.webhook_service.send_payment_failure_webhook"
        ) as mock_webhook:
            mock_webhook.side_effect = Exception("Webhook delivery failed")

            with patch(
                "app.services.notification_service.create_fallback_notification"
            ) as mock_fallback:

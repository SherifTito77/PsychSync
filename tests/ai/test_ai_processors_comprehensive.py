...
class TestMBTIProcessor:
    ...
    def test_dimension_distribution(self, processor):
        ...
        extraverted_dims = [dims['extraversion'] for t, dims in dimensions_by_type.items() if t[0] == 'E']
        introverted_dims = [dims['extraversion'] for t, dims in dimensions_by_type.items() if t[0] == 'I']

        assert all(e > 0.5 for e in extraverted_dims), "All extraverted types should have high extraversion"
        assert all(i < 0.5 for i in introverted_dims), "All introverted types should have low extraversion"

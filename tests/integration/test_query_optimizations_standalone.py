    ...
    test_queries = [
        {
            ...
            """,
            "expected_patterns": ["Index Scan", "Seq Scan"],  # Accept either - PG planner decides
    ...
    with engine.connect() as conn:
        for query in test_queries:
            ...
            try:
                ...
                found_patterns = [pattern for pattern in query["expected_patterns"]
                ...
                if found_patterns:
                    ...
                    print(f"    ⚠️  Expected patterns not found: {query['expected_patterns']}")
                    print(f"    Output: {explain_output[:200]}...")
                    failed += 1

                # Show execution time
                if "Execution Time:" in explain_output:
                    time_str = explain_output.split("Execution Time:")[-1].strip().split()[0]

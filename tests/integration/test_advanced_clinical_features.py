...
class TestLSASAssessment:
    ...
    async def test_lsas_submit_complete_assessment(self, client: AsyncClient, auth_headers):
...
class TestEAT26Assessment:
    ...
    async def test_eat26_above_threshold(self, client: AsyncClient, auth_headers):
        ...
        item_responses = {i: 3 if i <= 7 else 0 for i in range(1, 27)}  # 7 items × 3 = 21
...
class TestYBOCSAssessment:
    ...
    async def test_ybocs_subclinical(self, client: AsyncClient, auth_headers):
        ...
        response = await client.post(
            f"{BASE_URL}/api/v1/screening/ybocs",
            json=responses,
            headers=auth_headers
        )
...
class TestTelehealthService:
    ...
    async def test_schedule_consultation(self, client: AsyncClient, auth_headers, test_user, test_clinician, test_db: AsyncSession):
    ...
    async def test_check_availability(self, client: AsyncClient, auth_headers, test_clinician, test_db: AsyncSession):
    ...
    async def test_get_upcoming_sessions(self, client: AsyncClient, auth_headers):
        ...
        response = await client.get(
            f"{BASE_URL}/api/v1/telehealth/upcoming?role=patient",
            headers=auth_headers
        )
...
class TestAIChatbot:
    ...
    async def test_multiple_crisis_keywords(self, client: AsyncClient, auth_headers):
        ...
        for phrase in crisis_phrases:
            ...
            assert response.status_code == 200
            data = response.json()
            assert data['crisis_detected'] == True, f"Failed to detect crisis in: {phrase}"
...
class TestClinicalAnalytics:
    ...
    async def test_user_trends(self, client: AsyncClient, auth_headers, test_user, test_db: AsyncSession):
...
class TestEndToEndWorkflow:
    ...
    async def test_complete_assessment_workflow(self, client: AsyncClient, auth_headers, test_user, test_db: AsyncSession):
...
async def test_cleanup(test_db: AsyncSession):
    ...
    await test_db.execute(ClinicalScreening.__table__.delete().where(ClinicalScreening.screening_type == 'LSAS'))
    await test_db.commit()

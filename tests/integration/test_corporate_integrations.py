...
class TestEmailIntegration:
        ...
        return {
            ...
            'payload': {
                'headers': [
                    ...
                    {'name': 'Subject', 'value': 'URGENT: Project deadline approaching'},
    ...
    def test_calculate_behavioral_signals(self, email_extractor):
        ...
        signals = email_extractor.calculate_behavioral_signals(emails, time_window_days=30)
...
class TestCalendarIntegration:
    ...
    def test_calculate_behavioral_signals(self, calendar_extractor):
        ...
        events = [
            CalendarEvent(
                event_id=f'event_{i}',
                title='Meeting',
                start_time=datetime.now().replace(hour=10, minute=0) + timedelta(days=i),
        ...
        signals = calendar_extractor.calculate_behavioral_signals(events, time_window_days=30)
    ...
    def test_meeting_classification(self, calendar_extractor):
        ...
        for title, attendees, duration, expected_type in test_cases:
            meeting_type = calendar_extractor._classify_meeting_type(title, attendees, duration)
...
class TestSlackIntegration:
    ...
    def test_calculate_behavioral_signals(self, slack_extractor):
        ...
        signals = slack_extractor.calculate_behavioral_signals(messages, time_window_days=30)
    ...
    def test_detect_burnout_indicators(self, slack_extractor):
        ...
        assert len(indicators) > 0
        assert any('after-hours' in ind.lower() or 'slack' in ind.lower() for ind in indicators)
...
class TestBehavioralPipeline:
    ...
    async def test_generate_behavioral_profile(self, pipeline_orchestrator):
        ...
        # Mock API responses
        with patch('app.integrations.email_integration.GmailAPIIntegration.fetch_recent_emails') as mock_emails:
            mock_emails.return_value = []

            with patch('app.integrations.calendar_integration.GoogleCalendarAPIIntegration.fetch_events') as mock_events:
                mock_events.return_value = []

                with patch('app.integrations.slack_integration.SlackAPIIntegration.fetch_all_messages') as mock_messages:
    ...
    def test_severity_determination(self, pipeline_orchestrator):
        ...
        test_cases = [
            ...
            (['indicator1', 'indicator2', 'indicator3', 'indicator4', 'indicator5'], InsightSeverity.CRITICAL)
        ]

        for indicators, expected_severity in test_cases:
            severity = pipeline_orchestrator._determine_severity(indicators, critical_threshold=4)
            assert severity == expected_severity, f"Failed for {len(indicators)} indicators"

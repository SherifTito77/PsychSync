...
class LoggingMonitoringSecurityTester:
        ...
        try:
            ...
            if not security_file.exists():
                ...
                return TestResult(
                    ...
                    findings=findings,
                    recommendations=[
                        "Implement security.py with failed login tracking"
                    ],
                    details={},
                )
            ...
            if "log_login_attempt" in content:
                ...
                recommendations.append(
                    "Implement log_login_attempt function to track failed logins"
                )
            ...
            if "failed_login" in content.lower() or "login_attempts" in content.lower():
                ...
                if (
                    "increment_login_attempts" in content
                    or "login_attempts +=" in content
                ):
            ...
            if (
                ...
                recommendations.append(
                    "Add notification system for failed login alerts"
                )
                score += 5
            ...
            if audit_file.exists():
                ...
                if "AUTHENTICATION_FAILED" in audit_content:
                    ...
                    findings.append(
                        "⚠️  AuditLogger may not track authentication failures"
                    )
                    score += 10
        ...
        try:
            ...
            if audit_file.exists():
                ...
                if "class AuditAction" in content:
                    ...
                    if missing_actions:
                        findings.append(
                            f"⚠️  Missing audit actions: {', '.join(missing_actions[:5])}"
                        )
                        recommendations.append(
                            "Add missing audit actions to AuditAction enum"
                        )
                        score -= 10
                    else:
                    ...
                    recommendations.append(
                        "Implement AuditAction enum with all critical operations"
                    )
                    score -= 20
            ...
            if endpoints_dir.exists():
                ...
                for endpoint_file in endpoints_dir.glob("*.py"):
                    try:
                        ...
                        if (
                            "audit_logger" in content
                            or "AuditLogger" in content
                            or "log_event" in content
                        ):
                            has_audit = True
                ...
                if endpoints_with_audit:
                    findings.append(
                        f"✅ {len(endpoints_with_audit)} endpoints use audit logging"
                    )
                    score += min(20, len(endpoints_with_audit) * 2)

                if endpoints_without_audit:
                    findings.append(
                        f"⚠️  {len(endpoints_without_audit)} endpoints missing audit logs"
                    )
                    for ep in endpoints_without_audit[:3]:
                        findings.append(f"   - {ep}")
                    recommendations.append(
                        "Add audit logging to all security-critical endpoints"
                    )
        ...
        try:
            ...
            if logging_config.exists():
                ...
                if "log_sanitizer" in content or "SensitiveDataFilter" in content:
                    findings.append(
                        "✅ Log sanitization implemented (prevents log injection)"
                    )
                    score += 25
                else:
                ...
                if "structured" in content.lower() or "json" in content.lower():
                    ...
                    recommendations.append(
                        "Implement structured logging for better tamper resistance"
                    )
                    score += 10
            ...
            if has_external_logging:
                ...
                recommendations.append(
                    "Configure external log aggregation (Datadog, Sentry, etc.)"
                )
                score += 10
            ...
            if log_dir.exists():
                ...
                if secure_logs:
                    findings.append(
                        f"✅ {len(secure_logs)} log files have secure permissions (600)"
                    )
                    score += min(20, len(secure_logs) * 5)

                if insecure_logs:
                    findings.append(
                        f"⚠️  {len(insecure_logs)} log files have insecure permissions"
                    )
                    recommendations.append(
                        "Set log files to 600 (owner read/write only)"
                    )
                    score -= 10

            # Check for hash-based log integrity
            if (
                "checksum" in logging_config.read_text().lower()
                or "hash" in logging_config.read_text().lower()
            ):
                ...
                recommendations.append(
                    "Implement log file hash-based integrity verification"
                )
                score += 10
    ...
    def test_abnormal_activity_alerting(self) -> TestResult:
        ...
        try:
            ...
            sec_monitor_file = (
                self.project_root / "app" / "core" / "security_monitoring.py"
            )
            ...
            if "class SecurityMonitoringEngine" in content:
                ...
                if methods_found:
                    findings.append(
                        f"✅ Found {len(methods_found)} anomaly detection methods"
                    )
                ...
                if "AlertSeverity" in content:
                    findings.append(
                        "✅ Alert severity levels defined (LOW/MEDIUM/HIGH/CRITICAL)"
                    )
                    score += 10
            ...
            if monitoring_configured:
                findings.append(
                    f"✅ External monitoring configured: {', '.join(monitoring_configured)}"
                )
                score += 15
            else:
        ...
        try:
            ...
            if "CRITICAL" in content or "AlertSeverity.CRITICAL" in content:
                ...
                if "CRITICAL" in content and "lock" in content.lower():
                    ...
                    findings.append(
                        "⚠️  CRITICAL alerts may not trigger immediate action"
                    )
                    recommendations.append(
                        "Implement automatic account lock on CRITICAL alerts"
                    )
                    score += 10
            else:
                findings.append("❌ CRITICAL severity level not defined")
                recommendations.append(
                    "Define CRITICAL severity for intrusion attempts"
                )
                score -= 20
            ...
            if "threshold" in content.lower() or "BRUTE_FORCE_THRESHOLD" in content:
                ...
                recommendations.append(
                    "Configure escalation thresholds for automatic alerts"
                )
                score += 10
            ...
            if alerts_found:
                findings.append(
                    f"✅ Found {len(alerts_found)} intrusion detection alerts"
                )
                score += min(15, len(alerts_found) * 3)
            else:
                findings.append("⚠️  No specific intrusion alerts found")
                recommendations.append(
                    "Add detection for account takeover, privilege escalation, data exfiltration"
                )
                score += 5

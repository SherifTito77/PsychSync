...
class LogSecurityTester:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.log_directories = config.get(
            "log_directories",
            [
                "./logs",
                "/var/log",
    ...
    async def scan_directory_for_logs(
        self, directory: str, patterns: Dict[SensitiveDataType, List[str]]
    ):
    ...
    async def analyze_log_file(
        self, file_path: str, patterns: Dict[SensitiveDataType, List[str]]
    ):
        ...
        try:
            ...
            if file_size > self.max_file_size:
                self.logger.warning(
                    f"⚠️  Skipping large file {file_path} ({file_size} bytes)"
                )
                return
    ...
    def mask_sensitive_data(self, data: str, sensitive_type: SensitiveDataType) -> str:
        ...
        if sensitive_type in [SensitiveDataType.CREDENTIALS, SensitiveDataType.TOKENS]:
            ...
            # Default masking
            return (
                data[:5] + "*" * (len(data) - 10) + data[-5:]
                if len(data) > 10
                else "*" * len(data)
            )

    def determine_severity(
        self, sensitive_type: SensitiveDataType, matched_data: str
    ) -> str:
        """Determine severity based on type and content"""
        if sensitive_type in [SensitiveDataType.CREDENTIALS, SensitiveDataType.TOKENS]:
            if any(
                key in matched_data.lower()
                for key in ["password", "secret", "private", "key"]
            ):
                return "CRITICAL"
            return "HIGH"
            ...
            if any(
                cmd in matched_data.lower()
                for cmd in ["drop", "delete", "<script>", ";"]
            ):
                return "HIGH"
            return "MEDIUM"
        else:
    ...
    def get_recommendation(self, sensitive_type: SensitiveDataType) -> str:
        ...
        return recommendations.get(
            sensitive_type, "Review and sanitize sensitive data in logs"
        )
        ...
        for finding in self.findings:
            try:
                ...
                if mode[2] in ["4", "5", "6", "7"]:  # Others have read permission
                    ...
                    self.findings.append(finding)
                    self.logger.warning(
                        f"⚠️  Insecure permissions for {finding.log_file}: {mode}"
                    )
                ...
                if mode[2] in ["2", "3", "6", "7"]:  # Others have write permission
                    ...
                    self.findings.append(finding)
                    self.logger.warning(
                        f"🚨 World-writable log file: {finding.log_file}: {mode}"
                    )

            except Exception as e:
                self.logger.debug(
                    f"Error checking permissions for {finding.log_file}: {str(e)}"
                )
                        ...
                        async with session.post(
                            ...
                            if response.status in [
                                ...
                                if await self.check_logs_for_injection(payload):
                                    ...
                                    self.findings.append(finding)
                                    self.logger.warning(
                                        f"⚠️  Log injection detected: {repr(payload)}"
                                    )
                        ...
                        if any(
                            ...
                            try:
                                ...
                                if file_age < cutoff_date:
                                    ...
                                    self.findings.append(finding)
                                    self.logger.warning(
                                        f"⚠️  Old log file: {file_path} ({(datetime.now() - file_age).days} days)"
                                    )
                        ...
                        if any(
                            ...
                            try:
                                ...
                                if content:
                                    ...
                                    await self.check_log_integrity_issues(
                                        file_path, content
                                    )
                            except Exception as e:
                                finding = LogFinding(
    ...
    async def check_log_integrity_issues(self, file_path: str, content: str):
        ...
        for i, line in enumerate(lines, 1):
            if (
                line and len(line) > 1000
            ):  # Unusually long line might indicate truncation
                finding = LogFinding(
                    log_file=file_path,
                    line_number=i,
                    sensitive_type=SensitiveDataType.SYSTEM,
        ...
        for line in lines:
            if line.strip():
                has_timestamp = any(
                    re.search(pattern, line) for pattern in timestamp_patterns
                )
    ...
    async def test_log_monitoring(self):
        ...
        try:
            ...
            if not monitoring_found:
                ...
                self.logger.info(
                    f"✅ Log monitoring tools found: {', '.join(monitoring_found)}"
                )
    ...
    async def generate_report(self) -> Dict[str, Any]:
        ...
        for finding in self.findings:
            ...
            sensitive_type = finding.sensitive_type.value
            findings_by_type[sensitive_type] = (
                findings_by_type.get(sensitive_type, 0) + 1
            )
        ...
        # Save report
        report_file = (
            f"log_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )

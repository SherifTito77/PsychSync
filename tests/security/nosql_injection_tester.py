...
class NoSQLInjectionTester:
        ...
        payloads.extend(
            [
                ...
                InjectionPayload(
                    ...
                    payload={
                        "$or": [{"username": "admin"}, {"password": {"$ne": None}}]
                    },
                    injection_type=InjectionType.OPERATOR_INJECTION,
    ...
    async def test_api_endpoints(self):
        ...
        endpoints = [
            ...
            },
            {
                "method": "POST",
                "path": "/api/v1/users",
                "body_fields": ["filter", "query", "search"],
            },
            {
                "method": "PUT",
                "path": "/api/v1/users/{id}",
                "body_fields": ["filter", "query"],
            },
            {
                "method": "GET",
                "path": "/api/v1/analytics",
                "params": ["filter", "group_by"],
            },
        ]
    ...
    async def test_endpoint(self, session: aiohttp.ClientSession, endpoint: Dict):
        ...
        for payload in critical_payloads:
            try:
                if method == "GET":
                    await self.test_get_endpoint(
                        session, path, payload, endpoint.get("params", [])
                    )
                elif method == "POST":
                    await self.test_post_endpoint(
                        session, path, payload, endpoint.get("body_fields", [])
                    )
                elif method == "PUT":
                    await self.test_put_endpoint(
                        session, path, payload, endpoint.get("body_fields", [])
                    )
            ...
            try:
                ...
                async with session.get(
                    ...
                    await self.analyze_response(
                        response, payload, "GET", path, param, end_time - start_time
                    )
    ...
    async def test_post_endpoint(
        ...
        for field in body_fields:
            try:
                ...
                start_time = time.time()
                async with session.post(
                    test_url, json=test_data, timeout=aiohttp.ClientTimeout(total=10)
                ) as response:
                    end_time = time.time()
                    await self.analyze_response(
                        response, payload, "POST", path, field, end_time - start_time
                    )
        ...
        for field in body_fields:
            try:
                ...
                async with session.put(
                    ...
                    end_time = time.time()
                    await self.analyze_response(
                        response, payload, "PUT", path, field, end_time - start_time
                    )
    ...
    async def analyze_response(
        ...
        success_indicators = [
            ...
            isinstance(data, str)
            and "error" in data.lower()
            and "injection" not in data.lower(),
        ]
...
async def main():
    ...
    try:
        ...
        report_file = (
            f"nosql_injection_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        critical_vulns = [
            v for v in tester.vulnerabilities if v.get("severity") == "CRITICAL"
        ]
        if critical_vulns:

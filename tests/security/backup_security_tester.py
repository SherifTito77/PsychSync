...
class BackupSecurityTester:
    def __init__(self, config: Dict[str, Any]):
        ...
        self.backup_directories = config.get(
            "backup_directories",
            [
                "./backups",
                "/var/backups",
    ...
    async def analyze_backup_file(self, file_path: str):
        ...
        try:
            ...
            if not encryption_result["is_encrypted"]:
                ...
                self.logger.info(
                    f"✅ Encrypted backup detected: {file_path} ({encryption_result['encryption_type']})"
                )
            ...
            for pattern_name, pattern in sensitive_patterns.items():
                ...
                if match_count > 0:
                    ...
                    self.logger.critical(
                        f"🚨 Sensitive data in {file_path}: {pattern_name} ({match_count} instances)"
                    )

        except Exception as e:
            self.logger.error(
                f"Error scanning for sensitive data in {file_path}: {str(e)}"
            )
                ...
                for file_info in zip_file.filelist[:5]:  # Check first 5 files
                    if not file_info.is_dir():
                        with zip_file.open(file_info) as file:
                            content += file.read(1024 * 512).decode(
                                "utf-8", errors="ignore"
                            )
                ...
                for bucket in buckets["Buckets"]:
                    ...
                    try:
                        ...
                        if "ServerSideEncryptionConfigurationNotFoundError" in str(e):
                            ...
                            self.findings.append(finding)
                            self.logger.warning(
                                f"⚠️  S3 bucket {bucket_name} not encrypted"
                            )
                    ...
                    try:
                        ...
                        if (
                            '"Effect": "Allow"' in policy_str
                            and '"Principal": "*"' in policy_str
                        ):
                            finding = BackupFinding(
                                backup_path=f"s3://{bucket_name}",
                                backup_type=BackupType.CLOUD_BACKUP,
    ...
    async def test_azure_backup_security(self):
        ...
        self.logger.info(
            "ℹ️  Azure backup security testing requires Azure SDK and credentials"
        )
    ...
    async def test_gcs_backup_security(self):
        ...
        self.logger.info(
            "ℹ️  GCS backup security testing requires Google Cloud SDK and credentials"
        )
    ...
    async def test_backup_retention(self):
        ...
        if old_backups:
            ...
            self.findings.append(finding)
            self.logger.warning(
                f"⚠️  Found {len(old_backups)} backup files older than 1 year"
            )
        ...
        for finding in self.findings:
            if finding.backup_type in [BackupType.SQL_DUMP, BackupType.MONGODB_DUMP]:
                try:
                    ...
                    self.logger.error(
                        f"Error testing integrity for {finding.backup_path}: {str(e)}"
                    )
    ...
    async def test_mongodb_dump_integrity(self, file_path: str):
        ...
        self.logger.info(
            f"ℹ️  MongoDB dump integrity testing requires BSON library for {file_path}"
        )
    ...
    async def generate_report(self) -> Dict[str, Any]:
        ...
        for finding in self.findings:
            finding_dict = {
                ...
                "created_date": (
                    finding.created_date.isoformat() if finding.created_date else None
                ),
            }
            ...
            "encryption_status": {
                "encrypted": len(
                    [
                        ...
                        if f.encryption_detected != EncryptionStandard.UNKNOWN
                    ]
                ),
                "unencrypted": len(
                    [f for f in self.findings if f.issue_type == "Unencrypted Backup"]
                ),
            },
        }
        ...
        # Save report
        report_file = (
            f"backup_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
async def main():
    ...
    config = {
        ...
        "cloud_config": {
            "aws": {"check_s3": True, "regions": ["us-east-1", "us-west-2"]}
        },
    }

            ...
            for env_file in env_files:
                if Path(env_file).exists():
                    ...
                    jwt_lines = [
                        ...
                        if "SECRET_KEY=" in line or "JWT_" in line
                    ]
                    ...
                    if "password123" in content or "admin" in content.lower():
                        security_issues.append(
                            f"Default credentials found in {env_file}"
                        )
                        security_score -= 25
            ...
            async with self.session.get(f"{self.backend_url}/health") as response:
                ...
                if response.status >= 400:
                    try:
                        ...
                        if (
                            "stack" in str(error_data).lower()
                            or "traceback" in str(error_data).lower()
                        ):
                            vulnerabilities.append(
                                "API may disclose sensitive error information"
                            )
                            security_score -= 15
                    except Exception as e:
                        pass
    ...
    def analyze_input_validation(self):
        ...
        try:
                ...
                if tsx_file.exists():
                    try:
                        ...
                        if "dangerouslySetInnerHTML" in content:
                            vulnerabilities.append(
                                f"{tsx_file.name}: Uses dangerouslySetInnerHTML"
                            )
                            security_score -= 15
            ...
            if frontend_validation_issues > 0:
                recommendations.append(
                    f"Add input validation to {frontend_validation_issues} frontend forms"
                )

            details = (
                f"Analyzed {total_files} backend files, {len(tsx_files)} frontend files"
            )
            details += (
                f" | Issues found: {len(vulnerabilities)}, Score: {security_score}/100"
            )
    ...
    def analyze_dependency_security(self):
        ...
        try:
                ...
                if Path(req_file).exists():
                    ...
                    for package in packages:
                        if package.lower() in vulnerable_packages:
                            vulnerabilities.append(
                                f"Potentially vulnerable package: {package}"
                            )
                            security_score -= 5
                ...
                try:
                    ...
                    for package in dependencies:
                        if package in vulnerable_npm_packages:
                            vulnerabilities.append(
                                f"Potentially vulnerable npm package: {package}"
                            )
                            security_score -= 5
            ...
            if not has_security_tools:
                recommendations.append(
                    "Install security scanning tools (safety, bandit, npm audit)"
                )
                security_score -= 15
        ...
        for result in self.results:
            if result.security_score is not None:
                status = (
                    ...
                    else "🟡" if result.security_score >= 60 else "🔴"
                )
    ...
    async def run_security_analysis(self):
        ...
        for analysis_func in analyses:
            ...
            if result.success:
                ...
                print(
                    f"✅ {result.test_name}: COMPLETED ({result.duration:.3f}s){score_display}"
                )

...
class BusinessLogicSecurityTester:
    ...
    def create_test_user(self, email: str, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                logger.warning(
                    f"Failed to create test user {email}: {response.status_code}"
                )
                return {
                    "success": False,
        ...
        for scenario in test_scenarios:
            try:
                ...
                if scenario["method"] == "GET":
                    ...
                    response = self.session.post(
                        endpoint_url, json=scenario.get("data", {})
                    )
    ...
    def test_deleted_account_access(self) -> None:
        ...
        if test_user["success"]:
            ...
            auth_result = self.authenticate_user(
                "deletetest@example.com", "password789"
            )
                ...
                try:
                    ...
                    for endpoint in test_endpoints:
                        try:
                            ...
                            vulnerability_found = access_response.status_code in [
                                200,
                                201,
                            ]
                            ...
                            logger.error(
                                f"Error testing deleted account access to {endpoint}: {e}"
                            )
    ...
    def test_team_limits_bypass(self) -> None:
        ...
        try:
            ...
            create_team_response = self.session.post(
                f"{self.base_url}/api/v1/teams", json=team_data
            )
                    ...
                    },
                    {
                        ...
                        "data": {
                            "assessments": [
                                ...
                                for i in range(
                                    50
                                )  # Try to create 50 assessments at once
                            ]
                        },
    ...
    def run_all_tests(self) -> None:
        ...
        logger.info(
            f"✅ Security testing completed. Found {len([r for r in self.results if r.vulnerability_found])} potential vulnerabilities"
        )
    ...
    def generate_report(self, save_to_file: bool = True) -> str:
        ...
        report.append(
            f"- **Overall Risk Level:** {'CRITICAL' if critical_vulns else 'HIGH' if high_vulns else 'MEDIUM' if vulnerabilities else 'LOW'}"
        )
        report.append("")
        ...
        for result in self.results:
            ...
            report.append(
                f"| {result.test_name} | {status} | {severity} | {description} |"
            )

...
class TestBillingService:
    ...
    def test_billing_calculation(self):
        ...
        monthly_total = self.billing_service.calculate_billing_amount(
            plan="pro", billing_period="monthly", team_members=3
        )
        ...
        annual_total = self.billing_service.calculate_billing_amount(
            plan="pro", billing_period="annual", team_members=3
        )
...
class TestBillingAPI:
    ...
    def setup_method(self):
        """Set up test data"""
        self.user_headers = {"Authorization": "Bearer test_token"}
    ...
    def test_create_payment_intent_endpoint(self, mock_create_intent):
        ...
        response = client.post(
            ...
            json={"amount_cents": 4900, "currency": "usd"},
            headers=self.user_headers,
        )
    ...
    def test_cancel_subscription_endpoint(self, mock_cancel_subscription):
        ...
        response = client.delete(
            "/api/v1/billing/subscriptions/sub_test_123456", headers=self.user_headers
        )
    ...
    def test_get_billing_history_endpoint(self, mock_get_history):
        ...
        # Test API call
        response = client.get("/api/v1/billing/history", headers=self.user_headers)
    ...
    def test_get_subscription_tiers_endpoint(self, mock_get_tiers):
        ...
        mock_get_tiers.return_value = {
            ...
            "pro": {"price": 49, "features": ["Advanced features"]},
        }
    ...
    def test_unauthorized_billing_access(self):
        ...
        response = client.post(
            "/api/v1/billing/customers", json={"email": "test@example.com"}
        )
        assert response.status_code == 401
    ...
    def test_subscription_validation(self, mock_create_subscription):
        ...
        response = client.post(
            "/api/v1/billing/subscriptions",
            json={"price_id": "", "payment_method_id": ""},
            headers=self.user_headers,
        )
        assert response.status_code == 422
    ...
    def test_usage_calculation_endpoint(self, mock_calculate_usage):
        ...
        # Test API call
        response = client.get("/api/v1/billing/usage", headers=self.user_headers)
...
class TestSubscriptionWorkflow:
    ...
    @patch("app.services.billing.BillingService.create_payment_intent")
    @patch("app.services.billing.BillingService.create_subscription")
    def test_complete_subscription_workflow(
        self, mock_create_sub, mock_create_intent, mock_create_customer
    ):
        ...
        customer = billing_service.create_customer(
            email="test@example.com", name="Test User", user_id=123
        )
        ...
        payment_intent = billing_service.create_payment_intent(
            amount_cents=4900, currency="usd"  # $49.00
        )
    ...
    def test_plan_upgrade_workflow(self):
        ...
        enterprise_monthly = billing_service.calculate_billing_amount(
    ...
    def test_usage_limit_enforcement(self):
        ...
        assert billing_service.check_usage_limit(
            "team_members", 5, free_limits["team_members"]
        )
        assert not billing_service.check_usage_limit(
            "team_members", 6, free_limits["team_members"]
        )
        ...
        assert billing_service.check_usage_limit(
            "team_members", 50, pro_limits["team_members"]
        )
        assert not billing_service.check_usage_limit(
            "team_members", 51, pro_limits["team_members"]
        )

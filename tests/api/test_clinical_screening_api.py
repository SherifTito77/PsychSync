...
class TestConsentFlow:
    ...
    def test_screening_without_consent_fails(
    ...
    def test_consent_creation_allows_screening(
        ...
        consent_response = client.post(
            ...
            json={"consent_type": "screening", "screening_types": ["PHQ9", "GAD7"]},
            headers=auth_headers,
        )
...
class TestPHQ9Endpoint:
    ...
    def test_phq9_low_risk_screening(
        self, client: TestClient, auth_headers: dict, test_user_consent: dict
    ):
    ...
    def test_phq9_with_suicide_ideation_creates_alert(
        self,
        client: TestClient,
        auth_headers: dict,
        db: Session,
        test_user_consent: dict,
    ):
        ...
        alerts = (
            ...
            .filter(ClinicalAlert.user_id == test_user_consent["user_id"])
            .all()
        )
...
class TestGAD7Endpoint:
    ...
    def test_gad7_moderate_anxiety(
        self, client: TestClient, auth_headers: dict, test_user_consent: dict
    ):
...
class TestCSSRSEndpoint:
    ...
    def test_cssrs_recent_attempt_critical_alert(
        self,
        client: TestClient,
        auth_headers: dict,
        db: Session,
...
class TestPSS10Endpoint:
    ...
    def test_pss10_severe_stress_triggers_crisis(
        self, client: TestClient, auth_headers: dict, test_user_consent: dict
    ):
...
class TestDatabaseIntegration:
    ...
    def test_screening_saves_to_database(
        self,
        client: TestClient,
        auth_headers: dict,
        db: Session,
        ...
        screening = (
            db.query(ClinicalScreening)
            .filter(ClinicalScreening.id == data["id"])
            .first()
        )
    ...
    def test_crisis_alert_creates_additional_records(
        self,
        client: TestClient,
        auth_headers: dict,
        db: Session,
...
class TestSecurityAndCompliance:
    ...
    def test_unauthenticated_request_rejected(self, client: TestClient):
        ...
        response = client.post("/api/v1/screening/phq9", json={"q1_interest": 1})
    ...
    def test_phi_not_in_url_parameters(
        self, client: TestClient, auth_headers: dict, test_user_consent: dict
    ):
...
def test_user_consent(test_user, test_db):
    ...
    return {"user_id": test_user.id, "consent_id": consent.id}

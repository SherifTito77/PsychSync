...
class TestTimezoneEdgeCases:
    ...
    def test_utc_timezone_conversion(self):
        ...
        utc_time = datetime.datetime(2024, 1, 15, 14, 30, 0, tzinfo=datetime.timezone.utc)
        ...
        for timezone_str, expected in timezone_tests:
            ...
            assert converted_time.isoformat().startswith(expected[:16]), \
    ...
    def test_daylight_saving_time_transitions(self):
        ...
        after_spring = datetime.datetime(2024, 3, 10, 3, 0, 1)     # After DST (skips 2:00 AM)

        eastern_tz = ZoneInfo('America/New_York')

        before_dst = spring_forward.replace(tzinfo=datetime.timezone.utc).astimezone(eastern_tz)
        after_dst = after_spring.replace(tzinfo=datetime.timezone.utc).astimezone(eastern_tz)
    ...
    def test_assessment_completion_time_logging(self):
        ...
        test_scenarios = [
            ...
            ('America/New_York', '2024-01-15T10:30:00', '2024-01-15T15:30:00'),  # EST (UTC-5)
            ('Europe/London', '2024-01-15T15:30:00', '2024-01-15T15:30:00'),  # GMT (UTC+0)
            ('Asia/Tokyo', '2024-01-16T00:30:00', '2024-01-15T15:30:00'),  # JST (UTC+9)
            ('Australia/Perth', '2024-01-15T23:30:00', '2024-01-15T15:30:00'),  # AWST (UTC+8)
        ]
        ...
        for user_timezone, local_time, expected_utc in test_scenarios:
            ...
            assert utc_time.replace(tzinfo=None) == expected_utc_dt, \
    ...
    def test_cross_timezone_assessment_analytics(self):
        ...
        assert time_diff.total_seconds() < 3600, "Assessments should be within 1 hour window"
    ...
    def test_session_duration_across_timezones(self):
        ...
        user_tz = ZoneInfo(user_timezone)
        start_dt = datetime.datetime.fromisoformat(session_start_local).replace(tzinfo=user_tz)
        end_dt = datetime.datetime.fromisoformat(session_end_local).replace(tzinfo=user_tz)

        # Calculate duration
        duration = end_dt - start_dt

        # Should be exactly 30 minutes regardless of timezone
        assert duration.total_seconds() == 30 * 60, "Session duration should be timezone-independent"
    ...
    def test_timezone_database_robustness(self):
        ...
        for tz_name in essential_timezones:
            try:
                tz = ZoneInfo(tz_name)
                assert tz is not None, f"Essential timezone {tz_name} should be available"
    ...
    def test_performance_with_multiple_timezone_conversions(self):
        ...
        assert conversion_time < 1.0, "1000 timezone conversions should complete in under 1 second"
    ...
    def test_server_timezone_independence(self):
        ...
        for server_tz in server_timezones:
            try:
                ...
                assert utc_time.hour == 22, "UTC conversion should be server timezone independent"
    ...
    def test_timezone_aware_analytics_queries(self):
        ...
        assessment_data = [
            {
                'id': 1,
                'created_at': datetime.datetime(2024, 1, 15, 15, 30, 0, tzinfo=datetime.timezone.utc),
                'user_timezone': 'America/New_York',
                'local_time': '2024-01-15T10:30:00'
            },
            {
                'id': 2,
                'created_at': datetime.datetime(2024, 1, 15, 15, 45, 0, tzinfo=datetime.timezone.utc),
    ...
    def test_timezone_aware_deadline_calculations(self):
        ...
        base_time = datetime.datetime(2024, 1, 15, 12, 0, 0, tzinfo=datetime.timezone.utc)
        ...
        for rule, delta in deadline_rules:
            for timezone_str in user_timezones:
                ...
                assert abs(time_diff - delta) < datetime.timedelta(seconds=1), \
    ...
    def test_timezone_aware_export_import(self):
        ...
        for assessment in export_data['assessments']:
            ...
            utc_start = datetime.datetime.fromisoformat(assessment['created_at_utc'].replace('Z', '+00:00'))
            local_start = datetime.datetime.fromisoformat(assessment['local_start_time'])
            user_tz = ZoneInfo(assessment['user_timezone'])

            # Should be able to reconstruct UTC time from local time
            reconstructed_utc = local_start.replace(tzinfo=user_tz).astimezone(datetime.timezone.utc)
            assert abs((reconstructed_utc - utc_start).total_seconds()) < 60, "Timezone reconstruction failed"

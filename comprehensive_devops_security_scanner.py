...
class DevOpsSecurityScanner:
                ...
                for file_path in dir_path.rglob("*"):
                    if file_path.is_file():
                        try:
                            ...
                            if any(s in file_path.name.lower() for s in ["key", "secret", "password", "token", "credential"]):
    ...
    def scan_public_file_exposures(self):
        ...
        for public_dir in public_dirs:
            ...
            for pattern, description in sensitive_patterns.items():
                ...
                if "*" in pattern:
                    parts = pattern.split("*")
                    for file_path in public_dir.rglob(f"*{parts[1]}" if len(parts) > 1 else parts[0]):
                        if file_path.is_file() and file_path.name not in found_sensitive:
                            issues.append(SecurityIssue(
                                severity="critical",
        ...
        env_files = [f for f in env_files if f.is_file() and "example" not in f.name.lower()]
        ...
        if env_files:
            ...
            if not env_protected:
                issues.append(SecurityIssue(
                    ...
                    file_path=str(self.project_root / ".gitignore") if gitignore.exists() else None,
                                    ...
                                    for vuln in vuln_result.get("Vulnerabilities", []):
                                        ...
                                        if severity in ["critical", "high"]:
                                            issues.append(SecurityIssue(
                                                ...
                                                description=vuln.get("Title", "Security vulnerability"),
    ...
    def scan_docker_compose_security(self):
        ...
        for compose_file in compose_files:
            ...
            try:
                ...
                if "ports:" in content:
                    for line in content.split('\n'):
                        if '- "' in line or '- \'' in line or '- "' in line or ":80:" in line:
                ...
                if "/:/host" in content or "volumes:" in content:
                    for i, line in enumerate(content.split('\n')):
                        if ":/" in line and (":/host" in line or ":/root" in line or ":/var/run/docker.sock" in line):
                            issues.append(SecurityIssue(
                                severity="high",
                                category="orchestration_security",
                    ...
                    for secret_type, pattern in self.secret_patterns.items():
                        if re.search(pattern, line, re.IGNORECASE):
                            ...
                            if not any(placeholder in line.lower() for placeholder in
                                      ["change", "example", "your-", "placeholder", "xxx", "localhost"]):
    ...
    def scan_hardcoded_secrets(self):
        ...
        code_extensions = {'.py', '.js', '.ts', '.jsx', '.tsx', '.yaml', '.yml', '.json', '.sh', '.md'}

        # Files to skip
        skip_patterns = {
            'node_modules', '.git', 'venv', '__pycache__', '.pytest_cache',
            'dist', 'build', '.next', 'coverage', '.backup'
        }
    ...
    def _is_false_positive(self, line: str) -> bool:
        ...
        false_positive_indicators = [
            ...
            "placeholder", "your-", "change-me", "xxx",
            "<", ">",
    ...
    def generate_summary_report(self) -> Dict[str, Any]:
        ...
        critical_count = sum(len([i for i in r.issues if i.severity == "critical"]) for r in self.results)
        high_count = sum(len([i for i in r.issues if i.severity == "high"]) for r in self.results)
        medium_count = sum(len([i for i in r.issues if i.severity == "medium"]) for r in self.results)
        low_count = sum(len([i for i in r.issues if i.severity == "low"]) for r in self.results)

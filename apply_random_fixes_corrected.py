...
REPLACEMENTS = [
    ...
    (
        r'random\.shuffle\(([^)]+)\)',
        r"secrets.SystemRandom().shuffle(\1)"
    ),
    # secrets.SystemRandom().sample(x, n) -> secrets.SystemRandom().sample(x, n)
    (
        r'random\.sample\(([^,]+),\s*([^)]+)\)',
        r"secrets.SystemRandom().sample(\1, \2)"
    ),
]
...
def main():
    ...
    py_files = [f for f in py_files if 'test' not in str(f) and 'venv' not in str(f) and '.venv' not in str(f)]

...
class BackupSecurityTester:
    ...
    def scan_for_backup_files(self) -> List[Dict]:
        ...
        backup_patterns = [
            "*.sql", "*.dump", "*.backup", "*.bak", "*.pg_dump",
            "*.tar", "*.tar.gz", "*.tgz", "*.zip", "*.7z",
            "*.enc", "*.gpg", "*.aes", "*.db", "*.sqlite",
            "*.csv", "*.json", "*backup*", "*dump*", "*backup_*"
        ]
        ...
        try:
            ...
            if not result["encrypted"]:
                ...
                if self.calculate_entropy(header) > 7.0:
                    ...
                    result["security_issues"].append("File has high entropy but no clear encryption signature")
            ...
            if os.access(file_path, os.R_OK | os.W_OK | os.X_OK):
                result["security_issues"].append("File has overly permissive permissions")
    ...
    def create_encrypted_backup(self, db_path: str) -> Optional[Path]:
        ...
        try:
            ...
            # Encrypt content
            encrypted_content = self.simulate_encryption(db_content.decode('utf-8', errors='ignore'))
    ...
    def test_backup_rotation_policy(self) -> Dict:
        ...
        if len(old_backups) > 0:
            result["security_issue"] = f"Found {len(old_backups)} backups older than 90 days"
            result["recommendation"] = "Implement backup retention policy and secure old backups"
    ...
    def generate_security_recommendations(self, test_results: Dict) -> List[Dict]:
        ...
        unencrypted_files = [f for f in test_results.get("file_encryption_tests", [])
        ...
        access_issues = [f for f in test_results.get("access_control_tests", [])
        ...
        if backup_encryption.get("postgresql_backup_test", {}).get("security_level") == "low":
            recommendations.append({
                "priority": "MEDIUM",
    ...
    def run_comprehensive_test(self) -> Dict:
        ...
        encrypted_files = len([f for f in file_encryption_results if f.get("encrypted", False)])
        access_issues = len([f for f in results.get("access_control_tests", [])
                           if f.get("security_issues")])
    ...
    def calculate_security_score(self, results: Dict) -> int:
        ...
        unencrypted = len([f for f in file_encryption_tests if not f.get("encrypted", False)])
...
def main():
    ...
    try:
        ...
        unencrypted_files = [f for f in results["file_encryption_tests"]
                           if not f.get("encrypted", False)]
        ...
        # Show access issues
        access_issues = [f for f in results["access_control_tests"]
                        if f.get("security_issues")]
        ...
        with open("/Users/sheriftito/Downloads/psychsync/backup_encryption_security_report.json", "w") as f:

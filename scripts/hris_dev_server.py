...
class HRISDevServer:
    ...
    def _generate_mock_employees(self) -> List[MockEmployee]:
        ...
        positions = ["Software Engineer", "HR Manager", "Sales Representative", "Marketing Manager",
                     "Financial Analyst", "Operations Manager", "Team Lead", "Director"]
        ...
        for i in range(1, 21):  # 20 mock employees
            emp = MockEmployee(
                ...
                hire_date=(datetime.now() - timedelta(days=secrets.randbelow(970) + 30)).strftime('%Y-%m-%d'),
        ...
        while current_date <= end_date:
            ...
            for employee in self.employees[:15]:  # Only active employees
                if secrets.SystemRandom().random() > 0.2:  # 80% attendance rate
                    ...
                    hours_worked = clock_out_hour - clock_in_hour - random.uniform(0.5, 1.5)  # Lunch break
    ...
    def _setup_routes(self):
        ...
        def get_employees():
            ...
            if status:
                filtered_employees = [emp for emp in filtered_employees if emp.employment_status == status]
        ...
        def get_attendance():
            ...
            if start_date:
                filtered_attendance = [att for att in filtered_attendance if att.date >= start_date]
            if end_date:
                filtered_attendance = [att for att in filtered_attendance if att.date <= end_date]
            if employee_id:
                filtered_attendance = [att for att in filtered_attendance if att.employee_id == employee_id]
        ...
        def get_leaves():
            ...
            if start_date:
                filtered_leaves = [leave for leave in filtered_leaves if leave.start_date >= start_date]
            if end_date:
                filtered_leaves = [leave for leave in filtered_leaves if leave.end_date <= end_date]
            if employee_id:
                filtered_leaves = [leave for leave in filtered_leaves if leave.employee_id == employee_id]
            if status:
                filtered_leaves = [leave for leave in filtered_leaves if leave.status == status]
    ...
    def _setup_orangehrm_routes(self):
        ...
        def orangehrm_employees():
            ...
            return jsonify({
                "data": [asdict(emp) for emp in self.employees if emp.employment_status == "active"]
            })

        @self.app.route('/api/v2/search/employees')
        def orangehrm_search():
            query = request.args.get('q', '').lower()
            matched = [emp for emp in self.employees if query in emp.first_name.lower() or query in emp.last_name.lower()]
            return jsonify({
                "data": [asdict(emp) for emp in matched]
            })
    ...
    def _setup_quickbooks_routes(self):
        ...
        def quickbooks_users():
            ...
            filtered = [emp for emp in self.employees if emp.employment_status == 'active'] if active else self.employees

            return jsonify({
                "results": [asdict(emp) for emp in filtered]
            })
        ...
        def quickbooks_timesheets():
            ...
            return jsonify({
                "results": [asdict(att) for att in filtered_attendance]
            })
    ...
    def _setup_generic_routes(self):
        ...
        def search_employees():
            ...
            matched = [emp for emp in self.employees if term in emp.first_name.lower() or term in emp.last_name.lower()]
            return jsonify({
                "employees": [asdict(emp) for emp in matched]
            })
        ...
        def headcount_report():
            ...
            return jsonify({
                ...
                "active": len([emp for emp in self.employees if emp.employment_status == "active"])
            })

    def run(self):
        """Run the development server."""
        logger.info(f"Starting {self.platform} HRIS development server on port {self.port}")
...
def main():
    ...
    parser.add_argument('--platform', choices=['orangehrm', 'quickbooks_workforce', 'generic'],
                       default='orangehrm', help='HRIS platform to simulate')

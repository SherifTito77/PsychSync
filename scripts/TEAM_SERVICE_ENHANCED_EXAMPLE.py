...
from app.core.safe_db_operations import safe_create, safe_delete, safe_update
from app.db.models.team import Team, TeamMember, TeamRole
from app.db.models.user import User
from app.monitoring.database_error_monitor import db_monitor, monitor_db_errors
...
class TeamServiceEnhanced:
    ...
    async def update_member_role(
        ...
        if not member:
            logger.warning(
                f"Team member not found: team_id={team_id}, user_id={user_id}"
            )
            return None
        ...
        if member:
            logger.info(
                f"Updated role for user {user_id} in team {team_id} to {role.value}"
            )

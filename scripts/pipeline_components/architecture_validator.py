...
class ArchitectureValidator:
    ...
    def _calculate_maintainability_index(
        self, content: str, tree: ast.AST, lines_of_code: int
    ) -> float:
        """
        ...
        if operators == 0 or operands == 0:
            ...
            halstead_volume = (
                length * (vocabulary / (operators + operands))
                if vocabulary > 0
                else 1.0
            )
        ...
        try:
            ...
            mi = (
                ...
                - 0.23 * self._calculate_cyclomatic_complexity(tree)
    ...
    def _calculate_dependency_depth(
        self, file_path: Path, dependencies: List[str]
    ) -> int:
    ...
    def _detect_class_patterns(self, class_node: ast.ClassDef) -> List[str]:
        ...
        # Singleton Pattern
        if any(
            "instance" in method_name or "getInstance" in method_name
        ...
        if any(
            ...
            or "build" in method_name
            or "factory" in method_name.lower()
            for method_name in method_names
        ):
        ...
        # Observer Pattern
        if any(
            "notify" in method_name
            or "update" in method_name
            or "attach" in method_name
            or "detach" in method_name
            for method_name in method_names
        ):
            patterns.append("Observer")

        # Strategy Pattern
        if "strategy" in class_name or any(
            "execute" in method_name for method_name in method_names
        ):
            patterns.append("Strategy")

        # Decorator Pattern
        if "decorator" in class_name or any(
            "wrap" in method_name or "decorate" in method_name
            for method_name in method_names
        ):
            patterns.append("Decorator")

        # Command Pattern
        if "command" in class_name or any(
            "execute" in method_name or "undo" in method_name
            for method_name in method_names
        ):
        ...
        if any(
            "template" in class_name.lower()
            or (method_name.startswith("_") and method_name.endswith("_template"))
            for method_name in method_names
        ):
    ...
    def _detect_anti_patterns(self, content: str, tree: ast.AST) -> List[str]:
        ...
        if len(unused_imports) > 3:
            anti_patterns.append(
                f"Unused Imports: {len(unused_imports)} potentially unused"
            )

        return anti_patterns[:20]  # Limit to top 20 anti-patterns

    def _detect_code_smells(
        self, content: str, tree: ast.AST, classes: List[str], functions: List[str]
    ) -> List[str]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                if len(node.args.args) > 7:
                    code_smells.append(
                        f"Long Parameter List: {node.name} ({len(node.args.args)} parameters)"
                    )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                for method in methods:
                    ...
                    if method_calls > 10:
                        code_smells.append(
                            f"Feature Envy: {class_name}.{method.name} ({method_calls} external calls)"
                        )
        ...
        for combo, count in parameters_seen.items():
            if count > 3:
                code_smells.append(
                    f"Data Clump: Parameters {combo} appear together {count} times"
                )
        ...
        for class_name, deps in class_dependencies.items():
            if len(deps) > 10:
                code_smells.append(
                    f"Inappropriate Intimacy: {class_name} depends on {len(deps)} other classes"
                )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if len(abstract_methods) > len(methods) * 0.7:
                    code_smells.append(
                        f"Speculative Generality: {node.name} has too many abstract methods"
                    )
    ...
    def _validate_solid_principles(
        self, tree: ast.AST, classes: List[str]
    ) -> List[str]:
    ...
    def _check_srp_violations(self, class_node: ast.ClassDef) -> List[str]:
        ...
        if len(methods) > 15:
            violations.append(
                f"SRP Violation: {class_node.name} has {len(methods)} methods (too many responsibilities)"
            )
        ...
        for method in methods:
            ...
            if any(
                word in method.name.lower()
                for word in ["save", "load", "read", "write", "persist"]
            ):
                topic_words.add("data")
            if any(
                word in method.name.lower() for word in ["validate", "check", "verify"]
            ):
                topic_words.add("validation")
            if any(
                word in method.name.lower()
                for word in ["render", "display", "show", "view"]
            ):
                topic_words.add("ui")
            if any(
                word in method.name.lower()
                for word in ["send", "notify", "email", "message"]
            ):
        ...
        if len(all_topics) > 3:
            violations.append(
                f"SRP Violation: {class_node.name} handles {len(all_topics)} different concerns"
            )
        ...
        for method in class_node.body:
            if isinstance(method, ast.FunctionDef):
                ...
                if self._count_decision_points(method) > 5:
                    violations.append(
                        f"OCP Violation: {class_node.name}.{method.name} has complex conditional logic"
                    )
                ...
                if isinstance(method, ast.FunctionDef):
                    ...
                    for node in ast.walk(method):
                        if isinstance(node, ast.Raise):
                            violations.append(
                                f"LSP Violation: {class_node.name}.{method.name} may break substitution"
                            )
                            break
    ...
    def _check_isp_violations(self, class_node: ast.ClassDef) -> List[str]:
        ...
        if len(methods) > 12:
            violations.append(
                f"ISP Violation: {class_node.name} has {len(methods)} methods (consider splitting interface)"
            )
        ...
        for node in ast.walk(class_node):
            if isinstance(node, ast.Call):
                if isinstance(node.func, ast.Name):
                    ...
                    violations.append(
                        f"DIP Violation: {class_node.name} depends on concrete class {node.func.id}"
                    )
    ...
    def _compile_analysis_results(
        ...
        dependency_analysis = {
            ...
            "average_coupling": (
                sum(m.coupling_score for m in metrics) / len(metrics) if metrics else 0
            ),
            "max_dependency_depth": (
                max(m.dependency_depth for m in metrics) if metrics else 0
            ),
        ...
        recommendations = self._generate_recommendations(
            metrics, complexity_distribution, maintainability_distribution
        )

        # Identify critical issues
        critical_issues = self._identify_critical_issues(
            metrics, anti_patterns_found, solid_violations
        )
    ...
    def _calculate_maintainability_distribution(
        self, metrics: List[ArchitectureMetrics]
    ) -> Dict[str, int]:
    ...
    def _detect_isolated_modules(self, metrics: List[ArchitectureMetrics]) -> List[str]:
        ...
        for metric in metrics:
            ...
            if (
                self.dependency_graph.out_degree(module_name) == 0
                and self.dependency_graph.in_degree(module_name) == 0
            ):
    ...
    def _calculate_overall_quality_score(
        ...
        avg_maintainability = sum(m.maintainability_index for m in metrics) / len(
            metrics
        )
        ...
        maintainability_penalty = (
            max(0, self.maintainability_threshold - avg_maintainability) / 2
        )
    ...
    def _generate_recommendations(
        ...
        maintainability_dist: Dict[str, int],
    ) -> List[str]:
    ...
    def _identify_critical_issues(
        ...
        if very_complex:
            critical_issues.append(
                f"CRITICAL: {len(very_complex)} files with very high complexity (>50)"
            )
        ...
        if very_poor_maintainability:
            critical_issues.append(
                f"CRITICAL: {len(very_poor_maintainability)} files with very poor maintainability (<20)"
            )

        # Critical anti-patterns
        critical_anti_patterns = {
            k: v for k, v in anti_patterns.items() if "God Class" in k and v > 0
        }
        if critical_anti_patterns:
            critical_issues.append(
                f"CRITICAL: God Class pattern detected - refactoring required"
            )
        ...
        if circular_deps:
            critical_issues.append(
                f"CRITICAL: {len(circular_deps)} circular dependency chains detected"
            )

...
class GDPRComplianceTester:
    ...
    def test_gdpr_data_portability(self) -> GDPRTestResult:
        ...
        try:
            ...
            for fmt in export_formats:
                try:
                    response = requests.get(
                        ...
                        headers={
                            "Authorization": f"Bearer test_token_{user['user_id']}"
                        },
                        timeout=10,
                    )
                    ...
                    if response.status_code == 200:
                        ...
                        violation_details.append(
                            f"Export format {fmt.upper()} not available (HTTP {response.status_code})"
                        )

                except Exception as e:
                    format_availability[fmt] = {"available": False, "error": str(e)}
                    violation_details.append(
                        f"Export format {fmt.upper()} failed: {str(e)}"
                    )
            ...
            if format_availability.get("json", {}).get("available"):
                try:
                    ...
                    if data_completeness_score > 0:
                        ...
                        violation_details.append(
                            "Exported data missing required data types"
                        )

                except Exception as e:
                    violation_details.append(
                        f"Data completeness check failed: {str(e)}"
                    )

            # Test 3: Verify response time compliance (within 30 days = immediate for testing)
            response_time_score = 25
            total_response_time = sum(
                fmt.get("response_time", 0)
                for fmt in format_availability.values()
                if fmt.get("available")
            )

            if (
                total_response_time > 0 and total_response_time < 30
            ):  # 30 seconds for testing
                compliance_score += response_time_score
            else:
                violation_details.append(
                    f"Response time exceeds compliance threshold: {total_response_time:.2f}s"
                )
            ...
            try:
                ...
                violation_details.append(
                    f"Machine-readable format validation failed: {str(e)}"
                )
            ...
            return GDPRTestResult(
                ...
                violation_details=(
                    "; ".join(violation_details) if violation_details else None
                ),
            ...
            return GDPRTestResult(
                ...
                recommendations=[
                    "Investigate data export API implementation",
                    "Ensure GDPR Article 20 compliance",
                ],
            )
        ...
        try:
            ...
            try:
                ...
                if response.status_code in [200, 201, 202]:
                    ...
                    expected_completion = deletion_response.get(
                        "expected_completion_date"
                    )
                else:
                    violation_details.append(
                        f"Deletion request failed: HTTP {response.status_code}"
                    )
            ...
            try:
                ...
                if response.status_code == 400:
                    ...
                    violation_details.append(
                        "Identity verification bypass detected - GDPR violation"
                    )
            ...
            try:
                if deletion_request_available:
                    ...
                    if response.status_code == 200:
                        ...
                        estimated_completion = status_response.get(
                        ...
                        if created_at and estimated_completion:
                            ...
                            if days_to_complete <= 30:
                                ...
                                violation_details.append(
                                    f"Deletion timeline exceeds 30 days: {days_to_complete} days"
                                )
                                deletion_timeline = days_to_complete
                        else:
                            violation_details.append(
                                "Deletion timeline information missing"
                            )
                    else:
                        violation_details.append(
                            "Deletion status tracking not available"
                        )
            ...
            try:
                ...
                response = requests.get(
                    f"{self.backend_url}/api/v1/legal/retention-policy", timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    if legal_obligations and retention_periods:
                        ...
                        violation_details.append(
                            "Legal retention policy not documented"
                        )
                else:
            ...
            return GDPRTestResult(
                ...
                compliance_score=compliance_score,
                deletion_timeline=(
                    deletion_timeline if "deletion_timeline" in locals() else None
                ),
                legal_basis_verified=True,
                data_access_points=[
                    f"/api/v1/users/{user['user_id']}/request-deletion"
                ],
                violation_details=(
                    "; ".join(violation_details) if violation_details else None
                ),
            ...
            return GDPRTestResult(
                ...
                recommendations=[
                    "URGENT: Implement GDPR Article 17 compliance",
                    "Add legal review of deletion processes",
                ],
            )
    ...
    def test_cookie_consent_rules(self) -> GDPRTestResult:
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if consent_elements_found >= 4:  # Require multiple consent elements
                        ...
                        violation_details.append(
                            "Insufficient cookie consent elements detected"
                        )
                else:
                    violation_details.append(
            ...
            try:
                ...
                response = requests.get(
                    f"{self.backend_url}/api/v1/cookies/categories", timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    found_categories = [
                        cat.get("name", "").lower() for cat in categories
                    ]
                    category_compliance = len(
                        [cat for cat in required_categories if cat in found_categories]
                    )
                    ...
                    if category_compliance >= 3:
                        ...
                        violation_details.append(
                            f"Insufficient cookie categories: {category_compliance}/4 required"
                        )
                else:
            ...
            try:
                ...
                if response.status_code in [200, 201]:
                    ...
                    if consent_id:
                        ...
                        violation_details.append(
                            "Consent recording failed - no ID returned"
                        )
                else:
                    violation_details.append(
                        f"Consent submission failed: HTTP {response.status_code}"
                    )
            ...
            try:
                ...
                if response.status_code in [403, 401, 422]:
                    ...
                    violation_details.append(
                        f"Tracking without consent allowed: HTTP {response.status_code}"
                    )
            ...
            if compliance_score >= 80:
                result = TestResult.PASS
                recommendations = [
                    "Continue monitoring consent compliance",
            ...
            return GDPRTestResult(
                ...
                recommendations=[
                    "URGENT: Implement cookie consent compliance",
                    "Review ePrivacy Directive requirements",
                ],
            )
    ...
    def test_psychometric_profile_export(self) -> GDPRTestResult:
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        violation_details.append(
                            "Psychometric profile export not in valid JSON format"
                        )
                else:
                    violation_details.append(
                        f"Psychometric profile export failed: HTTP {response.status_code}"
                    )
            ...
            if profile_export_available:
                try:
                    ...
                    if data_completeness_score >= 80:
                        ...
                        violation_details.append(
                            f"Psychometric data incomplete: {len(found_components)}/{len(required_components)} components"
                        )
            ...
            if profile_export_available:
                try:
                    ...
                    if len(accuracy_checks) >= 2:
                        ...
                        violation_details.append(
                            "Psychometric data accuracy or consistency issues"
                        )
            ...
            if profile_export_available:
                try:
                    ...
                    metadata_present = sum(
                        ...
                        if meta in profile_data.get("metadata", {})
                    )
                    ...
                    if metadata_present >= 3:
                        ...
                        violation_details.append(
                            f"Missing export metadata: {metadata_present}/{len(required_metadata)} fields"
                        )

                except Exception as e:
                    violation_details.append(
                        f"Format compliance check failed: {str(e)}"
                    )
            ...
            if compliance_score >= 80:
                result = TestResult.PASS
                recommendations = [
                    "Continue monitoring profile export accuracy",
                    "Regular psychometric data audits",
                ]
            elif compliance_score >= 60:
                result = TestResult.WARNING
                recommendations = [
            ...
            return GDPRTestResult(
                ...
                legal_basis_verified=True,
                data_access_points=[
                    f"/api/v1/users/{user['user_id']}/psychometric-profile/export"
                ],
                violation_details=(
                    "; ".join(violation_details) if violation_details else None
                ),
            ...
            return GDPRTestResult(
                ...
                recommendations=[
                    "URGENT: Implement psychometric profile export",
                    "Ensure GDPR Article 20 compliance for specialized data",
                ],
            )
    ...
    def test_data_anonymization_after_deletion(self) -> GDPRTestResult:
        ...
        try:
            ...
            try:
                ...
                if response.status_code in [200, 202]:
                    ...
                    if anonymization_id:
                        ...
                        violation_details.append(
                            "Anonymization process failed - no ID returned"
                        )
                else:
                    violation_details.append(
                        f"Anonymization API failed: HTTP {response.status_code}"
                    )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    for field in personal_data_fields:
                        if field in data_status:
                            ...
                            if isinstance(field_value, str):
                                ...
                                if (
                                    field_value.startswith("anon_")
                                    or field_value.startswith("hash_")
                    ...
                    if len(anonymized_fields) >= 3:
                        ...
                        violation_details.append(
                            f"Insufficient field pseudonymization: {len(anonymized_fields)}/{len(personal_data_fields)}"
                        )
                else:
            ...
            try:
                ...
                response = requests.get(
                    f"{self.backend_url}/api/v1/legal/data-retention", timeout=10
                )
                ...
                if response.status_code == 200:
                    ...
                    found_elements = sum(
                        ...
                        if element in retention_policy
                    )
                    ...
                    if found_elements >= 3:
                        ...
                        violation_details.append(
                            f"Missing retention policy elements: {found_elements}/{len(required_elements)}"
                        )
                else:
            ...
            try:
                ...
                if all(removal_verification):
                    ...
                    violation_details.append(
                        f"Incomplete sensitive data removal: {sum(removal_verification)}/{len(removal_verification)}"
                    )

            except Exception as e:
                violation_details.append(
                    f"Complete removal verification failed: {str(e)}"
                )
            ...
            if compliance_score >= 80:
                result = TestResult.PASS
                recommendations = [
                    "Continue monitoring anonymization processes",
            ...
            return GDPRTestResult(
                ...
                recommendations=[
                    "URGENT: Implement GDPR Article 5 compliance",
                    "Review data protection by design principles",
                ],
            )
    ...
    def run_comprehensive_gdpr_test_suite(self) -> List[GDPRTestResult]:
        ...
        for test_func in test_functions:
            try:
                ...
                print(
                    f"{status_icon} {result.scenario_type.value}: {result.result.value.upper()} ({result.compliance_score:.1f}% compliance)"
                )
                ...
                error_result = GDPRTestResult(
                    ...
                    violation_details="Test execution failed - potential GDPR compliance issue",
                    recommendations=[
                        "URGENT: Investigate GDPR compliance implementation"
                    ],
                )
    ...
    def generate_gdpr_compliance_report(
        self, results: List[GDPRTestResult]
    ) -> Dict[str, Any]:
        ...
        avg_compliance_score = (
            sum(r.compliance_score for r in results) / total_tests
            if total_tests > 0
            else 0
        )
        ...
        report = {
            "gdpr_compliance_summary": {
                ...
                "legal_risk_level": (
                    "HIGH"
                    if critical_tests > 0
                    else "MEDIUM" if avg_compliance_score < 75 else "LOW"
                ),
            },
            "gdpr_test_results": [],
            "compliance_gaps": {
                "critical_violations": [
                    r for r in results if r.result == TestResult.CRITICAL
                ],
                "compliance_warnings": [
            ...
            "gdpr_articles_assessed": [
                ...
                },
                {
                    ...
                    "compliant": any(
                        r.scenario_type == GDPRTestScenario.PROFILE_EXPORT
                ...
                },
                {
                    ...
                    "compliant": any(
                        r.scenario_type == GDPRTestScenario.RIGHT_TO_ERASURE
                        and r.result != TestResult.CRITICAL
                        for r in results
                    ),
                },
                {
                    ...
                    "compliant": any(
                        r.scenario_type == GDPRTestScenario.DATA_PORTABILITY
                        and r.result != TestResult.CRITICAL
                        for r in results
                    ),
                },
                {
                    ...
                    "compliant": any(
                        r.scenario_type == GDPRTestScenario.DATA_ANONYMIZATION
                        and r.result != TestResult.CRITICAL
                        for r in results
                    ),
                },
    ...
    try:
        ...
        if critical_violations:
            ...
            for violation in critical_violations:
                print(
                    f"  • {violation.scenario_type.value.replace('_', ' ').title()}: {violation.violation_details}"
                )

...
class PsychSyncTestingFramework:
    ...
    def load_test_suites(self) -> Dict[str, TestSuite]:
        ...
        print(
            f"Loaded {len(test_suites)} test suites with {sum(len(s.test_methods) for s in test_suites.values())} test methods"
        )
        return test_suites
    ...
    def execute_test_suite(self, suite: TestSuite) -> Dict[str, Any]:
        ...
        try:
            ...
            for test_method in suite.test_methods:
                ...
                print(
                    f"  {'✅' if result.status == TestStatus.PASSED else '❌'} {test_method}: {result.status.value} ({result.execution_time_ms:.1f}ms)"
                )
        ...
        print(
            f"  📊 Suite {suite.name}: {suite_results['status']} "
    ...
    def calculate_performance_summary(self) -> Dict[str, Any]:
        ...
        all_results = [
            r for suite in self.test_results.values() for r in suite.get("results", [])
        ]
    ...
    def generate_recommendations(self, execution_summary: Dict[str, Any]) -> List[str]:
        ...
        if avg_time > 500:
            recommendations.append(
                f"Optimize test performance - average execution time ({avg_time:.1f}ms) exceeds 500ms"
            )
        ...
        if security_summary.get("status") == "failed":
            recommendations.append(
                "Address security test failures to ensure platform safety"
            )
    ...
    def save_human_readable_report(
        ...
        try:
            with open(report_path, "w") as f:
                ...
                f.write(
                    f"**Environment**: {execution_summary.get('environment', 'unknown')}\n"
                )
                f.write(
                    f"**Execution Time**: {execution_summary.get('start_time')} - {execution_summary.get('end_time')}\n"
                )
                f.write(
                    f"**Duration**: {execution_summary.get('execution_duration_seconds', 0):.1f} seconds\n\n"
                )
                ...
                f.write(
                    f"- **Total Tests**: {execution_summary.get('total_tests', 0)}\n"
                )
                ...
                if security_summary.get("status") != "not_executed":
                    ...
                    f.write(
                        f"- **Total Security Tests**: {security_summary.get('total_security_tests', 0)}\n\n"
                    )

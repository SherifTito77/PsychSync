...
class DataCorruptionAnalyzer(ast.NodeVisitor):
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        self.has_db_param = any(
            arg.arg in ['db', 'session', 'async_session']
    ...
    def _analyze_function_patterns(self, func_node):
        ...
        write_ops = [op for op in self.db_operations if op['type'] in ['add', 'delete', 'merge']]
    ...
    def analyze_file(self, filepath: str) -> List[Dict]:
        ...
        try:
            ...
            if re.search(read_write_pattern, content, re.MULTILINE | re.DOTALL):
                ...
                if 'select_for_update' not in content and 'with_for_update' not in content:
                    self.issues.append({
            ...
            bulk_pattern = r'for\s+\w+\s+in\s+\[[^\]]*\]:\s*(?:\n.*)*?db\.(add|delete|merge)'
    ...
    for filepath, file_issues in sorted(all_issues.items()):
        ...
        for issue_type, issues in file_issues.items():
            ...
            for issue in issues[:5]:  # Limit to 5 per type per file
                ...
                report_lines.append(f"{severity_icon} **Line {issue.get('line', '?')}**: {issue.get('message', 'Unknown issue')}")
...
def main():
    ...
    # Print summary
    total_issues = sum(len(issues) for file_issues in all_issues.values() for issues in file_issues.values())
    print(f"\n📈 Summary:")

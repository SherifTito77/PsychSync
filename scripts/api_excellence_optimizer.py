...
class APIExcellenceOptimizer:
            ...
            async with self.session.get(f"{self.base_url}/openapi.json") as response:
                if response.status == 200:
                    ...
                    return {
                        ...
                        "recommendations": [
                            "Ensure OpenAPI spec is accessible at /openapi.json"
                        ],
                    }
        except Exception as e:
            return {
        ...
        rate_limit_results.update(
            {
                ...
                "rate_limiting_detected": any(
                    code in [429, 503] for code in status_codes
                ),
            }
        )
    ...
    async def analyze_caching_strategy(self) -> Dict[str, Any]:
        ...
        cacheable_endpoints = [
            ep for ep in self.endpoints_to_test if ep.startswith("/api/v1/")
        ]
        ...
        endpoints_with_cache_control = sum(
            1 for r in cache_headers_results if r["has_cache_control"]
        )
        ...
        cache_analysis.update(
            {
                ...
                "cache_implementation_rate": (
                    endpoints_with_cache_control / len(cacheable_endpoints)
                    if cacheable_endpoints
                    else 0
                ),
                "etag_implementation_rate": (
                    endpoints_with_etag / len(cacheable_endpoints)
                    if cacheable_endpoints
                    else 0
                ),
        ...
        if not any(
            r["headers"]["cache-control"] and "max-age" in r["headers"]["cache-control"]
            for r in cache_headers_results
        ):
            recommendations.append(
                "Set appropriate max-age values for cacheable responses"
            )
    ...
    async def run_load_test(
        ...
        print(
            f"🏋️  Running load test: {concurrent_users} concurrent users for {duration_seconds}s"
        )
                    ...
                    try:
                        ...
                        async with self.session.get(
                            ...
                            load_test_results["status_codes"][response.status] = (
                                load_test_results["status_codes"].get(
                                    response.status, 0
                                )
                                + 1
                            )
            ...
            load_test_results.update(
                {
                    "avg_response_time": statistics.mean(
                        load_test_results["response_times"]
                    ),
                    "p95_response_time": (
                        statistics.quantiles(load_test_results["response_times"], n=20)[
                            18
                        ]
                        if len(load_test_results["response_times"]) > 20
                        else max(load_test_results["response_times"])
                    ),
                    "p99_response_time": (
                        statistics.quantiles(
                            load_test_results["response_times"], n=100
                        )[98]
                        if len(load_test_results["response_times"]) > 100
                        else max(load_test_results["response_times"])
                    ),
                    "requests_per_second": load_test_results["requests_completed"]
                    / duration_seconds,
                    "error_rate": (
                        load_test_results["requests_failed"]
                        / (
                            load_test_results["requests_completed"]
                            + load_test_results["requests_failed"]
                        )
                        if (
                            load_test_results["requests_completed"]
    ...
    async def _benchmark_endpoint(
            ...
            try:
                async with self.session.get(url) as response:
                    ...
                    status_codes[response.status] = (
                        status_codes.get(response.status, 0) + 1
                    )
        ...
        if response_times:
            ...
            error_rate = (
                sum(count for code, count in status_codes.items() if code >= 400)
                / num_requests
            )
        else:
    ...
    async def _check_broken_object_level_authorization(
        self,
        ...
        for endpoint in test_endpoints:
            try:
                ...
                logger.error(
                    f"Error checking object level authorization for {endpoint}: {e}"
                )
    ...
    async def _check_lack_of_resources_and_rate_limiting(
        self,
    ) -> List[SecurityVulnerability]:
    ...
    async def _check_broken_function_level_authorization(
        self,
    ) -> List[SecurityVulnerability]:
    ...
    async def _check_mass_assignment(self) -> List[SecurityVulnerability]:
        ...
        for payload in test_payloads:
            try:
                async with self.session.post(
                    f"{self.base_url}/api/v1/users/", json=payload
                ) as response:
    ...
    async def _check_insufficient_logging_and_monitoring(
        self,
    ) -> List[SecurityVulnerability]:
    ...
    def _validate_openapi_spec(self, openapi_spec: Dict) -> Dict[str, Any]:
        ...
        total_checks = (
            len(required_fields) + 3 + (len(openapi_spec.get("paths", {}).keys()) * 2)
        )
        ...
        if any(
            "description" not in openapi_spec.get("info", {})
    ...
    async def generate_optimization_report(self) -> Dict[str, Any]:
        ...
        rate_limiting_score = (
            100
            if rate_limiting_analysis["rate_limiting_status"] == "IMPLEMENTED"
            else 0
        )
        ...
        critical_vulns = [
            v for v in security_vulnerabilities if v.severity == "CRITICAL"
        ]
        if critical_vulns:
    ...
    try:
        ...
        if report["security_vulnerabilities"]:
            ...
            for vuln in report["security_vulnerabilities"][:5]:
                print(
                    f"   • {vuln['severity']}: {vuln['vulnerability_type']} on {vuln['endpoint']}"
                )
        ...
        if report["performance_metrics"]:
            ...
            for metric in report["performance_metrics"][:3]:
                print(
                    f"   • {metric['endpoint']}: {metric['avg_response_time']:.2f}ms avg, {metric['error_rate']:.1%} error rate"
                )

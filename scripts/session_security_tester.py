...
class SessionSecurityTester:
    ...
    def test_session_fixation(self) -> SessionTestResult:
        ...
        try:
            ...
            response1 = initial_session.get(f"{self.base_url}/api/v1/health", timeout=10)
            ...
            for cookie_name in pre_auth_cookies:
                if cookie_name in post_auth_cookies:
                    ...
                    cookie_analysis[cookie_name] = {
                        ...
                        "changed": pre_auth_cookies[cookie_name] != post_auth_cookies[cookie_name]
                    }
            ...
            old_session_response = old_session.get(f"{self.base_url}/api/v1/me", timeout=10)
            ...
            risk_level = "CRITICAL" if attack_successful else "HIGH" if not session_changed else "LOW"

            recommendations = []
            if not session_changed:
                recommendations.append("Session identifiers should be regenerated after authentication")
            if old_session_response.status_code == 200:
                recommendations.append("Old session tokens should be invalidated after login")
            if attack_successful:
                recommendations.append("Application should reject forced session identifiers")

            if not vulnerability_found:
                recommendations.append("Session fixation protection appears to be working correctly")
            ...
            return SessionTestResult(
                ...
                risk_level="MEDIUM",
                details={"error": str(e)},
                recommendations=["Session fixation test failed - manual review recommended"],
                evidence=str(e)
            )
    ...
    def test_session_timeout(self) -> SessionTestResult:
        ...
        try:
            ...
            immediate_response = self.session.get(f"{self.base_url}/api/v1/me", timeout=10)
            ...
            for duration in timeout_durations:
                ...
                timeout_response = self.session.get(f"{self.base_url}/api/v1/me", timeout=10)
            ...
            refresh_response = self.session.post(f"{self.base_url}/api/v1/refresh", timeout=10)
            ...
            abrupt_response = abrupt_session.get(f"{self.base_url}/api/v1/me", timeout=10)
            ...
            if immediate_access and len([r for r in timeout_results.values() if r["access_granted"]]) == len(timeout_results):
            ...
            if abrupt_access and len(timeout_results) > 0:
                ...
                if timeout_results[f"{longest_test}s"]["access_granted"]:
                    vulnerabilities.append("Session timeout may be too long or disabled")
                    risk_level = "MEDIUM"
            ...
            if "Session appears to have infinite timeout" in vulnerabilities:
                recommendations.append("Implement reasonable session timeout (30 minutes to 2 hours)")
            ...
            return SessionTestResult(
                ...
                risk_level="MEDIUM",
                details={"error": str(e)},
                recommendations=["Session timeout test failed - manual review recommended"],
                evidence=str(e)
            )
        ...
        try:
            ...
            for i in range(5):
                ...
                if token:
                    ...
                    session_id = hashlib.md5(str(session_cookies).encode()).hexdigest()[:8]
            ...
            for i, (session_obj, token) in enumerate(zip(self.concurrent_sessions, concurrent_tokens)):
                try:
            ...
            concurrent_results = []
            with ThreadPoolExecutor(max_workers=len(self.concurrent_sessions)) as executor:
                futures = [
            ...
            if len(concurrent_tokens) > 3 and not session_limit_reached:
                ...
                risk_level = "MEDIUM"
                recommendations.append("Consider implementing concurrent session limits")

            # Check session isolation
            failed_sessions = len([r for r in concurrent_results if not r.get("success", False)])
            if failed_sessions > 0:
                vulnerabilities.append(f"{failed_sessions} concurrent sessions failed")
                risk_level = "HIGH"
                recommendations.append("Investigate concurrent session stability issues")

            # Check logout invalidation
            sessions_invalidated = sum(1 for s in session_statuses.values() if s.get("invalidated_by_logout", False))
            ...
            return SessionTestResult(
                ...
                risk_level="MEDIUM",
                details={"error": str(e)},
                recommendations=["Concurrent session test failed - manual review recommended"],
                evidence=str(e)
            )
    ...
    def test_token_rotation(self) -> SessionTestResult:
        ...
        try:
            ...
            refresh_response = refresh_session.post(f"{self.base_url}/api/v1/refresh", timeout=10)
            refresh_works = refresh_response.status_code == 200
            ...
            old_token_session.headers.update({"Authorization": f"Bearer {initial_token}"})

            old_token_response = old_token_session.get(f"{self.base_url}/api/v1/me", timeout=10)
            old_token_valid = old_token_response.status_code == 200

            # Step 6: Test token expiration handling
            # Create a fake expired token
            expired_token_parts = token_parts.copy() if len(token_parts) >= 3 else ["", "", ""]
            if len(expired_token_parts) >= 2:
                ...
                try:
                    ...
                    payload_data = json.loads(base64.urlsafe_b64decode(expired_token_parts[1] + '==').decode())
                    payload_data['exp'] = int(time.time()) - 3600  # Expired 1 hour ago

                    expired_payload = base64.urlsafe_b64encode(json.dumps(payload_data).encode()).decode().rstrip('=')
                    expired_token = f"{expired_token_parts[0]}.{expired_payload}.{expired_token_parts[2]}"

                    expired_session = requests.Session()
                    expired_session.headers.update({"Authorization": f"Bearer {expired_token}"})
            ...
            return SessionTestResult(
                ...
                details={
                    ...
                    "token_changed_after_refresh": new_token != initial_token if new_token else False,
            ...
            return SessionTestResult(
                ...
                risk_level="MEDIUM",
                details={"error": str(e)},
                recommendations=["Token rotation test failed - manual review recommended"],
                evidence=str(e)
            )
    ...
    def test_session_store_leakage(self) -> SessionTestResult:
        ...
        try:
            ...
            sensitive_patterns = [
                "session", "token", "jwt", "secret", "key", "password",
                "sessionid", "csrf", "auth", "credential"
            ]
            ...
            for pattern in sensitive_patterns:
                if pattern.lower() in response_text.lower():
                    ...
                    if any(x in response_text.lower() for x in [":", "=", "\"", "'"]):
                        leaked_info.append(f"Potential {pattern} information in response")

            # Check response headers for sensitive information
            sensitive_headers = ["Set-Cookie", "Authorization", "X-Session-ID", "X-Auth-Token"]
            header_leakage = {}

            for header in sensitive_headers:
                if header in response_headers:
                    header_value = response_headers[header]
                    if len(header_value) > 50:  # Long header values might contain sensitive data
            ...
            for cookie_name, cookie_value in self.session.cookies.items():
                cookie_security[cookie_name] = {
                    ...
                    "value_prefix": cookie_value[:10] + "..." if len(cookie_value) > 10 else cookie_value
                }
            ...
            for endpoint in error_endpoints:
                try:
                    error_response = self.session.get(f"{self.base_url}{endpoint}", timeout=5)
            ...
            for i in range(5):
                ...
                if token:
                    ...
                    session_id = hashlib.md5(str(dict(test_session.cookies)).encode()).hexdigest()[:8]
                    session_ids.append(session_id)

            # Check for predictable patterns
            session_entropy = len(set(session_ids)) / len(session_ids) if session_ids else 0
            ...
            if leaked_info:
                ...
                risk_level = "MEDIUM"
                recommendations.append("Remove sensitive session information from API responses")

            if any(resp.get("contains_stack_trace", False) for resp in error_responses.values()):
                vulnerabilities.append("Error responses contain stack traces")
                risk_level = "HIGH"
                recommendations.append("Remove detailed error information from responses")
            ...
            for cookie_name, security_info in cookie_security.items():
                if security_info["value_length"] < 20:
                    cookie_recommendations.append(f"Cookie {cookie_name} may have insufficient entropy")
            ...
            return SessionTestResult(
                ...
                risk_level="MEDIUM",
                details={"error": str(e)},
                recommendations=["Session leakage test failed - manual review recommended"],
                evidence=str(e)
            )
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        try:
            ...
            critical_recommendations = [r for r in unique_recommendations if any(x in r.lower() for x in ['critical', 'immediate', 'urgent'])]
...
def main():
    ...
    parser.add_argument("--test", choices=["fixation", "timeout", "concurrent", "rotation", "leakage"], help="Run specific test")

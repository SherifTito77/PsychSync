...
This is an automated message from the PsychSync Test Execution System
            ...
            notification_file = Path("test_reports") / self.results['execution_id'] / "notification.txt"
    ...
    def _display_results(self):
        ...
        summary = self.results['summary']
        performance = self.results.get('analytics', {}).get('performance', {}).get('summary', {})
        security = self.results.get('analytics', {}).get('security', {}).get('security_scores', {})
        ...
        # Performance metrics
        perf_data = [
            ["Avg Performance Score", f"{performance.get('avg_performance_score', 0):.1f}/100"],
        ...
        print("\n🚀 PERFORMANCE METRICS")
        print(tabulate.tabulate(perf_data, headers=["Metric", "Value"], tablefmt="grid"))
        ...
        print("\n🔒 SECURITY METRICS")
        print(tabulate.tabulate(security_data, headers=["Metric", "Value"], tablefmt="grid"))
        ...
        print(f"📊 Detailed reports available in: test_reports/{self.results['execution_id']}/")
        print("=" * 60)
...
Examples:
  ...
  python run_integration_tests.py --output custom_reports
    ...
    parser.add_argument(
        '--timeout',
        type=int,
        help='Test execution timeout in minutes',
        default=30
    )
...
async def main():
    ...
    if not HAS_DEPS:
        ...
        print("Install with: pip install pytest tabulate matplotlib seaborn pandas plotly")
        return 1

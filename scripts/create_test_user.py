...
def create_organization(db: Session, name: str) -> Organization:
    ...
    org = Organization(name=name, created_at=datetime.utcnow())
    db.add(org)
    db.commit()
    db.refresh(org)
...
def create_team(
    db: Session, name: str, organization_id: int, created_by_id: int
) -> Team:
    """Create test team"""
    team = Team(
        name=name,
...
def add_team_member(
    db: Session, team_id: int, user_id: int, role: TeamRole = TeamRole.MEMBER
):
    """Add user to team"""
    member = TeamMember(
...
def create_complete_test_environment():
    ...
    try:
        ...
        dev_team = create_team(
            db, name="Development Team", organization_id=org1.id, created_by_id=admin.id
        )

        design_team = create_team(
            db, name="Design Team", organization_id=org1.id, created_by_id=admin.id
        )

        startup_team = create_team(
            db, name="Core Team", organization_id=org2.id, created_by_id=admin2.id
        )
        ...
        logger.info(
            "    Devs:    dev1@techstartup.com - dev3@techstartup.com / Dev123!"
        )

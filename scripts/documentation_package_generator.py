    ...
    async def generate_api_documentation(self) -> Dict[str, Any]:
        ...
        if api_spec:
                ...
                for path, path_item in api_spec["paths"].items():
                    for method, operation in path_item.items():
                        if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                            endpoint_doc = await self._generate_endpoint_documentation(
                                path, method, operation, api_spec
                            )
            ...
            return {
                ...
                "issues": [
                    "Could not retrieve OpenAPI specification - is the application running?"
                ],
            }
    ...
    async def generate_developer_documentation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "functions_analyzed": sum(
                len(analysis["functions"]) for analysis in code_analysis
            ),
            "docstring_coverage": (
                sum(analysis["docstring_coverage"] for analysis in code_analysis)
                / len(code_analysis)
                if code_analysis
                else 0
            ),
        ...
        for category, endpoints in categories.items():
            ...
            for path, methods in endpoints.items():
                ...
                for method, operation in methods.items():
                    if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                        category_doc += self._format_endpoint_doc(
...
"""
    ...
    async def _analyze_deployment_setup(self) -> Dict[str, Any]:
        ...
        if os.path.exists(k8s_dir):
            ...
            deployment_analysis["environments"] = [
                f
                for f in os.listdir(k8s_dir)
                if os.path.isfile(os.path.join(k8s_dir, f))
            ]
    ...
    async def _analyze_performance_optimization(self) -> List[Dict[str, Any]]:
        ...
        # Check for caching
        cache_files = ["app/core/enhanced_cache.py", "app/core/cache.py"]
        ...
        db_optimizations = ["app/core/database_optimization.py", "alembic/versions/"]
...
"""
    ...
    async def calculate_documentation_metrics(self) -> DocumentationMetrics:
        ...
        api_coverage = (
            (endpoints_documented / total_endpoints * 100) if total_endpoints > 0 else 0
        )
    ...
    def _calculate_documentation_quality_score(
        ...
        api_coverage = (
            (metrics.api_endpoints_documented / metrics.total_api_endpoints * 100)
    ...
    def _generate_documentation_recommendations(
        ...
        if metrics.docstring_coverage < 80:
            recommendations.append(
                f"Improve docstring coverage: {metrics.docstring_coverage:.1f}% (target: >80%)"
            )
        ...
        if metrics.total_api_endpoints > 0:
            ...
            if api_coverage < 90:
                recommendations.append(
                    f"Document more API endpoints: {api_coverage:.1f}% covered (target: >90%)"
                )

        if metrics.examples_count < 50:
            recommendations.append(
                f"Add more code examples: {metrics.examples_count} examples (target: >50)"
            )

        if metrics.total_sections < 30:
            recommendations.append(
                f"Expand documentation sections: {metrics.total_sections} sections (target: >30)"
            )
...
"""
    ...
    async def _generate_component_documentation(
        self, architecture_analysis: Dict
    ) -> List[Dict[str, Any]]:
        """Generate component documentation"""
        components = []
    ...
    async def _generate_troubleshooting_guides_content(
        self, issue_analysis: List[Dict]
    ):
...
async def main():
    ...
    try:
        ...
        print(
            f"   Functions Documented: {metrics['documented_functions']}/{metrics['total_functions']}"
        )
        print(f"   Docstring Coverage: {metrics['docstring_coverage']:.1f}%")
        print(
            f"   API Endpoints Documented: {metrics['api_endpoints_documented']}/{metrics['total_api_endpoints']}"
        )
        ...
        if doc_results["api_documentation"]["openapi_spec_available"]:
            print(
                f"   ✅ API Documentation: {doc_results['api_documentation']['total_endpoints']} endpoints"
            )
        else:
            print(f"   ⚠️  API Documentation: OpenAPI spec not available")

        print(
            f"   ✅ Architecture Documentation: {doc_results['architecture_documentation']['components_documented']} components"
        )
        print(
            f"   ✅ Developer Documentation: {doc_results['developer_documentation']['functions_analyzed']} functions analyzed"
        )
        print(
            f"   ✅ Deployment Documentation: {len(doc_results['deployment_documentation']['deployment_analysis']['methods'])} methods"
        )
        print(
            f"   ✅ User Guides: {doc_results['user_guides']['features_documented']} features"
        )
        print(
            f"   ✅ Security Documentation: {doc_results['security_documentation']['security_implementations']} implementations"
        )
        print(
            f"   ✅ Performance Documentation: {doc_results['performance_documentation']['optimizations_documented']} optimizations"
        )
        print(
            f"   ✅ Troubleshooting Guides: {doc_results['troubleshooting_guides']['issues_documented']} common issues"
        )

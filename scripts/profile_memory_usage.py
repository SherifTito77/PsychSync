...
class MemoryProfiler:
    ...
    def start_profiling(self):
        ...
        print(f"✅ Memory profiling started. Baseline: {self.baseline_memory['rss_mb']:.1f} MB")
    ...
    def take_snapshot(self, label: str = None):
        ...
        # Add memory delta
        snapshot['memory']['delta_rss_mb'] = current_memory['rss_mb'] - self.baseline_memory['rss_mb']
        snapshot['memory']['delta_vms_mb'] = current_memory['vms_mb'] - self.baseline_memory['vms_mb']

        self.snapshots.append(snapshot)

        print(f"📸 Memory snapshot '{label}': {current_memory['rss_mb']:.1f} MB "
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'memory_growth': {
                'total_growth_mb': self.snapshots[-1]['memory']['rss_mb'] - self.baseline_memory['rss_mb'],
                'peak_growth_mb': max(s['memory'].get('delta_rss_mb', 0) for s in self.snapshots)
            },
        ...
        # Add memory analysis
        report['analysis'] = {
            'has_memory_leak': report['memory_growth']['total_growth_mb'] > 100,  # >100MB growth
    ...
    def _calculate_growth_rate(self) -> float:
        ...
        growth = self.snapshots[-1]['memory']['rss_mb'] - self.snapshots[0]['memory']['rss_mb']
    ...
    def save_report(self):
        ...
        print(f"  Growth Rate: {report['analysis']['growth_rate_per_minute']:.1f} MB/min")
        print(f"  Potential Memory Leak: {'⚠️ YES' if report['analysis']['has_memory_leak'] else '✅ NO'}")

        if report['tracemalloc_stats'].get('top_allocations'):
            print("\n🔝 Top Memory Allocations:")
            for i, alloc in enumerate(report['tracemalloc_stats']['top_allocations'][:3], 1):
                print(f"  {i}. {alloc['filename']}:{alloc['line']} - {alloc['size_mb']:.1f} MB ({alloc['count']} objects)")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description="Memory profiling for PsychSync application")
    parser.add_argument('--output', '-o', default='memory-profile.json',

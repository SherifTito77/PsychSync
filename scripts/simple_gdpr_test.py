...
def test_gdpr_endpoints():
    ...
    for endpoint in gdpr_endpoints:
        try:
            ...
            print(
                f"{status_icon} {endpoint}: HTTP {response.status_code} ({len(response.content)} bytes)"
            )
    ...
    try:
        ...
        response = requests.get(
            f"{base_url}/api/v1/gdpr/data-summary-public", timeout=10
        )
    ...
    try:
        ...
        response = requests.post(
            f"{base_url}/api/v1/cookies/consent", json=consent_data, timeout=10
        )

...
def analyze_css_for_responsive_patterns() -> Dict:
    ...
    for css_file in css_files:
        try:
            ...
            responsive_indicators["flexbox_usage"] += len(
                re.findall(r"display:\s*flex", content)
            )
            responsive_indicators["grid_usage"] += len(
                re.findall(r"display:\s*grid", content)
            )

            # Check for responsive units
            responsive_indicators["responsive_units"] += len(
                re.findall(r"(vw|vh|vmin|vmax|rem|em)", content)
            )

            # Look for problematic patterns
            if re.search(r"overflow:\s*auto", content):
                responsive_indicators["issues"].append(
                    f"Potential overflow in {css_file.name}"
                )

        except Exception as e:
            responsive_indicators["issues"].append(
                f"Error analyzing {css_file.name}: {e}"
            )

    responsive_indicators["mobile_breakpoints"] = sorted(
        list(responsive_indicators["mobile_breakpoints"])
    )
        ...
        if page_result["loads_successfully"]:
            try:
                ...
                mobile_analysis = {
                    ...
                    "has_responsive_grid": "grid-cols-" in content
                    or "md:grid-cols-" in content,
                ...
                page_result["mobile_score_percent"] = round(
                    (mobile_score / max_score) * 100, 1
                )
...
def generate_recommendations(critical_pages: Dict, css_analysis: Dict) -> List[Dict]:
    ...
    all_pages_good = all(
        data.get("mobile_score_percent", 0) >= 80 for data in critical_pages.values()
    )
    if all_pages_good:
        recommendations.append(
            {

...
def print_results(results):
    ...
    print(f"   Used Services:  {results['used_count']} ({100*results['used_count']//results['total']}%)")
    print(f"   Unused Services: {results['unused_count']} ({100*results['unused_count']//results['total']}%)")
    print()

    if results['used']:
        print(f"✅ USED SERVICES ({results['used_count']})")
        for service, info in sorted(results['used'].items(), key=lambda x: x[1]['usage_count'], reverse=True):

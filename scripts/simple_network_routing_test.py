...
class SimplifiedNetworkRoutingLeakTester:
    ...
    def test_basic_network_exposure(self) -> Dict[str, Any]:
        ...
        dev_exposed = [
            p for p in result["open_ports"] if p["port"] in dev_ports and p["open"]
        ]
        if dev_exposed:
                ...
                try:
                    ...
                    if "proxy_pass" in content and "proxy_set_header" not in content:
                        result["security_issues"].append(
                            f"Proxy pass without security headers in {routing_file}"
                        )

                    if "--net=host" in content:
                        result["security_issues"].append(
                            f"Host networking mode used in {routing_file}"
                        )

                    if "bridge" in content and "isolated" not in content:
                        result["security_issues"].append(
                            f"Default bridge network used in {routing_file}"
                        )

                    # Check for exposed ports in Docker configs
                    if '"8000:8000"' in content or '"5173:5173"' in content:
                        result["security_issues"].append(
                            f"Development ports exposed in {routing_file}"
                        )

                    # Check for localhost references
                    if "localhost" in content and routing_file.endswith(
                        (".yml", ".yaml")
                    ):
                        result["security_issues"].append(
                            f"Localhost references in {routing_file}"
                        )

                except Exception as e:
                    result["security_issues"].append(
                        f"Error analyzing {routing_file}: {e}"
                    )
            ...
            if file_path.exists():
                ...
                try:
                    ...
                    leak_patterns = [
                        (
                            r"print\(.*request\.",
                            "Debug routing exposes request information",
                        ),
                        (r"logging\.debug.*request", "Debug logging exposes requests"),
                        (
                            r"traceback.*=.*True",
                            "Tracebacks expose application internals",
                        ),
                    ...
                    for pattern, description in leak_patterns:
                        if re.search(pattern, content, re.IGNORECASE):
                            result["leak_vulnerabilities"].append(
                                f"Information leak in {app_file}: {description}"
                            )

                except Exception as e:
                    result["leak_vulnerabilities"].append(
                        f"Error analyzing {app_file}: {e}"
                    )

        result["vulnerable"] = len(result["leak_vulnerabilities"]) > 0
        result["risk_level"] = (
            "HIGH" if len(result["leak_vulnerabilities"]) > 2 else "MEDIUM"
        )

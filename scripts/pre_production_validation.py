...
class PreProductionValidator:
        ...
        try:
            ...
            for ts_file in frontend_path.rglob('**/*.ts'):
                ...
                with open(ts_file, 'r', encoding='utf-8', errors='ignore') as f:
                    content = f.read()
                    count = sum(content.count(pattern) for pattern in
                              ['console.log', 'console.debug', 'console.info',
                               'console.warn', 'console.error'])
                    if count > 0:
                        console_count += count
                        files_with_consoles.append((str(ts_file.relative_to(frontend_path)), count))
            ...
            if console_count == 0:
                ...
                check.details = [
                    ...
                    *[f"  - {file}: {count}" for file, count in files_with_consoles[:10]]
                ]
            ...
            if 'Found 0 potential security bypasses' in output:
                ...
                if match:
                    ...
                    if bypass_count == 0:
                        ...
                        check.status = 'FAIL'
                        check.message = f"Found {bypass_count} debug mode security bypasses"
    ...
    def check_security_todos(self) -> None:
        ...
        try:
            ...
            critical_match = re.search(r'🔴 CRITICAL.*?\((\d+) items\)', output, re.DOTALL)
    ...
    def check_environment_config(self) -> None:
        ...
        if git_env.exists():
            issues.append(".env file exists in repository root (should be in .gitignore)")
                ...
                with open(config_file, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    for pattern, description in secret_patterns:
                        if re.search(pattern, content, re.IGNORECASE):
                            issues.append(f"{description} in {config_file.relative_to(self.root_path)}")

...
class AutomationSuite:
    ...
    def convert_to_playwright_automation(
        self, test_case: ManualTestCase
    ) -> AutomationScript:
        ...
        for step in test_case.steps:
            if "navigate" in step.lower():
                ...
                test_steps.append(
                    f"    await page.fill('[data-testid=\"input-field\"], '{step}');"
                )
            elif "click" in step.lower() or "submit" in step.lower():
                test_steps.append(
                    f'    await page.click(\'[data-testid="submit-button"];'
                )
        ...
        for result in test_case.expected_results:
            if "success" in result.lower():
                assertions.append(
                    f"    await expect(page.locator('[data-testid=\"success-message\"]')).toContainText('{result}');"
                )
            else:
                assertions.append(
                    f"    await expect(page.locator('[data-testid=\"result\"]')).toContainText('{result}');"
                )
  ...
  return data.text;
    ...
    def execute_complete_automation_suite(self):
        ...
        for capability, data in results["automation_capabilities"].items():
            ...
            print(
                f"{status_icon} {capability.replace('_', ' ').title()}: {data['status']}"
            )

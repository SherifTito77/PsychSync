...
def fix_exception_chains(content: str) -> tuple[str, int]:
    ...
    while i < len(lines):
        ...
        if re.search(r'except\s+\w+\s+as\s+\w+:', line):
            ...
            j = i + 1
            while j < len(lines) and not lines[j].strip().startswith('except') and not lines[j].strip().startswith('finally'):
                if 'raise HTTPException(' in lines[j] or 'raise ' in lines[j]:
                    ...
                    if match:
                        ...
                        if 'raise HTTPException(' in raise_line or 'raise ' in raise_line:
                            # Add 'from exc_var' before the closing parenthesis
                            if raise_line.rstrip().endswith(')'):
                                raise_line_fixed = raise_line.rstrip()[:-1] + f') from {exc_var}'
    ...
    def replacer1(match):
        ...
        if ' from ' not in exc_block:
            ...
            if last_paren != -1:
                ...
                fixed = exc_block[:last_paren] + f') from {exc_var}' + exc_block[last_paren+1:]
                fixes_count += 1
                return fixed
        return exc_block
...
def main():
    ...
    parser.add_argument('--all', action='store_true', help='Fix all files in app directory')

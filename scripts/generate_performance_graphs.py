...
class PerformanceGraphGenerator:
    ...
    def generate_response_time_graph(self, df: pd.DataFrame):
        ...
        axes[0, 0].plot(df['Timestamp'], df['Average Response Time'],
                       linewidth=2, alpha=0.8, color='blue')
        axes[0, 0].fill_between(df['Timestamp'], df['Average Response Time'],
        ...
        if '95%' in df.columns and '99%' in df.columns:
            axes[0, 1].plot(df['Timestamp'], df['95%'],
                           label='95th Percentile', linewidth=2)
            axes[0, 1].plot(df['Timestamp'], df['99%'],
                           label='99th Percentile', linewidth=2)
            axes[0, 1].plot(df['Timestamp'], df['Average Response Time'],
                           label='Average', linewidth=2, alpha=0.8)
        ...
        axes[1, 0].hist(df['Average Response Time'], bins=30, alpha=0.7,
                       color='skyblue', edgecolor='black')
        axes[1, 0].axvline(df['Average Response Time'].mean(),
        ...
        if 'Requests/s' in df.columns:
            scatter = axes[1, 1].scatter(df['Requests/s'], df['Average Response Time'],
                                       alpha=0.6, s=30, c=df['User Count'],
                                       cmap='viridis')
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/response_time_analysis.png",
    ...
    def generate_throughput_graph(self, df: pd.DataFrame):
        ...
        if 'Requests/s' in df.columns:
            axes[0, 0].plot(df['Timestamp'], df['Requests/s'],
                           linewidth=2, alpha=0.8, color='green')
            axes[0, 0].fill_between(df['Timestamp'], df['Requests/s'],
        ...
        if 'Total Requests' in df.columns:
            axes[0, 1].plot(df['Timestamp'], df['Total Requests'],
        ...
        if 'User Count' in df.columns and 'Requests/s' in df.columns:
            axes[1, 0].scatter(df['User Count'], df['Requests/s'],
                             alpha=0.7, s=40, color='purple')

            # Add trend line
            z = np.polyfit(df['User Count'], df['Requests/s'], 1)
            p = np.poly1d(z)
            axes[1, 0].plot(df['User Count'].sort_values(),
        ...
        if 'Failures/s' in df.columns:
            axes[1, 1].plot(df['Timestamp'], df['Failures/s'],
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/throughput_analysis.png",
                   dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def generate_error_analysis_graph(self, df: pd.DataFrame):
        ...
        if 'Error Count' in df.columns and 'Total Requests' in df.columns:
            ...
            axes[0, 0].plot(df['Timestamp'], error_rate,
                           linewidth=2, alpha=0.8, color='red')
            axes[0, 0].fill_between(df['Timestamp'], error_rate,
                                   alpha=0.3, color='red')
        ...
        if 'Total Requests' in df.columns and 'Error Count' in df.columns:
            success_rate = ((df['Total Requests'] - df['Error Count']) /
                          df['Total Requests'] * 100).fillna(100)
            axes[0, 1].plot(df['Timestamp'], success_rate,
                           linewidth=2, alpha=0.8, color='green')
            axes[0, 1].fill_between(df['Timestamp'], success_rate,
                                   alpha=0.3, color='green')
        ...
        if 'Error Count' in df.columns:
            axes[1, 0].bar(range(len(df)), df['Error Count'],
                          alpha=0.7, color='red')
        ...
        if 'Error Count' in df.columns:
            ...
            # Plot response time
            line1 = axes[1, 1].plot(df['Timestamp'], df['Average Response Time'],
                                  'b-', label='Response Time', linewidth=2, alpha=0.8)
            axes[1, 1].set_xlabel('Timestamp')
            ...
            line2 = ax2.plot(df['Timestamp'], df['Error Count'],
                           'r-', label='Error Count', linewidth=2, alpha=0.8)
    ...
    def generate_performance_summary_graph(self, df: pd.DataFrame):
        ...
        fig.suptitle('Performance Test Summary Dashboard', fontsize=18, fontweight='bold')

        # Key Metrics
        avg_response_time = df['Average Response Time'].mean()
        max_response_time = df['Average Response Time'].max()
        total_requests = df['Total Requests'].iloc[-1] if 'Total Requests' in df.columns else 0
        ...
        ax_response.plot(df['Timestamp'], df['Average Response Time'],
                        linewidth=2, color='blue', alpha=0.8)
        ax_response.fill_between(df['Timestamp'], df['Average Response Time'],
                                alpha=0.3, color='blue')
        ...
        if 'Requests/s' in df.columns:
            ...
            ax_throughput.plot(df['Timestamp'], df['Requests/s'],
                             linewidth=2, color='green', alpha=0.8)
            ax_throughput.fill_between(df['Timestamp'], df['Requests/s'],
                                     alpha=0.3, color='green')
        ...
        if 'Error Count' in df.columns and 'Total Requests' in df.columns:
            ...
            error_rate = (df['Error Count'] / df['Total Requests'] * 100).fillna(0)
            ax_errors.plot(df['Timestamp'], error_rate,
                         linewidth=2, color='red', alpha=0.8)
            ax_errors.fill_between(df['Timestamp'], error_rate,
                                 alpha=0.3, color='red')
        ...
        ax_hist.hist(df['Average Response Time'], bins=20, alpha=0.7,
                    color='skyblue', edgecolor='black')
        ax_hist.axvline(avg_response_time, color='red', linestyle='--',
                       linewidth=2, label=f"Mean: {avg_response_time:.2f}ms")
        ...
        if 'User Count' in df.columns and 'Requests/s' in df.columns:
            ...
            heatmap_data = df.groupby([user_bins, rps_bins])['Average Response Time'].mean().unstack()

            sns.heatmap(heatmap_data, annot=True, fmt=".1f", cmap="YlOrRd",
                       ax=ax_heatmap, cbar_kws={'label': 'Response Time (ms)'})
        ...
        plt.savefig(f"{self.output_dir}/performance_summary_dashboard.png",
                   dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _generate_query_performance_graph(self, query_data: List[Dict]):
        ...
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle('Database Query Performance Analysis', fontsize=16, fontweight='bold')

        # Query Response Times
        response_times = [q['avg_time_ms'] for q in query_data]
        query_names = [q['description'][:30] + '...' if len(q['description']) > 30 else q['description']
                      for q in query_data]
        ...
        for metric in metrics:
            if all(metric in q for q in query_data):
                ...
                axes[0, 1].plot(values, label=metric.replace('_', ' ').title(), linewidth=2, marker='o')
        ...
        sorted_indices = sorted(range(len(performance_score)), key=lambda i: performance_score[i], reverse=True)
        sorted_scores = [performance_score[i] for i in sorted_indices]
        sorted_names = [query_data[i]['description'][:25] + '...' if len(query_data[i]['description']) > 25 else query_data[i]['description']
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/query_performance_analysis.png",
                   dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _generate_concurrent_access_graph(self, concurrent_data: List[Dict]):
        ...
        fig.suptitle('Concurrent Access Performance Analysis', fontsize=16, fontweight='bold')

        # Throughput vs Thread Count
        axes[0].plot(df['num_threads'], df['throughput_qps'],
        ...
        axes[1].plot(df['num_threads'], df['avg_time_ms'],
                    marker='s', linewidth=2, markersize=8, color='red')
        axes[1].plot(df['num_threads'], df['p95_time_ms'],
                    marker='^', linewidth=2, markersize=8, color='orange', label='P95')
        ...
        if 'p99_time_ms' in df.columns:
            axes[2].plot(df['num_threads'], df['p99_time_ms'],
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/concurrent_access_analysis.png",
                   dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def _generate_write_performance_graph(self, write_data: List[Dict]):
        ...
        axes[0].plot(df['batch_size'], df['avg_time_ms'],
        ...
        axes[1].plot(df['batch_size'], df['throughput_records_per_second'],
        ...
        plt.tight_layout()
        plt.savefig(f"{self.output_dir}/write_performance_analysis.png",
                   dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    parser.add_argument('--output-dir', default='performance_graphs',

...
class ConsoleLogRemover:
    ...
    def has_logger_import(self, content: str) -> bool:
        ...
        return bool(re.search(r"import\s+logger.*from.*['\"@/utils/logger'\"]", content))
    ...
    def create_comment(self, content: str, line_num: int, original: str) -> str:
        ...
        return f"// TODO(human): Review and convert to appropriate logger method\n" \
               f"// Original: {original}\n"
    ...
    def replace_simple_console(self, content: str, matches) -> Tuple[str, int]:
        ...
        conversions = content.count('console.error') + content.count('console.warn') - \
                     new_content.count('console.error') - new_content.count('console.warn')
        ...
        for file_path in files:
            ...
            if result['modified'] or result.get('needs_manual_review'):
                ...
                if result.get('logs_removed'):
                    print(f"   ├─ Auto-converted: {result['logs_removed']} console statements")
                if result.get('remaining_consoles'):
                    print(f"   ├─ Needs manual review: {result['remaining_consoles']} statements")
    ...
    def print_summary(self):
        ...
        needs_review = sum(1 for r in self.__dict__.get('_results', []) if r.get('needs_manual_review'))
        if needs_review > 0:
            print(f"\n⚠️  {needs_review} files need manual review for complex console statements")
            print("   These files have console statements that couldn't be auto-converted")
...
def main():
    ...
    parser.add_argument(
        ...
        help='Path to scan (default: frontend/src)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',

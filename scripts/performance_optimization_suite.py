...
class PerformanceOptimizer:
    ...
    async def analyze_frontend_performance(self):
        ...
        try:
            ...
            async with self.session.get(self.frontend_url) as response:
                ...
                if inline_styles > 5:
                    recommendations.append(
                        "Move inline styles to external stylesheets for better caching"
                    )
        ...
        try:
            ...
            for endpoint in endpoints:
                ...
                try:
                    async with self.session.get(
                        ...
                        metrics[
                            f"{endpoint.replace('/', '_').replace('-', '_')}_time"
            ...
            if response_times:
                ...
                if max_response_time > 5.0:
                    recommendations.append(
                        "Some endpoints are slow - optimize database queries and implement caching"
                    )

                if len(response_times) < len(endpoints):
                    recommendations.append(
                        "Some endpoints are failing - check API configuration"
                    )

                details = (
                    f"API endpoints: {len(response_times)}/{len(endpoints)} responding"
                )
    ...
    async def analyze_memory_usage(self):
        ...
        try:
            ...
            metrics["system_memory_used_gb"] = round(
                system_memory.used / 1024 / 1024 / 1024, 2
            )
            ...
            if metrics["process_memory_mb"] > 1000:  # > 1GB
                recommendations.append(
                    "Process memory usage is high - check for memory leaks"
                )

            if metrics["system_memory_percent"] > 80:
                recommendations.append(
                    "System memory usage is high - consider freeing up memory"
                )
    ...
    def analyze_code_quality(self):
        ...
        try:
            ...
            if metrics["backend_lines_of_code"] > 30000:
                recommendations.append(
                    "Backend codebase is large - consider microservices for better scalability"
                )
            ...
            if len(test_files) < len(backend_py_files) // 2:
                recommendations.append(
                    "Consider increasing test coverage for better code quality"
                )

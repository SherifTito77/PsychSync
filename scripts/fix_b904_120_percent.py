...
def check_syntax_detailed(file_path: str) -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        return False, [f"Line {e.lineno}: {e.msg}", f"  {e.text if e.text else ''}"]
    ...
    try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                if len(node.body) > 50:
                    improvements.append(
                        f"Function {node.name} is very long ({len(node.body)} statements)"
                    )
...
def validate_fix(file_path: str) -> Dict[str, any]:
    ...
    result = subprocess.run(
        ["ruff", "check", file_path, "--select", "B904"], capture_output=True, text=True
    )
    ...
    validation["can_commit"] = validation["syntax_ok"] and validation["b904_count"] == 0

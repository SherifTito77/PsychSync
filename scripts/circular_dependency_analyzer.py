...
class CircularDependencyAnalyzer:
    ...
    def build_module_map(self):
        ...
        for file_path in python_files:
            ...
            if file_path.name == "__init__.py":
                ...
                module_name = (
                    ".".join(module_parts) if module_parts else file_path.parent.name
                )
            else:
        ...
        try:
            ...
            for rel_imp in visitor.relative_imports:
                ...
                if rel_imp == ".":
                    ...
                    level = rel_imp.count(".")
                    base_parts = (
                        current_module_parts[:-level]
                        if level > 0
                        else current_module_parts
                    )
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"Total Dependencies Found: {sum(len(deps) for deps in self.dependency_graph.values())}"
        )
        report.append("")
        ...
        if self.circular_dependencies:
            ...
            for i, cycle in enumerate(self.circular_dependencies, 1):
                ...
                if len(cycle) == 2:
                    ...
                    solutions.append(
                        "   - Consider moving shared functionality to a new module"
                    )

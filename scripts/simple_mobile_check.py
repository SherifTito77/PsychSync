        ...
        try:
            ...
            for mq in media_queries:
                responsive_patterns["media_queries"].append(
                    {"file": str(css_file), "query": mq.strip()}
                )
...
def analyze_react_components() -> Dict:
    ...
    for tsx_file in tsx_files:
        try:
            ...
            if fixed_widths:
                issues.extend(
                    [f"Fixed width found: {width}" for width in fixed_widths[:3]]
                )
            ...
            if small_buttons:
                issues.extend(
                    [f"Small touch target: {size}px" for _, size in small_buttons]
                )
...
def generate_mobile_report() -> Dict:
    ...
    responsive_components = sum(
        1 for comp in component_analysis.values() if comp["has_responsive_logic"]
    )
    mobile_responsive_rate = (
        (responsive_components / total_components * 100) if total_components > 0 else 0
    )

    # Count issues
    total_issues = sum(
        len(comp["potential_issues"]) for comp in component_analysis.values()
    )
    components_with_issues = sum(
        1 for comp in component_analysis.values() if comp["potential_issues"]
    )
    ...
    problematic_components = sorted(
        [
            (name, comp)
            for name, comp in component_analysis.items()
    ...
    good_examples = sorted(
        [
            (name, comp)
            for name, comp in component_analysis.items()
            if comp["has_responsive_logic"] and not comp["potential_issues"]
        ],
        key=lambda x: len(x[1]["uses_responsive_classes"])
        + len(x[1]["uses_responsive_hooks"]),
        reverse=True,
    )[:5]
    ...
    report = {
        "summary": {
            ...
            "css_media_queries": len(
                css_analysis["responsive_patterns"]["media_queries"]
            ),
...
def print_report(report: Dict) -> None:
    ...
    print(
        f"✅ Mobile-Responsive: {summary['responsive_components']} ({summary['mobile_responsive_rate']}%)"
    )

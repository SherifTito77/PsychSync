...
class LoadTestingSuite:
    ...
    async def test_api_endpoints_load(self) -> None:
        ...
        for endpoint in endpoints:
            ...
            if result.average_response_time > 1000:  # 1 second
                print(
                    f"⚠️  WARNING: {endpoint['path']} has high average response time: {result.average_response_time:.2f}ms"
                )
            else:
                print(
                    f"✅ {endpoint['path']} - {result.requests_per_second:.1f} RPS, {result.average_response_time:.2f}ms avg"
                )
    ...
    async def test_concurrent_user_load(self) -> None:
        ...
        for user_count in user_counts:
            ...
            result = await self.simulate_concurrent_users(
                user_count=user_count, duration_seconds=60
            )
            ...
            if result.error_rate > 5.0:  # 5% error rate threshold
                print(
                    f"⚠️  HIGH ERROR RATE: {result.error_rate:.2f}% at {user_count} users"
                )
            elif result.requests_per_second < user_count * 2:  # Minimum 2 RPS per user
                print(
                    f"⚠️  LOW THROUGHPUT: {result.requests_per_second:.1f} RPS for {user_count} users"
                )
            else:
                print(
                    f"✅ {user_count} users - {result.requests_per_second:.1f} RPS, {result.error_rate:.2f}% errors"
                )
    ...
    async def test_database_performance(self) -> None:
        ...
        result = await self.test_database_operations_load(
            operation_type="create", concurrent_operations=50, duration_seconds=30
        )
        self.test_results.append(result)

        # Test data querying load
        result = await self.test_database_operations_load(
            operation_type="query", concurrent_operations=100, duration_seconds=30
        )
        self.test_results.append(result)

        # Test complex analytics queries
        result = await self.test_database_operations_load(
            operation_type="analytics", concurrent_operations=20, duration_seconds=30
        )
        self.test_results.append(result)

        print(
            f"✅ Database operations tested - Create: {self.test_results[-3].average_response_time:.2f}ms, "
    ...
    async def test_authentication_load(self) -> None:
        ...
        print(
            f"✅ Authentication load tested - Login: {self.test_results[-2].average_response_time:.2f}ms, "
    ...
    async def test_assessment_system_load(self) -> None:
        ...
        print(
            f"✅ Assessment system tested - Creation: {self.test_results[-3].average_response_time:.2f}ms, "
    ...
    async def test_stress_conditions(self) -> None:
        ...
        while current_users <= max_concurrent_users:
            ...
            result = await self.simulate_concurrent_users(
                user_count=current_users, duration_seconds=30
            )
            ...
            if result.error_rate > 10.0:  # 10% error rate threshold
                ...
                print(
                    f"⚠️  HIGH RESPONSE TIME at {current_users} users: {result.average_response_time:.2f}ms"
                )
                break
            else:
    ...
    async def test_spike_load(self) -> None:
        ...
        # Check recovery
        if (
            recovery_result.average_response_time
            <= baseline_result.average_response_time * 1.5
        ):
    ...
    async def test_endurance(self) -> None:
        ...
        result = await self.simulate_concurrent_users(
            user_count=concurrent_users, duration_seconds=duration_minutes * 60
        )
        ...
        if result.error_rate < 1.0 and result.average_response_time < 2000:
            print(
                f"✅ Endurance test passed - {result.requests_per_second:.1f} RPS over {duration_minutes} minutes"
            )
        else:
            print(
                f"⚠️  Performance degradation detected over {duration_minutes} minutes"
            )
    ...
    async def _continuous_requests(
        self, make_request_func, duration_seconds, request_interval
    ):
    ...
    async def simulate_concurrent_users(
        ...
        for _ in range(user_count):
            task = asyncio.create_task(
                self._continuous_user_session(user_session, duration_seconds)
            )
            tasks.append(task)
    ...
    async def test_database_operations_load(
        self, operation_type: str, concurrent_operations: int, duration_seconds: int
    ) -> LoadTestResult:
    ...
    def generate_performance_report(self, total_time: float) -> Dict[str, Any]:
        ...
        if self.system_metrics_history:
            avg_cpu = statistics.mean(
                [m.cpu_percent for m in self.system_metrics_history]
            )
            max_cpu = max([m.cpu_percent for m in self.system_metrics_history])
            avg_memory = statistics.mean(
                [m.memory_percent for m in self.system_metrics_history]
            )
        ...
        slow_endpoints = [
            r for r in self.test_results if r.average_response_time > 1000
        ]
        if slow_endpoints:
            print(f"├─ ⚠️  {len(slow_endpoints)} endpoints need optimization:")
            for endpoint in slow_endpoints:
                print(
                    f"   • {endpoint.test_name}: {endpoint.average_response_time:.2f}ms avg"
                )
        ...
        report_path = (
            f"load_testing_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )

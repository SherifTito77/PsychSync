...
def get_all_python_files():
    ...
    for root, dirs, files in os.walk("app"):
        ...
        dirs[:] = [
            d for d in dirs if d != "__pycache__" and d != ".venv" and d != "venv"
        ]
...
def print_results(results):
    ...
    print(
        f"   Used Services:  {results['used_count']} ({100*results['used_count']//results['total_services']}%)"
    )
    print(
        f"   Unused Services: {results['unused_count']} ({100*results['unused_count']//results['total_services']}%)"
    )
    print()

...
def test_mbti_frontend_browser():
    ...
    try:
        ...
        checks = {
            ...
            "Error message": "error" in page_source.lower()
            or "something went wrong" in page_source.lower(),
            "MBTI content": "MBTI" in page_source
            or "assessment" in page_source.lower(),
            "Questions": "question" in page_source.lower()
            or "At parties" in page_source,
            "Submit button": "submit" in page_source.lower()
            or "button" in page_source.lower(),
        }
...
def test_direct_api():
    ...
    try:
        ...
        response = requests.get(
            "http://localhost:8000/api/v1/assessment-questions/mbti"
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"📊 Question count: {len(data.get('assessment', {}).get('questions', []))}"
            )

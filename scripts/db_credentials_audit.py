...
class DBCredentialsAuditor:
    ...
    def check_environment_variables(self):
        ...
        env_files = [".env.dev", ".env.prod", ".env", ".env.example"]
                            ...
                            findings.append(
                                {
                                    ...
                                    "matches": matches,
                                    "risk_level": (
                                        "HIGH" if "password" in pattern else "MEDIUM"
                                    ),
                                }
                            )
            ...
            try:
                ...
                for pattern in config_patterns:
                    ...
                    if matches:
                        ...
                        filtered_matches = [
                            m for m in matches if not self.is_false_positive(m)
                        ]
                        ...
                        if filtered_matches:
                            findings.append(
                                {
                                    "file": str(
                                        config_file.relative_to(self.base_path)
                                    ),
                                    "pattern": pattern,
                            ...
                            print(
                                f"    ⚠️  Found credentials in {config_file.relative_to(self.base_path)}"
                            )
                ...
                for indicator in rotation_indicators:
                    if indicator in content:
                        ...
                        if any(ctx in content for ctx in credential_contexts):
                            ...
                            print(
                                f"    ✓ Found rotation evidence in {file_path.relative_to(self.base_path)}"
                            )
                            break
    ...
    def check_backup_security(self):
        ...
        backup_patterns = ["*.backup", "*.bak", "*.sql", "*.dump", "*.pg_dump"]
    ...
    def generate_recommendations(self, findings):
        ...
        hardcoded_creds = [
            f for f in findings if "file" in f and f.get("risk_level") == "HIGH"
        ]
        ...
        weak_connections = [
            f
            for f in findings
            if f.get("type") == "database_connection" and f.get("security_issue")
        ]
        if weak_connections:
            recommendations.append(
                {
        ...
        report = {
            ...
            "summary": {
                ...
                "high_risk_findings": len(
                    [f for f in all_findings if f.get("risk_level") == "HIGH"]
                ),
                "medium_risk_findings": len(
                    [f for f in all_findings if f.get("risk_level") == "MEDIUM"]
                ),
...
def main():
    ...
    try:
        ...
        # Save report
        with open(
            "/Users/sheriftito/Downloads/psychsync/db_credentials_audit_report.json",
            "w",
        ) as f:

...
class TestingExcellenceSuite:
    ...
    async def validate_test_environment(self) -> TestEnvironmentValidation:
        ...
        try:
            result = subprocess.run(
                [
                    sys.executable,
                    "-c",
                    'import asyncio; from app.core.database import get_db_session; asyncio.run(get_db_session().execute("SELECT 1"))',
                ],
                cwd=self.project_root,
                capture_output=True,
                timeout=30,
        ...
        # Check test data
        test_data_files = ["tests/conftest.py", "tests/fixtures/", "tests/factories.py"]
        env_validation.test_data_available = any(
            os.path.exists(os.path.join(self.project_root, f)) for f in test_data_files
        )
        ...
        env_validation.configuration_valid = any(
            os.path.exists(os.path.join(self.project_root, f)) for f in env_files
        )
        if not env_validation.configuration_valid:
            env_validation.issues.append(
                "Test environment configuration file not found"
            )
        ...
        try:
            ...
            for line in stdout_lines:
                if " passed in " in line:
                    parts = line.split()
                    if "passed" in parts:
                        integration_results["tests_passed"] = int(
                            parts[parts.index("passed") - 1]
                        )
                    if "failed" in parts:
                        integration_results["tests_failed"] = int(
                            parts[parts.index("failed") - 1]
                        )
                    if "in" in parts and "s" in parts[-1]:
                        integration_results["duration"] = float(
                            parts[-1].replace("s", "")
                        )
        ...
        for root, dirs, files in os.walk(os.path.join(self.project_root, "app")):
            for file in files:
                if file.endswith(".py") and not file.startswith("__"):
                    ...
                    if not test_file_path or not os.path.exists(
                        os.path.join(self.project_root, test_file_path)
                    ):
                        missing_tests.append(
                            {
                                "source_file": relative_path,
                                "suggested_test_file": test_file_path
                                or self._generate_test_file_path(relative_path),
                                "type": "missing_test_file",
                                "priority": (
                                    "HIGH"
                                    if "api/" in relative_path
                                    or "services/" in relative_path
                                    else "MEDIUM"
                                ),
                            }
                        )
                    ...
                    elif test_file_path and os.path.exists(
                        os.path.join(self.project_root, test_file_path)
                    ):
                        functions_missing_tests = self._find_untested_functions(
    ...
    async def generate_test_optimization_report(self) -> Dict[str, Any]:
        ...
        if not environment_validation.database_connection:
            critical_recommendations.append(
                "CRITICAL: Test database connection is not working"
            )
        ...
        if missing_tests:
            high_missing_tests = [
                t for t in missing_tests if t.get("priority") == "HIGH"
            ]
    ...
    def _get_coverage_from_object(self, cov) -> TestCoverageMetrics:
        ...
        try:
            data = cov.get_data()
            total_lines = sum(
                len(data.lines(filename)) for filename in data.measured_files()
            )
            covered_lines = sum(
                len(data.lines(filename) - set(data.arcs(filename)))
    ...
    def _parse_test_results(self, test_data: Dict) -> TestPerformanceMetrics:
        ...
        slowest_tests = sorted(
            [
                {"name": t.get("nodeid", ""), "duration": t.get("duration", 0)}
                for t in tests
            ],
        ...
        failed_test_details = [
            ...
            for t in tests
            if "call" in t and "longrepr" in t.get("call", {})
        ]
    ...
    def _calculate_test_performance_score(
        ...
        # Slow test penalty
        if (
            metrics.slowest_tests and metrics.slowest_tests[0]["duration"] > 10
        ):  # 10 seconds
    ...
    def _calculate_environment_score(
        self, validation: TestEnvironmentValidation
    ) -> float:
    ...
    try:
        ...
        if report["missing_tests"]:
            ...
            for missing in report["missing_tests"][:3]:
                print(
                    f"   • {missing['source_file']} → {missing.get('suggested_test_file', missing.get('test_file', ''))}"
                )

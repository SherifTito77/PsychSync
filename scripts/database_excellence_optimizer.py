...
class DatabaseExcellenceOptimizer:
    ...
    def __init__(self):
        self.db_url = getattr(settings, "DATABASE_URL", "").replace(
            ...
            try:
                ...
                for query_data in slow_queries:
                    ...
                    optimization_suggestions = (
                        self._generate_query_optimization_suggestions(
                            query_text, mean_time, rows_returned, hit_percent
                        )
                    )
                    performance_grade = self._calculate_performance_grade(
                        mean_time, hit_percent
                    )
                        ...
                        missing_indexes.append(
                            {
                                ...
                                "estimated_impact": (
                                    "HIGH"
                                    if primary_col["selectivity"] > 100
                                    else "MEDIUM"
                                ),
    ...
    async def apply_automatic_optimizations(self) -> Dict[str, Any]:
        ...
        try:
            async with self.connection_pool.acquire() as conn:
                ...
                await self._create_critical_indexes(
                    conn, optimizations_applied, warnings
                )
    ...
    def _generate_query_optimization_suggestions(
        ...
        if hit_percent < 90:
            suggestions.append(
                "Low buffer cache hit ratio - query may be reading too many disk pages"
            )

        if rows == 0 and exec_time > 100:
            suggestions.append(
                "Empty result set taking too long - check query logic and indexes"
            )
    ...
    def _extract_index_usage(self, query_text: str) -> List[str]:
        ...
        if "Index Scan" in query_text:
            ...
            index_matches = re.findall(
                r"Index Scan.*?using\s+(\w+)", query_text, re.IGNORECASE
            )
            indexes.extend(index_matches)
        return indexes

    def _generate_index_recommendation(
        self, usage_count: int, size_mb: float, efficiency: float
    ) -> str:
    ...
    def _get_overall_grade(
        self, performance_score: int, optimization_score: int
    ) -> str:
    ...
    async def _optimize_postgres_settings(self, conn, optimizations: List[str]):
        ...
        optimizations.append(
            "Generated PostgreSQL performance settings recommendations"
        )

    async def _create_critical_indexes(
        self, conn, optimizations: List[str], warnings: List[str]
    ):
    ...
    try:
        ...
        if report["slow_queries_analysis"]:
            ...
            for i, query in enumerate(report["slow_queries_analysis"][:5], 1):
                print(
                    f"   {i}. Grade {query['performance_grade']} - {query['execution_time']:.2f}ms"
                )

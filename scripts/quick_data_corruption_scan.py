...
def run_grep(pattern, path, include_pattern="*.py"):
    ...
    try:
        cmd = [
            'grep', '-r', '-n', '--include=' + include_pattern,
            pattern, path
        ]
...
def analyze_database_operations():
    ...
    add_delete_lines = run_grep(r'(db\.add\(|db\.delete\(|session\.add\(|session\.delete\()', app_dir)
    ...
    file_operations = defaultdict(lambda: {
        'add_delete': [],
        'commit': [],
        'rollback': [],
        'try_except': False,
    ...
    for line in update_files:
        if line:
            filepath = line.split(':', 1)[0]
            if filepath not in file_operations or not file_operations[filepath]['for_update']:
    ...
    for severity in ['CRITICAL', 'HIGH', 'MEDIUM', 'LOW']:
        ...
        for idx, issue in enumerate(issues[severity][:20], 1):  # Limit to 20 per severity
            rel_path = issue['file'].replace('/Users/sheriftito/Downloads/psychsync/', '')
            report_lines.append(f"\n### {idx}. {rel_path}")

...
class MBTIUserFlowTest:
    ...
    def test_assessment_page_access(self):
        ...
        try:
            ...
            self.log_action(
                "✅ User visits MBTI assessment page", f"Status: {response.status_code}"
            )
    ...
    def simulate_question_progression(self):
        ...
        questions = [
            ...
            },
            {
                ...
                "options": {
                    "T": "Focus on facts and improvements",
        ...
        for i, question in enumerate(questions):
            ...
            self.log_action(f"📖 Question {i + 1}/8: {question['text'][:50]}...")
        ...
        self.log_action(
            "📊 User completed all questions", f"Total time: {total_time:.1f} seconds"
        )
    ...
    def simulate_submit_button_click(self):
        ...
        self.log_action(
            "🧠 Client-side calculation", f"Calculated type: {calculated_type}"
        )
        ...
        self.log_action(
            "📤 Sending to backend API", f"URL: {self.backend_url}/mbti-test-submit"
        )
    ...
    def analyze_results(self, results: Dict[str, Any]):
        ...
        self.log_action(
            "📊 Confidence",
            f"Score: {results.get('confidence', 0):.2f} ({results.get('confidence', 0)*100:.0f}%)",
        )
        self.log_action(
            "📊 Description", f"Personality: {results.get('description', 'N/A')}"
        )
        ...
        if scoring:
            ...
            self.log_action(
                f"   • Total Questions: {scoring.get('total_questions', 0)}"
            )
            ...
            if dim_scores:
                ...
                for dim, scores in dim_scores.items():
                    self.log_action(
                        f"     - {dim}: E={scores.get('E', 0)}, I={scores.get('I', 0)}"
                    )

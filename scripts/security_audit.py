...
class SecurityAudit:
    ...
    def check_xss_vulnerabilities(self):
        ...
        if vulnerable_files:
            ...
            logger.warning(
                f"⚠️  Found {len(vulnerable_files)} potential XSS vulnerabilities"
            )
        else:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                try:
                    ...
                    logger.warning(
                        f"⚠️  Found {len(vulnerable_packages)} vulnerable dependencies"
                    )
    ...
    def check_authentication(self):
        ...
        for py_file in self.project_root.rglob("*.py"):
            try:
                ...
                auth_endpoints += len(
                    re.findall(r'@router\.post\(["\']/(login|register|auth)', content)
                )
                auth_endpoints += len(
                    re.findall(r"def (login|register|authenticate)", content)
                )
            except Exception:
                pass
    ...
    def check_csrf_protection(self):
        ...
        if csrf_count > 0:
            ...
            self.issues["medium"].append(
                {"check": "CSRF Protection", "message": "No CSRF protection found"}
            )
    ...
    def check_sensitive_data_logs(self):
        ...
        if issues_found:
            ...
            logger.warning(
                f"⚠️  Found {len(issues_found)} potential sensitive data logs"
            )
        else:
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if self.issues["critical"]:
            recommendations.append(
                "🚨 CRITICAL: Address all critical security issues immediately before production deployment"
            )
        ...
        if len(self.issues.get("SQL Injection", [])) > 0:
            recommendations.append(
                "💡 Use parameterized queries (SQLAlchemy) instead of string concatenation"
            )

        if len(self.issues.get("XSS Vulnerabilities", [])) > 0:
            recommendations.append(
                "💡 Install and use bleach library for HTML sanitization"
            )

        if len(self.issues.get("Hardcoded Secrets", [])) > 0:
            recommendations.append(
                "💡 Move all secrets to environment variables or secret manager (AWS Secrets Manager, HashiCorp Vault)"
            )

        if len(self.issues.get("Dependencies", [])) > 0:
            recommendations.append(
                "💡 Run 'pip install --upgrade <package>' to update vulnerable dependencies"
            )
...
def main():
    ...
    parser.add_argument(
        "--sql-only", action="store_true", help="Check SQL injection only"
    )
    parser.add_argument(
        "--dependencies", action="store_true", help="Check dependencies only"
    )

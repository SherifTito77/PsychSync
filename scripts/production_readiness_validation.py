...
class ProductionReadinessValidator:
                    ...
                    if len(parts) >= 3:
                        ...
                        self.results.append(
                            ValidationResult(
                                ...
                                status=(
                                    "PASS"
                                    if cpu_percent < 80 and mem_status == "PASS"
                                    else "WARN"
                                ),
                ...
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        if (
                            ...
                            and health_data["database"].get("status") == "healthy"
                        ):
                            self.results.append(
                                ValidationResult(
                                    category="Database",
    ...
    async def validate_external_apis(self):
        ...
        external_services = ["Stripe API", "SendGrid API", "Redis Cache"]
    ...
    def generate_final_report(self) -> Dict[str, Any]:
        ...
        pass_rate = (
            (status_counts["PASS"] / total_tests * 100) if total_tests > 0 else 0
        )
        ...
        for category, results in categories.items():
            ...
            report["results_by_category"][category] = {
                ...
                "pass_rate": (
                    round((category_pass / category_total * 100), 1)
                    if category_total > 0
                    else 0
                ),
                "tests": [
        ...
        report["recommendations"] = self.generate_recommendations(
            status_counts, categories
        )

        return report

    def generate_recommendations(
        self, status_counts: Dict, categories: Dict
    ) -> List[str]:
        ...
        if status_counts["FAIL"] > 0:
            recommendations.append(
                "🚨 CRITICAL: Address all failing tests before production deployment"
            )

        if status_counts["WARN"] > 5:
            recommendations.append(
                "⚠️ Review and address warnings to improve production readiness"
            )
        ...
        for category, results in categories.items():
            ...
            if fails:
                if category == "Security Headers":
                    recommendations.append(
                        "🔒 Implement missing security headers for improved security posture"
                    )
                elif category == "Performance":
                    recommendations.append(
                        "⚡ Optimize slow endpoints to meet performance requirements"
                    )
                elif category == "Database":
                    recommendations.append(
                        "🗄️ Address database connectivity or performance issues"
                    )
                elif category == "Authentication":
                    recommendations.append(
                        "🔐 Fix authentication issues to ensure proper access control"
                    )

        if (
            status_counts["PASS"]
            == status_counts["PASS"] + status_counts["FAIL"] + status_counts["WARN"]
        ):
            recommendations.append(
    ...
    def save_text_report(self, report: Dict[str, Any], filename: str):
        ...
        with open(filename, "w") as f:
            ...
            for category, data in report["results_by_category"].items():
                ...
                f.write(
                    f"  Pass Rate: {data['pass_rate']}% ({data['pass_count']}/{data['total_tests']})\n"
                )
...
async def main():
    ...
    try:
        ...
        if summary["overall_status"] == "FAIL":
            ...
            logger.warning(
                "⚠️ Production validation passed with warnings - review recommendations"
            )
            return 0
        else:

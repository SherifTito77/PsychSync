        ...
        for org in organizations:
            ...
            try:
                ...
                for component_name, component in cus.components.items():
                    ...
                    print(
                        f"   {trend_icon} {component_name.title():15} {component.score:5.1f}/100 "
                ...
                if "engagement" in cus.components:
                    ...
                    print(
                        f"   • DAU/MAU Ratio: {eng.metrics.get('dau_mau_ratio', 0):.2f}"
                    )
                    print(
                        f"   • Session Frequency: {eng.metrics.get('session_frequency', 0):.1f} assessments/user"
                    )
                    print(
                        f"   • Feature Breadth: {eng.metrics.get('feature_breadth', 0):.1%}"
                    )

                if "adoption" in cus.components:
                    ado = cus.components["adoption"]
                    print(
                        f"   • Activation Rate: {ado.metrics.get('activation_rate', 0):.1%}"
                    )
                    print(
                        f"   • Team Adoption: {ado.metrics.get('team_adoption_rate', 0):.1%}"
                    )
                    print(
                        f"   • Seat Utilization: {ado.metrics.get('seat_utilization', 0):.1%}"
                    )
...
async def demo_score_calculation_steps():
    ...
    async for db in get_async_db():
        ...
        assessments = await service._get_assessment_count(
            str(org.id), start_date, end_date
        )
        ...
        engagement_score = (
            (dau_mau_ratio * 40) + (min(session_frequency / 4, 1.0) * 30) + 30
        )

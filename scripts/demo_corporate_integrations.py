...
def demo_email_integration():
    ...
    from app.integrations.email_integration import EmailMetadata, EmailMetadataExtractor
    ...
    print(
        f"   Work-Life Imbalance Score: {signals['work_life_imbalance_score']:.2f} (0-1)"
    )
    print(
        f"   Communication Overload: {'⚠️ YES' if signals['communication_overload'] else '✅ NO'}"
    )
...
def demo_calendar_integration():
    ...
    print(
        f"   Average Meeting Hours/Day: {signals['avg_meeting_hours_per_day']:.2f} hours"
    )
...
def demo_slack_integration():
    ...
    from app.integrations.slack_integration import SlackMessage, SlackMetadataExtractor
    ...
    print(
        f"   Social Interaction Score: {signals['social_interaction_score']:.2f} (0-1)"
    )
    print(
        f"   After-Hours Percentage: {signals['after_hours_message_percentage']:.1f}%"
    )
...
def demo_unified_pipeline():
    ...
    print(
        f"   💼 Engagement: {engagement:.2f} {'✅ GOOD' if engagement > 0.7 else '⚠️ NEEDS ATTENTION'}"
    )
    print(f"   👥 Retention Risk: {retention:.2f}")
    print(
        f"   ⚖️  Work-Life Balance: {work_life:.2f} {'✅ BALANCED' if work_life > 0.6 else '⚠️ IMBALANCED'}"
    )

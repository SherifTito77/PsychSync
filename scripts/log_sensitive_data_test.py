...
class LogSecurityTester:
    ...
    def setup_sensitive_patterns(self):
        ...
        self.sensitive_patterns = [
            ...
            (
                r'(diagnosis|medical|health|assessment)["\']?\s*[:=]\s*["\']([^"\']{10,})["\']',
                "PHI data in logs",
            ),
            (
                r'(blood_pressure|heart_rate|weight|height)["\']?\s*[:=]\s*["\']([^"\']{3,})["\']',
                "Health metrics in logs",
            ),
    ...
    def analyze_log_file(self, file_path: Path) -> Dict:
        ...
        try:
            ...
            result["summary"] = {
                ...
                "unique_pattern_types": len(
                    set(f["pattern_type"] for f in result["sensitive_findings"])
                ),
            }
    ...
    def mask_sensitive_data(
        self, data: str, mask_char: str = "*", show_last: int = 4
    ) -> str:
    ...
    def check_additional_issues(self, result: Dict, content: str):
        ...
        lines = content.split("\n")
        debug_lines = [
            line for line in lines if "debug" in line.lower() or "trace" in line.lower()
        ]
        ...
        if re.search(
            r"(request|response).*(body|payload|data)", content, re.IGNORECASE
        ):
    ...
    def check_database_logs(self) -> List[Dict]:
        ...
        sqlite_files = list(self.base_path.rglob("*.db")) + list(
            self.base_path.rglob("*.sqlite")
        )
    ...
    def check_application_logs(self) -> List[Dict]:
        ...
        app_log_paths = ["logs/", "var/log/", "/tmp/", "."]
        ...
        for log_path in app_log_paths:
            ...
            if full_path.exists():
                log_files = list(full_path.rglob("*.log")) + list(
                    full_path.rglob("*.out")
                )
    ...
    def generate_log_security_recommendations(
        ...
        high_risk_files = [
            r
            for r in analysis_results.get("log_file_analysis", [])
        ...
        password_findings = [
            f for f in all_findings if "password" in f["pattern_type"].lower()
        ]
        ...
        token_findings = [
            f for f in all_findings if "token" in f["pattern_type"].lower()
        ]
        if token_findings:
            recommendations.append(
                {
    ...
    def run_comprehensive_test(self) -> Dict:
        ...
        # Generate summary
        total_log_files = (
            len(log_file_analysis)
            + len(results["database_logs"])
            + len(results["application_logs"])
        )
        high_risk_files = len(
            [r for r in log_file_analysis if r.get("risk_level") == "HIGH"]
        )
        total_sensitive_findings = sum(
...
def main():
    ...
    try:
        ...
        high_risk_files = [
            r for r in results["log_file_analysis"] if r.get("risk_level") == "HIGH"
        ]
        ...
        if high_risk_files:
            ...
            for file_result in high_risk_files[:5]:  # Show first 5
                ...
                print(
                    f"     → {file_result.get('summary', {}).get('sensitive_findings', 0)} sensitive findings"
                )
        ...
        with open(
            "/Users/sheriftito/Downloads/psychsync/log_sensitive_data_security_report.json",
            "w",
        ) as f:

...
class UATFeedbackQuestion:
    def __init__(
        ...
        self.category = (
            category  # Usability, Functionality, BusinessValue, Performance, Overall
        )
...
class UATFeedbackSystem:
        ...
        return {
            ...
            "questions": [
                {
                    ...
                    "questions": [
                        ...
                        if q.category in ["Usability", "Performance"]
                    ],
                },
                {
            ...
            "demographics": {
                ...
                "questions": [
                    ...
                    for q in self.feedback_questions
                    if q.question_id in ["UAT-QL-022", "UAT-QL-023", "UAT-QL-024"]
                ],
            },
        }
    ...
    def analyze_feedback_results(self, feedback_data):
        ...
        for question in self.feedback_questions:
            if (
                ...
                and question.question_id in feedback_data
            ):
        ...
        boolean_success_rate = (
            (boolean_positive / boolean_total * 100) if boolean_total > 0 else 0
        )
    ...
    def _calculate_category_scores(self, feedback_data):
        ...
        for category in ["Usability", "Functionality", "BusinessValue", "Overall"]:
            category_questions = [
                q
                for q in self.feedback_questions
                if q.category == category and q.question_type == "rating"
            ]
        ...
        for question in self.feedback_questions:
            if (
                ...
                if response and any(
                    ...
                    for word in ["good", "excellent", "love", "great", "helpful"]
                ):
    ...
    def generate_feedback_report(self, all_feedback_data):
        ...
        for category in ["Usability", "Functionality", "BusinessValue", "Overall"]:
            category_scores = [
                insight["category_scores"].get(category, 0)
                for insight in all_insights
                if category in insight["category_scores"]
            ]
            if category_scores:
                aggregate_scores[category] = sum(category_scores) / len(category_scores)

        overall_score = sum(insight["overall_score"] for insight in all_insights) / len(
            all_insights
        )
    ...
    def _analyze_user_segments(self, feedback_data):
        ...
        segments = {"experience_levels": {}, "team_sizes": {}, "industries": {}}

        for feedback in feedback_data:
            if "UAT-QL-022" in feedback:  # Experience level
                exp = feedback["UAT-QL-022"]
                segments["experience_levels"][exp] = (
                    segments["experience_levels"].get(exp, 0) + 1
                )
            ...
            if "UAT-QL-024" in feedback:  # Industry
                ...
                segments["industries"][industry] = (
                    segments["industries"].get(industry, 0) + 1
                )
...
def main():
    ...
    print(
        f"   Estimated Completion Time: {feedback_form['instructions']['estimated_time']}"
    )

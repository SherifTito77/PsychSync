...
class SimpleCVEScanner:
    ...
    def scan_software_versions(self):
        ...
        try:
            result = subprocess.run(
                ["redis-server", "--version"], capture_output=True, text=True, timeout=5
            )
            if result.returncode == 0:
    ...
    def check_vulnerabilities(self):
        ...
        for package in vulnerable_packages:
            try:
                ...
                result = subprocess.run(
                    ["dpkg", "-l", pkg_name], capture_output=True, text=True
                )
        ...
        if self.results["vulnerabilities_found"]:
            ...
            for vuln in self.results["vulnerabilities_found"]:
                if isinstance(vuln, dict):
                    print(
                        f"   • Update {vuln['software']} to address: {vuln['vulnerability']}"
                    )
                else:
                    print(f"   • {vuln}")
        else:
    ...
    def generate_report(self):
        ...
        print(f"\n📊 EXECUTIVE SUMMARY:")
        print(
            f"   • Software Packages Detected: {len(self.results['software_detected'])}"
        )
        print(
            f"   • Vulnerabilities Found: {len(self.results['vulnerabilities_found'])}"
        )
        print(
            f"   • Security Recommendations: {len(self.results['security_recommendations'])}"
        )
        ...
        report_file = (
            f"cve_vulnerability_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )

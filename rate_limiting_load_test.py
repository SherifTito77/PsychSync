...
class RateLimitingLoadTester:
    ...
    def __init__(self, base_url: str = "http://localhost:8000", max_concurrent: int = 100):
    ...
    async def make_request(self, endpoint: Dict, user_id: Optional[str] = None) -> LoadTestResult:
    ...
    async def run_constant_load_test(self, requests_per_second: int, duration: int,
                                    users: List[str], endpoint_types: List[str]) -> List[LoadTestResult]:
        """Run constant load test with specified RPS for duration"""
        print(f"🚀 Running constant load test: {requests_per_second} RPS for {duration}s")
    ...
    async def run_burst_test(self, users: List[str], burst_size: int,
                           burst_interval: float = 1.0) -> List[LoadTestResult]:
        ...
        for burst in range(bursts):
            ...
            for i in range(burst_size):
                user_id = random.choice(users) if users else None
                endpoint = random.choice(self.endpoints["public"] + self.endpoints["auth"])
                tasks.append(self.make_request(endpoint, user_id))
    ...
    async def run_tier_test(self, tier_users: Dict[str, List[str]]) -> List[LoadTestResult]:
    ...
    def calculate_summary(self, results: List[LoadTestResult], scenario: str) -> LoadTestSummary:
        """Calculate test summary statistics"""
        if not results:
            return LoadTestSummary(
                scenario=scenario, total_requests=0, successful_requests=0,
                rate_limited_requests=0, failed_requests=0, average_response_time=0,
                min_response_time=0, max_response_time=0, p95_response_time=0,
                p99_response_time=0, requests_per_second=0, duration=0,
                rate_limit_hit_rate=0, error_rate=0
            )
    ...
    def print_summary(self, summary: LoadTestSummary) -> None:
        ...
        print(f"Rate Limited: {summary.rate_limited_requests:,} ({summary.rate_limit_hit_rate:.1f}%)")
    ...
    def save_detailed_report(self, results: List[LoadTestResult], summary: LoadTestSummary,
                           filename: str) -> None:
        ...
        report_data = {
            ...
            "analysis": {
                ...
                "rate_limiting_working": "✅" if summary.rate_limit_hit_rate > 0 else "❌",
                "performance_acceptable": "✅" if summary.average_response_time < 1000 else "❌",
...
async def main():
    ...
    parser.add_argument("--scenario", choices=["basic", "burst", "tier", "comprehensive"],
                       default="basic", help="Test scenario to run")
    parser.add_argument("--users", type=int, default=50, help="Number of concurrent users")
    parser.add_argument("--rps", type=int, default=100, help="Requests per second for basic test")
    parser.add_argument("--duration", type=int, default=60, help="Test duration in seconds")
    parser.add_argument("--burst-size", type=int, default=1000, help="Burst size for burst test")
    ...
    async with RateLimitingLoadTester(args.url, args.max_concurrent) as tester:
        ...
        success = (
            summary.rate_limit_hit_rate > 0 and  # Rate limiting should be working
            summary.error_rate < 10 and           # Error rate should be reasonable
            summary.average_response_time < 2000  # Response time should be acceptable
        )

...
class PromptEngineer:
    ...
    def _load_registry(self) -> Dict:
        ...
        if not self.registry_path.exists():
            console.print(f"[red]❌ Registry file not found: {self.registry_path}[/red]")
            sys.exit(1)
...
"""
    ...
    def display_categories(categories: List[Dict]):
        ...
        table = Table(title="📋 Prompt Categories", show_header=True, header_style="bold magenta")
    ...
    def display_prompt_details(prompt: Dict):
        ...
        metadata_table.add_row("Category", f"{category.get('icon', '')} {category.get('name', 'N/A')}")
        metadata_table.add_row("Complexity", prompt.get('complexity', 'unknown').upper())
        metadata_table.add_row("Estimated Time", prompt.get('estimated_time', 'N/A'))
        metadata_table.add_row("Scope", ', '.join(prompt.get('scope', [])))

        console.print(Panel(metadata_table, title="📊 Prompt Metadata", border_style="cyan"))
...
def cmd_search(args):
    ...
    UIHelpers.display_banner()
    console.print(f"[bold cyan]🔍 Searching for: [yellow]{args.query}[/yellow][/bold cyan]\n")
...
def cmd_filter(args):
    ...
    if args.scope:
        console.print(f"[bold cyan]🔍 Filtering by scope: [yellow]{args.scope}[/yellow][/bold cyan]\n")
        results = pe.filter_by_scope(args.scope)
        filter_name = f"Scope: {args.scope}"

    elif args.complexity:
        console.print(f"[bold cyan]🔍 Filtering by complexity: [yellow]{args.complexity}[/yellow][/bold cyan]\n")
    ...
    while True:
        ...
        if choice == "0":
            ...
            for cat in categories:
                console.print(f"  {cat['icon']} [cyan]{cat['id']}[/cyan] - {cat['name']}")

            cat_choice = Prompt.ask(
                "\n[bold cyan]Enter category ID[/bold cyan]",
            ...
            if prompt:
                ...
                if Confirm.ask("[bold cyan]Export this prompt?[/bold cyan]", default=False):
                    filename = Prompt.ask(
            ...
            # Filter by scope
            scopes = ["backend", "frontend", "api", "database", "devops", "testing",
                     "security", "architecture", "all", "analytics"]
            console.print(f"\n[bold]Available scopes:[/bold] {', '.join(scopes)}")

            scope = Prompt.ask(
                "[bold cyan]Enter scope[/bold cyan]",
                choices=scopes
            )
            ...
            complexity = Prompt.ask(
                "[bold cyan]Enter complexity level[/bold cyan]",
...
Examples:
  ...
  %(prog)s stats
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help='Path to prompts registry YAML file'
    )
    ...
    list_parser.add_argument(
        '--category',
        type=str,
        help='List prompts in a specific category'
    )
    ...
    filter_group.add_argument('--complexity', type=str,

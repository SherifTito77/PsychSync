...
# Integration Monitoring Setup Guide

**Network Resilience Enhancement - February 2026**

This section provides specific setup instructions for the new unified integration monitoring system.

## Quick Start for Integration Monitoring

### 1. Verify Integration Endpoints
...
# HTML dashboard
open http://localhost:8000/api/v1/monitoring/status
```

### 2. Start Prometheus with Integration Configuration
...
docker run -d \
  --name prometheus \
  -p 9090:9090 \
  -v $(pwd)/monitoring/prometheus_integration.yml:/etc/prometheus/prometheus.yml \
  -v $(pwd)/monitoring/alerts_integration.yml:/etc/prometheus/alerts.yml \
  prom/prometheus \
  --config.file=/etc/prometheus/prometheus.yml
...
psychsync_redis_circuit_breaker_open

# HRIS connector success rate (per connector)
psychsync_hris_success_rate{connector="odoo_production"}

# Overall integration health
psychsync_integration_healthy_count / psychsync_integration_total_count
```

### 4. Integration-Specific Alerts

| Alert | Trigger | Severity |
|-------|---------|----------|
| `DatabaseCircuitBreakerOpen` | DB circuit open > 1m | Critical |
| `RedisCircuitBreakerOpen` | Redis circuit open > 1m | Critical |
| `DatabaseLowSuccessRate` | Success rate < 95% for 5m | Warning |
| `HRISCircuitBreakerOpen` | HRIS circuit open > 2m | Warning |
| `IntegrationHealthCritical` | < 25% integrations healthy | Critical |
...
## Files Added for Integration Monitoring

| File | Purpose |
|------|---------|
| `app/monitoring/integration_metrics.py` | Unified metrics collection |
| `app/api/v1/endpoints/monitoring_integration.py` | API endpoints |
| `app/core/redis_client.py` | Enhanced with retry + circuit breaker |
| `app/core/database.py` | Enhanced with retry + circuit breaker |
| `app/integrations/hris/resilient_adapter.py` | Resilient HRIS connectors |
| `monitoring/prometheus_integration.yml` | Prometheus scrape config |
| `monitoring/alerts_integration.yml` | Alert rules |
...
- Max half-open attempts: 3 test calls
...
## Testing the Resilience Features
...
# 2. Make multiple API calls to trigger failures
for i in range(10):
    requests.get("http://localhost:8000/api/v1/cache/test")

# 3. Check circuit breaker state
curl http://localhost:8000/api/v1/monitoring/metrics | jq '.redis.circuit_breaker_state'

# Expected: "open" after 5 failures

# 4. Restart Redis and watch recovery
docker-compose start redis

# 5. After 60 seconds, circuit should transition to HALF_OPEN then CLOSED
```
...
tail -f logs/app.log | grep -E "retry|backoff|circuit"

# Example output:
# Redis GET failed (attempt 1/3): Connection timeout. Retrying in 1.23s...
# Redis GET failed (attempt 2/3): Connection timeout. Retrying in 2.45s...
# Redis GET succeeded after 2 retries
```

### Load Test with Resilience
...
# Check metrics during/after test
curl http://localhost:8000/api/v1/monitoring/metrics | jq '.database'

# Check Prometheus for metrics
open http://localhost:9090/graph?g0.expr=rate(psychsync_db_total_calls%5B5m%5D)
```

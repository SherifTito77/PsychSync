...
class AsyncConsistencyCompleter:
    ...
    def analyze_all_issues(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for py_file in self.project_root.rglob("*.py"):
            if any(
                skip in str(py_file)
                for skip in [".venv", "__pycache__", "node_modules", "migrations"]
            ):
                continue
        ...
        print(
            f"📊 Analyzed {total_files} files, found issues in {len(all_issues)} files"
        )
        return all_issues
    ...
    def analyze_file_issues(self, file_path: Path) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            async_func_indent = 0
            has_async_imports = (
                "from sqlalchemy.ext.asyncio import" in content
                or "from sqlalchemy.ext.asyncio import AsyncSession" in content
            )
                ...
                if in_async_func:
                    for pattern in sync_patterns_in_async:
                        if re.search(pattern, line) and "await" not in line:
                            ...
                            if any(
                                db_ref in line
                                for db_ref in ["db.", "session.", "self.db."]
                            ):
                                issues.append(
                                    {
                                        "file": str(file_path),
    ...
    def fix_file_completely(
        self, file_path: Path, issues: List[Dict[str, Any]]
    ) -> bool:
        ...
        try:
            ...
            if (
                ...
                and "from sqlalchemy.ext.asyncio import" not in content
            ):
                # Find import section
                    ...
                    if "from sqlalchemy import select" not in content:
                        ...
                        for i, line in enumerate(lines[:import_section_end]):
                            if line.strip().startswith("from sqlalchemy import"):
                                existing_imports = line.strip()[
                                    len("from sqlalchemy import ") :
                                ]
                                if "select" not in existing_imports:
                                    lines[i] = (
                                        f"from sqlalchemy import {existing_imports}, select"
                                    )
                                    modified = True
                                break
                        else:
            ...
            patterns_to_fix = [
                # Query patterns
                (
                    r"self\.db\.query\(([^)]+)\)\.first\(\)",
                    r"await self.db.execute(select(\1).limit(1)).scalar_one_or_none()",
                ),
                (
                    r"self\.db\.query\(([^)]+)\)\.all\(\)",
                    r"await self.db.execute(select(\1)).scalars().all()",
                ),
                (r"self\.db\.query\(([^)]+)\)", r"select(\1)"),
                (
                    r"db\.query\(([^)]+)\)\.first\(\)",
                    r"await db.execute(select(\1).limit(1)).scalar_one_or_none()",
                ),
                (
                    r"db\.query\(([^)]+)\)\.all\(\)",
                ...
                (
                    ...
                    r"await session.execute(select(\1).limit(1)).scalar_one_or_none()",
                ),
                (
                    r"session\.query\(([^)]+)\)\.all\(\)",
            ...
            for i, line in enumerate(lines):
                ...
                if any(
                    ...
                    if not stripped.startswith("await ") and (
                        i == 0 or not lines[i - 1].strip().endswith("await")
                    ):
                        if "await" not in line:
                            lines[i] = line.replace(
                                stripped[:4], "await " + stripped[:4], 1
                            )
                            modified = True
            ...
            if modified:
                ...
                for pattern, _ in patterns_to_fix:
                    self.transformations_applied[pattern] = (
                        self.transformations_applied.get(pattern, 0) + 1
                    )

...
class EncryptionSecurityTester:
    ...
    def test_password_hashing(self) -> TestResult:
        ...
        try:
                ...
                if "bcrypt" in schemes.lower():
                    ...
                    if rounds_match:
                        ...
                        if rounds >= 12:
                            ...
                            findings.append(
                                f"⚠️  bcrypt rounds: {rounds} (adequate, recommend 12+)"
                            )
                            score += 15
                        else:
                            findings.append(
                                f"⚠️  bcrypt rounds: {rounds} (weak, recommend 12+)"
                            )
                            score += 10
                            recommendations.append(
                                "Increase bcrypt rounds to at least 12"
                            )

                else:
                    findings.append(
                        f"⚠️  CryptContext configured but not using bcrypt: {schemes}"
                    )
                    recommendations.append(
                        "Configure bcrypt as the default password hashing algorithm"
                    )

            # Check for salt
            if "salt" in content.lower() or bcrypt_found:
                findings.append(
                    "✅ Password hashing includes salt (bcrypt auto-generates salt)"
                )
                score += 20
            ...
            if not recommendations:
                recommendations.append(
                    "Continue monitoring for best practices in password hashing"
                )
    ...
    def test_database_field_encryption(self) -> TestResult:
        ...
        try:
            ...
            for model_file in models_dir.glob("*.py"):
                ...
                for sensitive in sensitive_fields:
                    ...
                    if re.search(pattern, content, re.IGNORECASE):
                        ...
                        if "hash" not in content.lower() or sensitive not in [
                            "password",
                            "pass",
                        ]:
                            unencrypted_sensitive_fields.append(
                                {"file": model_file.name, "field": sensitive}
                            )

            details["encrypted_fields_count"] = len(encrypted_fields_found)
            details["encrypted_fields"] = list(set(encrypted_fields_found))[
                :20
            ]  # First 20
            ...
            if len(encrypted_fields_found) > 0:
                findings.append(
                    f"✅ Found {len(encrypted_fields_found)} encrypted field(s)"
                )
                score += 40
            ...
            if fernet_found:
                ...
                recommendations.append(
                    "Use Fernet (AES-128) or stronger for field encryption"
                )
                score += 10
            ...
            if len(unencrypted_sensitive_fields) == 0:
                ...
                findings.append(
                    f"⚠️  Found {len(unencrypted_sensitive_fields)} potentially unencrypted sensitive field(s)"
                )
    ...
    def test_secure_token_generation(self) -> TestResult:
        ...
        try:
            ...
            secure_patterns = [
                (
                    r"secrets\.token_[a-z]+\(",
                    "secrets.token_*() - cryptographically secure",
                ),
                (r"secrets\.choice\(", "secrets.choice() - cryptographically secure"),
                (
                    r"secrets\.randbelow\(",
                    "secrets.randbelow() - cryptographically secure",
                ),
            ...
            if len(secure_usage) > 0:
                findings.append(
                    f"✅ Found {len(secure_usage)} secure token generation usage(s)"
                )
                score += 50
            ...
            if len(insecure_usage) == 0:
                ...
                findings.append(
                    f"❌ Found {len(insecure_usage)} insecure random usage(s)"
                )
                for usage in insecure_usage[:5]:
                    findings.append(f"   - {usage['file']}: {usage['pattern']}")
                recommendations.append(
                    "Replace all 'random' module usage with 'secrets' module"
                )
                score -= 30
            ...
            if token_length >= 32:
                ...
                findings.append(
                    f"⚠️  Token length: {token_length} chars (recommend 32+)"
                )
                score += 5
    ...
    def test_weak_ciphers(self) -> TestResult:
        ...
        try:
            ...
            if len(weak_crypto_found) == 0:
                ...
                findings.append(
                    f"❌ Found {len(weak_crypto_found)} weak cipher usage(s)"
                )
                for crypto in weak_crypto_found[:5]:
                    findings.append(
                        f"   - {crypto['file']}: {crypto['cipher']} ({crypto['description']})"
                    )
            ...
            if len(strong_crypto) > 0:
                findings.append(
                    f"✅ Found {len(set(c['file'] for c in strong_crypto))} files using strong crypto"
                )
                score += 40

            # Check JWT algorithm
            settings_file = (
                self.project_root / "app" / "core" / "config" / "settings.py"
            )
            if settings_file.exists():
                content = settings_file.read_text()
                jwt_match = re.search(
                    r'JWT_ALGORITHM[^=]*=\s*[\""]([^\"\']+)[\""]', content
                )
                if jwt_match:
                    ...
                    if jwt_algo in [
                        ...
                        findings.append(
                            f"❌ JWT algorithm: {jwt_algo} (CRITICAL - no signature!)"
                        )
                        recommendations.append(
                            "Immediately change JWT algorithm to HS256 or RS256"
                        )
                        score -= 50
                    else:
                        findings.append(
                            f"⚠️  JWT algorithm: {jwt_algo} (verify security)"
                        )
                        score += 5
    ...
    def test_encryption_key_rotation(self) -> TestResult:
        ...
        try:
            ...
            rotation_file = (
                self.project_root / "app" / "core" / "credential_rotation.py"
            )
            ...
            if rotation_file.exists():
                ...
                interval_match = re.search(
                    r"rotation.*interval.*(\d+)\s*(day|week|month|hour)",
                    content,
                    re.IGNORECASE,
                )
                if interval_match:

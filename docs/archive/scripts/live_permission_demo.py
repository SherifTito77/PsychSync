...
class LivePermissionDemo:
    ...
    async def test_with_headers(
        self, headers: Dict[str, str], scenario: Dict[str, Any], user_type: str
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                response = await client.get(
                    f"{self.base_url}{scenario['endpoint']}", headers=headers
                )
    ...
    def _print_result(self, result: Dict[str, Any]) -> None:
        ...
        if result.get("error"):
            ...
            if result["response_data"]:
                ...
                print(
                    f"      📄 Response: {data_preview}{'...' if len(str(result['response_data'])) > 100 else ''}"
                )
    ...
    def _generate_permission_comparison(self, scenarios: List[Dict[str, Any]]) -> None:
        ...
        for scenario in scenarios:
            ...
            admin_status = (
                "✅" if admin_result and admin_result["expectation_met"] else "❌"
            )
            user_status = (
                "✅" if user_result and user_result["expectation_met"] else "❌"
            )

            # Truncate description to fit
            desc = (
                scenario["description"][:25] + "..."
                if len(scenario["description"]) > 25
                else scenario["description"]
            )

            print(
                f"{scenario['name'][:30]:<30} {admin_status:<8} {user_status:<12} {desc}"
            )
    ...
    def _show_security_metrics(self) -> None:
        ...
        print(
            f"   • Admin Tests: {admin_success}/{len(admin_tests)} ({(admin_success/len(admin_tests)*100):.1f}%)"
        )
        print(
            f"   • User Tests: {user_success}/{len(user_tests)} ({(user_success/len(user_tests)*100):.1f}%)"
        )
    ...
    def _find_result(
        self, user_type: str, scenario_name: str
    ) -> Optional[Dict[str, Any]]:
        """Find test result by user type and scenario name"""
        for result in self.test_results:
            if (
                result["user_type"] == user_type
                and result["scenario_name"] == scenario_name
            ):
                return result
        return None
...
async def main():
    ...
    try:
        ...
        report_file = (
            f"live_permission_demo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )

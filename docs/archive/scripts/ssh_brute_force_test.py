...
class SSHBruteForceTester:
                            ...
                            if setting == "Port" and value == "22":
                                ...
                                try:
                                    ...
                                    if tries > 6:
                                        ...
                                        print(
                                            f"  ✅ {config['description']}: {value} (Secure)"
                                        )
                                ...
                                try:
                                    ...
                                    if grace_time > 120:
                                        ...
                                        print(
                                            f"  ✅ {config['description']}: {value}s (Secure)"
                                        )
                                        config_analysis["hardening_score"] += 5
                                except Exception as e:
                                    pass
                            else:
                                if value == config.get("secure") or (
                                    setting == "Port" and value != "22"
                                ):
                                    print(
                                        f"  ✅ {config['description']}: {value} (Secure)"
                                    )
                                    ...
                                    and config.get("secure") != "not_default"
                                ):
                                    print(
    ...
    def test_connection_limiting(self, test_duration=30):
        ...
        while time.time() - start_time < test_duration:
            ...
            for i in range(5):  # 5 concurrent connections
                thread = threading.Thread(
                    target=rapid_connection_test, args=(test_counter,)
                )
        ...
        if connection_stats["connection_times"]:
            avg_connection_time = sum(connection_stats["connection_times"]) / len(
        ...
        resistance_score = {
            ...
            "attempts_per_second": round(
                connection_stats["total_attempts"] / total_time, 2
            ),
        }
        ...
        print(
            f"   Successful connections: {resistance_score['successful_connections']}"
        )
        ...
        # Analyze results
        if (
            resistance_score["blocked_connections"]
            > resistance_score["successful_connections"]
        ):
            print(
    ...
    def generate_report(self):
        ...
        resistance = self.results.get("brute_force_resistance", {}).get(
            "brute_force_resistance", "UNKNOWN"
        )

        print(f"   • SSH Configuration Security Score: {config_score}/100")
        print(f"   • Brute Force Resistance: {resistance}")
        print(
            f"   • Total Security Recommendations: {len(self.results.get('security_recommendations', []))}"
        )

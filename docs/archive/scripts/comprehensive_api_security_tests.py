...
class APISecurityTester:
    ...
    async def test_rate_limiting_effectiveness(self) -> APISecTestResult:
        ...
        if endpoints_dir.exists():
            ...
            rate_limit_coverage = (
                (endpoints_with_rate_limit / total_endpoints * 100)
            ...
            if rate_limit_coverage < 50:
                findings.append(
                    f"Only {rate_limit_coverage:.0f}% of endpoints have rate limiting"
                )
                recommendations.append(
                    f"Add rate limiting to remaining endpoints (target: 100%)"
                )
                status = "fail"
                severity = "high"
            else:
                findings.append(
                    f"Rate limiting coverage: {rate_limit_coverage:.0f}% of endpoints"
                )
    ...
    async def test_input_validation(self) -> APISecTestResult:
        ...
        for file_path in files_to_check:
            ...
            if re.search(r"\.execute\s*\(", content):
                ...
                if re.search(
                    r'execute\s*\(\s*f["\']|execute\s*\(\s*\.format|execute\s*\(\s*%',
                    content,
                ):
            ...
            if re.search(r"os\.system\s*\(|subprocess\.(call|run|Popen)\s*\(", content):
                if (
                    r"request\." in content
                    or r"form\." in content
                    or r"args\." in content
                ):
        ...
        if schemas_dir.exists():
            ...
            if schema_files:
                findings.append(
                    f"Pydantic validation schemas found ({len(schema_files)} files)"
                )
        ...
        for file_path in files_to_check:
            ...
            if re.search(r"(upload|download|file)", content, re.IGNORECASE):
                if "sanitize_path" not in content and "safe_path" not in content:
                    findings.append(
                        f"File operations in {file_path.name} may lack path sanitization"
                    )
                    recommendations.append(
                        "Use path sanitization utilities for all file operations"
                    )
                    status = "warning"
        ...
        for file_path in files_to_check:
            ...
            if re.search(r"User\(\*\*request\.|user\.update\(|item\.update\(", content):
                findings.append(
                    f"Potential mass assignment vulnerability in {file_path.name}"
                )
                recommendations.append(
                    "Use explicit field assignment instead of **kwargs unpacking"
                )
                status = "warning"
        ...
        for config_file in [cors_config, main_app]:
            ...
            if "CORSMiddleware" in content or "cors" in content.lower():
                ...
                if 'allow_origins=["*"]' in content or "allow_origins=['*']" in content:
                    ...
                    recommendations.append(
                        "Use specific origin list instead of wildcard"
                    )
                ...
                if 'allow_origins=["*"]' in content and "allow_credentials" in content:
                    findings.append(
                        "CORS allows credentials with wildcard origins (security issue)"
                    )
                    recommendations.append(
                ...
                if "allow_origin_regex" in content or "origin" in content.lower():
                    findings.append(
                        "CORS may reflect Origin header (potential for reflection attacks)"
                    )
                    recommendations.append(
                        "Validate origins against whitelist, don't reflect"
                    )
                    status = "warning"
                ...
                if "allow_origins" in content:
                    ...
                    origins_match = re.search(
                        r"allow_origins\s*=\s*\[(.*?)\]", content, re.DOTALL
                    )
                    if origins_match:
                        origins = origins_match.group(1)
                        if len(origins) > 10 and "localhost" in origins:
                            findings.append(
                                "CORS configured with specific origins (including localhost)"
                            )
                        elif len(origins) > 10:
        ...
        if frontend_api.exists():
            ...
            if "credentials:" in content or "withCredentials:" in content:
                ...
                if overly_permissive:
                    recommendations.append(
                        "Ensure CORS allows credentials only for trusted origins"
                    )
    ...
    async def test_api_versioning(self) -> APISecTestResult:
        ...
        if endpoints_dir.exists():
            ...
            if deprecated_count > 0:
                findings.append(
                    f"Found {deprecated_count} endpoints marked as deprecated"
                )
    ...
    async def test_broken_authentication(self) -> APISecTestResult:
        ...
        if endpoints_dir.exists():
            ...
            for endpoint_file in endpoints_dir.glob("*.py"):
                ...
                for route_match in routes:
                    ...
                    route_context = content[
                        route_match.start() : route_match.start() + 500
                    ]
                    ...
                    is_sensitive = any(
                        kw in route_context.lower() for kw in sensitive_keywords
                    )

                    # Check if protected
                    has_auth = (
                        "get_current_user" in route_context
                        or "Depends" in route_context
                    )
            ...
            if unprotected_sensitive:
                findings.append(
                    f"Found {len(unprotected_sensitive)} sensitive endpoints without authentication"
                )
                recommendations.append(
                ...
                findings.append(
                    f"Sensitive endpoints appear protected ({protected_count} with auth)"
                )
            ...
            if "ACCESS_TOKEN_EXPIRE" in content:
                ...
                if expire_match:
                    ...
                    if expire_minutes > 1440:  # More than 24 hours
                        ...
                        recommendations.append(
                            "Reduce access token expiration to 30 minutes or less"
                        )
                        severity = "high"
                        status = "fail"
        ...
        if auth_service.exists():
            ...
            if "login" in content.lower():
                if (
                    "regenerate" not in content.lower()
                    and "new_session" not in content.lower()
                ):
...
async def main():
    ...
    output_file = (
        project_root
        / f"api_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )

...
def test_business_logic_bypass():
    ...
    for test_case in test_cases:
        ...
        try:
            ...
            if test_case["expected_failure"] and response.status_code not in [
                400,
                401,
                403,
                404,
                ...
                print(
                    f"🚨 VULNERABILITY DETECTED: Expected failure but got {response.status_code}"
                )
            ...
            print(f"❌ Error: {str(e)}")
            results.append(
                {"test_name": test_case["name"], "error": str(e), "vulnerable": False}
            )

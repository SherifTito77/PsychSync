...
class TestProfileSettingsSecurity(unittest.TestCase):
    ...
    def test_xss_prevention_in_email_field(self):
        ...
        for xss_attempt in self.malicious_inputs["xss_attempts"]:
            ...
            is_safe_email = (
                ...
                or "<" in xss_attempt
                or ">" in xss_attempt
                or "javascript:" in xss_attempt.lower()
            )
    ...
    def test_html_tag_sanitization(self):
        ...
        for tag in dangerous_tags:
            with self.subTest(tag=tag):
                test_input = (
                    f"Normal text {tag}malicious content{tag.replace('<', '</')}"
                )
                ...
                self.assertNotIn(
                    tag, sanitized, f"Dangerous tag {tag} should be removed from input"
                )
    ...
    def test_script_event_sanitization(self):
        ...
        for handler in event_handlers:
            with self.subTest(handler=handler):
                ...
                sanitized = re.sub(
                    r'\s+on\w+="[^"]*"', "", test_input, flags=re.IGNORECASE
                )
    ...
    def test_parameterized_query_usage(self):
        ...
        for key, value in query_params.items():
            self.assertIsInstance(value, str, f"Query parameter {key} should be string")
            self.assertFalse(
                ";DROP TABLE" in value.upper(),
        ...
        for filename, size in test_files:
            with self.subTest(filename=filename, size=size):
                ...
                if size > max_size_bytes:
                    self.assertFalse(
                        is_valid_size, f"File {filename} too large at {size} bytes"
                    )
    ...
    def test_csrf_token_presence(self):
        ...
        self.assertGreater(
            len(csrf_token), 10, "CSRF token should be sufficiently long"
        )
    ...
    def test_same_site_cookie_attribute(self):
        ...
        self.assertEqual(
            cookie_config["session"]["sameSite"],
            "Strict",
    ...
    def test_sensitive_data_masking(self):
        ...
        self.assertNotIn(
            "4532-1234-5678", masked_credit_card, "Credit card should be masked"
        )
    ...
    def test_data_encryption_in_transit(self):
        ...
        self.assertIn(
            "Strict-Transport-Security", secure_headers, "HSTS header should be present"
        )
    ...
    def test_data_retention_policies(self):
        ...
        self.assertGreater(
            retention_policies["deleted_user_data_retention_days"],
            0,
    ...
    def test_api_rate_limiting(self):
        ...
        for endpoint, limit in rate_limits.items():
            self.assertIsInstance(
                limit, int, f"Rate limit for {endpoint} should be integer"
            )
            self.assertGreater(
                limit, 0, f"Rate limit for {endpoint} should be positive"
            )
    ...
    def test_brute_force_protection(self):
        ...
        self.assertGreater(
            max_failed_attempts, 3, "Should allow some failed attempts before lockout"
        )
        self.assertGreater(
            lockout_duration_minutes, 5, "Lockout duration should be meaningful"
        )

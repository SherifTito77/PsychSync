...
def test_mbti_flow():
    ...
    try:
        ...
        for i, q in enumerate(questions[:3], 1):  # Show first 3 questions
            ...
            print(
                f"   Options: {', '.join([opt['text'] for opt in q.get('options', [])])}"
            )
            print()
    except Exception as e:
    ...
    for personality in test_personalities:
        ...
        try:
            response = requests.post(
                ...
                json={"assessment_type": "mbti", "responses": personality["responses"]},
            )
    ...
    try:
        ...
        backend_response = requests.get(
            f"{backend_url}/api/v1/assessment-questions/mbti"
        )

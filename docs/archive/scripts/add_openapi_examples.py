...
EXAMPLE_TEMPLATES = {
    ...
    "users_update": {
        ...
        "request_example": {"full_name": "John Smith", "phone": "+1234567890"},
        "responses": {
            200: {
                ...
                "content": {
                    "application/json": {
                        "example": {
                            "id": 123,
                            "status": "completed",
                            "score": {"openness": 4.2, "conscientiousness": 3.8},
...
def add_openapi_to_file(file_path: str) -> Tuple[int, int]:
    ...
    def replace_decorator(match):
        ...
        function_name_match = re.search(
            r"def\s+(\w+)\s*\(", content[match.end() : match.end() + 100]
        )
        function_name = (
            function_name_match.group(1) if function_name_match else "unknown"
        )

        endpoint_type = detect_endpoint_type(file_path, function_name)
        template = EXAMPLE_TEMPLATES.get(
            endpoint_type, EXAMPLE_TEMPLATES["default_success"]
        )
    ...
    for endpoint_file in base_path.glob("*.py"):
        ...
        try:
            ...
            if endpoints > 0:
                ...
                if modified > 0:
                    print(
                        f"✅ {endpoint_file.name}: Enhanced {modified}/{endpoints} endpoints"
                    )
                else:
                    print(
                        f"⏭️  {endpoint_file.name}: Already documented ({endpoints} endpoints)"
                    )

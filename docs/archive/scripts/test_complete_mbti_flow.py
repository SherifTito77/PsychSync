...
def test_complete_mbti_assessment():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                print(
                    f"   📋 First question: {questions[0].get('question_text', 'N/A')}"
                )
            else:
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        try:
            response = requests.post(
                ...
                json={"assessment_type": "mbti", "responses": scenario["responses"]},
            )
            ...
            if response.status_code == 200:
                ...
                storage_response = requests.post(
                    f"{base_url}/api/v1/assessment-results-simple",
                    json={
                ...
                if storage_response.status_code == 200:
                    ...
                    if storage_data.get("success"):
                        print(
                            f"      💾 Results stored: {storage_data.get('result_id', 'N/A')}"
                        )
                    else:
                        print(
                            f"      ❌ Storage failed: {storage_data.get('message', 'N/A')}"
                        )
                else:
    ...
    try:
        frontend_response = requests.get(
            "http://localhost:5174/assessments/mbti/start", timeout=5
        )

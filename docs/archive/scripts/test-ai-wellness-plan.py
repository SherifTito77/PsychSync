...
async def test_ai_wellness_plan():
    ...
    print(
        f"   ✅ AI Data Preparation: {len(ai_assessment_data['wellness_domains'])} domains processed"
    )
    print(
        f"   ✅ Pattern Recognition: Consistency at {ai_assessment_data['response_patterns']['consistency']:.0%}"
    )
    print(
        f"   ✅ Predictive Analysis: Trend is {ai_assessment_data['response_patterns']['improvement_trend']}"
    )
    ...
    for domain, insights in ai_analysis["domain_insights"].items():
        print(
            f"   {domain.title()}: {insights['current_assessment']} → {insights['improvement_potential']} potential"
        )
    ...
    for goal in wellness_plan["goals"]:
        print(
            f"   {goal['domain'].title()}: {goal['current_score']}% → {goal['target_score']}% ({goal['priority']} priority)"
        )
    ...
    print(
        f"   ✅ Predictive insights (success probability: {predictive['success_probability']:.0%})"
    )
...
if __name__ == "__main__":
    ...
    print(
        f"   Plan contains {len(result.get('goals', []))} goals and {len(result.get('ai_recommendations', []))} AI recommendations"
    )

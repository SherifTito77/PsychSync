...
pwd_context = CryptContext(
    schemes=["bcrypt"], default="bcrypt", deprecated="auto", bcrypt__rounds=12
)
...
def test_password_verification():
    ...
    print(
        f"Hash algorithm: {stored_hash.split('$')[1] if len(stored_hash.split('$')) > 1 else 'unknown'}"
    )
    print(
        f"Hash rounds: {stored_hash.split('$')[2] if len(stored_hash.split('$')) > 2 else 'unknown'}"
    )
    print()

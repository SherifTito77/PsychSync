...
class SimpleSSLDowngradeTester:
    ...
    def test_tls_version_support(self) -> Dict[str, Any]:
        ...
        successful_connections = [
            r for r in result["test_results"].values() if r["connection_successful"]
        ]
        ...
        for endpoint, data in result["test_results"].items():
            if data["connection_successful"]:
                if data.get("cipher"):
                    ...
                    if any(
                        weak in cipher_name
                        for weak in ["rc4", "des", "3des", "md5", "null"]
                    ):
                        security_issues.append(
                            f"Weak cipher detected: {cipher_name} on {endpoint}"
                        )
    ...
    def test_certificate_validation(self) -> Dict[str, Any]:
        ...
        result["cert_files_found"] = [
            str(f.relative_to(self.base_path)) for f in cert_files[:5]
        ]
            ...
            try:
                ...
                if cert_size < 1000:
                    result["security_issues"].append(
                        f"Small certificate file: {cert_file.name} ({cert_size} bytes)"
                    )
                elif cert_size > 100000:
                    result["security_issues"].append(
                        f"Large certificate file: {cert_file.name} ({cert_size} bytes)"
                    )
                ...
                try:
                    ...
                    if file_mode == "644":  # World-readable
                        result["security_issues"].append(
                            f"Certificate file world-readable: {cert_file.name} (permissions: {file_mode})"
                        )
                ...
                result["security_issues"].append(
                    f"Error analyzing {cert_file.name}: {e}"
                )
                ...
                try:
                    ...
                    if file_ssl_settings:
                        result["ssl_settings"].append(
                            {"file": config_file, "settings": file_ssl_settings}
                        )

                    # Check for security issues
                    if "http://" in content and "https://" not in content:
                        result["security_issues"].append(
                            f"HTTP only configuration in {config_file}"
                        )

                    if (
                        "verify=false" in content.lower()
                        or "ssl_verify=false" in content.lower()
                    ):
                        result["security_issues"].append(
                            f"SSL verification disabled in {config_file}"
                        )
    ...
    def test_http_security_headers(self) -> Dict[str, Any]:
        ...
        # Test endpoints
        test_endpoints = ["http://localhost:8000/", "http://localhost:5174/"]
            ...
            try:
                ...
                for header in security_headers:
                    if header in response.headers:
                        ...
                        endpoint_result["security_headers"][header] = response.headers[
                            header
                        ]
                    else:
...
def main():
    ...
    try:
        ...
        print(
            f"🎯 Overall SSL Security Score: {summary['overall_ssl_security_score']}/100"
        )
        ...
        with open(
            "/Users/sheriftito/Downloads/psychsync/ssl_security_test_report.json", "w"
        ) as f:

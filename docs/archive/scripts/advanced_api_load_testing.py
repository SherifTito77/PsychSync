...
class LoadTestMetrics:
    ...
    def __post_init__(self):
        if self.median_response_time_ms == 0 and self.total_requests > 0:
            response_times = [
                r.response_time_ms for r in self._results if r.response_time_ms > 0
            ]
            if response_times:
                self.median_response_time_ms = statistics.median(response_times)
                self.p95_response_time_ms = sorted(response_times)[
                    int(len(response_times) * 0.95)
                ]
                self.p99_response_time_ms = sorted(response_times)[
...
class AdvancedAPILoadTester:
    ...
    async def make_request(
        self, user_id: int, endpoint_config: Dict[str, Any]
    ) -> TestResult:
    ...
    async def run_user_simulation(
        self, user_id: int, duration_seconds: int
    ) -> List[TestResult]:
    ...
    async def run_ramp_up_load_test(self):
        ...
        logger.info(
            f"Starting ramp-up load test: {self.concurrent_users} users over {self.ramp_up_time_seconds}s"
        )
    ...
    def generate_report(
        self, metrics: LoadTestMetrics, results: List[TestResult]
    ) -> Dict[str, Any]:
        ...
        return {
            "test_summary": {
                ...
                "total_duration_minutes": (
                    self.test_duration_seconds + self.ramp_up_time_seconds
                )
                / 60,
    ...
    def _generate_recommendations(
        self, metrics: LoadTestMetrics, errors: Dict[str, int]
    ) -> List[str]:
        ...
        if metrics.avg_response_time_ms > 2000:
            recommendations.append(
                "⚠️ High average response time (>2s). Consider optimizing database queries and adding caching."
            )

        if metrics.p95_response_time_ms > 5000:
            recommendations.append(
                "🚨 Very high P95 response time (>5s). Server is struggling under load."
            )

        if metrics.error_rate_percentage > 5:
            recommendations.append(
                f"❌ High error rate ({metrics.error_rate_percentage:.1f}%). Investigate server capacity and error handling."
            )

        if "timeout" in str(errors).lower():
            recommendations.append(
                "⏱️ Timeout errors detected. Consider increasing timeouts or optimizing performance."
            )

        if "connection" in str(errors).lower():
            recommendations.append(
                "🔗 Connection errors detected. Check connection pool limits and server capacity."
            )

        if metrics.requests_per_second < 100 and self.concurrent_users > 1000:
            recommendations.append(
                "📈 Low RPS for user count. Server may be bottlenecked. Consider horizontal scaling."
            )

        if metrics.error_rate_percentage < 1 and metrics.avg_response_time_ms < 500:
            recommendations.append(
                "✅ Excellent performance! System handled load well."
            )
...
async def main():
    ...
    async with AdvancedAPILoadTester(BASE_URL) as tester:
        ...
        try:
            ...
            print(
                f"   • Failed: {metrics.failed_requests:,} ({metrics.error_rate_percentage:.1f}%)"
            )
            ...
            for code, count in sorted(status_codes.items()):
                ...
                print(
                    f"   • HTTP {code}: {count:,} ({percentage:.1f}%) - {status_text}"
                )

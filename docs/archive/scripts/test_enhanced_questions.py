...
def test_enhanced_wellness_questions():
    ...
    for i in range(3):
        ...
        for domain in domains:
            ...
            print(
                f"     {domain.title()}: {count} questions - {selected_questions[domain][0][:30]}..."
            )

        total_selected = sum(
            len(questions) for questions in selected_questions.values()
        )
    ...
    print(
        f"   - Randomization Layers: 7 (Domain, Question, Order, Variant, Options, ID, Time)"
    )
...
if __name__ == "__main__":
    ...
    print(f"\n🚀 Enhancement Status:")
    print(
        f"   ✅ Question bank massively expanded: {results['total_questions']} questions"
    )
    print(f"   ✅ Randomization enhanced: {results['randomization_layers']} layers")
    print(
        f"   ✅ Unpredictability achieved: {results['combinations']:.2e} combinations"
    )
    print(
        f"   ✅ Anti-gaming implemented: {results['anti_gaming_score']}% effectiveness"
    )

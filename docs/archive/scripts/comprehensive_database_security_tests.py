...
class ComprehensiveDatabaseSecurityTester:
    ...
    async def test_nosql_injection(self) -> Dict[str, Any]:
        ...
        for py_file in python_files:
            ...
            try:
                ...
                if uses_db:
                    ...
                    unsafe_patterns = [
                        (
                            r"dict\s*\(\s*\*\*user_input",
                            "Dictionary unpacking with user input",
                        ),
                        (
                            r"\.find\s*\(\s*\{[^}]*\+\s*\w+",
                        ...
                        (
                            r"\.update\s*\(\s*\{[^}]*\$\w+",
                            "Direct use of MongoDB operators with user input",
                        ),
        ...
        config_files = list(self.base_path.rglob("*.json")) + list(
            self.base_path.rglob("*.yml")
        )
        ...
        result["risk_level"] = (
            "CRITICAL"
            if result["successful_injections"] > 5
            else "HIGH" if result["successful_injections"] > 0 else "LOW"
        )
    ...
    async def test_credential_rotation(self) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if file_issues:
                        result["credential_issues"].append(
                            {"file": config_file, "issues": file_issues}
                        )
        ...
        for py_file in python_files:
            ...
            try:
                ...
                if re.search(
                    r"rotate.*credential|credential.*rotation", content, re.IGNORECASE
                ):
                    rotation_checks["has_rotation_function"] = True

                # Check for automated rotation scheduling
                if re.search(
                    r"cron|schedule|periodic.*rotation", content, re.IGNORECASE
                ):
                ...
                if re.search(
                    r"alert.*rotation|rotation.*alert|notify.*credential",
                    content,
                    re.IGNORECASE,
                ):
        ...
        result["vulnerable"] = (
            len(result["credential_issues"]) > 0 or len(stale_credentials) > 0
        )
    ...
    async def test_backup_encryption(self) -> Dict[str, Any]:
                ...
                try:
                    ...
                    # Check for plaintext SQL indicators
                        ...
                        backup_info["issues"].append(
                            "Plain text SQL backup - readable by anyone"
                        )
                    # Check for base64 encoding (weak)
                        ...
                        try:
                            ...
                            if b"CREATE TABLE" in decoded or b"INSERT INTO" in decoded:
                                is_encrypted = False
                                backup_info["issues"].append(
                                    "Base64 encoded backup - not true encryption"
                                )
                    ...
                    if not is_encrypted:
                        result["security_issues"].append(
                            f"Unencrypted backup: {backup_info['file']}"
                        )
        ...
        backup_scripts = list(self.base_path.rglob("*backup*.py")) + list(
            self.base_path.rglob("*backup*.sh")
        )
        ...
        if not encryption_in_script:
            result["security_issues"].append(
                "No encryption mechanism found in backup scripts"
            )
        ...
        result["risk_level"] = (
            "CRITICAL" if result["unencrypted_backups"] > 0 else "MEDIUM"
        )
    ...
    async def test_privilege_escalation(self) -> Dict[str, Any]:
        ...
        result["risk_level"] = (
            "CRITICAL" if len(result["escalation_attempts"]) > 5 else "HIGH"
        )
    ...
    async def test_log_security(self) -> Dict[str, Any]:
            ...
            if log_file.is_file() and log_file.stat().st_size > 0:
                try:
                    ...
                    if file_findings["sensitive_data"]:
                        ...
                        result["security_issues"].append(
                            f"Sensitive data in {log_file.name}"
                        )
        ...
        for py_file in python_files:
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    # Check for debug logging with sensitive data
                    if re.search(
                        r"logger\.debug.*password|logger\.debug.*token|logger\.debug.*key",
                        line,
                        re.IGNORECASE,
                    ):
                    ...
                    if re.search(
                        r"print\s*\(.*(password|token|secret|key)", line, re.IGNORECASE
                    ):
                        debug_logging_issues.append(
                            {
                                "file": str(py_file.relative_to(self.base_path)),
        ...
        if debug_logging_issues:
            result["security_issues"].append(
                f"{len(debug_logging_issues)} instances of debug logging with sensitive data"
            )
        ...
        result["vulnerable"] = (
            len(result["sensitive_data_found"]) > 0 or len(debug_logging_issues) > 0
        )
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        total_issues = sum(
            len(r.get("security_issues", []) + r.get("vulnerabilities", []))
            for r in results
        )
        ...
        summary = {
            ...
            "database_security_score": max(
                0, 100 - (vulnerable_tests * 15) - (total_issues * 2)
            ),
        }
...
async def main():
    ...
    try:
        ...
        for i, test_result in enumerate(results["test_results"], 1):
            ...
            if test_result.get("vulnerable", False):
                ...
                if "injection_attempts" in test_result:
                    print(
                        f"      Injection attempts: {test_result['injection_attempts']}"
                    )
                if "backups_found" in test_result:
                    print(
                        f"      Unencrypted backups: {test_result['unencrypted_backups']}"
                    )
                if "escalation_attempts" in test_result:
                    print(
                        f"      Escalation attempts: {len(test_result['escalation_attempts'])}"
                    )
                if "logs_analyzed" in test_result:
                    print(
                        f"      Logs with sensitive data: {len(test_result['sensitive_data_found'])}"
                    )
                ...
                if "vulnerabilities" in test_result and test_result["vulnerabilities"]:
                    for vuln in test_result["vulnerabilities"][:2]:
                        print(
                            f"      📁 {vuln['file']}: {len(vuln['vulnerabilities'])} issues"
                        )
        ...
        with open(
            "/Users/sheriftito/Downloads/psychsync/database_security_test_suite_report.json",
            "w",
        ) as f:

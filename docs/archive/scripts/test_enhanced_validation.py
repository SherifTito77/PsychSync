...
def test_enhanced_validation():
    ...
    def validate_complete_responses_enhanced(tool_questions, responses):
        ...
        if len(responses) != len(tool_questions):
            print(
                f"Incomplete responses: {len(responses)}/{len(tool_questions)} questions answered"
            )
            return False
        ...
        for response in responses:
            if not is_valid_score_js_style(response.get("score")):
                print(
                    f"Invalid score {response.get('score')} for response to question {response['question_id']}"
                )
                return False
    ...
    for test in test_cases:
        ...
        if "tool_questions" in test:
            complete_valid = validate_complete_responses_enhanced(
                test["tool_questions"], test["responses"]
            )
            complete_correct = complete_valid == test["expected_valid"]

        if score_correct and valid_correct and complete_correct:
            print(
                f"    ✅ PASSED: Score={calc_result['score']}, Valid={calc_result['isValid']}"
            )
            if "tool_questions" in test:
            ...
            print(f"    ❌ FAILED:")
            print(
                f"      Score: {calc_result['score']} (expected {test['expected_score']})"
            )
            print(
                f"      Valid: {calc_result['isValid']} (expected {test['expected_valid']})"
            )
            if "tool_questions" in test:
                print(
                    f"      Complete: {complete_valid} (expected {test['expected_valid']})"
                )
...
def test_real_phq9_scenarios():
    ...
    for scenario in scenarios:
        ...
        if total_correct and risk_correct:
            ...
            print(
                f"  ❌ {scenario['name']}: Total={total} (exp {scenario['expected_total']}), Risk={risk} (exp {scenario['expected_risk']})"
            )

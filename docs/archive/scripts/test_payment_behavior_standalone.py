...
class TestFailedPaymentBehavior:
    ...
    def test_insufficient_funds_handling(self):
        ...
        with patch.object(
            BillingService, "create_payment_intent", side_effect=mock_stripe_error
        ):
            try:
    ...
    def test_smart_retry_logic_for_transient_failures(self):
        ...
        test_scenarios = [
            {"name": "Invalid Card", "should_retry": False, "expected_retries": 0},
        ]
    ...
    def test_user_friendly_error_messages(self):
        ...
        for scenario in error_scenarios:
            ...
            for phrase in scenario["expected_message_contains"]:
                ...
                assert (
                    False
                ), f"Message '{error_result['message']}' should contain '{phrase}'"
            ...
            for expected_suggestion in scenario["expected_suggestions"]:
                if any(
                    expected_suggestion.lower() in s.lower()
                    for s in error_result["suggestions"]
                ):
                    found_suggestions += 1

            assert (
                found_suggestions > 0
            ), f"Should have at least one matching suggestion from {scenario['expected_suggestions']}"

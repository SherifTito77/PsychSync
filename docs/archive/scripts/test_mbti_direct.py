...
def test_mbti_loading():
    ...
    try:
        ...
        react_indicators = ["react-dom", "React", "root", 'id="root"']
        ...
        has_loading = (
            "Loading MBTI Assessment" in content or "loading-skeleton" in content
        )
        print(f"   Loading skeleton: {has_loading}")

        # Look for error content
        has_error = (
            "error" in content.lower() or "something went wrong" in content.lower()
        )
    ...
    try:
        ...
        if api_response.status_code == 200:
            ...
            print(
                f"   Questions: {len(data.get('assessment', {}).get('questions', []))}"
            )
        else:

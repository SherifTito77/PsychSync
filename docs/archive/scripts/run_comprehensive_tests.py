...
def install_python_dependencies():
    ...
    for dep in dependencies:
        success, _, _ = run_command(
            f"pip install {dep}", f"Installing {dep}", timeout=120
        )
        if not success:
...
def run_backend_tests():
    ...
    backend_tests = [
        (
            "python -m pytest tests/test_enhanced_ai_service.py -v",
            "Enhanced AI Service Tests",
            120,
        ),
        (
            "python -m pytest tests/test_assessment_integration.py -v",
            "Assessment Integration Tests",
            120,
        ),
        (
            "python -m pytest tests/test_enhanced_backend.py -v",
            "Enhanced Backend Tests",
            180,
        ),
        (
            "python -m pytest tests/ -k 'test_' --tb=short --maxfail=3",
            "All Backend Tests",
            300,
        ),
    ]
...
def run_integration_tests():
    ...
    backend_tests = [
        (
            ...
            "Enhanced AI Service Import Test",
            30,
        ),
        (
...
def generate_test_report(all_results):
    ...
    total_time = sum(
        r["execution_time"] for results in all_results.values() for r in results
    )
    ...
    for category, results in all_results.items():
        ...
        report["test_categories"][category] = {
            ...
            "success_rate_percent": round(
                (category_passed / category_total * 100) if category_total > 0 else 0, 2
            ),

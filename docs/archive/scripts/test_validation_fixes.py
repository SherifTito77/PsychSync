...
def test_password_validation_fixes():
    ...
    test_cases = [
        ...
        (
            "SecurePassword123!",
            False,
            "Contains common pattern 'password' even with uppercase",
        ),
    ]
    ...
    for password, email, name, should_be_valid in schema_test_cases:
        try:
            user_data = {"email": email, "full_name": name, "password": password}
    ...
    try:
        ...
        print(
            f"❌ Password Change Schema - Unexpected validation error: {e.errors()[0]['msg']}"
        )
    except Exception as e:
    ...
    print(
        f"Core Validation Tests: {successful_tests}/{total_tests} passed ({success_rate:.1f}%)"
    )

...
class DashboardWidgetVerifier:
    ...
    def test_endpoint(
        self, endpoint: str, description: str, expected_status: int = 200
    ) -> Dict[str, Any]:
    ...
    def verify_health_endpoints(self):
        ...
        for endpoint, description, expected in health_endpoints:
            ...
            if result["success"]:
                ...
                print(
                    f"   ✅ {description}: {result['status_code']} ({result['response_time']:.3f}s)"
                )
            else:
    ...
    def verify_metrics_endpoints(self):
        ...
        metrics_endpoints = [
            ...
            (
                "/api/v1/api/v1/metrics/business",
                "Business Metrics Dashboard",
                401,
            ),  # Requires auth
            (
                "/api/v1/api/v1/metrics/cache",
                "Cache Performance Metrics",
                401,
            ),  # Requires auth
            (
                "/api/v1/api/v1/query-performance/dashboard",
    ...
    def verify_assessment_endpoints(self):
        ...
        assessment_endpoints = [
            (
                "/api/v1/api/v1/assessments",
                "Assessment List Dashboard",
                401,
            ),  # Requires auth
            (
                "/api/v1/api/v1/dashboard/test-assessment-id",
                "Reliability Validity Dashboard",
                401,
            ),  # Requires auth
        ]
    ...
    def print_summary(self):
        ...
        success_rate = (
            (self.results["successful_widgets"] / self.results["total_widgets"] * 100)
            if self.results["total_widgets"] > 0
            else 0
        )
        ...
        print("\n🔐 Security Analysis:")
        auth_required = sum(
            1 for r in self.results["widget_results"] if r["expected_status"] == 401
        )
        auth_working = sum(
            1
            for r in self.results["widget_results"]
            if r["expected_status"] == 401 and r["status_code"] == 401
        )
        ...
        avg_response_time = sum(
            r["response_time"] for r in self.results["widget_results"]
        ) / len(self.results["widget_results"])
        print(f"Average response time: {avg_response_time:.3f}s")

        slow_endpoints = [
            r for r in self.results["widget_results"] if r["response_time"] > 2.0
        ]
        if slow_endpoints:
            print(f"Slow endpoints (>2s): {len(slow_endpoints)}")
            for endpoint in slow_endpoints:
                print(
                    f"  - {endpoint['description']}: {endpoint['response_time']:.3f}s"
                )

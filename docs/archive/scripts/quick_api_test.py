...
class QuickAPITester:
            ...
            try:
                ...
                if test_case["method"] == "GET":
                    ...
                    if isinstance(test_case.get("data"), str):
                        ...
                        response = requests.post(
                            test_case["url"], json=test_case.get("data", {}), timeout=10
                        )
                ...
                status = (
                    "✅"
                    if graceful and has_error_structure
                    else "⚠️" if graceful else "❌"
                )

                print(f"{status} Test {i}: {test_case['name']}")
                print(
                    f"    HTTP {response.status_code} ({response_time:.0f}ms) - Expected {test_case['expected_status']}"
                )

                if response.status_code < 500:
                    print(
                        f"    Graceful: Yes | Structure: {'✅' if has_error_structure else '❌'}"
                    )
    ...
    def run_mini_load_test(self, num_requests=50):
        ...
        def make_request():
            ...
            try:
                ...
                return {
                    ...
                    == 401,  # 401 is expected for unauthenticated health check
                }
            except Exception as e:
                return {
                    "status_code": 0,
        ...
        print(
            f"   • Min/Max Response Time: {min_response:.0f}ms / {max_response:.0f}ms"
        )
    ...
    def generate_summary(self):
        ...
        print(
            f"Graceful Responses: {graceful_count}/{len(self.results)} ({graceful_count/len(self.results)*100:.1f}%)"
        )
        print(
            f"Structured Responses: {structured_count}/{len(self.results)} ({structured_count/len(self.results)*100:.1f}%)"
        )

        if graceful_count == len(self.results) and structured_count == len(
            self.results
        ):
            print(

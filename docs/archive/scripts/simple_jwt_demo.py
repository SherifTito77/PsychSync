...
class SimpleJWTTester:
    ...
    def generate_mock_jwt(self, user_email: str, expires_in_minutes: int = 30) -> str:
        ...
        payload = {
            ...
            "iat": int(time.time()),
            "type": "access",
            "jti": hashlib.sha256(f"{user_email}{time.time()}".encode()).hexdigest()[
                :16
            ],
            "version": "1.0",
        }
    ...
    def test_jwt_expiration(self, token: str) -> Dict[str, Any]:
        ...
        result = {"test_name": "JWT Expiration Testing", "status": "pass", "issues": []}
        ...
        try:
            ...
            if exp_time and iat_time:
                ...
                if time_remaining <= 0:
                    ...
                    result["issues"].append(
                        f"Token duration too short: {duration} seconds"
                    )
                elif duration > 2100:  # More than 35 minutes
                    result["issues"].append(
                        f"Token duration too long: {duration} seconds"
                    )
            else:
                result["status"] = "fail"
        ...
        try:
            ...
            if found_keywords:
                security_issues.append(
                    f"Sensitive data in payload: {', '.join(found_keywords)}"
                )
    ...
    def test_invalid_tokens(self) -> Dict[str, Any]:
        ...
        for i, invalid_token in enumerate(invalid_tokens):
            try:
                ...
                result["issues"].append(
                    f"Token {i+1} was accepted (should be rejected)"
                )
            except Exception:
    ...
    def run_demo_tests(self) -> List[Dict[str, Any]]:
        ...
        tokens = {
            ...
            "valid_7day": self.generate_mock_jwt(
                "user@example.com", 7 * 24 * 60
            ),  # 7 days
    ...
    def generate_demo_report(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self.generate_recommendations(
                results, avg_security_score
            ),
        }

    def generate_recommendations(
        self, results: List[Dict[str, Any]], security_score: float
    ) -> List[str]:
        ...
        if any(
            "duration" in issue.lower()
            for issue in sum([r.get("issues", []) for r in results], [])
        ):
            recommendations.append(
                "Adjust JWT token lifetimes to appropriate durations"
            )

        if any(
            "sensitive data" in issue.lower()
            for issue in sum([r.get("issues", []) for r in results], [])
        ):
...
def main():
    ...
    try:
        ...
        print(
            f"  python postman_test_runner.py --collection postman_jwt_token_collection.json"
        )

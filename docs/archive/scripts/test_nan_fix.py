...
def test_safe_score_calculation():
    ...
    def calculate_safe_total_score(responses):
        ...
        for response in responses:
            if isinstance(response.get("score"), (int, float)) and (
                float("-inf") < float(response["score"]) < float("inf")
            ):
    ...
    for i, test in enumerate(test_cases):
        ...
        if score_correct and valid_correct:
            ...
            print(
                f"  ❌ FAILED: Score={result['score']} (expected {test['expected_score']}), Valid={result['isValid']} (expected {test['expected_valid']})"
            )
...
def test_complete_response_validation():
    ...
    def validate_complete_responses(tool_questions, responses):
        ...
        for response in responses:
            if not isinstance(response.get("score"), (int, float)) or not (
                float("-inf") < float(response["score"]) < float("inf")
            ):
                return False

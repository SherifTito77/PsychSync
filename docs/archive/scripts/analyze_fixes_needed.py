...
def test_issue_reproduction():
    ...
    for val in test_values:
        ...
        print(
            f"  Value: {repr(val):<10} is numeric: {is_num:<5} is finite: {is_finite}"
        )
...
def test_improved_validation():
    ...
    def is_valid_score(score):
        ...
        return (
            ...
            and not math.isnan(score)  # Exclude booleans
    ...
    for test in test_cases:
        result = calculate_safe_total_score_improved(
            test["responses"], test.get("expected_question_count")
        )
        ...
        if result["isValid"] == test["expected_valid"]:
            ...
            print(
                f"  ❌ {test['name']}: FAILED (valid={result['isValid']}, expected={test['expected_valid']})"
            )

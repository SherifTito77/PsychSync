...
class PrivilegeEscalationTester:
    ...
    async def test_postgresql_functions(
        self, conn, current_user: str, privileges: Dict[str, bool]
    ):
            ...
            if create_func_check:
                ...
                try:
                    ...
                    self.logger.critical(
                        f"🚨 UDF privilege escalation possible for {current_user}"
                    )
            ...
            for role in roles:
                ...
                try:
                    ...
                    if new_privileges.get("superuser") and not await conn.fetchval(
                        ...
                        self.findings.append(finding)
                        self.logger.critical(
                            f"🚨 Role abuse: {current_user} -> {role_name}"
                        )
        ...
        try:
            ...
            if backup_privilege:
                try:
                    ...
                    copy_test = (
                        "COPY (SELECT 'test') TO '/tmp/pg_backup_test.csv' WITH CSV;"
                    )
                    await conn.execute(copy_test)

                    # Clean up
                    await conn.execute(
                        "SELECT pg_read_file('/tmp/pg_backup_test.csv');"
                    )
    ...
    async def test_mongodb_privilege_escalation(self):
        ...
        try:
            ...
            try:
                ...
                current_user = (
                    user_info["users"][0]["user"] if user_info["users"] else "unknown"
                )
                current_roles = (
                    user_info["users"][0]["roles"] if user_info["users"] else []
                )
            except Exception as e:
    ...
    async def test_mongodb_dangerous_roles(
        self, client, current_user: str, current_roles: List[Dict]
    ):
            ...
            for payload in test_payloads:
                try:
                    ...
                    if result:
                        ...
                        self.findings.append(finding)
                        self.logger.critical(
                            f"🚨 JavaScript injection possible for {current_user}"
                        )
                        break
    ...
    async def test_mongodb_aggregation_abuse(self, client, current_user: str):
        ...
        try:
            ...
            for stage in dangerous_stages:
                try:
                    pipeline = [stage]
                    result = await db.testcollection.aggregate(pipeline).to_list(
                        length=1
                    )
        ...
        try:
            ...
            for command, description, severity in dangerous_commands:
                try:
                    ...
                    self.findings.append(finding)
                    self.logger.warning(
                        f"⚠️  Dangerous Redis command accessible: {command}"
                    )
    ...
    async def test_mongo_to_pg_escalation(self):
        ...
        try:
            ...
            self.logger.info(
                "ℹ️  Cross-database escalation testing requires network access checks"
            )
    ...
    async def test_pg_to_mongo_escalation(self):
        ...
        try:
            ...
            self.logger.info(
                "ℹ️  Cross-database escalation testing requires foreign data wrapper checks"
            )

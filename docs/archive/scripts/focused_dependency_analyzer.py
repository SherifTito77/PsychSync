...
class FocusedDependencyAnalyzer:
            ...
            for imp in imports:
                ...
                if "." in imp:
                    ...
                    for i in range(len(parts), 0, -1):
                        ...
                        if test_imp in [
                            self.get_module_name_from_path(f) for f in python_files
                        ]:
                            base_imp = test_imp
                            break
    ...
    def analyze_patterns(self):
        ...
        for module, deps in self.import_graph.items():
            ...
            if "core." in module:
                for dep in deps:
                    if any(
                        x in dep for x in ["services.", "api.endpoints.", "db.models."]
                    ):
                        issues.append(
                            {
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"Total Dependencies: {sum(len(deps) for deps in self.import_graph.values())}"
        )
        report.append("")
        ...
        if issues:
            ...
            for issue_type, type_issues in issue_types.items():
                ...
                report.append(
                    f"  Severity: {severity_counts['critical']} critical, {severity_counts['high']} high, {severity_counts['medium']} medium"
                )
                report.append("  Examples:")

                for issue in type_issues[:3]:  # Show first 3 examples
                    report.append(
                        f"    {issue['from']} -> {issue['to']} ({issue['severity']})"
                    )
        ...
        most_imported = [
            (module, len(importers)) for module, importers in imported_by.items()
        ]
    ...
    def suggest_solutions(self) -> str:
        ...
        if self.circular_deps:
            ...
            for i, cycle in enumerate(self.circular_deps, 1):
                ...
                if len(cycle) == 2:
                    ...
                    solutions.append(
                        "   - Use dependency injection to break the circularity"
                    )
        ...
        if issues:
            ...
            if "model_importing_service" in issue_types:
                ...
                solutions.append(
                    "   - Use domain events or signals for model callbacks"
                )
                solutions.append(
                    "   - Consider using SQLAlchemy events for model-level hooks"
                )
            ...
            if "core_importing_app_module" in issue_types:
                ...
                solutions.append(
                    "   - Core modules should not depend on application modules"
                )

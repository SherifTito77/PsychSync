...
class SimpleDatabaseSecurityScanner:
    ...
    def scan_code_for_sql_injection(self) -> Dict[str, Any]:
        ...
        result["total_vulnerabilities"] = sum(
            len(v["vulnerabilities"]) for v in result["vulnerabilities"]
        )
        result["vulnerable"] = result["total_vulnerabilities"] > 0
        result["risk_level"] = (
            "HIGH" if result["total_vulnerabilities"] > 5 else "MEDIUM"
        )
    ...
    def check_hardcoded_credentials(self) -> Dict[str, Any]:
        ...
        result["vulnerable"] = result["total_vulnerabilities"] > 0
        result["risk_level"] = (
            "CRITICAL" if result["total_vulnerabilities"] > 3 else "HIGH"
        )
            ...
            if file_path.exists():
                ...
                try:
                    ...
                    if "port=5432" in content and "firewall" not in content:
                        file_issues.append(
                            "Default database port exposed without firewall mention"
                        )
                    ...
                    result["config_issues"].append(
                        {"file": config_file, "issues": [f"Error analyzing file: {e}"]}
                    )

        result["total_issues"] = sum(
            len(issues["issues"]) for issues in result["config_issues"]
        )
    ...
    def run_comprehensive_database_scan(self) -> Dict[str, Any]:
        ...
        total_vulnerabilities = sum(
            r.get("total_vulnerabilities", r.get("total_issues", 0)) for r in results
        )
    ...
    try:
        ...
        for i, scan_result in enumerate(results["scan_results"], 1):
            ...
            if scan_result.get("vulnerable", False):
                ...
                vuln_count = scan_result.get(
                    "total_vulnerabilities", scan_result.get("total_issues", 0)
                )
        ...
        with open(
            "/Users/sheriftito/Downloads/psychsync/database_security_scan_report.json",
            "w",
        ) as f:

...
def test_mbti_flow():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success"):
                ...
                print(
                    f"   🎯 Dimensions: {len(set(q['dimension'] for q in assessment['questions']))}/4"
                )
                ...
                print(
                    f"   📋 Options: {', '.join([opt['value'] for opt in q['options']])}"
                )
            else:
    ...
    try:
        response = requests.get(
            "http://localhost:5173/assessments/mbti/start", timeout=10
        )
        if response.status_code == 200:
    ...
    try:
        response = requests.get(
            "http://localhost:8000/api/v1/assessment-results-test?assessment_type=mbti&limit=5",
            timeout=5,
        )
        if response.status_code == 200:
            data = response.json()
            if data.get("success") and data.get("count", 0) > 0:
                ...
                print(
                    "⚠️  No MBTI results found (normal if no assessments completed yet)"
                )
        else:
            print(f"❌ HTTP {response.status_code}")
    except Exception as e:

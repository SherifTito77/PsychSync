...
class ProfileSettingsTestRunner:
    ...
    def _display_file_results(self, test_file: str, result):
        ...
        status_emoji = (
            "✅"
            if file_results["success_rate"] >= 90
            else "⚠️" if file_results["success_rate"] >= 70 else "❌"
        )
    ...
    def _generate_report(self):
        ...
        for test_file, results in self.results["test_suites"].items():
            if "error" in results:
                ...
                status_emoji = (
                    ...
                    else "⚠️" if results["success_rate"] >= 70 else "❌"
                )
                print(
                    f"   {status_emoji} {test_file}: {results['success_rate']:.1f}% ({results['tests_run']} tests)"
                )
        ...
        if self.results["execution_time"] > 0:
            tests_per_second = (
                self.results["total_tests"] / self.results["execution_time"]
            )
            print(f"\n⚡ PERFORMANCE ANALYSIS:")
            print(f"   Tests per Second: {tests_per_second:.1f}")
            print(
                f"   Average Test Time: {(self.results['execution_time'] / self.results['total_tests']):.3f}s"
            )
        ...
        if overall_success_rate >= 95:
            print(
                "   🌟 EXCELLENT: Test suite is production-ready with high confidence"
            )
        ...
        if self.results["failed_tests"] > 0:
            print(
                f"   • Review {self.results['failed_tests']} failing tests and fix issues"
            )
        if self.results["error_tests"] > 0:
            print(
                f"   • Address {self.results['error_tests']} test errors (configuration/environment)"
            )

        success_rate_issues = [
            file
            for file, results in self.results["test_suites"].items()
            ...
            "summary": {
                ...
                "executed_files": len(
                    [
                        ...
                        if f in self.results["test_suites"]
                        and "error" not in self.results["test_suites"][f]
                    ]
                ),
            },
        }

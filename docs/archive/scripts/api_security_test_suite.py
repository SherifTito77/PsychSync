...
class APISecurityTester:
    ...
    async def _test_endpoint_rate_limiting(self, endpoint: str):
        ...
        for i in range(request_count):
            try:
                ...
                if status == 429:  # Too Many Requests
                    ...
                    status == 503
                ):  # Service Unavailable (often used for rate limiting)
                    rate_limit_hit = True
                    print(
                        f"  ✅ Service unavailable (likely rate limiting) at request {i+1}"
                    )
                    break
        ...
        rate_limiting_effective = (
            rate_limit_hit or requests_per_second < 10
        )  # Less than 10 req/sec indicates limiting
        ...
        self.results.append(
            {
                ...
                "status_code_distribution": {
                    ...
                    "others": len(
                        [s for s in status_codes if s not in [200, 401, 429, 503]]
                    ),
                },
        ...
        if rate_limiting_effective:
            print(
                f"  ✅ Rate limiting is effective ({requests_per_second:.2f} req/sec)"
            )
        else:
            print(
                f"  🚨 Rate limiting may be missing or ineffective ({requests_per_second:.2f} req/sec)"
            )
    ...
    def _get_test_data_for_endpoint(self, endpoint: str) -> Dict[str, Any]:
        ...
        if endpoint == "/api/v1/auth/login":
            return {"email": base_email, "password": "testpassword123"}
        elif endpoint == "/api/v1/auth/register":
            return {
        ...
        for test_id in test_case["test_ids"]:
            try:
                ...
                if status == 200:
                    ...
                    response_data = (
                        json.loads(response_text) if response_text.strip() else {}
                    )

                    # Check if actual data was returned (not just error message)
                    if (
                        isinstance(response_data, dict) and len(response_data) > 3
                    ):  # More than typical error response
                        is_vulnerable = True
                        vulnerabilities_found += 1
                        print(
                            f"  🚨 IDOR VULNERABILITY: Unauthorized access to {endpoint_url}"
                        )
                    ...
                    if any(
                        ...
                        for error in ["sql", "mysql", "postgresql", "sqlite"]
                    ):
                        is_vulnerable = True
                        vulnerabilities_found += 1
                ...
                async with self.session.get(url, headers=headers) as response:
                    ...
                    if auth_status == 200:
                        ...
                        response_data = (
                            json.loads(auth_response_text)
                            if auth_response_text.strip()
                            else {}
                        )
    ...
    async def _test_mass_assignment_endpoint(self, test_case: Dict[str, Any]):
        ...
        legitimate_data = {
            field: "test_value" for field in test_case["legitimate_fields"]
        }
        ...
        try:
            ...
            if (
                mass_assign_status == legitimate_status == 200
                or mass_assign_status == legitimate_status == 201
            ):
                # Compare responses to see if extra fields were processed
                try:
                    legitimate_data_json = (
                        json.loads(legitimate_response)
                        if legitimate_response.strip()
                        else {}
                    )
                    mass_assign_data_json = (
                        json.loads(mass_assign_response)
                        if mass_assign_response.strip()
                        else {}
                    )
                    ...
                    for field in test_case["suspicious_fields"]:
                        if (
                            field in mass_assign_data_json
                            and field not in legitimate_data_json
                        ):
                            is_vulnerable = True
                            print(
                                f"  🚨 MASS ASSIGNMENT VULNERABILITY: Field '{field}' was accepted!"
                            )
                            break
                except Exception as e:
            ...
            # Check if the request was rejected (good protection)
                ...
                is_vulnerable = False
                print(
                    f"  ✅ Mass assignment properly rejected (HTTP {mass_assign_status})"
                )
                            ...
                            try:
                                async with self.session.post(
                                    ...
                                    if gql_status == 200:
                                        ...
                                        print(
                                            f"    🚨 GraphQL introspection working - Schema exposed"
                                        )
                                ...
                                print(
                                    f"    ❌ GraphQL query {i+1} failed: {str(gql_e)}"
                                )
    ...
    async def _test_endpoint_data_leakage(self, endpoint: str):
        ...
        try:
            ...
            for header, value in response_headers.items():
                if any(
                    pattern.lower() in value.lower() for pattern in sensitive_patterns
                ):
            ...
            try:
                ...
                if isinstance(response_json, dict):
                    ...
                    sensitive_fields = [
                        ...
                        if any(
                            pattern in field.lower() for pattern in sensitive_patterns
                        )
                    ]
                    if sensitive_fields:
                        leakage_indicators.append(
                            f"Sensitive fields: {sensitive_fields[:3]}"
                        )
            except Exception as e:
                pass

            # Determine if vulnerable
            if len(leakage_indicators) > 3 or any(
                "password" in ind.lower()
                or "secret" in ind.lower()
                or "key" in ind.lower()
            ...
            if is_vulnerable:
                print(
                    f"  🚨 Data leakage detected: {len(leakage_indicators)} indicators"
                )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        vulnerabilities_by_type = {
            "RATE_LIMITING": [
                ...
                if r["test_type"] == "RATE_LIMITING"
                and not r.get("rate_limiting_effective", True)
            ],
            "IDOR_VULNERABILITY": [
                r
                for r in self.results
                if r["test_type"] == "IDOR_VULNERABILITY"
                and r.get("vulnerabilities_found", 0) > 0
            ],
            "MASS_ASSIGNMENT": [
                r
                for r in self.results
                if r["test_type"] == "MASS_ASSIGNMENT" and r.get("vulnerable", False)
            ],
            "GRAPHQL_SECURITY": [
                r
                for r in self.results
                if r["test_type"] == "GRAPHQL_SECURITY"
                and not r.get("graphql_secure", True)
            ],
            "DATA_LEAKAGE": [
                r
                for r in self.results
                if r["test_type"] == "DATA_LEAKAGE" and r.get("vulnerable", False)
            ],
        }

        total_vulnerabilities = sum(
            len(vulns) for vulns in vulnerabilities_by_type.values()
        )
        ...
        return {
            ...
            "security_score": max(
                0, 100 - (total_vulnerabilities * 10)
            ),  # Simple scoring
        }

    def _generate_api_recommendations(
        self, vulnerabilities_by_type: Dict[str, List]
    ) -> List[str]:
        ...
        if not any(vulnerabilities_by_type.values()):
            recommendations.append(
                "✅ No API vulnerabilities found - maintain current security practices"
            )
            ...
            for vuln_type, vulns in report["vulnerabilities_by_type"].items():
                if vulns:
                    ...
                    for vuln in vulns[:2]:  # Show first 2 examples
                        if "endpoint" in vuln:
                            print(
                                f"    - {vuln['endpoint']}: {vuln.get('status_code', 'N/A')}"
                            )

            ...
            try:
                ...
                if fixed_widths:
                    breakpoint_analysis["breakpoint_issues"].extend(
                        [
                            f"Fixed width {width} will break at 320px"
                            for width in fixed_widths[:3]
                        ]
                    )
                ...
                breakpoint_analysis["breakpoint_issues"].append(
                    f"Error reading {css_file}: {e}"
                )
        ...
        if comp_path.exists():
            try:
                ...
                fixed_patterns = re.findall(
                    r'(width|height|size)=["\']?\d{3,}["\']?', content
                )
                if fixed_patterns:
                    analysis["has_fixed_dimensions"] = True
                    analysis["issues"].extend(
                        [
                            f"Fixed dimension: {pattern}"
                            for pattern in fixed_patterns[:3]
                        ]
                    )
...
def generate_small_mobile_recommendations(
    breakpoint_analysis: Dict, component_analysis: Dict
) -> List[Dict]:
    ...
    # Check touch targets
    components_without_touch_friendly = [
        name
        for name, data in component_analysis.items()
...
def main():
    ...
    recommendations = generate_small_mobile_recommendations(
        breakpoint_analysis, component_analysis
    )
    ...
    print(
        f"✅ Has 320px breakpoint: {'Yes' if breakpoint_analysis['has_320px_breakpoint'] else 'No'}"
    )
    print(
        f"✅ Has small mobile support: {'Yes' if breakpoint_analysis['has_small_mobile_support'] else 'No'}"
    )
    ...
    responsive_components = sum(
        1
        for data in component_analysis.values()
        if data.get("has_responsive_classes", False)
    )
    mobile_optimized = sum(
        1
        for data in component_analysis.values()

...
class LoggingMonitoringFixer:
    ...
    def print_status(self, message: str, status: str = "info"):
        ...
        color = {"success": GREEN, "warning": YELLOW, "error": RED, "info": BLUE}.get(
            status, BLUE
        )
    ...
    def fix_log_permissions(self):
        ...
        for log_file in log_dir.glob("*.log"):
            try:
                ...
                self.print_status(
                    f"   ✅ Fixed {log_file.name}: {current_mode}", "success"
                )
                fixed_count += 1
        ...
        if fixed_count > 0:
            ...
            self.print_status(
                f"   📊 Fixed {fixed_count} log file permissions", "success"
            )
    ...
    def implement_structured_logging(self):
        ...
        try:
            ...
            if "StructuredFormatter" in content or "JSONFormatter" in content:
                self.print_status(
                    "   ℹ️  Structured logging already implemented", "info"
                )
                return
...
'''
            ...
            if import_end > 0:
                ...
                self.fixes_applied.append(
                    "Implemented structured JSON logging formatter"
                )
...
'''
            ...
            os.system(
                f"cd {self.project_root} && python -m app.core.log_integrity create"
            )
    ...
    def add_audit_logging_to_endpoints(self):
        ...
        for endpoint_path, endpoint_name in endpoints_to_fix.items():
            ...
            if not endpoint_file.exists():
                self.print_status(
                    f"   ⚠️  {endpoint_name} endpoint not found: {endpoint_path}",
                    "warning",
                )
                continue
            ...
            try:
                ...
                if "audit_logger" in content or "AuditLogger" in content:
                    self.print_status(
                        f"   ℹ️  {endpoint_name} already has audit logging imports",
                        "info",
                    )
                    continue
                ...
                for i, line in enumerate(lines):
                    ...
                    elif import_insert_index > 0 and not line.startswith(
                        ("from ", "import ")
                    ):
                        break
...
"""
                ...
                self.fixes_applied.append(
                    f"Added audit logging imports to {endpoint_name}"
                )
                self.print_status(
                    f"   ✅ Added audit logging to {endpoint_name}", "success"
                )

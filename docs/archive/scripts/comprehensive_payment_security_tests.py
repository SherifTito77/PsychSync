...
class PaymentSecurityTester:
    ...
    async def test_double_charging(self) -> TestResult:
        ...
        billing_files = ["app/api/v1/endpoints/billing.py", "app/services/billing.py"]
            ...
            if file_path.exists():
                ...
                for pattern in idempotency_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        ...
                        details.append(
                            f"✅ Found idempotency pattern: {pattern} in {file_path_str}"
                        )
                        break
        ...
        if not duplicate_prevention:
            ...
            details.append(
                f"{YELLOW}⚠️  WARNING: No explicit duplicate prevention found{RESET}"
            )
        ...
        if not rate_limit_found:
            ...
            details.append(
                f"{YELLOW}⚠️  WARNING: Payment rate limiting not confirmed{RESET}"
            )
        else:
        ...
        for file_path in frontend_files[:50]:  # Check first 50 files
            try:
                if (
                    "billing" in file_path.name.lower()
                    or "payment" in file_path.name.lower()
                ):
                    content = file_path.read_text()
        ...
        if not debounce_found:
            score -= 10
            details.append(
                f"{YELLOW}⚠️  INFO: Frontend click protection not confirmed{RESET}"
            )
        else:
    ...
    async def test_refund_race_conditions(self) -> TestResult:
        ...
        if billing_service.exists():
            ...
            refund_section = re.search(
                r"def.*refund.*\(|async def.*refund.*\(", content, re.IGNORECASE
            )
            if refund_section:
            ...
            if not transaction_found:
                ...
                details.append(
                    f"{RED}❌ CRITICAL: No transaction protection found{RESET}"
                )
            ...
            if not refund_idempotency:
                ...
                details.append(
                    f"{YELLOW}⚠️  WARNING: Refund idempotency not confirmed{RESET}"
                )
            ...
            if not webhook_found:
                score -= 20
                details.append(
                    f"{YELLOW}⚠️  INFO: Refund webhook handling not confirmed{RESET}"
                )
    ...
    async def test_pci_compliance(self) -> TestResult:
            ...
            try:
                ...
                for pattern in card_data_patterns:
                    ...
                    if (
                        ...
                        and "migration" not in str(py_file).lower()
                    ):
                        if re.search(pattern, content, re.IGNORECASE):
                            ...
                            if "=" in content:
                                ...
                                details.append(
                                    f"{RED}❌ CRITICAL: {pattern} in {py_file.name}{RESET}"
                                )
                                break
            except Exception:
                pass
        ...
        if not tls_configured:
            score -= 15
            details.append(
                f"{YELLOW}⚠️  WARNING: HTTPS/TLS configuration not confirmed{RESET}"
            )
                ...
                for pattern in log_patterns:
                    if re.search(pattern, content, re.IGNORECASE):
                        ...
                        for match in line_matches:
                            ...
                            if re.search(
                                r"\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}", line_content
                            ):
                                logging_safe = False
                                issues.append(
                                    SecurityIssue(
                                ...
                                details.append(
                                    f"{RED}❌ CRITICAL: Possible card logging in {py_file.name}{RESET}"
                                )
            except Exception:
                pass
        ...
        if not webhook_verified:
            score -= 10
            details.append(
                f"{YELLOW}⚠️  WARNING: Webhook signature verification not confirmed{RESET}"
            )
    ...
    async def test_card_data_leakage(self) -> TestResult:
        ...
        if billing_endpoints.exists():
            ...
            for pattern in return_patterns:
                ...
                for match in matches:
                    ...
                    if any(
                        field in return_statement.lower()
                        for field in [
                            "number",
                            "cvv",
                            "cvc",
        ...
        for schema_file in schema_files:
            try:
                ...
                if re.search(
                    r"(card_number|cvv|cvc|pin|expiry)", content, re.IGNORECASE
                ):
                    # Check if it's a legitimate model or just a field definition
                    if "class" in content or "BaseModel" in content:
                        card_exposure = True
                        details.append(
                            f"{YELLOW}⚠️  Card fields in {schema_file.name}{RESET}"
                        )
            except Exception:
                pass
        ...
        if risky_storage:
            ...
            details.append(
                f"{RED}❌ CRITICAL: Possible card data storage in frontend{RESET}"
            )
        else:
        ...
        for file in frontend_files[:100]:
            try:
                ...
                if re.search(r"\?.*card\s*=", content, re.IGNORECASE):
                    url_card_data = True
                    details.append(
                        f"{YELLOW}⚠️  Card data may be in URL: {file.name}{RESET}"
                    )
            except Exception:
                pass
        ...
        for log_file in log_files:
            try:
                ...
                if re.search(r"\d{4}[-\s]?\d{4}[-\s]?\d{4}[-\s]?\d{4}", content):
                    card_in_logs = True
                    details.append(
                        f"{RED}❌ CRITICAL: Card-like numbers in {log_file.name}{RESET}"
                    )
                    break
            except Exception:
                pass
        ...
        for env_file in env_files:
            try:
                ...
                if re.search(r"4242.*4242.*4242.*4242", content):
                    card_in_env = True
                    details.append(
                        f"{YELLOW}⚠️  Test card numbers in {env_file.name} (development){RESET}"
                    )
            except Exception:
                pass
    ...
    async def test_subscription_cancellation(self) -> TestResult:
        ...
        if billing_service.exists():
            ...
            if billing_endpoints.exists():
                ...
                if (
                    "current_user" in endpoint_content
                    and "subscription_id" in endpoint_content
                ):
                    ownership_check = True
            ...
            if not ownership_check:
                ...
                details.append(
                    f"{RED}❌ CRITICAL: No ownership verification found{RESET}"
                )
            ...
            if (
                "immediate" in service_content.lower()
                and "cancel" in service_content.lower()
            ):
            ...
            if not (immediate_cancel and period_end_cancel):
                score -= 15
                details.append(
                    f"{YELLOW}⚠️  WARNING: Cancellation options incomplete{RESET}"
                )
            ...
            refund_policy = False
            refund_patterns = [r"refund.*policy", r"proration", r"refund.*remaining"]
            ...
            if not audit_logging:
                score -= 10
                details.append(
                    f"{YELLOW}⚠️  INFO: Cancellation audit logging not confirmed{RESET}"
                )
            ...
            if not webhook_cancellation:
                score -= 10
                details.append(
                    f"{YELLOW}⚠️  INFO: Webhook cancellation handling not confirmed{RESET}"
                )
        ...
        if not access_revocation:
            ...
            details.append(
                f"{YELLOW}⚠️  WARNING: Access revocation not confirmed{RESET}"
            )

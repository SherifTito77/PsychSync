...
class InputValidationSecurityTester:
    ...
    async def _test_sql_injection_endpoint(
        self, endpoint: Dict[str, Any], payload: str
    ):
        ...
        try:
            ...
            is_vulnerable = (
                ...
                and payload in test_data.values()  # Unexpected success
            )
            ...
            if is_vulnerable:
                logger.warning(
                    f"🚨 SQL INJECTION DETECTED: {endpoint['path']} - Payload: {payload}"
                )
            else:
    ...
    async def _test_xss_endpoint(self, endpoint: Dict[str, Any], payload: str):
        ...
        try:
            ...
            if is_vulnerable:
                logger.warning(
                    f"🚨 XSS DETECTED: {endpoint['path']} - Payload: {payload}"
                )
            else:
    ...
    async def _test_parameter_pollution(
        self, endpoint: Dict[str, Any], pollution_test: Dict[str, Any]
    ):
        ...
        try:
            ...
            is_vulnerable = status == 200 and any(
                indicator not in response_text.lower()
                for indicator in ["error", "invalid"]
            )
            ...
            if is_vulnerable:
                logger.warning(
                    f"🚨 PARAMETER POLLUTION DETECTED: {endpoint['path']} - Param: {pollution_test['param']}"
                )
            else:
            ...
            logger.error(
                f"❌ Parameter Pollution Test Error: {endpoint['path']} - {str(e)}"
            )
    ...
    async def _test_html_injection_endpoint(
        self, endpoint: Dict[str, Any], payload: str
    ):
        ...
        try:
            ...
            is_vulnerable = (
                status == 200
                and any(
                    indicator in response_text.lower() for indicator in html_indicators
                )
            ...
            if is_vulnerable:
                logger.warning(
                    f"🚨 HTML INJECTION DETECTED: {endpoint['path']} - Payload: {payload[:50]}..."
                )
            else:
    ...
    async def test_business_logic_bypass(self):
        ...
        bypass_tests = [
            # Price/amount manipulation
            {
                ...
                "method": "POST",
                "data": {"price": -100, "title": "test"},
            },
            {
                "endpoint": "/api/v1/assessments",
                "method": "POST",
                "data": {"price": 0, "title": "test"},
            },
            {
                "endpoint": "/api/v1/assessments",
                "method": "POST",
                "data": {"price": 0.01, "title": "test"},
            },
            # Role manipulation
            {
            ...
            },
            {
                "endpoint": "/api/v1/assessments",
                "method": "GET",
                "params": {"limit": "999999 OR 1=1"},
            },
            # Status manipulation
            {
                "endpoint": "/api/v1/assessments",
                "method": "POST",
                "data": {"title": "test", "status": "approved"},
            },
            {
                "endpoint": "/api/v1/assessments",
                "method": "POST",
            ...
            # Date manipulation
            {
                "endpoint": "/api/v1/assessments",
                "method": "POST",
                "data": {"title": "test", "created_at": "2099-12-31"},
            },
            {
                "endpoint": "/api/v1/assessments",
                "method": "POST",
                "data": {"title": "test", "expires_at": "1970-01-01"},
            },
        ]
    ...
    async def _test_business_logic_bypass(self, test_case: Dict[str, Any]):
        ...
        try:
            ...
            should_fail = (
                "admin" in str(data)
                or -1 in str(data).values()  # Trying to set admin role
                or -1 in params.values()
                or 99999 in str(url)  # Negative values
                or 0 in str(data).values()  # Very large ID
                or 0 in params.values()  # Zero values
            )
            ...
            if is_vulnerable:
                logger.warning(
                    f"🚨 BUSINESS LOGIC BYPASS DETECTED: {test_case['endpoint']} - Data: {data}"
                )
            else:
            ...
            logger.error(
                f"❌ Business Logic Test Error: {test_case['endpoint']} - {str(e)}"
            )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "vulnerabilities_by_type": {
                "SQL_INJECTION": [
                    r for r in vulnerabilities if r["test_type"] == "SQL_INJECTION"
                ],
                "XSS_EVENT_HANDLERS": [
                    r for r in vulnerabilities if r["test_type"] == "XSS_EVENT_HANDLERS"
                ],
                "HTTP_PARAMETER_POLLUTION": [
                    r
                    for r in vulnerabilities
                    if r["test_type"] == "HTTP_PARAMETER_POLLUTION"
                ],
                "HTML_INJECTION": [
                    r for r in vulnerabilities if r["test_type"] == "HTML_INJECTION"
                ],
                "BUSINESS_LOGIC_BYPASS": [
                    r
                    for r in vulnerabilities
                    if r["test_type"] == "BUSINESS_LOGIC_BYPASS"
                ],
            },

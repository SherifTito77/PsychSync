        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(token_url, data=token_data) as response:
                if response.status != 200:
                    raise HTTPException(
                        status_code=400, detail="Fitbit token exchange failed"
                    )
...
class HealthDataService:
    ...
    async def connect_device(
        self, user_id: int, device_connect: DeviceConnectRequest
        ...
        try:
            ...
            self.logger.error(
                f"Failed to connect device {device_connect.device_type}: {str(e)}"
            )
            return DeviceConnectResponse(
    ...
    async def sync_device_data(self, device_id: int) -> bool:
        ...
        try:
            device = (
                self.db.query(HealthDevice).filter(HealthDevice.id == device_id).first()
            )
            ...
            raw_data = await adapter.fetch_data(
                device.access_token, start_date, end_date
            )
            ...
            self.logger.info(
                f"Synced {len(normalized_data)} data points for device {device_id}"
            )
            return True
    ...
    async def get_health_summary(
        self, user_id: int, time_range: str = "7days"
    ) -> HealthDataSummary:
    ...
    async def generate_insights(
        self, metrics: Dict[str, Any], data_points: List[HealthDataPoint]
    ) -> List[Dict[str, Any]]:
    ...
    async def calculate_trend(self, user_id: int, start_date: datetime) -> float:
        ...
        second_half_steps = sum(
            p.value for p in second_half if p.metric_type == "steps"
        )
...
async def connect_health_device(
    user_id: int, device_connect: DeviceConnectRequest, db: Session = Depends(get_db)
):
...
async def sync_device_data(device_id: int, db: Session = Depends(get_db)):
    """Sync data from connected device"""
    service = HealthDataService(db)
...
async def get_health_summary(
    user_id: int, time_range: str = "7days", db: Session = Depends(get_db)
):
...
async def disconnect_device(
    user_id: int, device_id: int, db: Session = Depends(get_db)
):
    """Disconnect health device"""
    device = (
        db.query(HealthDevice)
        .filter(HealthDevice.id == device_id, HealthDevice.user_id == user_id)
        .first()
    )

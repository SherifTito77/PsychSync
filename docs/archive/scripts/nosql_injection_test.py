...
class NoSQLInjectionTester:
    ...
    async def test_endpoint_injection(
        self, session: aiohttp.ClientSession, endpoint: str, payload: Dict
    ) -> Dict[str, Any]:
    ...
    async def check_injection_indicators(
        self, test_result: Dict, content: str, response: aiohttp.ClientResponse
    ):
    ...
    async def run_injection_tests(self) -> List[Dict[str, Any]]:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            print(
                f"📊 Testing {len(self.test_endpoints)} endpoints with {len(self.injection_payloads)} payloads each..."
            )
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "dangerous_payloads": dict(
                sorted(payload_danger.items(), key=lambda x: x[1], reverse=True)[:10]
            ),
            "recommendations": [],
        }
...
async def main():
    ...
    try:
        ...
        with open(
            "/Users/sheriftito/Downloads/psychsync/nosql_injection_report.json", "w"
        ) as f:

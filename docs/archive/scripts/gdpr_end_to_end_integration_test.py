...
class GDPRIntegrationTester:
    ...
    def log_test_step(self, step: str, status: str, details: str = ""):
        ...
        self.test_results.append(
            {"timestamp": timestamp, "step": step, "status": status, "details": details}
        )
    ...
    def test_backend_connectivity(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                self.log_test_step(
                    "Backend Connectivity Check", "PASS", "Backend responding normally"
                )
                return True
            else:
                self.log_test_step(
                    "Backend Connectivity Check", "FAIL", f"HTTP {response.status_code}"
                )
                return False
        ...
        for endpoint in endpoints:
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.log_test_step(
                        f"GDPR Endpoint: {endpoint}", "PASS", "Endpoint available"
                    )
                else:
                    results[endpoint] = {
                        "status": "error",
    ...
    def test_data_export_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            formats_response = requests.get(
                f"{self.backend_url}/api/v1/gdpr/export-formats", timeout=10
            )
            ...
            if formats_response.status_code == 200:
                ...
                for format_type in available_formats:
                    try:
                        ...
                        if export_response.status_code in [
                            200,
                            201,
                            401,
                            ...
                            export_results[format_type] = (
                                f"error_{export_response.status_code}"
                            )
                            self.log_test_step(
                                f"Export Format: {format_type}",
                                "FAIL",
                        ...
                        self.log_test_step(
                            f"Export Format: {format_type}", "FAIL", str(e)
                        )
                ...
                self.log_test_step(
                    "Data Export Formats",
                    "FAIL",
                    f"HTTP {formats_response.status_code}",
                )
        ...
        try:
            ...
            if categories_response.status_code == 200:
                ...
                self.log_test_step(
                    ...
                    "PASS",
                    f"Found {len(consent_categories)} categories",
                )
                ...
                if consent_response.status_code in [200, 201]:
                    ...
                    self.log_test_step(
                        ...
                        "PASS",
                        f"Consent recorded: {consent_result.get('consent_id', 'N/A')}",
                    )
                    ...
                    self.log_test_step(
                        "Cookie Consent Recording",
                        "FAIL",
                        f"HTTP {consent_response.status_code}",
                    )
                    return {
                        "categories_available": len(consent_categories),
                ...
                self.log_test_step(
                    "Cookie Consent Categories",
                    "FAIL",
                    f"HTTP {categories_response.status_code}",
                )
    ...
    def test_data_retention_policies(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.backend_url}/api/v1/gdpr/data-retention-policy", timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                found_elements = [
                    ...
                    if elem in policy.get("policy", {})
                ]

                self.log_test_step(
                    "Data Retention Policy",
                    "PASS",
                    f"Found {len(found_elements)}/{len(required_elements)} required elements",
                )
                ...
                return {
                    ...
                    "compliance_score": len(found_elements)
                    / len(required_elements)
                    * 100,
                }
            else:
                self.log_test_step(
                    "Data Retention Policy", "FAIL", f"HTTP {response.status_code}"
                )
    ...
    def test_privacy_policy_accessibility(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.backend_url}/api/v1/gdpr/privacy-policy", timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                self.log_test_step(
                    "Privacy Policy Access", "PASS", "Privacy policy accessible"
                )
                ...
                self.log_test_step(
                    "Privacy Policy Access", "FAIL", f"HTTP {response.status_code}"
                )
                return {"policy_accessible": False, "error_code": response.status_code}
    ...
    def test_processing_activities_registry(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.backend_url}/api/v1/gdpr/processing-activities", timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                self.log_test_step(
                    ...
                    "PASS",
                    f"Found {len(processing_activities)} activities",
                )

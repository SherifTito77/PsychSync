...
from app.schemas.assessment import AssessmentBase, QuestionCreate
        ...
        for email in valid_emails:
            try:
                user_data = UserRegister(
                    email=email, password="ValidPassword123!", full_name="Test User"
                )
                passed = True
        ...
        for q_type in valid_question_types:
            try:
                question_data = QuestionCreate(
                    question_type=q_type, question_text="Sample question text"
                )
                passed = True
                self.log_test_result(
    ...
    def test_xss_prevention(self):
        ...
        for payload in xss_payloads:
            try:
                ...
                passed = (
                    not contains_script
                )  # For this test, we consider rejection as better
                self.log_test_result(
    ...
    def test_length_validation(self):
        ...
        try:
            assessment_data = AssessmentBase(
                title=very_long_string, description="Test assessment", category="test"
            )
            passed = False
            self.log_test_result(

...
class MobileViewportTester:
    ...
    def check_text_readability(self) -> Tuple[bool, List[str]]:
        ...
        try:
            ...
            if overflow_elements:
                issues.extend(
                    [f"Text overflow in: {elem}" for elem in overflow_elements]
                )
    ...
    def test_page(
        self, page_name: str, route: str, viewport_name: str, viewport_size: Dict
    ) -> PageTestResult:
        """Test a single page at specific viewport size"""
        print(
            f"🔍 Testing {page_name} at {viewport_size['width']}x{viewport_size['height']} ({viewport_name})"
        )
        ...
        try:
            ...
            try:
                viewport_meta = self.driver.find_element(
        ...
        return PageTestResult(
            ...
            width_overflow=(
                has_horiz_scroll if "has_horiz_scroll" in locals() else False
            ),
            horizontal_scroll=(
                has_horiz_scroll if "has_horiz_scroll" in locals() else False
            ),
    ...
    def run_all_tests(self, viewports: List[str] = None) -> None:
        ...
        for viewport_name in viewports:
            ...
            print(
                f"📱 Testing viewport: {viewport_name} ({viewport_size['width']}x{viewport_size['height']})"
            )
            print("-" * 60)
            ...
            for page_name, route in PAGE_ROUTES.items():
                ...
                if result.load_success:
                    if result.issues:
                        ...
                        major_count = sum(
                            1 for issue in result.issues if issue.severity == "major"
                        )
                        minor_count = sum(
                            1 for issue in result.issues if issue.severity == "minor"
                        )
                        print(
                            f"⚠️  {page_name}: {critical_count} critical, {major_count} major, {minor_count} minor issues"
                        )
                    else:
    ...
    def generate_report(self) -> Dict:
        ...
        critical_issues = sum(
            len([r for r in result.issues if r.severity == "critical"])
            for result in self.results
        )
        major_issues = sum(
            len([r for r in result.issues if r.severity == "major"])
            for result in self.results
        )
        minor_issues = sum(
            len([r for r in result.issues if r.severity == "minor"])
            for result in self.results
        )
        ...
        for result in self.results:
            ...
            page_scores[result.page]["critical"] += sum(
                1 for issue in result.issues if issue.severity == "critical"
            )
        ...
        most_problematic = sorted(
            page_scores.items(),
            key=lambda x: (x[1]["critical"], x[1]["issues"]),
            reverse=True,
        )
        ...
        return {
            "summary": {
                ...
                "pages_with_failures": total_pages
                - len([p for p, score in page_scores.items() if score["load_success"]]),
            },
    ...
    def save_report(
        self, report: Dict, filename: str = "mobile_viewport_test_report.json"
    ) -> None:
    ...
    def print_summary(self, report: Dict) -> None:
        ...
        if report["most_problematic_pages"]:
            ...
            for page_name, issues in report["most_problematic_pages"][:5]:
                if issues["critical"] > 0 or not issues["load_success"]:
                    status = (
                        "❌ FAILED TO LOAD"
                        if not issues["load_success"]
                        else f"{issues['critical']} critical issues"
                    )
        ...
        if (
            ...
            and summary["major_issues"] == 0
            and summary["pages_with_failures"] == 0
        ):
            print(
                "  🎉 Excellent mobile compatibility! All pages passed basic usability tests."
            )
...
def main():
    ...
    parser.add_argument(
        "--base-url", default="http://localhost:5173", help="Base URL to test"
    )
    parser.add_argument("--width", type=int, default=375, help="Viewport width to test")
    parser.add_argument(
        "--height", type=int, default=667, help="Viewport height to test"
    )
    parser.add_argument(
        "--headless", action="store_true", default=True, help="Run headless"
    )
    parser.add_argument(
        "--screenshot-dir", default="./screenshots", help="Screenshot directory"
    )
    parser.add_argument(
        "--viewport",
        choices=["iphone_se", "small_mobile", "very_small_mobile"],
        default=["iphone_se", "small_mobile"],
        nargs="+",

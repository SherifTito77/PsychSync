...
class FilesystemSecurityTester:
    ...
    async def test_directory_traversal(self) -> TestResult:
            ...
            try:
                ...
                for pattern, desc in file_operations:
                    if pattern in content:
                        ...
                        function_code = self._extract_function_with_pattern(
                            content, pattern
                        )
                        ...
                        if function_code:
                            ...
                            if not has_validation:
                                ...
                                details.append(
                                    f"{RED}⚠️  {desc} in {filename} (no validation detected){RESET}"
                                )
                            else:
                ...
                details.append(
                    f"{YELLOW}⚠️  Error scanning {endpoint_file.name}: {e}{RESET}"
                )
                    ...
                    if risky in content:
                        ...
                        for i, line in enumerate(lines, 1):
                            if risky in line:
                                ...
                                details.append(
                                    f"{YELLOW}⚠️  Risky path in {py_file.name}:{i} - {risky}{RESET}"
                                )
            except Exception:
                pass
            ...
            if config_path.exists():
                ...
                if "StaticFiles" in content or "static" in content.lower():
                    ...
                    if "/static" in content or "staticfiles" in content.lower():
                        details.append(
                            f"{GREEN}✅ Static files configured in {config_file}{RESET}"
                        )
                    else:
                        # Check for root mounting
                        if '"/"' in content or 'path="/"' in content:
                            vulnerable_patterns.append(
                                (config_file, "Root static file mounting", '"/"')
                            )
                            details.append(
                                f"{RED}❌ CRITICAL: Root static mounting in {config_file}{RESET}"
                            )
                            score -= 30
        ...
        for endpoint_file in api_endpoints:
            try:
                ...
                if "upload" in content.lower() and "file" in content.lower():
                    ...
                    if (
                        "safe_filename" not in content.lower()
                        and "sanitize" not in content.lower()
                    ):
                        upload_endpoints.append(
                            (endpoint_file.name, "No filename sanitization")
                        )
                        details.append(
                            f"{YELLOW}⚠️  Upload endpoint without filename sanitization: {endpoint_file.name}{RESET}"
                        )
                        score -= 15
                    else:
                        details.append(
                            f"{GREEN}✅ Upload endpoint has sanitization: {endpoint_file.name}{RESET}"
                        )
            except Exception:
                pass
        ...
        if vulnerable_patterns:
            ...
            details.append(
                f"{GREEN}✅ No directory traversal vulnerabilities detected{RESET}"
            )
    ...
    def _extract_function_with_pattern(
    ...
    async def test_temp_directories(self) -> TestResult:
        ...
        for temp_dir in temp_dirs:
            ...
            if temp_path.exists():
                ...
                if self._is_web_accessible(temp_path):
                    ...
                    details.append(
                        f"{RED}❌ CRITICAL: Web-accessible temp directory: {temp_dir}{RESET}"
                    )
                    score -= 25
                else:
                    details.append(
                        f"{GREEN}✅ Temp directory not web-accessible: {temp_dir}{RESET}"
                    )
                ...
                for file_path in public_path.rglob("*"):
                    if file_path.is_file():
                        ...
                        if file_path.suffix in [".php", ".sh", ".bash", ".py"]:
                            details.append(
                                f"{RED}❌ CRITICAL: Executable script in public dir: {file_path.relative_to(project_root)}{RESET}"
                            )
                            score -= 15
                        ...
                        if file_path.suffix in [".env", ".key", ".pem", ".sql"]:
                            details.append(
                                f"{RED}❌ CRITICAL: Sensitive file in public dir: {file_path.name}{RESET}"
                            )
                            score -= 20
        ...
        for config in nginx_configs:
            try:
                ...
                if "location" in content and (
                    "tmp" in content.lower() or "temp" in content.lower()
                ):
                    details.append(
                        f"{YELLOW}⚠️  Nginx config may serve temp directory: {config.relative_to(project_root)}{RESET}"
                    )
                    score -= 15
                ...
                if "autoindex on" in content.lower():
                    details.append(
                        f"{YELLOW}⚠️  Directory listing enabled in: {config.relative_to(project_root)}{RESET}"
                    )
                    score -= 10
            except Exception:
                pass
    ...
    async def test_deployment_artifacts(self) -> TestResult:
            ...
            if artifact_name.startswith("*"):
                ...
                for match in project_root.rglob(artifact_name[1:]):
                    if match.is_file():
                        ...
                        details.append(
                            f"{YELLOW}⚠️  Found artifact: {rel_path} ({description}){RESET}"
                        )
                        score -= 2
            else:
                # Direct path
            ...
            for match in matches:
                if match.is_file():
                    ...
                    if "tests/" not in str(rel_path):
                        details.append(
                            f"{YELLOW}⚠️  Development file in root: {rel_path}{RESET}"
                        )
                        score -= 3
        ...
        for pattern in backup_patterns:
            for match in project_root.rglob(pattern):
                if match.is_file():
                    ...
                    details.append(
                        f"{RED}❌ Backup file: {rel_path} ({size} bytes){RESET}"
                    )
                    score -= 5
        ...
        if (project_root / ".git").exists():
            details.append(
                f"{YELLOW}⚠️  .git directory present (OK for development, remove for production){RESET}"
            )
            score -= 10
            ...
            if build_path.exists():
                ...
                if gitignore.exists():
                    ...
                    if build_dir not in gitignore_content:
                        details.append(
                            f"{YELLOW}⚠️  Build dir not in .gitignore: {build_dir}{RESET}"
                        )
                        score -= 5
                    else:
                        details.append(
                            f"{GREEN}✅ Build dir in .gitignore: {build_dir}{RESET}"
                        )
    ...
    async def test_env_file_exposure(self) -> TestResult:
        ...
        for env_file in env_files:
            ...
            if self._is_web_accessible(env_file.parent):
                details.append(
                    f"{RED}❌ CRITICAL: .env file in web-accessible location: {rel_path}{RESET}"
                )
                score -= 40
                ...
                try:
                    ...
                    if secrets_found:
                        details.append(
                            f"{YELLOW}⚠️  {rel_path} contains: {', '.join(secrets_found)}{RESET}"
                        )

                        # Check if it's the example/template
                        if (
                            "example" in str(env_file).lower()
                            or "template" in str(env_file).lower()
                        ):
                            details.append(
                                f"{GREEN}  → Appears to be a template (safe){RESET}"
                            )
                        else:
        ...
        if gitignore.exists():
            ...
            if missing_patterns:
                details.append(
                    f"{YELLOW}⚠️  .gitignore missing: {', '.join(missing_patterns)}{RESET}"
                )
                score -= 15
            else:
        ...
        try:
            ...
            if result.stdout.strip():
                ...
                details.append(
                    f"{RED}❌ CRITICAL: .env files tracked by git: {', '.join(tracked_env_files)}{RESET}"
                )
                score -= 30
            ...
            try:
                ...
                for pattern, desc in secret_patterns:
                    ...
                    for match in matches:
                        ...
                        details.append(
                            f"{RED}❌ Hardcoded {desc} in {py_file.relative_to(project_root)}:{line_num}{RESET}"
                        )
                        score -= 5
            except Exception:
                pass
                ...
                for pattern, desc in secret_patterns.items():
                    ...
                    if matches:
                        ...
                        if unique_contexts:
                            ...
                            details.append(
                                f"{RED}❌ {desc} found in {rel_path} ({len(matches)} occurrences){RESET}"
                            )
                            ...
                            for ctx in list(unique_contexts)[:2]:
                                ...
                                redacted = re.sub(
                                    r"ey[A-Za-z0-9+/=]{10,}", "ey***REDACTED***", ctx
                                )
                                redacted = re.sub(
                                    r"sk_[a-z]+_[A-Za-z0-9]{10,}",
                                    "sk_***REDACTED***",
                                    redacted,
                                    flags=re.IGNORECASE,
                                )
        ...
        for log_file in log_files[:20]:  # Check first 20 log files
            ...
            try:
                ...
                for pattern, desc in pii_patterns.items():
                    ...
                    if (
                        matches and len(matches) > 5
                    ):  # Only flag if more than 5 occurrences
                        details.append(
                            f"{YELLOW}⚠️  Possible {desc} in {rel_path} ({len(matches)} occurrences){RESET}"
                        )
                        score -= 10
            except Exception:
                pass
        ...
        for config_file in log_config_files:
            ...
            if config_path.exists():
                ...
                if "sanitize" in content.lower() or "redact" in content.lower():
                    details.append(
                        f"{GREEN}✅ Log sanitization configured in {config_file}{RESET}"
                    )
                else:
                    details.append(
                        f"{YELLOW}⚠️  No log sanitization found in {config_file}{RESET}"
                    )
                    score -= 10
                ...
                if "StreamHandler" in content or "console" in content.lower():
                    details.append(
                        f"{YELLOW}⚠️  Console logging in {config_file} (be careful with output){RESET}"
                    )
        ...
        for result in self.test_results:
            ...
            if result.issues:
                for issue in result.issues:
                    ...
                    print(
                        f"   {severity_color}● {issue.severity.upper()}: {issue.title}{RESET}"
                    )

        # Print issue count
        total_issues = sum(len(r.issues) for r in self.test_results)
        critical_issues = sum(
            1 for r in self.test_results for i in r.issues if i.severity == "critical"
        )

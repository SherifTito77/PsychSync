        ...
        try:
            if method == "GET":
                ...
                response = requests.post(
                    f"{API_URL}{endpoint}", headers=get_headers(), json={"test": "data"}
                )

            status_icon = (
                "✅"
                if response.status_code == 200
                else "❌" if response.status_code == 404 else "⚠️"
            )
            print(
                f"{method:4} {endpoint:40} {response.status_code:4} {status_icon} {description}"
            )

        ...
        return [
            {
                ...
                "conditions": [
                    {
                        "field": "description",
                        "pattern": r"((?i)union.*select|select.*from|insert.*values|update.*set|delete.*from)",
                    },
            ...
            },
            {
                ...
                "conditions": [
                    ...
                    {
                        "field": "description",
                        "pattern": r"((?i)download.*large|export.*bulk)",
                    },
                ],
                "actions": [
                    "immediate_alert",
            ...
            with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    ...
                    security_event = (
                        ...
                        or self.detect_privilege_escalation(log_entry)
                        or self.detect_data_exfiltration(log_entry)
                    )
            ...
            if event.event_type in rule["event_types"]:
                ...
                for condition in rule["conditions"]:
                    ...
                    if "pattern" in condition:
                        if not re.search(
                            condition["pattern"], str(field_value), re.IGNORECASE
                        ):
                            rule_match = False
                            break
                        ...
                        field_value_numeric = (
                            float(str(field_value)) if str(field_value).isdigit() else 0
                        )
                        condition_value = float(condition["value"])

                        if (
                            condition["operator"] == ">"
                            and field_value_numeric <= condition_value
                        ):
                            rule_match = False
                            break
                        elif (
...
Required Actions: {', '.join(alert['actions'])}
    ...
    def run_security_scan(self) -> Dict[str, Any]:
        ...
        scan_results["scan_summary"] = {
            ...
            "critical_events": len(
                [
                    e
                    for e in scan_results["events_detected"]
            ...
            "high_severity_events": len(
                [
                    e
                    for e in scan_results["events_detected"]
                    if e.severity == SecuritySeverity.HIGH
                ]
            ),
        }
    ...
    def store_security_metrics(self, scan_results: Dict):
        ...
        try:
            ...
            metrics = [
                ...
                (
                    "alerts_generated",
                    scan_results["scan_summary"]["alerts_generated_this_scan"],
                ),
                ("critical_events", scan_results["scan_summary"]["critical_events"]),
                (
                    "high_severity_events",
                    scan_results["scan_summary"]["high_severity_events"],
                ),
                ("total_alerts", len(self.alert_log)),
            ]
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                metrics_24h[metric_type].append(
                    {"value": metric_value, "timestamp": timestamp}
                )
    ...
    def start_continuous_monitoring(self, interval_seconds: int = 60):
        ...
        print(
            f"🔄 Starting continuous security monitoring (interval: {interval_seconds}s)"
        )
...
def main():
    ...
    if "error" not in dashboard:
        ...
        # Save dashboard
        with open(
            "/Users/sheriftito/Downloads/psychsync/security_dashboard.json", "w"
        ) as f:
    ...
    try:
        ...
        if choice == "2":
            interval = input(
                "Enter monitoring interval in seconds (default: 60): "
            ).strip()
            ...
            scan_results = monitoring_system.run_security_scan()
            print(
                f"✅ Scan complete. {scan_results['scan_summary']['total_events']} events detected."
            )

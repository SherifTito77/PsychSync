...
class APIErrorValidator:
    ...
    async def test_error_scenario(
        ...
        try:
            ...
            graceful_handling, error_message, validation_passed = (
    ...
    def _validate_error_response(
        ...
        if not response_body:
            ...
            missing_fields = [
                field for field in required_fields if field not in response_body
            ]
            ...
            if not has_helpful_fields and actual_status >= 400:
                error_message += (
                    "Missing additional context (details, error_code, etc.). "
                )
    ...
    async def test_business_logic_errors(self) -> List[ErrorTestResult]:
        ...
        tests = [
            ...
            (
                "invalid_assessment_update",
                "/api/v1/assessments/99999/complete",
                "POST",
                {},
                None,
                404,
            ...
            (
                ...
                "/api/v1/auth/login",
                "POST",
                {"email": "ratelimit@example.com", "password": "test123"},
                None,
                429,
            ),
        ]
    ...
    async def test_method_not_allowed(self) -> List[ErrorTestResult]:
        ...
        for test_name, endpoint, method in tests:
            result = await self.test_error_scenario(
                f"method_not_allowed_{test_name}", endpoint, method, None, None, 405
            )
            results.append(result)
    ...
    async def generate_error_validation_report(
        self, all_results: List[ErrorTestResult]
    ) -> Dict[str, Any]:
        ...
        return {
            "test_summary": {
                ...
                "graceful_handling_rate_percentage": (
                    (graceful_handled / total_tests) * 100 if total_tests > 0 else 0
                ),
            ...
            "test_type_breakdown": {
                test_type: {
                    ...
                    "pass_rate": (
                        len([r for r in results if r.validation_passed]) / len(results)
                    )
                    * 100,
                }
            ...
            "common_issues": {
                issue_type: list(set(messages))[:3]
                for issue_type, messages in common_issues.items()
            },
            "failed_tests": [
                {
            ...
            "recommendations": self._generate_error_recommendations(
                passed_tests, total_tests, common_issues
            ),
        }

    def _generate_error_recommendations(
        self, passed_tests: int, total_tests: int, common_issues: Dict[str, List[str]]
    ) -> List[str]:
        ...
        if pass_rate < 80:
            recommendations.append(
                "🚨 Low error handling pass rate. Implement comprehensive error responses for all endpoints."
            )
        ...
        if "missing" in str(common_issues).lower():
            recommendations.append(
                "📋 Add consistent error response structure with 'error', 'message', and 'details' fields."
            )

        if "timeout" in str(common_issues).lower():
            recommendations.append(
                "⏱️ Implement proper timeout handling and return appropriate timeout error responses."
            )

        if "empty" in str(common_issues).lower():
            recommendations.append(
                "📄 Ensure all error responses include meaningful JSON content."
            )

        if "internal_server" in str(common_issues).lower():
            recommendations.append(
                "🔧 Convert internal server errors to appropriate client error codes where possible."
            )

        if pass_rate >= 90:
            recommendations.append(
                "✅ Excellent error handling! Most scenarios are handled gracefully."
            )

        if pass_rate >= 80 and pass_rate < 90:
            recommendations.append(
                "👍 Good error handling. Address the remaining failed tests for full compliance."
            )
...
async def main():
    ...
    async with APIErrorValidator(BASE_URL) as validator:
        ...
        try:
            ...
            print(
                f"   • Passed: {summary['passed_tests']} ({summary['pass_rate_percentage']:.1f}%)"
            )
            print(f"   • Failed: {summary['failed_tests']}")
            print(
                f"   • Graceful Handling: {summary['graceful_handling_rate_percentage']:.1f}%"
            )
            print(
                f"   • Avg Response Time: {summary['average_response_time_ms']:.0f}ms"
            )
            ...
            for test_type, stats in report["test_type_breakdown"].items():
                status = (
                    ...
                    else "⚠️" if stats["pass_rate"] >= 60 else "❌"
                )
                print(
                    f"   {status} {test_type.title()}: {stats['passed']}/{stats['total']} ({stats['pass_rate']:.1f}%)"
                )

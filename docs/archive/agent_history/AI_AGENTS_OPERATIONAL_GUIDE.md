...
# AI agents are in the app.ai.agents/ directory
ls app.ai.agents/
...
python app.ai.agents/run_agents.py --category testing

# Run all performance agents
python app.ai.agents/run_agents.py --category performance
```
...
python app.ai.agents/run_agents.py --all --output agent_report.json
```
...
python app.ai.agents/run_agents.py --agent debug_code_remover
  ...
  ai-agents:
    ...
    steps:
      ...
      - name: Run critical agents
        run: |
          python app.ai.agents/run_agents.py \
...
0 2 * * * cd /path/to/psychsync && python app.ai.agents/run_agents.py --all --output daily_report.json >> /var/log/app.ai.agents.log 2>&1

# Run security agents every 6 hours
0 */6 * * * cd /path/to/psychsync && python app.ai.agents/run_agents.py --category security --output security_report.json >> /var/log/app.ai.agents.log 2>&1

# Run performance agents weekly on Sunday at 3 AM
0 3 * * 0 cd /path/to/psychsync && python app.ai.agents/run_agents.py --category performance --output perf_report.json >> /var/log/app.ai.agents.log 2>&1
```
...
```python
from app.ai.agents import AgentOrchestrator, AgentConfig
from app.ai.agents.code_quality_agents import CodeQualityMonitorAgent
...
```python
from app.ai.agents import BaseAgent, AgentConfig, AgentResult
...
ai-agents:
  ...
  script:
    ...
    - python app.ai.agents/run_agents.py --all --output results.json
...
from app.ai.agents import BaseAgent
...
0 2 * * * python app.ai.agents/run_agents.py --agent load_testing
0 3 * * 0 python app.ai.agents/run_agents.py --category performance
```
...
```bash
# Check agent logs
tail -f app.ai.agents/agent_execution.log
...
```bash
# Check agent logs
cat app.ai.agents/agent_execution.log

# Run with verbose output
python app.ai.agents/run_agents.py --agent <name> --debug
```
...
- **Agent Framework**: `app.ai.agents/agent_framework.py`
- **Agent Implementations**: `app.ai.agents/*_agents.py`
- **Orchestrator Script**: `app.ai.agents/run_agents.py`
- **Execution Logs**: `app.ai.agents/agent_execution.log`
- **Sample Reports**: `app.ai.agents/agent_report.json`
...
1. Check this guide
2. Review agent logs: `app.ai.agents/agent_execution.log`
3. Check agent implementations in `app.ai.agents/*_agents.py`

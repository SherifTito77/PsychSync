...
def merge_sboms(python_sbom, frontend_sbom, output_file):
    ...
    merged_sbom = {
        ...
        "metadata": {
            ...
            "tools": [
                {"vendor": "PsychSync", "name": "SBOM Merge Tool", "version": "1.0.0"},
                {"vendor": "CycloneDX", "name": "cyclonedx-python", "version": "1.0.0"},
            ],
            "component": {
                "name": "psychsync",
                "type": "application",
        ...
        for component in py_components["components"]:
            ...
            if bom_ref:
                ...
                component["properties"].append(
                    {"name": "psychsync:source", "value": "python-backend"}
                )

    # Process Frontend SBOM
    if fe_components and "components" in fe_components:
        print(
            f"📦 Processing {len(fe_components['components'])} frontend components..."
        )
            ...
            if bom_ref:
                if bom_ref in components_set:
                    ...
                    components_set[bom_ref]["properties"].append(
                        {"name": "psychsync:source", "value": "nodejs-frontend"}
                    )
                else:
                    # New component
                    if "properties" not in component:
    ...
    python_count = sum(
        1
        for c in merged_sbom["components"]

...
def zap_to_sarif(input_file: str, output_file: str, target_url: str):
    ...
    for alert in root.findall(".//alert"):
        ...
        if location_elem is not None:
            uri = (
                location_elem.find("uri").text
                if location_elem.find("uri") is not None
                else target_url
            )
        else:
            uri = target_url
        ...
        if plugin_id not in rules_seen:
            rule = {
                ...
                "properties": {
                    ...
                    "confidence": (
                        alert.find("confidence").text
                        if alert.find("confidence") is not None
                        else "0"
                    ),
                },
            }
    ...
    print(
        f"✅ Converted {len(sarif['runs'][0]['results'])} ZAP findings to SARIF format"
    )
...
def main():
    ...
    parser.add_argument(
        "--target-url", required=True, help="Target URL that was scanned"
    )

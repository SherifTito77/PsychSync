...
class PreDeploymentValidator:
    ...
    async def run_all_validations(self) -> Dict[str, Any]:
        ...
        print(
            f"🔍 Starting comprehensive pre-deployment validation for {self.environment} environment"
        )
        print("=" * 70)
            ...
            try:
                ...
                if result["status"] == "CRITICAL":
                    self.validation_results["critical_failures"].append(
                        f"{validation_name}: {result['message']}"
                    )
                    print(
                        f"❌ {validation_name.upper()}: CRITICAL - {result['message']}"
                    )
                    ...
                    print(
                        f"⚠️  {validation_name.upper()}: WARNING - {result['message']}"
                    )
                else:
    ...
    async def validate_database_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result and result[0] == 1:
                ...
                return {"status": "CRITICAL", "message": "Database query test failed"}
    ...
    async def validate_security_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                not hasattr(settings, "SECRET_KEY")
                or settings.SECRET_KEY == "your-secret-key"
            ):
            ...
            if hasattr(settings, "CORS_ORIGINS") and "*" in settings.CORS_ORIGINS:
                security_issues.append(
                    "CORS configured to allow all origins - not secure for production"
                )
    ...
    async def validate_backup_strategy(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "details": {
                    "backup_schedule": getattr(
                        settings, "BACKUP_SCHEDULE", "Not configured"
                    ),
                    "retention_days": getattr(
                        settings, "BACKUP_RETENTION_DAYS", "Not configured"
                    ),
                },
            }
    ...
    def print_final_report(self):
        ...
        if (
            not self.validation_results["critical_failures"]
            and not self.validation_results["warnings"]
        ):

...
2. **Create folders**
   ```bash
   mkdir -p ~/psychsync/{backend,frontend,infra,docs}
   cd ~/psychsync
   ```

3. **Create GitHub repos (web UI)**
   - Create a GitHub **organization** (optional) or just use your account.
   - Create two repos: `psychsync-backend`, `psychsync-frontend`.
   - Clone them locally:
     ```bash
     cd ~/psychsync
     git clone https://github.com/YOUR_ORG/psychsync-backend backend
...
## 1) Backend Skeleton (FastAPI) — Day 1

1. **Create virtualenv & install deps**
   ```bash
   cd ~/psychsync/backend
   python -m venv .venv
...
3. Backend: add `/integrations/slack/test` that posts a message to a channel using the token.
...
2. Use the official client to create events/reminders from your backend.
...
**Monitoring & Logs**
- Create Sentry projects (frontend + backend) and install SDKs.

...
class TestRateLimitingByRole(unittest.TestCase):
    ...
    def test_normal_user_rate_limiting(self):
        ...
        for i in range(rate_limit + 10):  # Exceed limit by 10 requests
            ...
            if result["status_code"] == 200:
                ...
                print(f"    Request {i+1}: ❌ Rate limited ({result.get('retry_after', 0)}s retry)")

        # Validate results
        successful_requests = [r for r in results if r["status_code"] == 200]
        rate_limited_requests = [r for r in results if r["status_code"] == 429]

        self.assertEqual(len(successful_requests), rate_limit,
                        f"Normal user should make exactly {rate_limit} successful requests")
        self.assertEqual(len(rate_limited_requests), 10,
                        f"Normal user should have 10 rate-limited requests")

        print(f"  ✅ Normal user rate limiting: {len(successful_requests)} successful, {len(rate_limited_requests)} rate-limited")
    ...
    def test_admin_user_rate_limiting(self):
        ...
        self.assertEqual(len(rate_limited_requests), 5,
                        f"Admin user should have 5 rate-limited requests")

        print(f"  ✅ Admin user rate limiting: {len(successful_requests)} successful, {len(rate_limited_requests)} rate-limited")
    ...
    def test_premium_user_rate_limiting(self):
        ...
        print(f"  ✅ Premium user rate limiting: {len(successful_requests)} successful, {len(rate_limited_requests)} rate-limited")
    ...
    def test_burst_capacity_handling(self):
        ...
        self.assertGreater(successful_burst, burst_size * 0.8,
                         "At least 80% of burst requests should succeed")

        print(f"  ✅ Burst handling: {successful_burst}/{burst_size} requests successful")
    ...
    def test_rate_limit_recovery(self):
        ...
        self.assertEqual(exhausted_result["status_code"], 429,
        ...
        self.assertEqual(recovery_result["status_code"], 200,
    ...
    def test_concurrent_user_rate_limiting(self):
        ...
        for user_key, summary in summary_by_user.items():
            expected_limit = min(requests_per_user, self.users[user_key]["rate_limit_per_minute"])

            if requests_per_user <= expected_limit:
                self.assertEqual(summary["successful"], requests_per_user,
                               f"{user_key} should succeed all requests within limit")
            else:
                self.assertGreaterEqual(summary["successful"], expected_limit * 0.9,

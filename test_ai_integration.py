...
def test_ai_integration():
    ...
    try:
        response = requests.get(f"{base_url}/personality-assessments/frameworks", timeout=5)
    ...
    try:
        ...
        response = requests.post(f"{base_url}/personality-assessments/process-public",
                               json=mbti_data, timeout=5)
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                result = data.get('results', {})
                print(f"   ✅ SUCCESS: {result.get('type')} - {result.get('processed_by')}")
                print(f"      📝 Description: {result.get('description', 'N/A')[:50]}...")
                print(f"      💡 Insights: {len(result.get('ai_insights', []))} generated")
            else:
    ...
    try:
        ...
        response = requests.post(f"{base_url}/personality-assessments/process-public",
                               json=enneagram_data, timeout=5)
        if response.status_code == 200:
            data = response.json()
            if data.get('success'):
                result = data.get('results', {})
                print(f"   ✅ SUCCESS: {result.get('type')} - {result.get('framework')}")

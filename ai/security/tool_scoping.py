    ...
    def check_permission(
        ...
        if context and tool.denied_contexts:
            if context in tool.denied_contexts:
                return False, (
                    f"Context '{context}' is explicitly denied"
                )

        return True, None

    def check_rate_limit(self, user_id: str, tool_name: str) -> tuple[bool, Optional[str]]:
        """
        ...
        with self.lock:
            ...
            if recent_calls >= tool.rate_limit:
                return False, (
                    ...
                    f"Max {tool.rate_limit} calls per minute."
                )
    ...
    def invoke_tool(
        ...
        if not has_perm:
            ...
            self._log_invocation(
                tool_name, user_id, context, parameters,
                approved=False, success=False, error=perm_error
            )
            return result
        ...
        if not within_limit:
            ...
            self._log_invocation(
                tool_name, user_id, context, parameters,
                approved=False, success=False, error=rate_error
            )
            return result
        ...
        if self.requires_approval(tool_name):
            if not approver_id:
                ...
                self._log_invocation(
                    tool_name, user_id, context, parameters,
                    approved=False, success=False, error=result["error"]
                )
                return result
        ...
        try:
            ...
            self._log_invocation(
                tool_name, user_id, context, parameters,
                approved=True, success=False, error=str(e)
            )
...
if __name__ == "__main__":
    ...
    print(f"user_123 -> file_read (credentials context): {has_perm} ({error or 'Allowed'})")

...
class SocialEngineeringSecurityTester:
    ...
    async def test_phishing_scenarios(self) -> SocialEngTestResult:
        ...
        if email_service.exists():
            ...
            if not re.search(r'expires?\s*(at|in|before|within)', content, re.IGNORECASE):
                findings.append("Email links may not have expiration times")
                recommendations.append("Add expiration time to all email verification/reset links")
                status = "fail"
                severity = "high"

            # Check for token uniqueness
            if re.search(r'token\s*=\s*["\']\w+["\']', content) or re.search(r'uuid\.\w*\(\)', content):
                pass  # Good: using UUID or tokens
            else:
                findings.append("Email tokens may not be sufficiently unique")
                recommendations.append("Use cryptographically secure random tokens (UUID4, secrets.token_urlsafe)")
                status = "fail"
            ...
            if 'html' in content.lower() and 'Content-Type' not in content:
                ...
                recommendations.append("Always set Content-Type headers for HTML emails")
                status = "warning"
        ...
        if auth_endpoints.exists():
            ...
            reset_func = re.search(r'def\s+(?:forgot_password|reset_password|send_reset_email)', content)
            if reset_func:
                # Look for rate limiting decorators or middleware
                func_context = content[reset_func.start():reset_func.start()+500]
                if 'rate_limit' not in func_context.lower() and 'limiter' not in func_context.lower():
                    findings.append("Password reset endpoint lacks rate limiting")
                    recommendations.append("Add rate limiting to prevent email bombing/spam")
                    status = "fail"
                    severity = "critical"
            ...
            if 'reset_password' in content.lower():
                if not re.search(r'(expires|exp|max_age|ttl)', content, re.IGNORECASE):
                    ...
                    recommendations.append("Set short expiration (15-60 minutes) for reset links")
                    status = "fail"
        ...
        if templates_dir.exists():
            for template_file in templates_dir.glob("*.html"):
                ...
                if 'click here' in content.lower() and 'psychsync' not in content.lower():
                    findings.append(f"Template {template_file.name} uses generic 'click here' links")
                    recommendations.append("Use specific, branded language in email CTAs")

                # Check for HTTP links
                if 'http://' in content and 'https://' not in content:
                    findings.append(f"Template {template_file.name} contains insecure HTTP links")
                ...
                if 'href=' in content and 'psychsync' not in content.lower():
                    findings.append(f"Template {template_file.name} may link to external domains")
                    recommendations.append("Verify all email links point to official domains")

        if not findings:
            findings.append("No critical phishing vulnerabilities detected")
            recommendations.append("Implement DMARC, SPF, and DKIM for email authentication")
    ...
    async def test_forgotten_password_manipulation(self) -> SocialEngTestResult:
        ...
        if auth_endpoints.exists():
            ...
            for match in reset_endpoints:
                ...
                if 'if.*email.*exists' in endpoint_content.lower() or 'if.*user' in endpoint_content.lower():
                    findings.append("Password reset may leak information about email existence")
                    recommendations.append("Always return same response regardless of email existence")
                    status = "fail"
                    severity = "high"

                # Check for immediate vs delayed responses
                if 'asyncio.sleep' in endpoint_content or 'time.sleep' in endpoint_content:
                    pass  # Good: timing attack protection
                else:
                    findings.append("Password reset may be vulnerable to timing attacks")
                    recommendations.append("Use constant-time responses to prevent email enumeration")
                    status = "warning"

            # Check for security questions (generally bad practice)
            if 'security_question' in content.lower() or 'mother_maiden' in content.lower():
                findings.append("Security questions detected (weak authentication method)")
            ...
            if 'old_password' in content.lower() and 'reset' in content.lower():
                ...
                recommendations.append("Ensure old password verification is properly implemented")
                status = "warning"
        ...
        if user_service.exists():
            ...
            if 'reset_token' in content.lower():
                if not re.search(r'(expires|exp|ttl|max_age)', content, re.IGNORECASE):
                    ...
                    recommendations.append("Set short expiration (15-60 minutes) for reset tokens")
                    status = "fail"
        ...
        if models.exists():
            ...
            if 'reset_token' in content.lower():
                if 'Column' in content and 'reset_token' in content:
                    ...
                    recommendations.append("Ensure only one valid reset token exists at a time")
                    status = "info"

        if not findings:
            findings.append("Password reset flow appears properly implemented")
            recommendations.append("Implement universal response: 'If email exists, reset link sent'")
            recommendations.append("Add rate limiting to prevent email enumeration attacks")
    ...
    async def test_helpdesk_impersonation(self) -> SocialEngTestResult:
        ...
        if api_endpoints.exists():
            for endpoint_file in api_endpoints.glob("*.py"):
                ...
                if re.search(r'impersonat|login_as|become_user|sudo', content, re.IGNORECASE):
                    findings.append(f"User impersonation feature in {endpoint_file.name}")
                    recommendations.append("Ensure impersonation requires MFA and detailed audit logging")
                    status = "fail"
                    severity = "critical"

                # Check for admin password reset
                if re.search(r'admin.*reset.*password|force.*reset|set_password', content, re.IGNORECASE):
                    findings.append(f"Admin password reset in {endpoint_file.name}")
                    recommendations.append("Require user confirmation for admin-initiated password changes")
                    status = "warning"

                # Check for user role modification
                if re.search(r'(change_role|update_role|set_role|make_admin)', content, re.IGNORECASE):
                    if not re.search(r'audit|log|track', content, re.IGNORECASE):
                        findings.append(f"Role changes in {endpoint_file.name} may not be audited")
                        recommendations.append("Log all role/permission changes with admin identity")
                        status = "fail"
                        severity = "high"
        ...
        if admin_endpoints.exists():
            ...
            for route in admin_routes:
                ...
                if 'admin' in route_context.lower():
                    ...
                    if 'current_user' not in route_context or 'is_admin' not in route_context.lower():
                        findings.append("Admin route may lack proper authorization check")
                        recommendations.append("Always verify admin role with get_current_user dependency")
                        status = "fail"
                        severity = "critical"
        ...
        if not audit_log.exists():
            ...
            recommendations.append("Implement comprehensive audit logging for admin actions")
            status = "warning"
        else:
            ...
            if has_impersonation_logging and has_role_change_logging:
                findings.append("Comprehensive audit logging for admin operations detected")
            elif 'impersonat' not in content.lower() and 'role_change' not in content.lower():
                findings.append("Audit logging may not cover impersonation/role changes")
                recommendations.append("Add audit events for sensitive admin operations")
                status = "warning"
        ...
        if security_config.exists():
            ...
            if 'mfa' not in content.lower() and 'two_factor' not in content.lower():
                ...
                recommendations.append("Require MFA for all admin actions and impersonation")
                status = "warning"
        ...
        if not findings:
            ...
            recommendations.append("Log all admin actions with timestamp, admin ID, and affected user")
    ...
    async def test_recovery_flows_identity_verification(self) -> SocialEngTestResult:
        ...
        if auth_endpoints.exists():
            content = auth_endpoints.read_text()
            auth_has_2fa = '2fa' in content.lower() or 'two_factor' in content.lower() or 'mfa' in content.lower()

            # Check for KBA endpoints
            if re.search(r'security_question|mother_maiden|pet_name|favorite_', content, re.IGNORECASE):
        ...
        if has_2fa:
            ...
            if has_2fa_service:
                ...
                if 'recovery_code' in service_content.lower() and 'single' in service_content.lower():
                    findings.append("2FA includes single-use recovery codes (best practice)")
            ...
            if deps_file.exists():
                ...
                if 'get_admin_user_with_mfa' in deps_content or 'get_current_user_with_mfa' in deps_content:
        ...
        if auth_endpoints.exists():
            recovery_func = re.search(r'def\s+(?:recover_account|restore_account|verify_recovery)', content)
            if recovery_func:
                func_context = content[recovery_func.start():recovery_func.start()+1000]

                # Check verification steps
                verification_count = len(re.findall(r'verif|confirm|validate|check', func_context, re.IGNORECASE))
                if verification_count < 2:
                    findings.append("Account recovery may have insufficient verification")
                    recommendations.append("Require multiple verification factors (email + 2FA)")
                    status = "fail"
        ...
        if users_service.exists():
            ...
            if 'change_email' in content.lower() or 'update_email' in content.lower():
                if not re.search(r'verif|confirm.*token|send.*email', content, re.IGNORECASE):
                    findings.append("Email changes may not require verification")
                    recommendations.append("Always require email verification for email changes")
                    status = "fail"
                    severity = "high"

            # Check password change verification
            if 'change_password' in content.lower():
                if not re.search(r'old_password|current_password|verify_password', content, re.IGNORECASE):
                    findings.append("Password changes may not require current password")
                    recommendations.append("Always require current password for password changes")
                    status = "fail"
                    severity = "high"
        ...
        if auth_service.exists():
            ...
            if 'recover' in content.lower() or 'reset' in content.lower():
                if not re.search(r'rate.?limit|throttle|attempt.*limit', content, re.IGNORECASE):
                    findings.append("Account recovery may lack rate limiting")
                    recommendations.append("Limit recovery attempts to prevent brute force")
                    status = "warning"

        if not findings:
            findings.append("No critical identity verification vulnerabilities detected")
            recommendations.append("Implement 2FA with single-use recovery codes")
            recommendations.append("Require multiple verification factors for account recovery")
    ...
    async def test_phone_verification_bypass(self) -> SocialEngTestResult:
        ...
        for file_path in [auth_endpoints, auth_service]:
            if file_path and file_path.exists():
                ...
                if re.search(r'sms|phone|verify.?phone|send.?code|otp', content, re.IGNORECASE):
                    phone_verification_found = True

                    # Check SMS code generation
                    if re.search(r'code\s*=\s*\d{4,6}|pin\s*=\s*\d+', content):
                        findings.append("SMS codes may be predictable (sequential/static)")
                        recommendations.append("Use cryptographically secure random codes (secrets module)")
                        status = "fail"
                        severity = "high"

                    # Check code expiration
                    if not re.search(r'expires?|ttl|max_age|valid_for', content, re.IGNORECASE):
                        findings.append("SMS codes may not expire")
                        recommendations.append("Set short expiration (5-10 minutes) for SMS codes")
                        status = "fail"
                    ...
                    if code_length_match:
                        ...
                        if code_length < 6:
                            findings.append(f"SMS codes are only {code_length} digits (weak)")
                            recommendations.append("Use at least 6-digit codes for SMS verification")
                            status = "warning"

                    # Check for rate limiting
                    if not re.search(r'rate.?limit|throttle|attempt.*limit', content, re.IGNORECASE):
                        findings.append("SMS verification may lack rate limiting")
                        recommendations.append("Limit SMS attempts to prevent SMS bombing and cost attacks")
                        status = "fail"
                        severity = "high"
                    ...
                    if 'twilio' in content.lower() or 'nexmo' in content.lower():
                        if 'voip' not in content.lower() and 'type' not in content.lower():
                            findings.append("SMS service may not filter VoIP numbers")
                            recommendations.append("Filter VoIP numbers which are easier to compromise")
                            status = "warning"

        if not phone_verification_found:
            findings.append("No phone/SMS verification system detected")
            recommendations.append("Consider implementing SMS verification as 2FA factor")
            recommendations.append("If using SMS, be aware of SS7 vulnerabilities and SIM swapping risks")
            status = "info"
            severity = "info"
        ...
        if security_config.exists():
            ...
            if 'totp' not in content.lower() and 'authenticator' not in content.lower():
                ...
                recommendations.append("Implement TOTP (Google Authenticator) as more secure 2FA alternative")
                status = "warning"
            ...
            for file_path in [auth_endpoints, auth_service]:
                if file_path and file_path.exists():
                    ...
                    if 'verify' in content.lower() and 'code' in content.lower():
                        ...
                        if not re.search(r'use|consum|redeem|invalidate', content, re.IGNORECASE):
                            findings.append("SMS codes may not be invalidated after use (replay attack risk)")
                            recommendations.append("Mark codes as used after successful verification")
                            status = "fail"
                            severity = "high"
                        break

        if not findings or status == "info":
            findings = ["No phone verification bypass vulnerabilities detected (or no SMS system)"]
            recommendations = [
    ...
    async def run_all_tests(self) -> Dict[str, Any]:
        ...
        for test_name, test_method in test_methods:
            ...
            try:
                ...
                self.results.append(SocialEngTestResult(
                    category=test_name,
                    test_name=test_name,
                    severity="error",
                    status="error",
...
async def main():
    ...
    # Save report to JSON
    output_file = project_root / f"social_engineering_security_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

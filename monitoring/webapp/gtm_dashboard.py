...
async def gtm_dashboard(request: Request):
    ...
    return templates.TemplateResponse("gtm_dashboard.html", {
        "request": request,
...
async def get_gtm_overview(date_range: DateRangeRequest) -> GTMOverviewResponse:
    ...
    try:
        ...
        channel_performance = gtm_service.analyze_channel_performance(date_range.start_date, date_range.end_date)
...
async def get_funnel_analytics(date_range: DateRangeRequest) -> FunnelAnalyticsResponse:
    ...
    try:
        ...
        conversion_rates = {
            ...
            "lead_to_qualified": funnel_stages["qualified_leads"] / funnel_stages["leads"],
            "qualified_to_demo": funnel_stages["demo_scheduled"] / funnel_stages["qualified_leads"],
            "demo_to_proposal": funnel_stages["proposals_sent"] / funnel_stages["demo_completed"],
            "proposal_to_close": funnel_stages["closed_won"] / funnel_stages["proposals_sent"]
        }
        ...
        if conversion_rates["visitor_to_lead"] < 0.15:
            optimization_opportunities.append("Improve landing page conversion with better value proposition")
        if conversion_rates["lead_to_qualified"] < 0.5:
            optimization_opportunities.append("Enhance lead scoring and qualification criteria")
        if conversion_rates["qualified_to_demo"] < 0.7:
            optimization_opportunities.append("Optimize demo scheduling process and follow-up")
        if conversion_rates["demo_to_proposal"] < 0.4:
            optimization_opportunities.append("Improve demo quality and value demonstration")
        if conversion_rates["proposal_to_close"] < 0.5:
            optimization_opportunities.append("Streamline proposal process and address objections proactively")
...
async def get_revenue_analytics(date_range: DateRangeRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        mrr_breakdown = {
            ...
            "professional_services": revenue_metrics.get("professional_services_revenue", 0)
        }
    ...
    try:
        ...
        for campaign_id, campaign in gtm_service.campaigns.items():
            ...
            gtm_campaigns.append({
                ...
                "cost_per_lead": campaign.spend / max(1, performance.get("leads_generated", 1)),
...
async def get_customer_segmentation() -> Dict[str, Any]:
    ...
    try:
        ...
        behavioral_segments = {
            "power_users": {
                "count": 95,
                "characteristics": ["Daily login", "Multiple features", "Team collaboration"],
                "retention_rate": 0.98,
            ...
            "at_risk_users": {
                "count": 180,
                "characteristics": ["Infrequent login", "Limited feature adoption", "Low engagement"],
                "retention_rate": 0.45,
                "expansion_potential": 0.10
            },
            "new_users": {
                "count": 172,
                "characteristics": ["Recent signup", "Onboarding phase", "Discovery mode"],
                "retention_rate": 0.78,
        ...
        raise HTTPException(status_code=500, detail="Failed to get customer segmentation")
...
async def get_sales_performance(date_range: DateRangeRequest) -> Dict[str, Any]:
    ...
    try:
        ...
        for playbook_id, playbook in sales_enablement_service.playbooks.items():
            ...
            conversion_rate = sum(playbook.conversion_rates.values()) / len(playbook.conversion_rates)

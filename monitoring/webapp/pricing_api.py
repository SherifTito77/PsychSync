...
async def create_customer(customer_data: CustomerRequest):
    ...
    try:
        customer = subscription_service.create_customer(
            ...
            psychsync_app_url=(
                str(customer_data.psychsync_app_url)
                if customer_data.psychsync_app_url
                else None
            ),
        )
...
async def create_subscription(subscription_data: SubscriptionRequest):
    ...
    try:
        ...
        return {
            ...
            "monthly_price": pricing_service.get_tier_pricing(
                subscription.tier, BillingCycle.MONTHLY
            ),
            "yearly_price": pricing_service.get_tier_pricing(
                subscription.tier, BillingCycle.YEARLY
            ),
        }
...
async def cancel_subscription(
    subscription_id: str, reason: str = "", immediate: bool = False
):
...
async def get_tier_recommendation(metrics: MetricsRequest):
    ...
    try:
        ...
        value_prop = pricing_service.calculate_tier_value_proposition(
            recommended_tier, metrics_dict
        )
...
async def analyze_upgrade_opportunity(
    subscription_id: str, metrics: MetricsRequest, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        recommendation = (
            await upgrade_recommendation_service.generate_upgrade_recommendation(
        ...
        return {
            ...
            "value_proposition": {
                "additional_monthly_value": upgrade_proposal[
            ...
            "pricing_details": {
                "new_monthly_price": pricing_service.get_tier_pricing(
                    recommendation.recommended_tier, BillingCycle.MONTHLY
                ),
                "new_yearly_price": pricing_service.get_tier_pricing(
                    recommendation.recommended_tier, BillingCycle.YEARLY
                ),
                "yearly_savings": pricing_service.calculate_yearly_savings(
        ...
        raise HTTPException(
            status_code=500, detail="Failed to analyze upgrade opportunity"
        )
...
async def get_revenue_metrics(
    time_period: str = Query("month", description="time period: month, quarter, year")
):
...
def _generate_recommendation_reasoning(
    tier: SubscriptionTier, metrics: Dict[str, Any]
) -> List[str]:
    ...
    if tier == SubscriptionTier.ENTERPRISE:
        reasoning.append(
            "Your scale requires enterprise-grade features and SLA guarantees"
        )
        ...
        reasoning.append(
            "Growth tier provides advanced business intelligence for your scale"
        )
        ...
        reasoning.append(
            "Free tier is perfect for getting started with business intelligence"
        )
...
async def _track_recommendation(customer_id: str, subscription_id: str, recommendation):
    ...
    try:
        ...
        logger.info(
            f"Tracking recommendation: {recommendation.recommended_tier.value} for {customer_id}"
        )
    except Exception as e:

...
class NetworkSecurityAuditor:
    ...
    def audit_tls_configuration(self) -> Dict[str, Any]:
        ...
        for tls_name, method_name in tls_methods:
            try:
                ...
                if hasattr(ssl, f'PROTOCOL_{method_name.replace(".", "_")}'):
                    proto_const = getattr(ssl, f'PROTOCOL_{method_name.replace(".", "_")}')
        ...
        if not tls_versions.get("TLSv1.2", False) and not tls_versions.get("TLSv1.3", False):
            results["findings"].append({
                "severity": "HIGH",
        ...
        print(f"    TLS Versions: {', '.join([v for v, supported in tls_versions.items() if supported]) or 'None accessible'}")
        ...
        try:
            response = urllib.request.urlopen(f"http://{self.target_host}:{self.target_port}/", timeout=5)
            ...
            print(f"    Security headers found: {len([v for v in security_headers.values() if v])}/6")
    ...
    def test_ssl_downgrade_attacks(self) -> Dict[str, Any]:
        ...
        for proto_name, tls_version_enum in tls_versions_to_test:
            ...
            except (ssl.SSLError, OSError, ConnectionResetError, TimeoutError, AttributeError):
                results["tests"][f"downgrade_to_{proto_name}"] = {
                    "status": "protected",
        ...
        try:
            ...
            if hsts_header:
                ...
                if "max-age=" in hsts_header:
                    max_age = int(hsts_header.split("max-age=")[1].split(";")[0].strip())
                include_subdomains = "includeSubDomains" in hsts_header or "includesubdomains" in hsts_header
    ...
    def test_dns_poisoning_scenarios(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if resolv_conf.exists():
                ...
                if any(ip.startswith("127.") or ip == "localhost" for ip in dns_servers):
    ...
    def check_routing_leaks(self) -> Dict[str, Any]:
            ...
            try:
                ...
                if 300 <= response.getcode() < 400:
                    ...
                    if "evil.com" in location:
                        results["tests"][f"redirect_{test_url.split('=')[0].split('/')[-1]}"] = {
                            "status": "vulnerable",
        ...
        if not any("vulnerable" in str(v) and "path_traversal" in k for k, v in results["tests"].items()):
        ...
        for path in invalid_paths:
            try:
                ...
                found_sensitive = [info for info in sensitive_info if info.lower() in content.lower()]

                if found_sensitive:
                    results["tests"][f"info_disclosure_{path.replace('/', '_')[-30:]}"] = {
                        "status": "leaking",
                        "details": f"Found: {found_sensitive}"
                    }
                    results["findings"].append({
                        "severity": "MEDIUM",
                ...
                try:
                    ...
                    if "traceback" in error_content.lower() or "exception" in error_content.lower():
                        results["findings"].append({
                            "severity": "MEDIUM",

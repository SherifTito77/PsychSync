...
class ProductPromptsManager:
    ...
    def filter_prompts(self, category_id: Optional[str] = None,
        ...
        if search:
            ...
            prompts = [p for p in prompts if
                      ...
                      any(search_lower in uc.lower() for uc in p['use_cases'])]
...
def get_related_prompts(prompt_id):
    ...
    if prompt and prompt.get('related_prompts'):
        ...
        return jsonify({
            'success': False,
            'error': 'Prompt not found or no related prompts'
        }), 404
...
def get_workflow(goal):
    ...
    if workflow:
        ...
        return jsonify({
            'success': False,
            'error': f'Workflow not found: {goal}'
        }), 404
    ...
    workflows = [
        {
            ...
            'prompt_count': 5,
            'goals': ['Generate feature brief', 'Define specs', 'Map journey', 'Write criteria', 'Announce']
        },
        {
            ...
            'prompt_count': 5,
            'goals': ['Retention levers', 'Churn prediction', 'Lifecycle', 'Pain points', 'Success KPIs']
        },
        {

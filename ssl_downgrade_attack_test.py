...
class SSLDowngradeAttackTester:
    ...
    def test_ssl_v2_v3_support(self, hostname: str = "localhost", port: int = 443) -> Dict[str, Any]:
        ...
        if test_result["sslv2_supported"] or test_result["sslv3_supported"]:
            ...
            test_result["recommendation"] = "Disable SSLv2 and SSLv3 protocols immediately"
        else:
    ...
    def test_tls_version_downgrade(self, hostname: str = "localhost", port: int = 443) -> Dict[str, Any]:
        ...
        if test_result["weak_versions"]:
            ...
            test_result["recommendation"] = "Disable TLSv1.0 and TLSv1.1, enforce TLSv1.2+"
        else:
            test_result["vulnerable"] = False
            test_result["risk_level"] = "LOW"
    ...
    def test_cipher_suite_downgrade(self, hostname: str = "localhost", port: int = 443) -> Dict[str, Any]:
            ...
            with socket.create_connection((hostname, port), timeout=10) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    if cipher_info:
                        ...
                        is_weak = any(pattern.lower() in cipher_lower for pattern in weak_cipher_patterns)
        ...
        if test_result.get("weak_ciphers"):
            test_result["vulnerable"] = True
            test_result["risk_level"] = "HIGH"
            test_result["recommendation"] = "Disable weak cipher suites and use only strong ciphers"
        elif test_result.get("connection_failed"):
            test_result["vulnerable"] = True
            test_result["risk_level"] = "MEDIUM"
            test_result["recommendation"] = "Server connection failed - investigate TLS configuration"
        else:
            test_result["vulnerable"] = False
            test_result["risk_level"] = "LOW"
    ...
    def test_certificate_validation_bypass(self, hostname: str = "localhost", port: int = 443) -> Dict[str, Any]:
            ...
            try:
                ...
                with socket.create_connection((hostname, port), timeout=5) as sock:
                    with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                        ...
                        test_case_result["peer_certificate"] = ssock.getpeercert() is not None
        ...
        bypass_tests = [t for t in test_result["validation_tests"] if t.get("bypass_possible", False)]

        if bypass_tests:
            test_result["vulnerable"] = True
            test_result["risk_level"] = "HIGH"
            test_result["bypass_methods"] = [t["name"] for t in bypass_tests]
            test_result["recommendation"] = "Enforce strict certificate validation and hostname verification"
        else:
            test_result["vulnerable"] = False
            test_result["risk_level"] = "LOW"
    ...
    def test_application_ssl_downgrade(self, base_url: str = "http://localhost:8000") -> Dict[str, Any]:
        ...
        # Determine vulnerability
        successful_downgrades = [t for t in test_result["downgrade_tests"] if t.get("downgrade_successful", False)]

        if successful_downgrades:
            test_result["vulnerable"] = True
            test_result["risk_level"] = "CRITICAL"
            test_result["downgrade_methods"] = [t["name"] for t in successful_downgrades]
            test_result["recommendation"] = "Implement HSTS, HTTPS-only enforcement, and secure redirects"
        else:
            test_result["vulnerable"] = False
            test_result["risk_level"] = "LOW"
    ...
    def test_implementation_vulnerabilities(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                vulnerable_patterns = [
                    "1.0.1", "1.0.2-alpha", "1.0.2-beta", "1.0.2-rc"
                ]

                for pattern in vulnerable_patterns:
                    if pattern in version:
                        heartbleed_test["vulnerable"] = True
                        heartbleed_test["details"] = f"Vulnerable OpenSSL version detected: {version.strip()}"
                        break
                else:
                    heartbleed_test["details"] = f"OpenSSL version appears safe: {version.strip()}"
            else:
        ...
        vulnerable_tests = [t for t in test_result["implementation_tests"] if t.get("vulnerable", False)]

        if vulnerable_tests:
            test_result["vulnerable"] = True
            test_result["risk_level"] = "CRITICAL"
            test_result["vulnerabilities"] = [t["name"] for t in vulnerable_tests]
            test_result["recommendation"] = "Update OpenSSL and libraries to latest versions"
        else:
            test_result["vulnerable"] = False
            test_result["risk_level"] = "LOW"
    ...
    def generate_downgrade_protection_recommendations(self, test_results: Dict) -> List[Dict]:
        """Generate SSL downgrade protection recommendations"""
        recommendations = []

        # SSLv2/SSLv3 recommendations
        sslv_tests = [r for r in test_results.get("protocol_tests", []) if r.get("name") == "SSLv2/SSLv3 Protocol Support"]
        ...
        tls_tests = [r for r in test_results.get("protocol_tests", []) if r.get("name") == "TLS Version Downgrade Attack"]
        ...
        cipher_tests = [r for r in test_results.get("protocol_tests", []) if r.get("name") == "Cipher Suite Downgrade Attack"]
        ...
        cert_tests = [r for r in test_results.get("protocol_tests", []) if r.get("name") == "Certificate Validation Bypass"]
    ...
    def run_comprehensive_downgrade_test(self) -> Dict[str, Any]:
        ...
        vulnerable_tests = len([t for t in all_protocol_tests if t.get("vulnerable", False)])
        ...
        results["summary"] = {
            ...
            "overall_downgrade_security_score": self.calculate_downgrade_security_score(results)
        }
    ...
    def calculate_downgrade_security_score(self, results: Dict) -> int:
        ...
        vulnerable_protocols = len([t for t in protocol_tests if t.get("vulnerable", False)])
...
def main():
    ...
    try:
        ...
        print(f"🎯 Overall Downgrade Security Score: {summary['overall_downgrade_security_score']}/100")

        # Show vulnerable protocol tests
        vulnerable_protocols = [t for t in results.get("protocol_tests", []) if t.get("vulnerable", False)]
        if vulnerable_protocols:
            print(f"\n🚨 PROTOCOL DOWNGRADE VULNERABILITIES:")
            for test in vulnerable_protocols:
                print(f"  ❌ {test['test_name']} ({test['hostname']}:{test['port']}): {test.get('recommendation', 'Unknown issue')}")
        ...
        if summary['application_vulnerable']:
            ...
            print(f"  ❌ Application allows SSL downgrade: {app_test.get('recommendation', 'Unknown issue')}")
        ...
        if summary['implementation_vulnerable']:
            ...
            for vuln in vulnerabilities:
                print(f"  ❌ {vuln}: {impl_test.get('recommendation', 'Unknown issue')}")
        ...
        with open("/Users/sheriftito/Downloads/psychsync/ssl_downgrade_attack_report.json", "w") as f:

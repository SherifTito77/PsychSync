    ...
    for assessment in assessments:
        ...
        if nav_match:
            ...
            if actual_route == assessment["expected_route"]:
                ...
                print(f"❌ Route: {actual_route} (expected: {assessment['expected_route']})")
                all_correct = False
        else:
        ...
        state_match = re.search(r"state:\s*\{\s*assessmentType:\s*'([^']+)',\s*result:\s*([^}]+)\s*\}", content)
        if state_match:
            assessment_type = state_match.group(1)
            print(f"✅ State structure: assessmentType: '{assessment_type}', result: object")

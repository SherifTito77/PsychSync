...
class SecureModelTrainer:
    ...
    def train_model(
        ...
        try:
            ...
            data_validation_report = self._validate_training_data(train_data, val_data, corpus_id)

            if not data_validation_report['passed']:
                raise ValueError(f"Data validation failed: {data_validation_report['reason']}")
            ...
            report = TrainingReport(
                ...
                final_train_loss=self.metrics_history[-1].train_loss if self.metrics_history else 0.0,
                final_val_loss=validation_report.validation_timestamp.timestamp() if validation_report else 0.0,
                final_val_accuracy=validation_report.accuracy_score if validation_report else 0.0,
            ...
            logger.info(f"Training completed: {'PASSED' if training_passed else 'FAILED'}")
    ...
    def _validate_training_data(
        ...
        try:
            ...
            if isinstance(train_data, pd.DataFrame):
                missing_ratio = train_data.isnull().sum().sum() / (len(train_data) * len(train_data.columns))
    ...
    def _monitor_training_step(
        ...
        if metrics.val_accuracy is not None and len(self.metrics_history) > 5:
            recent_accuracies = [m.val_accuracy for m in self.metrics_history[-5:] if m.val_accuracy]
            if recent_accuracies:
                max_acc = max(recent_accuracies)
                if metrics.val_accuracy < max_acc * (1 - self.PERFORMANCE_DEGRADATION_THRESHOLD):
                    anomalies.append(AnomalySignal(
    ...
    def _detect_adversarial_patterns(
        ...
        if wrong_predictions.sum() > 0 and pred_probs is not None:
            ...
            if high_confidence_wrong > len(true_labels) * self.ADVERSARIAL_RATIO_THRESHOLD:
                anomalies.append(AnomalySignal(
    ...
    def _assess_training_completion(
        ...
        # Check F1 threshold
        if validation_report.f1_score and validation_report.f1_score < self.MIN_F1_THRESHOLD:
            return False
    ...
    def _compute_data_hash(self, data: Any) -> str:
        ...
        if isinstance(data, (pd.DataFrame, np.ndarray)):
            data_str = data.to_string() if isinstance(data, pd.DataFrame) else str(data.tobytes())
        ...
        if len(self.checkpoints) > self.max_checkpoints_to_keep:
            ...
            for checkpoint in self.checkpoints[:to_remove]:
                try:
                    ...
                    logger.warning(f"Failed to remove checkpoint {checkpoint.checkpoint_id}: {str(e)}")
...
def main():
    ...
    parser.add_argument(
        '--incident-id',
        help='Incident ID for this training run'
    )
    parser.add_argument(
        '--corpus-id',
        help='Corpus ID being used for training'
    )
    parser.add_argument(
        '--validate-only',
        action='store_true',
    ...
    parser.add_argument(
        '--model-path',
        help='Path to model file (for validation)'
    )

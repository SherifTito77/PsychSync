...
class DataPoisoningDetector:
    ...
    def detect_poisoning(
        ...
        if text_column:
            backdoor_signals = self._detect_backdoors(
                df, text_column
            )
        ...
        if self.enable_statistical:
            stat_signals = self._detect_statistical_anomalies(
                df, baseline_stats
            )
        ...
        affected_samples = self._identify_affected_samples(
            df, signals, limit=1000
        )
        ...
        logger.info(f"Detected: {report.poisoning_detected}, "
                        ...
                        if np.mean(variance) > np.median(variance) * 2:
                            signals.append(PoisoningSignal(
                                ...
                                evidence={
                                    ...
                                    'variance_ratio': np.mean(variance) / (np.median(variance) + 1e-9)
                                }
                            ))
            ...
            for value, count in duplicates.items():
                if count > duplicate_threshold:
                    signals.append(PoisoningSignal(
                        ...
                        evidence={
                            'text_hash': hashlib.sha256(value.encode()).hexdigest()[:16],
    ...
    def _generate_recommendations(
        ...
        if poisoning_type == PoisoningType.LABEL_FLIPPING:
            recommendations.append("📊 Verify label integrity with original data source")
        ...
        if len(signals) > 10:
            recommendations.append("📈 High number of anomalies detected - comprehensive audit recommended")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Detect data poisoning in ML corpora"
    )
    parser.add_argument(
        '--corpus',
        required=True,
        help='Path to corpus file (JSON) or directory'
    )
    parser.add_argument(
        '--corpus-id',
        required=True,
    ...
    parser.add_argument(
        '--text-column',
        help='Name of text/input column'
    )
    parser.add_argument(
        '--baseline',

...
async def check_data_integrity():
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            orphaned_users = await session.execute(text("""
                SELECT COUNT(*) FROM users
                WHERE organization_id IS NOT NULL
                AND organization_id NOT IN (SELECT id FROM organizations)
            """))
            orphaned_user_count = orphaned_users.scalar()
            print(f"   Users with invalid organization references: {orphaned_user_count}")

            # Team members with invalid team references
            orphaned_members = await session.execute(text("""
                SELECT COUNT(*) FROM team_members tm
                LEFT JOIN teams t ON tm.team_id = t.id
                WHERE t.id IS NULL
            """))
            orphaned_member_count = orphaned_members.scalar()
            print(f"   Team members with invalid team references: {orphaned_member_count}")

            # Team members with invalid user references
            invalid_user_members = await session.execute(text("""
                SELECT COUNT(*) FROM team_members tm
                LEFT JOIN users u ON tm.user_id = u.id
                WHERE u.id IS NULL
            """))
            invalid_user_member_count = invalid_user_members.scalar()
            print(f"   Team members with invalid user references: {invalid_user_member_count}")

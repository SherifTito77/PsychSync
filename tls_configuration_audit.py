...
class TLSConfigurationAuditor:
    ...
    def scan_tls_configurations(self) -> List[Dict]:
        ...
        config_files = [f for f in config_files if "node_modules" not in str(f) and ".git" not in str(f)]
        ...
        try:
            ...
            try:
                ...
                analysis = {
                    ...
                    "days_until_expiry": (cert_obj.not_valid_after - datetime.now()).days,
                ...
                if cert_obj.public_key().key_size < 2048:
                    issues.append(f"Weak key size: {cert_obj.public_key().key_size} bits")
        ...
        for config_file in config_files:
            ...
            if file_path.exists():
                try:
                    ...
                    cert_matches = re.findall(r'(cert|certificate|ssl_cert|tls_cert)["\']?\s*[:=]\s*["\']([^"\']+)["\']', content, re.IGNORECASE)
                    for match in cert_matches:
                        analysis["certificate_files"].append(match[1])

                    # Look for TLS version configuration
                    tls_version_matches = re.findall(r'(tls_version|ssl_version|proto)["\']?\s*[:=]\s*["\']([^"\']+)["\']', content, re.IGNORECASE)
                    for match in tls_version_matches:
                        analysis["tls_versions"].append(match[1])

                    # Check for security issues
                    if "http://" in content:
                        analysis["security_issues"].append("Plain HTTP configuration detected")
                    ...
                    analysis["security_issues"].append(f"Error analyzing {config_file}: {e}")
    ...
    def test_server_tls_configuration(self, hostname: str = "localhost", port: int = 8000) -> Dict[str, Any]:
        ...
        try:
            ...
            with socket.create_connection((hostname, port)) as sock:
                with context.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    expiry_date = datetime.strptime(cert_pem["notAfter"], "%b %d %H:%M:%S %Y %Z")
                    if expiry_date < datetime.now():
                        results["security_issues"].append("Certificate has expired")
                    elif (expiry_date - datetime.now()).days < 30:
                        results["security_issues"].append("Certificate expires within 30 days")
        ...
        for docker_file in docker_files:
            ...
            if file_path.exists():
                try:
                    ...
                    cert_volume_matches = re.findall(r'- ([^:]+/[^:]+\.crt|[^:]+/[^:]+\.pem|[^:]+/[^:]+\.key):([^:]+)', content)
                    for match in cert_volume_matches:
                        analysis["certificate_volumes"].append(match[0])

                    # Check for TLS environment variables
                    tls_env_matches = re.findall(r'(TLS_|SSL_|CERT_|KEY_)[^=]*=[^s]+', content, re.IGNORECASE)
                    if tls_env_matches:
                        analysis["docker_tls_configured"] = True

                    # Security checks
                    if "ports:" in content and "443" not in content:
                        analysis["security_issues"].append("Docker container exposes HTTP without HTTPS")

                    if "restart: always" not in content and "restart: unless-stopped" not in content:
                        analysis["security_issues"].append("Docker container lacks proper restart policy")

                except Exception as e:
                    analysis["security_issues"].append(f"Error analyzing {docker_file}: {e}")
    ...
    def check_nginx_ssl_configuration(self) -> Dict[str, Any]:
        ...
        for nginx_file in nginx_files:
            try:
                ...
                cert_matches = re.findall(r'(ssl_certificate|ssl_certificate_key)["\']?\s*([^;]+);', content)
                for match in cert_matches:
                    analysis["certificate_files"].append(match[1])

                # Security checks
                if "ssl_protocols TLSv1 TLSv1.1" in content:
                    analysis["security_issues"].append("Insecure TLS protocols (TLSv1.0, TLSv1.1) enabled")
                ...
                if "listen 80;" in content and "listen 443 ssl;" not in content:
                    analysis["security_issues"].append("HTTP port 80 enabled without HTTPS redirect")
    ...
    def generate_tls_recommendations(self, analysis_results: Dict) -> List[Dict]:
        ...
        certs_with_issues = [cert for cert in analysis_results.get("certificates", [])
        ...
        if app_tls.get("risk_level") in ["HIGH", "MEDIUM"]:
            recommendations.append({
                ...
                "issue": app_tls.get("risk_level", "") + " risk in application TLS configuration",
        ...
        failed_tests = [test for test in server_tests if not test.get("tls_supported", True)]
        if failed_tests:
            recommendations.append({
                "priority": "CRITICAL",
                "category": "Server TLS",
    ...
    def run_comprehensive_tls_audit(self) -> Dict[str, Any]:
        ...
        results["config_files"] = [str(f.relative_to(self.base_path)) for f in config_files[:10]]  # Limit to first 10
        ...
        results["certificates_with_issues"] = len([c for c in certificates if c.get("risk_level") in ["HIGH", "MEDIUM"]])
        ...
        results["servers_with_tls_issues"] = len([t for t in server_tests if not t.get("tls_supported", True)])
        ...
        results["summary"] = {
            ...
            "certificates_with_issues": len([c for c in certificates if c.get("risk_level") in ["HIGH", "MEDIUM"]]),
            "servers_tested": len(server_tests),
            "servers_with_tls_issues": len([t for t in server_tests if not t.get("tls_supported", True)]),
    ...
    def calculate_tls_security_score(self, results: Dict) -> int:
        ...
        cert_issues = len([c for c in results.get("certificates", [])
        ...
        server_issues = len([t for t in results.get("server_tests", [])
...
def main():
    ...
    try:
        ...
        print(f"🎯 Overall TLS Security Score: {summary['overall_tls_security_score']}/100")

        # Show certificate issues
        cert_issues = [c for c in results.get("certificates", [])
                      if c.get("risk_level") in ["HIGH", "MEDIUM"]]
        if cert_issues:
            print(f"\n🔒 CERTIFICATE SECURITY ISSUES:")
            for cert in cert_issues:
                print(f"  ❌ {cert['file']}: {', '.join(cert.get('security_issues', []))}")

        # Show server TLS issues
        server_issues = [t for t in results.get("server_tests", [])
                         if not t.get("tls_supported", True)]
        if server_issues:
            print(f"\n🌐 SERVER TLS ISSUES:")
            for test in server_issues:
                print(f"  ❌ {test['hostname']}:{test['port']}: {', '.join(test.get('security_issues', []))}")
        ...
        with open("/Users/sheriftito/Downloads/psychsync/tls_configuration_security_report.json", "w") as f:

...
def test_credential_rotation():
    ...
    for py_file in python_files:
        ...
        try:
            with open(py_file, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
            if re.search(r'rotate.*credential|credential.*rotation|cron.*password', content, re.IGNORECASE):
                has_rotation = True
                break
        except Exception:
            pass
...
def test_log_security():
    ...
    for py_file in python_files:
        ...
        try:
            ...
            for line in lines:
                if re.search(r'logger\.debug.*(password|token|secret|key)', line, re.IGNORECASE):
                    debug_issues += 1
                    break

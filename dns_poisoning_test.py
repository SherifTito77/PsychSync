...
class DNSPoisoningTester:
    ...
    def check_dns_servers_configuration(self) -> Dict[str, Any]:
        ...
        if resolv_conf_path.exists():
            try:
                ...
                if len(dns_servers) == 1:
                    result["security_issues"].append("Single DNS server configured (single point of failure)")

                # Check for localhost DNS
                if "127.0.0.1" in dns_servers:
                    result["security_issues"].append("Localhost DNS configured (may be secure if properly configured)")
        ...
        try:
            ...
            if "localhost" in resolved_domains and "127.0.0.1" not in str(resolved_domains["localhost"]):
                result["security_issues"].append("localhost does not resolve to 127.0.0.1")
    ...
    def test_dns_cache_poisoning(self, target_domain: str = "localhost") -> Dict[str, Any]:
        ...
        try:
            ...
            if dns_query.returncode == 0:
                ...
                if result["normal_resolution"] and result["normal_resolution"] in dns_query.stdout:
                    recursion_test["vulnerable"] = False
                else:
                    recursion_test["vulnerable"] = True
                    result["vulnerabilities"].append("DNS response differs from system resolution")
            else:
    ...
    def analyze_application_dns_usage(self) -> Dict[str, Any]:
        ...
        if hardcoded_domains:
            result["security_issues"].append(f"Hardcoded external domains: {', '.join(hardcoded_domains)}")

        if "localhost" in str(result["dns_references"]):
            result["security_issues"].append("Localhost references found in configuration")
    ...
    def test_dns_spoofing_vulnerability(self) -> Dict[str, Any]:
            ...
            try:
                ...
                for pattern in suspicious_patterns:
                    if re.search(pattern, hosts_content, re.IGNORECASE):
                        ...
                        result["vulnerabilities"].append(f"Suspicious hosts file entry: {pattern}")
        ...
        if not dns_cache_settings:
            ...
            cache_test["result"] = f"DNS cache configuration found: {', '.join(dns_cache_settings)}"
            result["vulnerabilities"].append("DNS cache configuration may be vulnerable to pollution")

        result["spoofing_tests"].append(cache_test)

        result["vulnerable"] = len(result["vulnerabilities"]) > 0
        result["risk_level"] = "HIGH" if len(result["vulnerabilities"]) > 1 else "MEDIUM"
                ...
                try:
                    ...
                    if file_dns_settings:
                        result["dns_settings"].append({
                            "file": docker_file,
                            "settings": file_dns_settings
                        })

                    # Check for security issues
                    if "8.8.8.8" in content or "8.8.4.4" in content:
                        result["security_issues"].append("Public DNS servers used in Docker configuration")

                    if "/etc/hosts" in content and "127.0.0.1" not in content:
                        result["security_issues"].append("Custom hosts mapping without localhost entry")

                except Exception as e:
                    result["security_issues"].append(f"Error analyzing {docker_file}: {e}")
    ...
    def generate_dns_security_recommendations(self, test_results: List[Dict]) -> List[Dict]:
...
def main():
    ...
    try:
        ...
        print(f"🎯 Overall DNS Security Score: {summary['overall_dns_security_score']}/100")
        ...
        with open("/Users/sheriftito/Downloads/psychsync/dns_poisoning_security_report.json", "w") as f:

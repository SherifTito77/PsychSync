...
class AdminVsUserPermissionDemo:
    ...
    def demonstrate_permission_differences(self):
        ...
        print(f"   • Permission Violations Prevented: {concurrent_results['violations_prevented']}")
        print(f"   • Average Response Time: {concurrent_results['avg_response_time_ms']:.2f}ms")
        ...
        print(f"   • Cross-User Data Access: {'❌ BLOCKED' if isolation_results['cross_user_blocked'] else '⚠️  ALLOWED'}")
        print(f"   • Privilege Escalation: {'❌ BLOCKED' if isolation_results['privilege_escalation_blocked'] else '⚠️  ALLOWED'}")
        print(f"   • Data Leakage: {'❌ PREVENTED' if isolation_results['data_leakage_prevented'] else '⚠️  DETECTED'}")
        print(f"   • Role Boundary Enforcement: {'✅ ACTIVE' if isolation_results['role_boundaries_enforced'] else '❌ INACTIVE'}")
        ...
        print(f"   • Permission Check Overhead: {performance_results['check_overhead_ms']:.2f}ms")
        print(f"   • Database Query Impact: {performance_results['db_query_impact_ms']:.2f}ms")
        print(f"   • Memory Usage Increase: {performance_results['memory_overhead_percent']:.1f}%")
        print(f"   • Cache Hit Rate: {performance_results['cache_hit_rate_percent']:.1f}%")
        ...
        print(f"   • Authentication Bypass Prevention: {'✅ SECURE' if security_assessment['auth_bypass_prevented'] else '❌ VULNERABLE'}")
        print(f"   • Authorization Enforcement: {'✅ ENFORCED' if security_assessment['authz_enforced'] else '❌ MISSING'}")
        print(f"   • Input Validation: {'✅ VALIDATED' if security_assessment['input_validated'] else '❌ MISSING'}")
        print(f"   • Audit Trail: {'✅ LOGGED' if security_assessment['audit_trail'] else '❌ MISSING'}")
    ...
    def _generate_permission_summary(self, scenarios: List[Dict[str, Any]]) -> Dict[str, int]:
        """Generate summary statistics from permission scenarios"""
        high_risk_count = sum(1 for s in scenarios if s['risk_level'].lower() in ['critical', 'high'])
        admin_required_count = sum(1 for s in scenarios if 'admin' in s['normal_access'].lower())
        normal_accessible_count = sum(1 for s in scenarios if 'denied' not in s['normal_access'].lower())

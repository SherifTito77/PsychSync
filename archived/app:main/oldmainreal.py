    ...
    # from app.api.v1.api import api_router #users, organizations, auth
    # # from app.api.v1 import users, organizations, auth
    ...
    # # this went to __init__.py    app.include_router(users.router, prefix="/api/v1/users", tags=["users"])
    # # app = FastAPI(title="PsychSync AI API")
    ...
    #     # Example: set a Redis key (make sure Redis is running)
    #     await set_key("welcome", "Hello, PsychSync!")
    ...
    #         logger.warning(f"Failed to cache optimization result: {str(e)}")
    ...
    #             return eval(cached_insights)
    #     except Exception as e:
    #         logger.warning(f"Failed to get cached insights: {str(e)}")

...
class EdgeChaosTester:
    ...
    def _generate_test_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "spam_submission": {
                ...
                "assessment_response": {"score": random.randint(1, 5) for _ in range(50)}
            }
        }
    ...
    def test_network_drop_mid_submission(self) -> ChaosTestResult:
        ...
        try:
            ...
            large_payload = {
                ...
                "user_responses": [{"question_id": f"q{i}", "answer": "x" * 1000} for i in range(100)],
                "metadata": {"timestamp": datetime.now().isoformat()}
            }

            # Start submission but interrupt with network simulation
            with session.post(f"{self.backend_url}/api/v1/assessments/submit",
                             json=large_payload,
                ...
                return ChaosTestResult(
                    ...
                    system_metrics={
                        ...
                        "after": system_after,
                        "memory_delta": system_after.get("memory_percent", 0) - system_before.get("memory_percent", 0)
                    },
    ...
    def test_disk_full_scenario(self) -> ChaosTestResult:
        ...
        try:
            ...
            if PSUTIL_AVAILABLE:
                ...
                print(f"Current disk usage: {current_usage:.1f}% ({free_space_gb:.1f}GB free)")
            else:
                current_usage = 0
            ...
            test_file_size_gb = min(0.5, free_space_gb * 0.8)  # Use up to 80% of free space, max 0.5GB
            ...
            if test_file_size_gb < 0.1:
                ...
                try:
                    ...
                    response = session.post(f"{self.backend_url}/api/v1/test/disk-stress",
                                           json=test_payload,
                                           timeout=30)
            ...
            return ChaosTestResult(
                ...
                recommendations=["Add comprehensive disk monitoring and error handling"]
            )
        ...
        try:
                            ...
                            if iteration % 10 == 0 and PSUTIL_AVAILABLE:
                                ...
                                if len(memory_samples) >= 3:
                                    ...
                                    if growth > memory_growth_threshold:
                                        leak_detected = True
                                        print(f"⚠️  Memory growth detected: {growth:.1f}%")
            ...
            return ChaosTestResult(
                ...
                system_metrics={
                    ...
                    "memory_variance": memory_variance if 'memory_variance' in locals() else 0,
    ...
    def test_malicious_large_input(self) -> ChaosTestResult:
        ...
        test_cases = [
            ...
            {"name": "Mixed Attack Vector", "data": self.test_data["massive_text"] + self.test_data["malicious_script"]}
        ]
        ...
        try:
            ...
            for i, test_case in enumerate(test_cases):
                ...
                try:
                    ...
                    if len(test_case['data']) > 1048576:  # 1MB
                        print(f"    ⚠️  Input size: {len(test_case['data'])} bytes (very large)")
                    ...
                    submit_start = time.time()
                    response = session.post(f"{self.backend_url}/api/v1/test/large-input",
                                           json=payload,
                                           timeout=30)
                    submit_time = time.time() - submit_start
                    ...
                    if response.status_code == 200:
                        ...
                        if response_size > 1048576:  # 1MB response
                            ...
                            case_impact = f"Large response returned: {response_size} bytes"
                        else:
                        ...
                        case_result = TestResult.WARNING
                        case_impact = f"Unexpected response code: {response.status_code}"
            ...
            if error_count > 0:
                ...
                user_impact = "System handles malicious inputs but with degraded performance"
                recommendations = [
            ...
            return ChaosTestResult(
                ...
                recommendations=["Add comprehensive error handling for input validation testing"]
            )
    ...
    def test_rapid_form_spam(self) -> ChaosTestResult:
        ...
        try:
            ...
            print(f"  Initiating {spam_count} rapid submissions with {submission_interval}s interval...")
            ...
            for i in range(spam_count):
                ...
                try:
                    response = session.post(f"{self.backend_url}/api/v1/assessments/submit",
                                           json=submission_data,
                                           timeout=5)

                    submit_time = time.time() - submission_start
                    response_time = response.elapsed.total_seconds() if hasattr(response, 'elapsed') else submit_time
            ...
            avg_response_time = sum(s['response_time'] for s in submissions if s['response_time'] > 0) / total_submissions

            response_times = [s['response_time'] for s in submissions if s['response_time'] > 0]
            if response_times:
    ...
    def run_comprehensive_test_suite(self) -> List[ChaosTestResult]:
        ...
        for test_func in test_functions:
            try:
                ...
                print(f"{status_icon} {result.scenario_type.value}: {result.result.value.upper()} ({result.execution_time:.2f}s)")
    ...
    def generate_test_report(self, results: List[ChaosTestResult]) -> Dict[str, Any]:
        ...
        overall_status = "PASS" if (passed_tests + warning_tests) >= total_tests * 0.8 else "FAIL"
        ...
        report = {
            ...
            "system_impact": {
                ...
                "performance_issues": [r for r in results if r.result == TestResult.WARNING],
    ...
    try:
        ...
        if critical_issues:
            ...
            for issue in critical_issues:
                print(f"  • {issue.scenario_type.value}: {issue.user_experience_impact}")
